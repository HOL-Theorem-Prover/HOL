(* Generate one of three chapter-listing files from a manual's
   chapters.txt:

     gen_chap_lists summary       > SUMMARY.md
     gen_chap_lists latex-include > chapters-include.tex
     gen_chap_lists stems         > chapter-stems.mk
...
fun emitSummary (bookTitle : string) (chaps : chap list) =
    let
      fun emitChap (stem, children) =
          (* The mdbook-only `references` chapter has no .smd
             source — its content is generated by `smdpp render-bib`
             from the manual's .bib file.  Wire it into the sidebar
             with a fixed title pointing at references.md. *)
          if stem = "references" then
              emit "- [References](references.md)\n"
          else if not (fileExists (stem ^ ".smd")) then ()
          else let
...
fun emitLatexInclude (chaps : chap list) =
    ...
    (* The mdbook-only `references` chapter is rendered from
       <manual>.bib via smdpp render-bib; the PDF path gets its
       bibliography from natbib's \bibliography{<manual>} directly,
       so skip it here. *)
    ; List.app (fn (s, _) =>
                   if s = "references" then ()
                   else emit ("\\include{" ^ s ^ "}\n")) chaps )

(* --- chapter-stems.mk output ----------------------------------- *)

(* Emit a Make-syntax assignment of every chapter stem (top-level
   plus nested sub-files, in chapters.txt order) into CHAPTER_STEMS,
   skipping the mdbook-only `references' bibliography chapter that
   has no .tex source.  Consumed by each manual's Holmakefile via
   `include chapter-stems.mk` so TEX_CORES doesn't drift from
   chapters.txt. *)
fun emitMakeStems (chaps : chap list) =
    ( emit "# Generated by Manual/Tools/gen_chap_lists from \
           \chapters.txt; do not edit by hand.\n"
    ; emit "CHAPTER_STEMS ="
    ; List.app (fn (s, kids) =>
                  ( if s = "references" then ()
                    else emit (" " ^ s)
                  ; List.app (fn k => emit (" " ^ k)) kids )) chaps
    ; emit "\n" )
