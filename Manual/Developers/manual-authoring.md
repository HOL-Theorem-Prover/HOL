...
| `Manual/theme/index.hbs`          | Shared Handlebars template — MathJax setup, manual-switcher strip, sidebar JS, `window.HOL_MANUALS.books` / `.identifierBooks`. |
| `Manual/theme/custom.css`         | Shared stylesheet.                                                                |
| `Manual/theme/hol-searcher.js`    | Cross-book searcher; replaces mdbook's bundled `searcher.js`.  Also holds the entry-name lookup layer (see below). |
| `Manual/<Manual>/identifiers.js`  | Generated entry-name table for one book; copied into `Manual/book/<Manual>/` by that book's `mdbook` recipe. |
...
### Entry-name lookup (`identifiers.js`)

Elasticlunr runs a `trimmer → stopWordFilter → stemmer` pipeline over the **query** as well as over the indexed text.
A page whose name is an English stop word or pure punctuation therefore cannot be reached through the index at all: the token is dropped on both sides, so the query produces no terms and the search returns nothing.
In the Reference manual that silently hides `bossLib.have`, `bossLib.by`, `Tactical.THEN`, `Tactical.EVERY`, `Tactical.IF`, `Lib.I`, `Lib.all`, `Lib.can`, `Lib.for`, `Lib.say`, and every punctuation entry (`&&`, `$`, `==`, `++`, `-->`, `PART_MATCH'`, …).
mdbook 0.5 exposes no knob for that pipeline, so no `book.toml` setting can fix it.

Each book therefore also emits a generated `identifiers.js` next to its `searchindex.js`, listing its entry names as literal strings.
`hol-searcher.js` loads every book's table (`window.HOL_MANUALS.identifierBooks` in `index.hbs`), matches the typed string against it directly — exact matches first, then prefix matches from three characters up — and renders those hits in their own group above the full-text results.
Nothing in that path touches Elasticlunr, which is the point: a name it cannot represent is still findable, and an exact entry-name match is no longer buried under the pages that merely mention it.

The format is one line per record, `[qualified, short, page]`:

    /* Generated -- do not edit. ... */
    window.hol_identifiers = {entries: [
    ["BasicProvers.&&","&&","BasicProvers..IYG.html"],
    ["bossLib.have","have","bossLib.have.html"],
    ]};

`qualified` and `short` are the **decoded** names a reader would type; `page` is the **encoded** stem plus `.html`, i.e. what mdbook actually renders.
`short` is a separate field rather than `qualified` split at its last dot, because a decoded HOL identifier may itself contain a dot.
The searcher upgrades `page` to the indexed `page.html#slug` when that book's index is loaded, so a name hit gets the same breadcrumbs and teaser as a full-text hit; when it isn't, the bare page link still works.

Two producers, one format:

  - **Reference** — `../Tools/gen_reference_summary --identifiers $(Processed)`.
    It already scans the Docfiles directory, and its `splitName` already yields the structure, the `UC_ASCII_Encode`-decoded function name, and the encoded filename.
    It renders the same scan that produces `SUMMARY.md`, so the sidebar and the table cannot disagree about which entries exist.
  - **Every other book** — `../Tools/smdpp dump-identifiers .`, a scan-only pass over `SUMMARY.md` alongside `dump-labels`.
    For a prose section the qualified and short names coincide.

Two design restrictions worth knowing before extending this:

  - The table covers **`SUMMARY.md`-level entries only**.
    In-page `###` headings are already separate Elasticlunr documents (`heading-split-level` 3) and are found by ordinary search; duplicating them here would drown the name group in section titles.
  - `identifiers.js` must **never** become an `additional-js` entry in a `book.toml`.
    mdbook content-hashes those, and the searcher loads siblings from a fixed cross-book URL — the same trap the `searchindex.js` symlink works around.
    It is installed by the `mdbook` recipe instead, with a `cp` **after** `mdbook build` (which empties the build dir) and before the `smdpp check-*` gates.
    A copy rather than a symlink, because a link pointing out of `book/` would not survive a deploy snapshot.

To give a new book a table: add the `identifiers.js` rule beside its `labels.tsv` rule, list `identifiers.js` among its `../book/<Manual>/index.html` prerequisites, add the `cp` line to that recipe, and add the book to `identifierBooks` in `index.hbs`.
...
# Adding a new manual
  ...
  2. Edit `book.toml`'s `title` and `description` (the description shows up on the lander card emitted by `gen_lander`); set `build-dir = "../book/<NewManual>"`; add an entry for the new manual to the `PORTS` table in `Manual/Tools/mdbook-preview.py` (3000–3007 are taken; 3002 is the unified site).
  ...
  6. Wire the new manual into the unified site:
       ...
       - Optionally give it an entry-name table so its sections can be found by name from any manual: see "Entry-name lookup (`identifiers.js`)" above for the four one-line edits.
