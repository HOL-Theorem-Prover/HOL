...
To use Holindex, add the following lines to the header of
...
correspond to the command line options of the merger. Their appearance
in the header is optional. One can insert \HOL{}-theorems by
\verb-\blockHOLthm{theory.thm}- or short ones by
\verb-\inlineHOLthm-. There is also \verb-\citeHOLthm{theory.thm}- to
cite theorems in the index. Similar \LaTeX{}-macros exist for terms
and types. There are macros for setting formating options as
well. However, it is recommended to use external files to define
terms, types and theorems and their formating. Finally, the \HOL{}
index is printed with \verb-\printHOLindex-.
...
\end{alltt}
will create \verb-jobfile.tix-. This file contains
all the pretty-printed \HOL{}-theorems, terms and types as well
as the \HOL{}-index. It will be used by further runs of \LaTeX{}.
For an example on how to use Holindex, please have a look
at \verb|holindex-demo.tex| in \varref{HOLDIR}\texttt{/src/TeX}.
