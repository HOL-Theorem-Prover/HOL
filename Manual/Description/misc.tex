...
\item[$\mathit{nm}_1$\texttt{/}$\mathit{nm}_2$] (For use with \holtm{}
  and \holthm{}.)
%
Causes $\mathit{nm}_1$ to be substituted for $\mathit{nm}_2$ in the
term or theorem.
%
This will rename free variables, or variables that are universally
...
Once created, a munger is run as a filter command, consuming its
...
The munger provides preprocessing of \LaTeX{} sources. In contrast
Holindex is interleaved with \LaTeX{}. Its main purpose is to generate
indices of \HOL{}-theorems, types and terms. However, it can also be used
to insert terms, types or theorems in the text.

To use Holindex, one needs to add the following lines to the header of
the \LaTeX{} source file.
%
\begin{alltt}
\bs{}usepackage{holindex}
\bs{}initHOLindex
\bs{}setHOLlinewidth{width}  
\bs{}setHOLoverrides{file}
\end{alltt}
%
The commands \verb-\setHOLlinewidth- and \verb-\setHOLoverrides-
correspond to the command line options of the merger. They are
optional. One can insert \HOL{}-theorems typeset in an extra block by
\verb-\blockHOLthm{theory.thm}- or short ones by
\verb-\inlineHOLthm-. There is also the option to just cite theorems
in the index.  Similar commands exist for terms and types. There are
commands for setting formating options as well. Moreover, external
files can be used for defining terms, types and theorems and their
formating. Finally, the index is printed with \verb-\printHOLindex-.

The workflow of Holindex is similar to that of tools like Bibtex.  The
first run of \LaTeX{} on a file \verb-jobfile.tex- will create an
auxiliary file \verb-jobfile.hix-. A call of 
\begin{alltt}
   munge.exe -index jobname
\end{alltt}
will proceed \verb-jobfile.tix-. This file contains
all the pretty-printed \HOL{}-theorems, terms and types as well
as the \HOL{} index. It will be used by further runs of \LaTeX{}.
In order to edit the sources on a machine without the munger, on
can just copy \verb-jobfile.tix- to the new mas-chine and continue
editing. For an example on how to use Holindex, please have a look
at \verb-holindex-demo.tex- in \varref{HOLDIR}\texttt{/TeX}.
