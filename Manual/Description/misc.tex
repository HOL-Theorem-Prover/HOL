...
\[
\begin{array}{lcl}
\textsl{make-file}& ::= & \prelim^* \;\; (\hmtab|\hmnewline)^*\;\;\hmrule^*\\
...
Finally, the commands can use two useful abbreviations for the
programs executed.  The string \texttt{HOLMOSMLC} is replaced by an
...
normal includes).  The string \texttt{MOSMLC} is replaced by an
invocation of the compiler along with just the normal includes.
