...
\index{holmake preexec files@\texttt{.hol\_preexec} files (\holmake{})}
...
When this scanning is complete, the contents of all these files will be executed as shell commands (using the \ml{OS.Process.system} command).
...
The commands are executed in a pre-order depth-first traversal of the directory hierarchy (relying on the behaviour of \ml{String.compare} on paths).
