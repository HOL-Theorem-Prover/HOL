...
\item $t$ is not a closed term;
\item The kind variables of $t$ are not contained within the kind variables of the type variables of $t$.
\end{myenumerate}
\index{extension, of HOL logic@extension, of \HOL{} logic!by type definition|)}
\index{theories, in HOL logic@theories, in \HOL{} logic!extension of|)}
\index{type definition extension, in HOL logic@type definition extension, in \HOL{} logic!ML function for@\ML\ function for|)}
...
%
\index{type specifications, in HOL logic@type specifications, in \HOL{} logic|(}
%
In \LOGIC{} a type specification
%
\index{type specification extension, of HOL logic@type specification extension, of \HOL{} logic!ML function for@\ML\ function for}
%
for a theory ${\cal T}$ 
is defined to be a pair:
...
\[\turn q_{bool}\ml{[}{\tau_1},\ldots,{\tau_n}/{\alpha_1},\ldots,{\alpha_n}\ml{]} \]
...
\begin{holboxed}
\index{new_type_specification@\ml{new\_type\_specification}|pin}
\begin{verbatim}
...
\noindent This theorem is stored,
with name $name$, as a definition in the current theory segment. A call to
\ml{new\_type\_specification} fails if:

\begin{myenumerate}
\item the theorem argument has a non-empty assumption list;
\item there are free variables or free type variables in the theorem argument;
\item $\tau_1$, $\dots$, $\tau_n$ are not distinct names;
\item the kind of some $\alpha_i$ does not contain all the kind
variables which occur in the term
\holtxt{?:$\alpha_1\ \cdots\ \alpha_n$.~$q$[$\alpha_1$, $\ldots$, $\alpha_n$]}.
\end{myenumerate}
%
\index{specification of types, in HOL logic@specification of types, in \HOL{} logic|)}
