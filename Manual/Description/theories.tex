...
does not as yet provide a well-rounded collection of definitions about
relations; indeed, relations are often treated informally in the
system. For example, $R_1 \subset R_2$ can be directly represented as 
{\small\verb+!x y. R1 x y ==> R2 x y+}. The theory {\tt relation}
currently provides support for transitive closure and wellfoundedness.
...
\begin{boxed}
\index{new_recursive_definition@\ml{new\_prim\_rec\_definition}|pin}
\begin{verbatim}
   new_recursive_definition : fixity -> Thm.thm -> string -> term -> thm
