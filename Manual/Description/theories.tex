...
as built-in constants \index{variables, in HOL logic@variables, in
  \HOL{} logic!with constant names} is sometimes inconvenient because
they are often wanted as mnemonic names for variables (\eg\ \ml{S} to
range over sets and \ml{o} to range over outputs).\footnote{Constants
...
names can be used in the current system if \ml{o}, \ml{S}, \ml{K},
\ml{I}, \ml{W}, and \ml{C} are first hidden (see
Section~\ref{hidden}).  In fact, this happens so often with the
constant \holtxt{C} that it is ``hidden'' by default.  While hidden,
it must be written in fully-qualified form, as \holtxt{combin\$C}.
%
\index{ function composition operator, in HOL logic@\ml{o} (function composition operator), in \HOL{} logic|)}
\index{function composition, in HOL logic@function composition, in \HOL{} logic|)}
...
\index{arithmeticTheory@\ml{arithmeticTheory}}
...
(\ml{FUNPOW}) is tail-recursive. An alternative presentation of the
recursion (\ml{FUNPOW\_SUC}) may be simpler to reason about in some
cases.
%
\begin{hol}
{\small
...
% This macro below didn't seem to work; don't know why.
% \def\cdotfill{
%%   \leavevmode
%%   \cleaders \hb@xt@ 1.2em{\hss$\cdot$\hss}\hfill
%%   \kern\z@}
...
\ml{datatype/equiv}.  The following equivalence relation is used to
...
The map \ml{n2w} is the primary means of constructing new words -- a
...
positive value.  This is captured the theorems \ml{WORD\_L\_LESS\_EQ}
...
%
The behaviour of \holtxt{MAP2} in the two cases when it is given
...
%
The predicate \holtxt{ALL\_DISTINCT} holds on a list  just
...
\index{lazy lists@``lazy'' lists!the \HOL{} theory of|(}
...
\index{labelled paths!the \HOL{} theory of|(}
\index{reduction sequences!the \HOL{} theory of|(}
