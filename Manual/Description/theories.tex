...
\index{ function application operator, in HOL logic@\ml{:>} ((reversed) function application operator), in \HOL{} logic}
There are no theorems about \ml{:>}; it use is as a convenient syntax for function applications.
...
The symbols \holtxt{univ} and \holtxt{$\mathbb{U}$} are high-priority ``true prefixes'' (see Section~\ref{sec:parseprint:fixities}), and overloaded patterns (see Section~\ref{sec:parser:syntactic-patterns}) mapping a value of the itself type to the corresponding \holtxt{UNIV} constant.
