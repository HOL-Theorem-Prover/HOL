...
\end{hol}
where $n$ is the cardinality of the set \holtxt{UNIV:'b set}.  The
type \ty{'b} could be infinite\footnote{\HOL{}'s type system does not
  allow type variables to range over finite types only.} and so the
dimension $n$ is actually taken to be \holtxt{dimindex(:\ty{'b})},
where \holtxt{dimindex} has value one when the universe is infinite.
For example, \fcp{'a}{\num} is similar to \ty{'a}, and \fcp{'a}{\bool}
is like \ty{'a}\hash\ty{'a}.
...
index : \fcp{'a}{'b}\rarr\num\rarr\ty{'a}
\end{hol}
and \oo{x}{i} denotes the value of \holtxt{x} at position \holtxt{i}.  Note that, in practice, indices are constrained to be less than the size of \ty{'b}.
...
then each bit can be accessed using the indexing map.
...
The theory \theoryimp{words} introduces a selection of polymorphic constants and operations -- these can be type instantiated to any word size.  For example, word addition
has type:
\begin{hol}
...
The function
\holtxt{word\_concat:\worda\rarr\wordb\rarr\wordc} concatenates words -- it composes \holtxt{w2w}
with
\holtxt{word\_join:\worda\rarr\wordb\rarr\fcp{\bool}{\ensuremath{\alpha+\beta}}}, which is
...
often not the intended type \eg{} the type \fcp{\bool}{\ty{32}}will be preferred to
\fcp{\bool}{\ty{16}+\ty{16}}.
...
respectively.  Three operations are provided for selecting bit
...
needs to be loaded.  This library also provides a \emph{compset} \ml{words\_compset}, which
can be used to build conversions that evaluate variable-free word expressions.

Words can be pretty-printed using different number bases. For example, the function
\ml{output\_words\_as\_bin} will select binary format for all words;
\ml{output\_words\_as} is more flexible and allows the number base to vary depending on
the word length and numeric value.  The session below demonstrates working with \ml{EVAL} and
pretty-printing.
\setcounter{sessioncount}{0}
\begin{session}
\begin{verbatim}
...
- wordsLib.output_words_as
     (fn x => if fst x = 32 then StringCvt.HEX else StringCvt.DEC);
> val it = () : unit
...
each bit \\
 & & \worda\rarr\worda \\
\holtxt{word\_join} & & \worda\rarr\wordb\rarr\fcp{\bool}{$\alpha+\beta$} & Join
words \\
...
String length (\holtxt{STRLEN}) is defined by primitive
