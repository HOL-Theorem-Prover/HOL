...
A useful subset of the collection of theories distributed with the \HOL\
system \index{ancestry, of HOL system theories@ancestry, of \HOL\ system
theories} is listed in Table \ref{nativeTheories}. 
...
realTheory & ({\it plus several others}) real numbers and analysis \\ \hline
\end{tabular}
...
constants\index{variables, in HOL logic@variables, in \HOL\ logic!with
constant names} is sometimes inconvenient because they are often wanted
...
and \ml{I} are first hidden (see Section~\ref{hidden}).  \index{
function composition operator, in HOL logic@\ml{o} (function composition
operator, in \HOL\ logic)|)} \index{function composition, in HOL
...
relation} currently provides definitions and theorems about
the transitive closure of a relation and for wellfounded relations.
...
{\small\verb%\(%}$x_1${\small\verb%,%}$x_2${\small\verb%).%}$t$
...
\begin{flushleft}
\begin{tabular}{@{}ll}
{\small\verb%   \(%}$x${\small\verb%,%}$y${\small\verb%).%}$t$
&$\leadsto\ \ ${\small\verb%UNCURRY(\%}$x\
y${\small\verb%.%}$t${\small\verb%)%}\\
{\small\verb%   \(%}$x_1${\small\verb%,%}$x_2${\small\verb%,%}$\ldots${\small\verb%,%}$x_n${\small\verb%).%}$t$
&$\leadsto\ \ ${\small\verb%UNCURRY(\%}$x_1${\small\verb%.\(%}$x_2${\small\verb%,%}$\ldots${\small\verb%,%}$x_n${\small\verb%).%}$t${\small\verb%)%}\\
{\small\verb%   \((%}$x_1${\small\verb%,%}$\ldots${\small\verb%,%}$x_n${\small\verb%),%}$y_1${\small\verb%,%}$\ldots${\small\verb%,%}$y_m${\small\verb%).%}t
...
$b$\ml{(}$x_1${\small\verb%,%}$x_2${\small\verb%).%}$t$
is parsed as
$b${\small\verb%(\(%}$x_1${\small\verb%,%}$x_2${\small\verb%).%}$t${\small\verb%)%}, and hence transformed as above.
For example, {\small\verb%!(x,y).x>y%} parses to
{\small\verb%$!(UNCURRY(\x.\y.$> x y))%} (where {\small{\tt >}} is an infixed
constant of the theory {\small\verb%arithmetic%} meaning `is greater than')\index{function abstraction, in HOL logic@function abstraction, in \HOL\ logic!paired|)}.
...
{\small\verb%   let %}$f\ v_1\ \ldots\ v_n${\small\verb% = %}$t_1${\small\verb% in %}$t_2$
&$\leadsto\ \ ${\small\verb%LET(\%}$f${\small\verb%.%}$t_2$
{\small\verb%)(\%}$v_1\ \ldots\ v_n${\small\verb%.%}$t_1${\small\verb%)%}\\
{\small\verb%   let (%}$v_1${\small\verb%,%}$\ldots${\small\verb%,%}$v_n${\small\verb%) = %}$t_1${\small\verb% in %}$t_2$
&$\leadsto\ \ ${\small\verb%LET(\(%}$v_1${\small\verb%,%}$\ldots${\small\verb%,%}$v_n${\small\verb%).%}$t_2${\small\verb%)%}$t_1$\\
{\small\verb%   let %}$v_1${\small\verb%=%}$t_1${\small\verb% and %}$\ldots${\small\verb% and %}$v_n${\small\verb%=%}$t_n${\small\verb% in %}$t$
&$\leadsto\ \ ${\small\verb%LET(%}$\ldots${\small\verb%(LET(LET(\%}$v_1\ldots v_n${\small\verb%.%}$t${\small\verb%)%}$t_1${\small\verb%)%}$t_2${\small\verb%)%}$\ldots${\small\verb%)%}$t_n$\\
\end{tabular}}
...
\noindent yields (ignoring the uniqueness of \ml{fn}) the existence theorem shown below:
...
calculations on numerals can be found in the \ml{numeral} theory;
these are mechanized by the \ml{reduce} library. Numerals may of course
...
The constant {\small\verb+& : num -> int+} denotes the injection
function from natural numbers to integers. For some reason it can 
be printed out as {\small\verb+fromNum : num -> int+}, although 
{\small\verb+fromNum+} is not a constant. What is going on here?


% Term `1i = &(1n + 0n)`;
%> val it = `1 = fromNum (1 + 0)` : Term.term
%- show_numeral_types := true;
%> val it = () : unit
%- Term `&1 = &(1n + 0n)`;
%> val it = `1i = fromNum (1n + 0n)` : Term.term

\section{Real numbers and analysis}

There is an extensive theory of real numbers and analysis in HOL, 
due to John Harrison \cite{jrh:thesis}.
...
uses this theorem to do automatic\index{primitive recursion
theorem!automated use of, in HOL system@automated use of, in \HOL\
system}  proofs of the existence of primitive
