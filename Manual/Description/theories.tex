...
\begin{hol}
\begin{verbatim}
   option_case_def |- (option_CASE NONE u f = u) /\
                      (option_CASE (SOME x) u f = f x)
\end{verbatim}
\end{hol}
%
...
For more on the ``indexed'' treatment of lists, see Section~\ref{sec:indexed-lists} below.
...
\begin{alltt}
   EL
     |- (\(\forall\)l. EL 0 l = HD l) \(\land\)
        (\(\forall\)l n. EL (SUC n) l = EL n (TL l))
\end{alltt}
\end{hol}
...
\begin{alltt}
   EL_MAP
     |- \(\forall\)n l. n < LENGTH l \(\Rightarrow\) \(\forall\)f. EL n (MAP f l) = f (EL n l)
   MEM_EL
     |- \(\forall\)l x. MEM x l \(\iff\) \(\exists\)n. n < LENGTH l \(\land\) (x = EL n l)
\end{alltt}
\end{hol}
...
\begin{alltt}
   LUPDATE_def
     |- (\(\forall\)e n. LUPDATE e n [] = []) \(\land\)
        (\(\forall\)e x l. LUPDATE e 0 (x::l) = e::l) \(\land\)
        \(\forall\)e n x l. LUPDATE e (SUC n) (x::l) = x::LUPDATE e n l
\end{alltt}
\end{hol}
...
\begin{alltt}
   EL_LUPDATE
     |- \(\forall\)ys x i k.
          EL i (LUPDATE x k ys) =
          if (i = k) \(\land\) k < LENGTH ys then x else EL i ys
\end{alltt}
\end{hol}
...
\begin{alltt}
   findi_def
     |- (\(\forall\)x. findi x [] = 0) \(\land\)
        \(\forall\)x h t. findi x (h::t) = if x = h then 0 else 1 + findi x t
\end{alltt}
\end{hol}
...
\begin{alltt}
   delN_def
     |- (\(\forall\)i. delN i [] = []) \(\land\)
        \(\forall\)i h t. delN i (h::t) = if i = 0 then t else h::delN (i - 1) t
\end{alltt}
\end{hol}
...
\begin{alltt}
   MEM_MAPi
     |- \(\forall\)x f l. MEM x (MAPi f l) \(\iff\) \(\exists\)n. n < LENGTH l \(\land\) (x = f n (EL n l))
\end{alltt}
\end{hol}
