...
\begin{verbatim}
  ...
  BIGINTER    |- !B. BIGINTER P = {x | !s. s IN B ==> x IN s}
\end{verbatim}}
%
...
straightforward. Since the universe of a type is always defined, the
complement ({\small\verb+COMPL+}) of a set is definable.
{\small
\begin{verbatim}
...
built from the empty set and created by a finite number of insertions.
%
{\small
\begin{verbatim}
 FINITE_DEF
...
\end{verbatim}}
%
A set is infinite iff it is not finite. As mentioned, set operations
apply to both finite and infinite sets. However, some operations, such
as cardinality, are only defined for finite sets (since, in \HOL{},
finite sets have cardinality ({\small\verb+CARD+}) defined as a
natural number). Thus {\small\verb+CARD+} has type
$(\alpha\to\konst{bool})\to\konst{num}$.
 The cardinality of an infinite set is not specified.
{\small
\begin{verbatim}
 CARD_DEF
      |- (CARD {} = 0) /\
...
definition of summing a finite set of numbers.
%
{\small
\begin{verbatim}
 SUM_IMAGE_DEF
...
theorems in {\small\verb+pred_set+} relate the various standard
operators to each other. Some notable other theorems are Koenig's
Lemma, which states that every finitely branching infinite tree has an
infinite path. There are many ways to formulate this theorem,
depending on how the notion of tree is formalized. In
{\small\verb+pred_set+}, finite branching is defined as a predicate on
a relation.
%
{\small
\begin{verbatim}
...
\end{verbatim}}
%
 From this, we obtain the following version of Koenig's Lemma:
{\small
\begin{verbatim}
 KoenigsLemma
  |- finitely_branching R ==>
            !x ~FINITE {y | RTC R x y} ==>
              ?f :num -> 'a. (f 0 = x) /\ !n. R (f n) (f (SUC n))
\end{verbatim}}
