...
using the natural numbers \ie{} via \holtxt{w2n} and \holtxt{n2w}.  In 
...
``less than'' we have \holtxt{255w < 0w:word8} because the word  \holtxt{n2w 
255} is actually taken to be representing the integer $-1$, whereas the 
unsigned version appears to be more natural: \holtxt{0w <+ 255w:word8}.
...
of writing, even with \ml{show\_types:=true}, the type of a word literal will 
not be printed.}  Any number base can be used in parsing a literal, but the 
reference \ml{allow\_octal\_input} must be true in order to parse octal numbers.
...
(\holtxt{PSUBSET}, infix), and power set (\holtxt{PSUBSET})
