...
The library \theoryimp{fcpLib} also provides a function \ml{mk\_index\_type},
which is used to introduce types of a given size into the logic.\footnote{Note
that  \ml{mk\_index\_type} can only be used to create \emph{one} type for a given
size \ie{} it cannot be called twice to create two distinct types of size 128.}
 For example, \ml{mk\_index\_type 128} introduces the type \ty{i128} and the
theorems:
\begin{hol}
\begin{verbatim}
finite_128:   |- FINITE (UNIV :i128 set)
dimindex_128: |- dimindex (:i128) = 128 .
\end{verbatim}
\end{hol}
...
\ml{wordsLib.mk\_word\_size}.  Each word type is an abbreviation for
\fcp{bool}{iN}, where $N$ is the word length; for example, with type
...
instantiated to \ty{i32} then this operation corresponds with 32-bit addition.
...
\mbox{\ty{bool}\,\holtxt{**}\,(\ty{i16}+\ty{i16})}.
