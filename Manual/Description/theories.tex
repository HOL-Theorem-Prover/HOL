...
\index{axioms!non-primitive, of HOL logic@non-primitive, of \HOL{} logic!for natural numberrs}
...
\begin{tabular}{@{}cccc}
Constant & Overloaded symbol & Strength & Associativity \\ \hline
{\small\verb+rat_geq+} &{\small\verb+>=+} & 450 & right \\
{\small\verb+rat_leq+} &{\small\verb+<=+} & 450 & right \\
{\small\verb+rat_gre+} &{\small\verb+>+}  & 450 & right \\
{\small\verb+rat_les+} &{\small\verb+<+}  & 450 & right \\
...
($n = 64$).  In the theory \theoryimp{words}, bit strings are represented as
...
no longer necessary to build an entirely new theory (with new constants and
operations) for each word size that is required.}

Wai Wong's theory \theoryimp{word} should be used when working with different
...
Curzon's \ml{rich\_list} theory.}  However, one key advantage of the finite
...
The \HOL{} theory \theoryimp{fcp} introduces a new infix type operator
...
\end{hol}
where $n$ is the size of \ty{'b}.  The type \ty{'b} could be
...
dimindex (:\ty{'b})
\end{hol}
where \holtxt{dimindex} has value one when the universe is infinite, and
otherwise it is the cardinality of the set.  For example, \fcp{'a}{\num} is
similar to \ty{'a}, and \fcp{'a}{\bool} is like \ty{'a}\hash\ty{'a}.
...
\%\%:(\fcp{'a}{'b})\rarr\num\rarr\ty{'a}
\end{hol}
where \oo{x}{i} is the value of \holtxt{x} at position \holtxt{i}.  Note that
the index \holtxt{i} is a natural number; when the index is greater than or
equal to the size of \ty{'b} the value of \oo{x}{i} cannot be determined, so
indices are constrained to be less than the size of \ty{'b}.
...
In order to define Cartesian products, the theory \theoryimp{fcp} introduces a
...
to say, the type \fcp{bool}{'a} can represent a binary word whose length
...
\holtxt{f:\num\rarr\bool} which gives the word's value at each bit position and
then each bit can be accessed using the indexing map \holtxt{\%\%}.

Numeral type names are supported, so one can freely work with indexing sets of any size \eg{} the type \ty{32} has thirty-two elements.
...
\holtxt{BIT\_MODIFY}. In this context, natural numbers are treated as words of
unbounded length.  The operations in \theoryimp{bit} are built on top of the
theory \theoryimp{arithmetic} \ie{} they are defined using \holtxt{DIV},
\holtxt{MOD} and \holtxt{EXP}.  For example, from the definition of
\holtxt{BIT}, the following theorem is derivable:
\begin{hol}
\begin{verbatim}
...
The theory \theoryimp{words} introduces types for a selection of
standard word sizes \eg{} \ty{word8} and \ty{word32}.  Any additional
word types can be introduced, as needed, with the function
\ml{wordsLib.mk\_word\_size}.  Each word type is simply an abbreviation for
\fcp{bool}{N}, where $N$ is the word length; for example, with type
abbreviation printing turned off, \ty{word32} will print as
\fcp{bool}{32}.  One can also use the notation \ty{N word} e.g. \ty{128 word}.
...
\end{hol}
where the notation \worda is used to denote \fcp{bool}{'a}. If \ty{'a} is
instantiated to \ty{32} then this operation corresponds with 32-bit addition.
...
\end{verbatim}
\end{hol}
The suffix \holtxt{w} is used in \HOL{} to denote a word literal \eg{}
\holtxt{255w} is the same as \holtxt{n2w 255}.

The function \holtxt{w2w:\worda\rarr\wordb} uses the functions \holtxt{w2n} and
\holtxt{n2w} to map between two word types:
\begin{hol}
\begin{verbatim}
|- !w. w2w w = n2w (w2n w)
\end{verbatim}
...
\cdots b_0
\end{eqnarray*}
The function \holtxt{sw2sw:\worda\rarr\wordb} is the sign extending version of
\holtxt{w2w}.

Words can be concatenated with the function
\holtxt{word\_concat:\worda\rarr\wordb\rarr\wordc}.  This applies \holtxt{w2w}
to the result of
\holtxt{word\_join:\worda\rarr\wordb\rarr$\bool^{(\alpha+\beta)}$}, which is
...
often not the intended type \eg{} the type \ty{word32} will be preferred to
\mbox{\ty{bool}\,\holtxt{**}\,(\ty{16}+\ty{16})}.

\begin{figure}
\begin{center}
\scalebox{.8}{\includegraphics{figs/concat}}
\caption{Joining two words: \holtxt{x = word\_join v w}.}
\label{fig:concat}
\end{center}
\end{figure}
...
\holtxt{255w < 0w:word8} because the word \holtxt{n2w 255} is actually
taken to be representing the integer $-1$, whereas the unsigned
version appears to be more natural: \holtxt{0w <+ 255w:word8}.
...
A word's bit ordering can be flipped over with \holtxt{word\_reverse}; see
Figure~\ref{fig:reverse}. Three operations are provided for selecting bit
ranges, or sub-words: \holtxt{word\_bits}, \holtxt{word\_slice} and
\holtxt{word\_extract}.  These operations are illustrated in
Figure~\ref{fig:bits}.
Again, these operations are defined using the Cartesian product binder; for
example, \holtxt{word\_bits} is defined as follows:
\begin{hol}
\begin{verbatim}
|- !h l x.
     (h -- l) x = FCP i. i + l <= MIN h (dimindex (:'a) - 1) /\ x %% (i + l) .
\end{verbatim}
\end{hol}
...
\begin{figure}
\begin{center}
\scalebox{.8}{\includegraphics{figs/reverse}}
\caption{Reversing the bit ordering: \holtxt{w = word\_reverse v}.}
\label{fig:reverse}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
\scalebox{.8}{\includegraphics{figs/bits}}
\caption{Selected bit ranges for the case when $l < h < n$.  The symbols
\holtxt{--}, \holtxt{<>} and \holtxt{><} are for \holtxt{word\_bits},
\holtxt{word\_slice} and \holtxt{word\_extract} respectively.  The word extract
function applies \holtxt{word\_bits} and then \holtxt{w2w}; typically the
...
\begin{figure}
\begin{center}
\scalebox{.8}{\includegraphics{figs/modify}}
...
(\holtxt{word\_rrx}).  These shifts are illustrated in Figure~\ref{fig:shifts};
...
(a) Logical shift left: \holtxt{w = v << n}. & (b) Logical shift right:
\holtxt{w = v >>> n}. \\[12pt]
\scalebox{.8}{\includegraphics{figs/asr}} &
\hspace{-5mm}\scalebox{.8}{\includegraphics{figs/ror}} \\
(c) Arithmetic shift right: \holtxt{w = v >> n}. & (d) Rotate right: \holtxt{w
= v \#>> n}. \\[12pt]
...
To evaluate ground terms, using \ml{EVAL}, the library \theoryimp{wordsLib}
needs to be loaded.  This library also provides a conversion \ml{WORDS\_CONV}
and a \emph{compset} \ml{words\_compset}, which can be used to build
...
sizes (introduced with \ml{mk\_word\_size}), the size theorems must be added to
the \emph{compset} manually.  The standard word sizes are listed at the end of
the script file \ml{\$HOLDIR/src/n-bit/wordsScript.sml}.  By editing this list,
it is of course possible to build a version of \HOL{} with a custom set of
standard word sizes.} The \emph{simpset} fragment \ml{SIZES\_ss} contains this
set of size theorems -- this fragment is useful when applying the simplifier to
word expressions.

Words can be pretty-printed using different number bases; for example,
\ml{pp\_word\_bin} will select binary format for all words.  The function
\ml{pp\_word} is more flexible and allows the number base to vary depending on
the word length.  The session below demonstrates working with \ml{EVAL} and
pretty-printing.
\setcounter{sessioncount}{0}
\begin{session}
\begin{verbatim}
...
- base_tokens.allow_octal_input:=true;
> val it = () : unit
- wordsLib.pp_word (fn x => if Type.compare(x,``:32``) = EQUAL then
...
\end{verbatim}
\end{session}
Note that type annotation is used to designate word sizes.  Any number
base can be used in parsing a literal, but the reference
\ml{allow\_octal\_input} must be true in order to parse octal numbers.
