...
\subsection{Restricted quantifiers}\label{res-quant}
...
\begin{verbatim}
  RES_EXISTS_UNIQUE_DEF |- RES_EXISTS_UNIQUE =
                            \P m. (?x :: P. m x) /\
                                  !x y :: P. m x /\ m y ==> (x = y)
...
Many other theorems may be found in \theoryimp{bool} theory.
...
such functions.  For example, the \HOL{} system defines a length
function, \ml{LENGTH}, on lists by the primitive recursive definition
on lists shown below:
...
\noindent
When this \ML{} expression is evaluated, \HOL{} uses \ml{list\_Axiom}
...
in the theory \ml{list} by the specifications:
...
An extensive development of set theory is available by loading the
theory {\small\verb+pred_set+}. Sets are represented by functions of
the type $\alpha \to \konst{bool}$, \ie, they are so-called
characteristic functions, and may be finite or infinite. All of the
elements in a set must have the same type. The basic notion that set
theory is usually based on is \emph{set membership}. In HOL, this is
represented by a definition (in theory {\small\verb+bool+}) 
%
{\small
\begin{verbatim}
  IN_DEF  |- IN = \x f. f x
\end{verbatim}}
%
The operator {\small\verb+IN+} is used as an infix and is merely a way
of applying the characteristic function to an item:
%
{\small
\begin{verbatim}
...
\end{verbatim}}
%
The following are standard definitions in set theory. The 
empty set is the characteristic function that is constantly
false. The universal set on a type is the characteristic 
function that is always true. The insertion of an element
into a set is defined with a set comprehension, as are 
several other concepts. The definition of set comprehension
...
\begin{verbatim}
 ...
 BIGUNION    |- !P. BIGUNION P = {x | ?p. p IN P /\ x IN p}
 BIGINTER    |- !B. BIGINTER B = {x | !P. P IN B ==> x IN P}
...
\end{verbatim}}
%
Finiteness is defined inductively. Finite sets have cardinality 
defined as a natural number. The cardinality of an infinite set
is not specified.
%
{\small
\begin{verbatim}
...
\end{verbatim}}
%
An extensive suite of theorems dealing with finiteness is available. The 
theory of sets has an analogue 
...
\subsection{Multisets (\theoryimp{bag})}
