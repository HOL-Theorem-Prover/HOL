...
\item Tjark Weber: {\it SMT Solvers: New Oracles for the HOL Theorem
  Prover}.  Published at the Workshop on Verified Software: Theory,
  Tools, and Experiments (VSTTE) 2009.
\item Sascha B{\"o}hme, Tjark Weber: {\it Fast LCF-Style Proof
  Reconstruction for Z3}.  Published at the International Conference
  on Interactive Theorem Proving (ITP) 2010.
\end{itemize}
...
fail).  As with other tactics, \ml{Tactical.TAC\_PROOF} and
\ml{Tactical.prove} can be used to derive functions of type \ml{goal
  -> thm} or \ml{term -> thm} from them.
...
\holtxt{y IN \{x | P x\}} is replaced by \holtxt{P y}.  The resulting
...
\ml{HolSmtLib} has been tested with CVC3~1.5, Yices~1.0.28 and
Z3~2.11.  Later versions of these SMT solvers might work as well;
however, Yices~2 is not supported.  To use \ml{HolSmtLib}, you need to
install at least one of these SMT solvers on your machine.  As
mentioned before, Yices supports a much larger fragment of
higher-order logic than the other solvers, but proof reconstruction
