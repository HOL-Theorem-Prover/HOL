...
each assumption in turn, additionally assuming earlier assumptions to
help in the simplification of later assumptions.  Each assumption is
added back into the goal's assumption list with the tactic
\ml{STRIP\_ASSUME\_TAC}.  This means that assumptions that become
conjunctions will have each conjunct assumed separately.  Assumptions
that become disjunctions, will cause one new sub-goal to be created
for each disjunct.  If an assumption is simplified to false, this will
solve the goal.
