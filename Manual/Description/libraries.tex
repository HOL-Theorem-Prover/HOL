...
caret (\ml{\^}, ASCII character~94).  In order to get a bare caret,
write two carets in a row. (This is analogous to the way in \ML{}
string syntax treats the back-slash.) Thus:
\begin{session}
\begin{hol}
\begin{verbatim}
- ``f ^` x ``;
...
The main use of the caret is to introduce \emph{antiquotations}.
Within a quotation, expressions of the form
{\small\verb+^(+}$t${\small\verb+)+}
%
