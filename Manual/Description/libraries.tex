...
with arguments with variables---so-called \emph{symbolic} evaluation---and 
...
\paragraph{Functions over numbers}

\ml{SUC\_TO\_NUMERAL\_DEFN\_CONV} is used to convert a function
defined by pattern-matching over Peano-style numbers to a function
over numerals, which is the format that \ml{EVAL} prefers. \ml{Define}
will automatically call \ml{SUC\_TO\_NUMERAL\_DEFN\_CONV} on its
result. 
