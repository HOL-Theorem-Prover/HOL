...
%
  ...
  given as \holtxt{thy\$ident}); \ml{APP} (an application of one form
...
special form.  For example, \holtxt{\holquote{\dol{}if~P}} turns into
the \ml{Absyn} form
\begin{verbatim}
...
\end{verbatim}
  ...
  \holtxt{\holquote{\$+~x}}, which generates the abstract syntax
\begin{verbatim}
   APP(IDENT "+", IDENT "x")
\end{verbatim}
  ...
  Thus, the above could be written \holtxt{\holquote{(+)~x}} for the
  same effect.
\index{tokens!suppressing parsing behaviour of|)}
...
function \ml{Unicode.unicode\_version}.  Then, while the trace
variable \ml{"Unicode"} is $0$, the ASCII syntax will be used for
...
\ml{Preterm} will translate into terms.  In essence, bound names turn
...
The most common use-case for the overload map is have names map to
constants.  In this way, for example, the various numeric theories can
map the string \ml{"+"} to the relevant notions of addition, each of
...
string \ml{"<=>"} maps to equality, but where the arguments are forced
to be of type \holquote{:bool}.
...
example, in \theoryimp{boolTheory} and its descendants, the string
\ml{"<>"} is overloaded to the term \holquote{\bs{}x~y.~\td{}(x~=~y)}.
Additionally, \ml{"<>"} is set up at the concrete syntax level as an
infix.  When the user inputs \holquote{x~\lt\gt~y}, the resulting
\ml{Absyn} value is
\begin{verbatim}
...
making otherwise redundant calls to the \ml{overload\_on} function.)
...
\index{Ntimes (controlling rewrite applications)@\ml{Ntimes} (controlling rewrite applications)}
...
When printing terms with type printing turned on, it is sometimes not possible to see the types of bit vector (sub-)expressions, which leads to ambiguity.  This is often the case when word-to-word conversions are present, as in the example below.
\begin{session}
\begin{verbatim}
...
\end{verbatim}
\end{session}
This pretty-printer can be removed with a call to \ml{wordsLib.remove\_word\_cast\_printer}.
...
\end{verbatim}
\end{session}
Of course, type annotations could have been added to avoid this problem entirely.  Note that, unlike \ml{deprecate\_int}, the function \ml{deprecate\_word} does not remove the overloadings, it simply lowers their priority.
...
A basic decision procedure for words is provided in the form of \ml{wordsLib.WORD\_DECIDE}.  This procedure is fairly light-weight, \eg{} it does not employ ``bit-blasting'' techniques for addition or multiplication.  However, it does provide some support for reasoning about word orderings, for example:
\begin{session}
\begin{verbatim}
...
\end{verbatim}
\end{session}
When the trace variable \ml{"word decide"} is set, a limited amount of feedback will be printed.

A custom decision procedure can be constructed using \ml{wordsLib.WORD\_DP}.  In fact, the default decision procedure is just a special instance of this routine, for example:
\begin{session}
\begin{verbatim}
...
\end{verbatim}
\end{session}
The supplied conversion is used to perform initial simplification, the problem is then mapped to the natural numbers and the decision procedure is applied.  With complex goals, the arithmetic decision procedures in \ml{intLib} provide better performance than \ml{bossLib.DECIDE}.
