...
\end{verbatim}
\end{hol}
A theorem ``wrapped'' in the \ml{Once} function will onl be applied
once when the simplifier runs.  A theorem wrapped in \ml{Ntimes} will
be applied as many times as given in the integer parameter.
These restrictions apply across multiple applications of the
simplifier to different goals.  This means that if \ml{tac} in
\begin{hol}
\begin{verbatim}
