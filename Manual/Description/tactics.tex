...
\index{tactics!sequencing of}%
 by use of the
tactical \ml{>>}\footnote{The \ml{>>} function can also be written as \ml{THEN}.}:\index{THEN@\ml{THEN}}\index{>>@\ml{>>}}
if $T_1$ and
...
\item{\bf Some other rewriting tactics} (based on \ml{rewrite\_tac}) are:
\begin{enumerate}
...
\index{>{"|}@\ml{\gt\pipe}~(\ml{THENL})}
\item With {\small\verb%THENL%} (see Section~\ref{sec:thenl});
...
to a particular tactic, namely \ml{DISCH\_TAC} (Section~\ref{sec:disch}).
...
\index{denoting assumptions}%
\index{assumptions!denoting of, in proofs}%
them, for which \ml{ASSUM\_LIST}
