...
not presently possible with the HOL parser.  The
\ml{set\_mapped\_fixity} function also requires us to provide the
``real'' name for the syntax (it's generally helpful to have names
that just include alphanumeric characters), so we pick \verb!redn! as
an abbreviation of ``reduction''.
...
\begin{session}
\begin{verbatim}
- set_fixity {tok = "-||->", term_name = "predn",
...
> val predn_rules =
  ...
  val predn_ind =
    |- !predn'.
         (!x. predn' x x) /\
         (!x y u v. predn' x y /\ predn' u v ==>
                    predn' (x # u) (y # v)) /\
         (!x y. predn' (K # x # y) x) /\
         (!f g x. predn' (S # f # g # x) (f # x # (g # x))) ==>
         !a0 a1. a0 -||-> a1 ==> predn' a0 a1 : thm
  val predn_cases =
    |- !a0 a1.
         a0 -||-> a1 =
         (a1 = a0) \/
...
also how, in establishing these two new relations, we have to use the
underlying symbols' ``real'' names.  First, we create the desired
symbol for the concrete syntax, and then we ``overload'' it so that
the parser will expand it to the desired form.  We can use the simpler
\ml{set\_fixity} function to make our new symbols infix, and not have
to bother about ``real'' names, because we're not defining a real HOL
constant.
\begin{session}
\begin{verbatim}
...
\begin{session}
\begin{verbatim}
e (PROVE_TAC [RTC_RTC, RTCredn_ap_monotonic]);
OK..
Meson search level: .............................
...
\begin{session}
\begin{verbatim}
- e (PROVE_TAC [RTC_rules, predn_RTCredn, RTCredn_RTCredn]);
OK..
Meson search level: .......
