...
There are actually only four different kinds of term in \HOL: variables, constants, function applications (\ml{``$t_1$\ $t_2$``}), and lambda abstractions ({\small\verb|``\|}$x$\ml{.}$t$\ml{``}).
...
For example, in the following case, the \HOL{} type checker used the known type \ml{bool->bool} of boolean negation ({\small\verb|~|}) to deduce that the variable \ml{x} must have type \ml{bool}.
...
Alternatively, it is possible to explicitly indicate the required types by using the notation \ml{``$term$:$type$``}, as illustrated below.
...
For example, the Law of Excluded Middle is bound to the \ML{} name {\small\verb|BOOL_CASES_AX|}:
...
Rules of inference are \ML{} functions that return values of type {\small\verb|thm|}.
...
which takes as arguments a term {\small\verb|``|}$a${\small\verb|``|} and a theorem {\small\verb%|- !%}$x${\small\verb|.|}$t[x]$ and returns the theorem {\small\verb%|- %}$t[a]$, the result of substituting $a$ for $x$ in $t[x]$.
...
section~\ref{chap:euclid}, use \ml{open} to bring declare all of the
