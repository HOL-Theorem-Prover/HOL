...
(terms of type \ml{bool} fulfill that role).
\end{itemize}
...
Since \HOLW{} is backwards compatible, it makes sense to learn \HOLW{} in
...
and lambda abstractions ({\small\verb|``\|}$x$\ml{.}$t$\ml{``}).  More
...
\ml{bool->bool} of boolean negation ({\small\verb|~|}) to deduce that
the variable \ml{x} must have type \ml{bool}.
...
type-checking of another. Thus \ml{x} is not constrained to have the
the type \ml{bool} in the second quotation.)
...
types by using the notation \ml{``$term$:$type$``}, as illustrated below.
...
quantifiers: \ml{!} and \ml{?}, universal and existential quantifiers.
...
\footnote{The \ml{Raise} function passes on all of the exceptions it
