...
\textsf{hol.unquote} and start by loading and opening \ml{bossLib}.
...
{\small\verb|!t. out t = if l5 t then l3 t else l4 t|}
...
pull the assumption that we do have out of the list and rewrite with a
specialised version of it.  We can do just this using
{\small\verb|PAT_ASSUM|}.  This tactic is of type \ml{term -> thm ->
  tactic}.  It selects an assumption that is of the form given by its
term argument, and passes it to the second argument, a function which
expects a theorem and returns a tactic.  Here it is in action:
...
function to the \ml{t} parameter.  In fact, the value that matched
  ...
  this using the \ml{arith\_ss} simpset that comes with \ml{bossLib}
