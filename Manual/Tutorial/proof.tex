...
\end{verbatim}\end{session}
    ...
    \ml{Th8} constitutes a proof in \HOL\ of the theorem \ml{|-\ t1
      ==> (t1 ==> t2) ==> t2}. In stardard logical notation this proof
    could be written:
...
tactics using tacticals (\eg\ see the definition of
...
$T_2$ are tactics, \index{tacticals!for alternation} then
$T_1${\small\verb| ORELSE |}$T_2$ evaluates to a tactic which applies
$T_1$ unless that fails; if it fails, it applies $T_2$. \ml{ORELSE} is
defined in \ML\ as a curried infix by\footnote{This is a minor
  simplification.}
...
\item {\small\verb|simpLib.SIMP_TAC|} of type \ml{simpLib.simpset ->
    thm list -> tactic}.  A \ml{simpset} is a special collection of
    rewriting theorems and other theorem-proving functionality.
    Values defined by \HOL\ include \ml{boolSimps.bool\_ss}, which has
    basic knowledge of the boolean connectives, and
    \ml{HOLSimps.hol\_ss}, which includes theorems appropriate for
    lists, pairs, and arithmetic.  Additional theorems for rewriting
    can be added using the second argument of \ml{SIMP\_TAC}.
\end{enumerate}
\end{itemize}
...
would then solve $g$, where \ml{THENL} \index{THENL@\ml{THENL}} is the
tactical that applies the respective elements of the tactic list to
the subgoals produced by \ml{T}.
