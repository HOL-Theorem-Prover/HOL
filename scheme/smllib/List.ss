(module List (lib "ml.scm" "lang")
  ...
  (define-structure
   ...
   (define null (match-lambda ((list) #t) (_ #f)))
   (define hd (match-lambda ((list) (raise (struct Empty ()))) ((cons x xr) x)))
   (define tl (match-lambda ((list) (raise (struct Empty ()))) ((cons x xr) xr)))
   (define last (match-lambda ((list) (raise (struct Empty ()))) ((cons x (list)) x) ((cons x xr) (last xr))))
   (define nth
     (match-lambda
            ...
            (lambda (_id10546)
              (lambda (_id10547)
                (match
                 ...
                 ((list-no-order (cons '1 (list)) (cons '2 _)) (raise (struct Subscript ())))
            ...
            (lambda (_id10549)
              (lambda (_id10550)
                (match
                 ...
                 ((list-no-order (cons '1 (list)) (cons '2 n)) (raise (struct Subscript ())))
            ...
            (lambda (_id10552)
              (lambda (_id10553)
                (match
                 ...
                 ((list-no-order (cons '1 xs) (cons '2 0)) (list))
                 ((list-no-order (cons '1 (list)) (cons '2 n)) (raise (struct Subscript ())))
           ...
           (lambda (_id10555)
             (lambda (_id10556)
               (match
                ...
                ((list-no-order (cons '1 (list)) (cons '2 k)) k)
   ...
   (match-define
     ...
     (let ()
       ...
       (list (lambda (xs) ((revAcc xs) (list))) (match-lambda ((list-no-order (cons '1 xs) (cons '2 ys)) ((revAcc xs) ys))))))
   (match-define
     (list @)
     (let ()
       (define append
   ...
   (define concat (match-lambda ((list) (list)) ((cons xs xsr) (append xs (concat xsr)))))
   (define app
     (lambda (_id10566)
       (lambda (_id10567)
         (match
          (list (cons '1 _id10566) (cons '2 _id10567))
          ((list-no-order (cons '1 f) (cons '2 (list))) (list))
     ...
     (lambda (_id10572)
       (lambda (_id10573)
         (match
          ...
          ((list-no-order (cons '1 p) (cons '2 (list))) (struct NONE ()))
     ...
     (lambda (_id10574)
       (lambda (_id10575)
         (match
          ...
          ((list-no-order (cons '1 p) (cons '2 (list))) (list))
               ...
               (lambda (_id10577)
                 (lambda (_id10578)
                   (match
                    ...
                    ((list-no-order (cons '1 (list)) (cons '2 are) (cons '3 |aren't|))
       ...
       (lambda (_id10582)
         (lambda (_id10583)
           (match
            ...
            ((list-no-order (cons '1 f) (cons '2 e) (cons '3 (list))) e)
     ...
     (lambda (_id10587)
       (lambda (_id10588)
         (match
          ...
          ((list-no-order (cons '1 p) (cons '2 (list))) #f)
     ...
     (lambda (_id10589)
       (lambda (_id10590)
         (match
          ...
          ((list-no-order (cons '1 p) (cons '2 (list))) #t)
          ((list-no-order (cons '1 p) (cons '2 (cons x xr))) ((match-lambda (#t ((all p) xr)) (#f #f)) (p x)))))))
   (define tabulate
     (match-lambda
       ((list-no-order (cons '1 n) (cons '2 f))
        (let ()
          (define h (lambda (i) (if (< i n) (cons (f i) (h (+ i 1))) (list))))
          (if (< n 0) (raise (struct Size ())) (h 0))))))
   (define getItem (match-lambda ((list) (struct NONE ())) ((cons x xr) (struct SOME ((list (cons '1 x) (cons '2 xr)))))))))
