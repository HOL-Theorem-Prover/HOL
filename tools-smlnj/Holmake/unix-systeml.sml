...
val isUnix = true
(* We set this true as it appears that SML/NJ always provides Word64,
   independent of target-size (see the unconditional appearance of Word64 in
   SML/NJ's (legacy) basis.cm, as opposed to Int{31,63} and Word{31,63}. *)
val haveWord64 = true

(* Adapted from mosml's unix-systeml.sml *)
local val cast : 'a -> int = Unsafe.cast
in
...
val build_log_dir = fullPath [HOLDIR, "tools-smlnj", "build-logs"]
...
fun emit_hol_script _ _ =
    (print "emit_hol_script not yet implemented for SML/NJ\n";
     OS.Process.failure)
end (* local *)
