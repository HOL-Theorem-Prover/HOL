...
open lcsymtacs ieee_arithTheory realLib wordsLib
...
val () = Hol_datatype `float_value = Float of real | Infinity | NaN`
...
val float_add_def = Define`
   float_add mode (x: ('t, 'w) float) (y: ('t, 'w) float) =
      case float_value x, float_value y of
       ...
       | _, Infinity => y
       | Float r1, Float r2 =>
            float_round mode (mode = roundTowardNegative) (r1 + r2)`
...
val exponent_boundary_lt = Q.prove(
   ...
   >- (match_mp_tac ieee_arithTheory.lt_mult
...
val ULP_lt_float_to_real = Q.prove(
   ...
   \\ simp [GSYM realTheory.REAL_LDISTRIB, realTheory.POW_2_LE1,
            ieee_arithTheory.le_mult,
            realLib.REAL_ARITH ``1r <= x /\ 0 <= n ==> 1 <= x + n``]
   )
      ...
      >- (`abs (float_to_real y) <= abs x` by fs [exponent_boundary_def]
          \\ metis_tac
               [realTheory.REAL_LE_TRANS, exponent_boundary_lt,
                ULP_lt_float_to_real, exponent_boundary_not_float_zero,
                realLib.REAL_ARITH
...
val float_sub_finite = Q.store_thm("float_sub_finite",
   `!mode x y r1 r2.
       (float_value x = Float r1) /\ (float_value y = Float r2) ==>
       (float_sub mode x y =
        float_round mode (if x.Sign = y.Sign
                             then mode = roundTowardNegative
                          else x.Sign = 0w) (r1 - r2))`,
   simp [float_sub_def]
   )
