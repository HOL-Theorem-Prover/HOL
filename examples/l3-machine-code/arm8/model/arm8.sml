(* arm8 - generated by L3 - Mon Feb 15 15:14:45 2016 *)
  ...
  in
    ...
    ; case (#EL) (!PSTATE) of
       ...
       | BitsN.B(0x1,2) =>
         ...
         ; if (not(BitsN.bit((!target),55))) andalso ((#TBI0) (!TCR_EL1))
             then target :=
  ...
  in
    if (not aligned) andalso
       ((acctype = AccType_ATOMIC) orelse
        ((acctype = AccType_ORDERED) orelse ((#A)(SCTLR ()))))
      then raise ALIGNMENT_FAULT
    else aligned
  end;
...
fun Replicate N l =
  BitsN.fromBitstring
    (Bitstring.replicate
       (l,Nat.div(BitsN.size(BitsN.B(0x0,N)),L3.length l)),N);
