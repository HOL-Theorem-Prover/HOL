(* mips - generated by L<3> - Wed Oct 29 16:00:41 2014 *)
  ...
  in
    if BigEndianMem ()
      then ( if l = (BitsN.B(0x0,3))
               then MEM :=
                    (Map.update
                       ((!MEM),BitsN.toNat a,BitsN.bits(MemElem,63,56)))
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x1,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,55,48)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x2,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,47,40)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x3,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,39,32)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x4,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,31,24)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x5,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,23,16)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x6,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,15,8)))
                    end
             else ()
           ; if (BitsN.<=+(l,BitsN.B(0x7,3))) andalso
               ...
               then let
                    ...
                    in
                      MEM :=
                      (Map.update
                         ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,7,0)))
                    end
             else ()
           )
             ...
             then let
                  ...
                  in
                    MEM :=
                    (Map.update
                       ((!MEM),BitsN.toNat x,BitsN.bits(MemElem,63,56)))
                  end
           else ()
         ; if (BitsN.<=+(l,BitsN.B(0x6,3))) andalso
              (BitsN.<=+(BitsN.B(0x6,3),h))
         ...
         ; if l = (BitsN.B(0x0,3))
             then MEM :=
                  (Map.update
                     ((!MEM),BitsN.toNat a,BitsN.bits(MemElem,7,0)))
           else ()
         )
  end;
...
fun Fetch () =
  if (BitsN.bits((!PC),1,0)) = (BitsN.B(0x0,2))
    then let
           ...
           val memword =
             BitsN.bits
               (memdoubleword,Nat.+(31,Nat.*(8,BitsN.toNat bytesel)),
...
fun dfn'ERET () =
  if ((#CU0)((#Status) (!CP0))) orelse (KernelMode ())
    then ( if (#ERL)((#Status) (!CP0))
             then ( BranchTo :=
                    (Option.SOME
                       (true,BitsN.-((#ErrorEPC) (!CP0),BitsN.B(0x4,64))))
                  ; let
                      val x0 = (#Status) (!CP0)
                    in
                      CP0 :=
                  ...
                  (Option.SOME
                     (true,BitsN.-((#EPC) (!CP0),BitsN.B(0x4,64))))
                ; let
                    val x0 = (#Status) (!CP0)
                  in
                    CP0 :=
...
fun p_r2 (s,(rs,rt)) =
  case s of
   ...
   | "rge" => OK(Trap(TGE(rs,rt)))
   | "rgeu" => OK(Trap(TGEU(rs,rt)))
   | "rlt" => OK(Trap(TLT(rs,rt)))
   | "rltu" => OK(Trap(TLTU(rs,rt)))
...
fun p_r3 (s,(rd,(rt,rs))) =
  case s of
     "sllv" => OK(Shift(SLLV(rs,(rt,rd))))
   | "srlv" => OK(Shift(SRLV(rs,(rt,rd))))
   | "srav" => OK(Shift(SRAV(rs,(rt,rd))))
   | "dsllv" => OK(Shift(DSLLV(rs,(rt,rd))))
   | "dsrlv" => OK(Shift(DSRLV(rs,(rt,rd))))
   | "dsrav" => OK(Shift(DSRAV(rs,(rt,rd))))
