(* arm - generated by L<3> - Tue Nov 18 15:31:11 2014 *)
...
fun CurrentModeIsUserOrSystem () =
  ...
  ; Set.mem((#M) (!CPSR),[BitsN.B(0x10,5),BitsN.B(0x1F,5)])
  );
           ...
           ((!CPSR),
            BitsN.bitFieldInsert
              (w,
               if Int.>=
                    (res1,
                     if Set.mem(op',[BitsN.B(0x1,2),BitsN.B(0x2,2)])
                       then 0
                     else 65536)
                 then BitsN.B(0x3,2)
    ...
    then BitsN.zeroExtend 64
           (BitsN.concat
              [BitsN.bits(imm8,7,7),BitsN.~(BitsN.bits(imm8,6,6)),
               BitsN.resize_replicate 5 (BitsN.bits(imm8,6,6),5),
               BitsN.bits(imm8,5,0),
               BitsN.resize_replicate 19 (BitsN.B(0x0,1),19)])
  else BitsN.concat
         [BitsN.bits(imm8,7,7),BitsN.~(BitsN.bits(imm8,6,6)),
          BitsN.resize_replicate 8 (BitsN.bits(imm8,6,6),8),
          BitsN.bits(imm8,5,0),
    ...
    if cond = (BitsN.B(0xF,4))
      then case boolify'28(BitsN.bits(w,27,0)) of
          ...
          | (true,
             ...
             (if Do(cond,Set.mem(Extension_VFP,(!Extensions)))
...
fun EncodeVFPImmediate (imm64,single_register) =
  if single_register
    then if ((((BitsN.bits(imm64,18,0)) = (BitsN.B(0x0,19))) andalso
      ...
      (Set.mem(BitsN.bits(imm64,61,54),[BitsN.B(0x0,8),BitsN.B(0xFF,8)]))) andalso
  ...
  case ast of
   ...
   | LoadUnprivileged(add,(postindex,(Rt,(Rn,immediate_form1 imm32)))) =>
      (if ((e = Enc_ARM) andalso postindex) andalso
       ...
       else if (((Set.mem(e,[Enc_Thumb,Enc_Wide])) andalso add) andalso
  ...
  case L3.uncurry String.tokens (fn c => Char.isSpace c,s) of
     [t] =>
      let
        val (l,r) =
          L3.splitl
            (fn c => (Char.isAlphaNum c) orelse (Set.mem(c,[#"_",#"."])),t)
      in
...
fun p_suffix s =
  case L3.uncurry String.fields (fn c => c = #".",s) of
     [cond,s] =>
      (case (p_cond cond,Set.mem(s,["w","n","f32","f64","32","64"])) of
