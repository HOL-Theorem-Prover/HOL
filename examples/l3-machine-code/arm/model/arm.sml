(* arm - generated by L<3> - Fri Nov 21 16:33:11 2014 *)
...
fun SignExtendFrom N (w,p) =
  let
    val s = Nat.-(Nat.-(BitsN.size(BitsN.B(0x0,N)),1),p)
  in
    BitsN.>>(BitsN.<<(w,s),s)
  end;
...
fun SignedSatQ M (i,N) =
  ...
  ; let
    ...
    in
      if Int.>(i,Int.-(max,1))
        then (BitsN.fromInt(Int.-(max,1),M),true)
      else if Int.<(i,Int.~ max)
        then (BitsN.fromInt(Int.~ max,M),true)
...
fun DataProcessing (opc,(setflags,(d,(n,(imm32,C))))) =
  let
    ...
    val (result,(carry,overflow)) =
      DataProcessingALU(opc,(rn,(imm32,(#C) (!CPSR))))
  in
...
fun DataProcessingPC (opc,(setflags,(n,imm32))) =
  let
    ...
    val (result,_) = DataProcessingALU(opc,(rn,(imm32,(#C) (!CPSR))))
  in
    if setflags
      then if CurrentModeIsHyp ()
             then TakeUndefInstrException ()
