(* arm - generated by L3 - Wed Jul 15 10:38:11 2015 *)
...
fun ThumbCondition () =
  if (ITSTATE ()) = (BitsN.B(0x0,8))
    then BitsN.B(0xE,4)
  else if not((BitsN.bits(3,0) ((#IT) (!CPSR))) = (BitsN.B(0x0,4)))
    then BitsN.bits(7,4) ((#IT) (!CPSR))
    ...
    val result =
      BitsN.fromBitstring
        ((Bitstring.fromBool carry_in)
           @
           (Bitstring.bits(Nat.-(BitsN.size(BitsN.B(0x0,N)),1),1)
...
fun doRegisterShiftedRegister (opc,(setflags,(d,(n,(m,(shift_t,s)))))) =
  let
    val (shifted,carry) =
      Shift_C 32
        (R m,(shift_t,(BitsN.toNat(BitsN.bits(7,0) (R s)),(#C) (!CPSR))))
  in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(1,0)
              (w,if Int.>=(res1,0) then BitsN.B(0x3,2) else BitsN.B(0x0,2))))
      end
    ; let
        val w = (#GE) (!CPSR)
      in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(3,2)
              (w,if Int.>=(res2,0) then BitsN.B(0x3,2) else BitsN.B(0x0,2))))
      end
    ; IncPC ()
    )
  end;
  ...
  in
    ...
    ; let
      ...
      in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(0,0) (w,BitsN.fromBit(Int.>=(res1,0)))))
      end
    ; let
        val w = (#GE) (!CPSR)
      in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(1,1) (w,BitsN.fromBit(Int.>=(res2,0)))))
      end
    ; let
        val w = (#GE) (!CPSR)
      in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(2,2) (w,BitsN.fromBit(Int.>=(res3,0)))))
      end
    ; let
        val w = (#GE) (!CPSR)
      in
        ...
        (PSR_GE_rupd
           ((!CPSR),
            BitsN.bitFieldInsert(3,3) (w,BitsN.fromBit(Int.>=(res4,0)))))
      end
    ; IncPC ()
    )
  end;
    ...
    case op' of
       BitsN.B(0x0,2) =>
          (Int.+
           ...
           Int.+
             (UInt 16 (BitsN.bits(31,16) Rn),
              UInt 16 (BitsN.bits(31,16) Rm)))
     | BitsN.B(0x1,2) =>
          (Int.-
...
fun UnsignedParallelAddSub8 (sub,(n,m)) =
  ...
  in
    if sub
      then (Int.-(UInt 8 (BitsN.bits(7,0) Rn),UInt 8 (BitsN.bits(7,0) Rm)),
            (Int.-
    ...
    else (Int.+(UInt 8 (BitsN.bits(7,0) Rn),UInt 8 (BitsN.bits(7,0) Rm)),
          (Int.+
             (UInt 8 (BitsN.bits(15,8) Rn),UInt 8 (BitsN.bits(15,8) Rm)),
           (Int.+
            ...
            (PSR_GE_rupd
               ((!CPSR),
                BitsN.bitFieldInsert(0,0)
                  (w,BitsN.fromBit(Int.>=(res1,ge_lim)))))
          end
        ; let
            val w = (#GE) (!CPSR)
          in
            ...
            (PSR_GE_rupd
               ((!CPSR),
                BitsN.bitFieldInsert(1,1)
                  (w,BitsN.fromBit(Int.>=(res2,ge_lim)))))
          end
        ; let
            val w = (#GE) (!CPSR)
          in
            ...
            (PSR_GE_rupd
               ((!CPSR),
                BitsN.bitFieldInsert(2,2)
                  (w,BitsN.fromBit(Int.>=(res3,ge_lim)))))
          end
        ; let
            val w = (#GE) (!CPSR)
          in
            ...
            (PSR_GE_rupd
               ((!CPSR),
                BitsN.bitFieldInsert(3,3)
                  (w,BitsN.fromBit(Int.>=(res4,ge_lim)))))
          end
        ; IncPC ()
        )
      end
...
fun dfn'UnsignedSumAbsoluteDifferences (d,(n,(m,a))) =
  let
    ...
    val absdiff1 =
      Int.abs
        (Int.-(UInt 8 (BitsN.bits(7,0) Rn),UInt 8 (BitsN.bits(7,0) Rm)))
    val absdiff2 =
      Int.abs
        (Int.-(UInt 8 (BitsN.bits(15,8) Rn),UInt 8 (BitsN.bits(15,8) Rm)))
    val absdiff3 =
      Int.abs
        (Int.-
           (UInt 8 (BitsN.bits(23,16) Rn),UInt 8 (BitsN.bits(23,16) Rm)))
...
fun dfn'BitFieldExtract (unsigned,(d,(n,(lsbit,widthminus1)))) =
  ...
  in
    ( write'R
        (if unsigned
           then BitsN.bits(msbit,lsbit) (R n)
         else SignExtendFrom 32
                (BitsN.bits(msbit,lsbit) (R n),widthminus1),d)
    ; IncPC ()
    )
  end;
   ...
   | ArithLogicImmediate(opc,(setflags,(d,(n,imm12)))) =>
      (if e = Enc_ARM
         ...
         then let
            ...
            (d = n)) andalso ((BitsN.bits(11,8) imm12) = (BitsN.B(0x0,4)))) andalso
          (not(((setflags = (not(c = (BitsN.B(0xE,4))))) orelse
                (BitsN.bit(d,3))) orelse (e = Enc_Wide)))
         then let
  ...
  case instructionFromString s of
   ...
   | WORD w =>
      ("",
       Option.SOME
         (String.concat
            [halfString(BitsN.bits(31,16) w)," ",
             halfString(BitsN.bits(15,0) w)]))
   | PENDING _ => ("Contains label",NONE)
   | FAIL err => (("Parse error: ") ^ err,NONE);
