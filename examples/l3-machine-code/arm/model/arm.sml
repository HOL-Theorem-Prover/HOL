(* arm - generated by L3 - Tue Jan 30 09:00:54 2018 *)
                 ...
                 then if BadMode(BitsN.bits(4,0) value)
                        then raise UNPREDICTABLE
                               ("CPSRWriteByInstr: BadMode: "
                                ...
                                ((BitsN.bits(4,0) value) =
                                 (BitsN.B(0x16,5)))
                               then raise UNPREDICTABLE "CPSRWriteByInstr"
                             else ()
...
fun FPAdd32 (op1,op2) = (L3.snd o FP32.add) (RoundingMode (),(op1,op2));

fun FPSub32 (op1,op2) = (L3.snd o FP32.sub) (RoundingMode (),(op1,op2));

fun FPMul32 (op1,op2) = (L3.snd o FP32.mul) (RoundingMode (),(op1,op2));

fun FPAdd64 (op1,op2) = (L3.snd o FP64.add) (RoundingMode (),(op1,op2));

fun FPSub64 (op1,op2) = (L3.snd o FP64.sub) (RoundingMode (),(op1,op2));

fun FPMul64 (op1,op2) = (L3.snd o FP64.mul) (RoundingMode (),(op1,op2));
...
fun dfn'vabs (dp_operation,(d,m)) =
  ( if dp_operation
      then write'D(FP64.abs(D m),d)
    else write'S(FP32.abs(S m),d)
  ; IncPC ()
  );

fun dfn'vneg (dp_operation,(d,m)) =
  ( if dp_operation
      then write'D(FP64.neg(D m),d)
    else write'S(FP32.neg(S m),d)
  ; IncPC ()
  );

fun dfn'vsqrt (dp_operation,(d,m)) =
  ( if dp_operation
      then write'D((L3.snd o FP64.sqrt) (RoundingMode (),D m),d)
    else write'S((L3.snd o FP32.sqrt) (RoundingMode (),S m),d)
  ; IncPC ()
  );
...
fun dfn'vdiv (dp_operation,(d,(n,m))) =
  ( if dp_operation
      then write'D((L3.snd o FP64.div) (RoundingMode (),(D n,D m)),d)
    else write'S((L3.snd o FP32.div) (RoundingMode (),(S n,S m)),d)
  ; IncPC ()
  );
...
fun dfn'vmla_vmls (dp_operation,(add,(d,(n,m)))) =
  ( if dp_operation
      then let
           ...
           val addend = if add then product else FP32.neg product
         in
...
fun dfn'vfma_vfms (dp_operation,(add,(d,(n,m)))) =
  ( if dp_operation
      then let
             val op64 = D n
             val op64 = if add then op64 else FP64.neg op64
           in
             write'D
               ((L3.snd o FP64.mul_add) (RoundingMode (),(op64,(D m,D d))),
                d)
           end
    else let
           val op32 = S n
           val op32 = if add then op32 else FP32.neg op32
         in
           write'S
             ((L3.snd o FP32.mul_add) (RoundingMode (),(op32,(S m,S d))),d)
         end
  ; IncPC ()
  );
...
fun dfn'vfnma_vfnms (dp_operation,(add,(d,(n,m)))) =
  ( if dp_operation
      then let
           ...
           in
             write'D
               ((L3.snd o FP64.mul_add)
                  (RoundingMode (),(op64,(D m,FP64.neg(D d)))),d)
           end
    else let
           val op32 = S n
           val op32 = if add then op32 else FP32.neg op32
         in
           write'S
             ((L3.snd o FP32.mul_add)
                (RoundingMode (),(op32,(S m,FP32.neg(S d)))),d)
         end
  ; IncPC ()
  );
  ...
  ( if dp_operation
      then let
         ...
         in
           case typ of
              VFPNegMul_VNMLA =>
                write'S(FPAdd32(FP32.neg(S d),FP32.neg product),d)
            | VFPNegMul_VNMLS => write'S(FPAdd32(FP32.neg(S d),product),d)
            | VFPNegMul_VNMUL => write'S(FP32.neg product,d)
         end
  ; IncPC ()
  );
    ...
    (String.concat
       ["Decode ",
        case mc of
         ...
         | ThumbEE opc =>
           (Bitstring.toBinString(BitsN.toBitstring opc)) ^ "; ThumbEE; "
         | Thumb2(opc1,opc2) =>
           String.concat
     ...
     (true,
      (true,
       ...
       let
         ...
         val args = (single_regs,(true,(wback,(d,(Rn,imm8)))))
       in
         (not((!VFPExtension) = NoVFP),
          (unpred,
           VFP(if (BitsN.fromBitstring([L'0],1)) = (BitsN.B(0x1,1))
                 then vldm args
               else vstm args)))
       end
   | (true,
    (true,
     ...
     let
       ...
       val args = (single_regs,(false,(wback,(d,(Rn,imm8)))))
     in
       (not((!VFPExtension) = NoVFP),
        (unpred,
         VFP(if (BitsN.fromBitstring([L'0],1)) = (BitsN.B(0x1,1))
   ...
   | (true,
    (true,
     ...
     let
       ...
       val args =
         (single_reg,
          (add,(d,(BitsN.fromBitstring([Rn'3,Rn'2,Rn'1,Rn'0],4),imm32))))
     in
       (not((!VFPExtension) = NoVFP),
        ("",
         VFP(if (BitsN.fromBitstring([L'0],1)) = (BitsN.B(0x1,1))
   ...
   | (true,
    (true,
     ...
     let
       ...
       val unpred = if Rt = (BitsN.B(0xF,4)) then "vmov_single" else ""
     in
       (not((!VFPExtension) = NoVFP),
        (unpred,VFP(vmov_single(to_arm_register,(Rt,n)))))
     end
   | (true,
    (true,
     (false,
     ...
     let
       ...
       val unpred =
         if (Rt = (BitsN.B(0xF,4))) orelse
           ...
           then "vmov_two_singles"
         else ""
     in
       (not((!VFPExtension) = NoVFP),
        (unpred,VFP(vmov_two_singles(to_arm_registers,(Rt,(Rt2,m))))))
     end
   | (true,
    (true,
     (false,
     ...
     in
       (not((!VFPExtension) = NoVFP),
        (unpred,VFP(vmov_double(to_arm_registers,(Rt,(Rt2,m))))))
     end
   | (true,
    (true,
     (true,
     ...
     in
       if (BitsN.fromBitstring([L'0],1)) = (BitsN.B(0x1,1))
         then (not((!VFPExtension) = NoVFP),("",VFP(vmrs Rt)))
              ...
              val unpred = if Rt = (BitsN.B(0xF,4)) then "msr" else ""
            in
              (not((!VFPExtension) = NoVFP),(unpred,VFP(vmsr Rt)))
            end
     end
      ...
      then case boolify'28(BitsN.bits(27,0) w) of
          ...
          | (true,
           (true,
            ...
            let
              val (defined,(unpred,i)) = DecodeVFP w
            in
              if Do(cond,defined)
                then ( if not(unpred = "")
                         then DECODE_UNPREDICTABLE
...
fun EncodeThumbImmediate x =
  let
    val x0 = x
    val b3 = BitsN.bits(31,24) x0
    val b2 = BitsN.bits(23,16) x0
    val b1 = BitsN.bits(15,8) x0
    val b0 = BitsN.bits(7,0) x0
  in
    case (b3 = (BitsN.B(0x0,8)),
     (b2 = (BitsN.B(0x0,8)),(b1 = (BitsN.B(0x0,8)),b0 = (BitsN.B(0x0,8))))) of
       (true,(true,(true,_))) => Option.SOME(BitsN.@@(BitsN.B(0x0,4),b0))
     | (true,(false,(true,false))) =>
       if b2 = b0 then Option.SOME(BitsN.@@(BitsN.B(0x1,4),b0)) else NONE
     | (false,(true,(false,true))) =>
       if b3 = b1 then Option.SOME(BitsN.@@(BitsN.B(0x2,4),b1)) else NONE
       ...
       in
         if BitsN.<+(n,BitsN.B(0x100,32))
           then Option.SOME
                  (BitsN.@@
                     (BitsN.+
                        (BitsN.-(BitsN.B(0x1F,5),BitsN.fromNat(m,5)),
                  ...
                  in
                    ARM(BitsN.concat
                          [c,BitsN.B(0x1D,5),D,BitsN.B(0x3,2),
                           BitsN.bits(7,4) v'0,Vd,BitsN.B(0x5,3),
                           BitsN.fromBit(not single_register),
                           BitsN.B(0x0,4),BitsN.bits(3,0) v'0])
                  end
...
fun e_system (c,(ast,e)) =
  case ast of
   ...
   | ChangeProcessorState(true,(true,_)) => BadCode "ChangeProcessorState"
   | ChangeProcessorState
     (enable,(disable,(affectA,(affectI,(affectF,changemode))))) =>
     let
...
fun s_load (c,ast) =
  case ast of
   ...
   | LoadExclusiveDoubleword(t,(t2,n)) =>
     ("ldrexd" ^ (s_cond c),String.concat[s_reg2(t,t2),", [",s_reg n,"]"]);
...
fun s_media (c,ast) =
  case ast of
   ...
   | ByteReversePackedHalfword(d,m) => ("rev16" ^ (s_cond c),s_reg2(d,m))
   | ByteReverseSignedHalfword(d,m) => ("revsh" ^ (s_cond c),s_reg2(d,m))
...
fun s_system (c,ast) =
  case ast of
   ...
   | HypervisorCall imm16 => ("hvc" ^ (s_cond c),"#" ^ (s_hex 16 imm16))
   ...
   | SupervisorCall imm32 => ("svc" ^ (s_cond c),"#" ^ (s_hex 32 imm32))
