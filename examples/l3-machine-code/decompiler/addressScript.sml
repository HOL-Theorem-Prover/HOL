...
val ALIGNED_n2w = store_thm("ALIGNED_n2w",
  ...
  \\ `n MOD 4 < 4` by METIS_TAC [DIVISION,DECIDE ``0<4:num``]
...
val ALIGNED_and_1 = store_thm("ALIGNED_and_1",
  ...
  \\ REWRITE_TAC [GSYM (EVAL ``2*2:num``),MULT_ASSOC]
...
val ADDR32_ADD_EQ_ZERO = store_thm("ADDR32_ADD_EQ_ZERO",
  ...
  \\ FULL_SIMP_TAC (bool_ss++SIZES_ss) [w2n_n2w,LESS_MOD,EVAL ``4 * 1073741824:num``]
  \\ DECIDE_TAC);
...
val word_add_and_3w = store_thm("word_add_and_3w",
  ...
  \\ `n = n DIV 4 * 4 + n MOD 4` by METIS_TAC [DIVISION,EVAL ``0<4:num``]
...
val word32_add_n2w = store_thm("word32_add_n2w",
  ...
  \\ `n' MOD 4294967296 < 4294967296` by METIS_TAC [arithmeticTheory.DIVISION,EVAL ``0 < 4294967296n``]
...
val ALIGNED_BITS = store_thm("ALIGNED_BITS",
  ...
  \\ FULL_SIMP_TAC (std_ss++SIZES_ss) [ALIGNED_n2w,bitTheory.BIT_def] THENL [
    ...
    \\ ASM_SIMP_TAC std_ss []
    \\ REWRITE_TAC [GSYM (EVAL ``2*2n``),MULT_ASSOC]
    ...
    \\ `0 < 2n` by EVAL_TAC \\ `?k. n = k * 2` by METIS_TAC [MOD_EQ_0_IMP]
    \\ FULL_SIMP_TAC std_ss [MOD_EQ_0_IMP,MULT_DIV]
    \\ FULL_SIMP_TAC bool_ss [GSYM (EVAL ``2*2n``),MULT_ASSOC]
    \\ FULL_SIMP_TAC bool_ss [GSYM (EVAL ``SUC 1n``),MULT_SUC_EQ]]);
...
val ALIGNED_ADD_EQ_LEMMA = prove(
  ...
  \\ ONCE_REWRITE_TAC [MATCH_MP (GSYM MOD_PLUS) (DECIDE ``0<4n``)]
...
val ALIGNED_MOD_4 = store_thm("ALIGNED_MOD_4",
  ...
  NTAC 2 STRIP_TAC
  \\ STRIP_ASSUME_TAC (Q.SPEC `n` (MATCH_MP DA (DECIDE ``0 < 4n``)))
...
val extract_byte = store_thm("extract_byte",
  ...
  \\ REWRITE_TAC [GSYM (EVAL ``256 * 16777216n``)]
  \\ SIMP_TAC bool_ss [arithmeticTheory.MOD_MULT_MOD,EVAL ``0 < 256n``,
       EVAL ``0 < 16777216n``]);
...
val w2w_eq_n2w = store_thm("w2w_eq_n2w",
  ...
  THEN IMP_RES_TAC (DECIDE ``k < 256 ==> k < 4294967296n``)
