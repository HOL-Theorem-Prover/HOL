(* x64 - generated by L<3> - Mon Mar 17 14:22:23 2014 *)
...
datatype Zinst =
    Zdec_fail of string
  | Zfull_inst of (BitsN.nbit list) * (instruction * (BitsN.nbit list))
...
val ICACHE = ref (Map.mkMap(SOME 18446744073709551616,NONE))
  : (BitsN.nbit option Map.map) ref

val MEM = ref (Map.mkMap(SOME 18446744073709551616,NONE))
  : (BitsN.nbit option Map.map) ref
...
fun mem8 addr =
  case Map.lookup((!MEM),BitsN.toNat addr) of
     Option.SOME b => b
   | NONE => raise BadMemAccess addr;

fun write'mem8 (b,addr) =
  if Option.isSome(Map.lookup((!MEM),BitsN.toNat addr))
    then MEM := (Map.update((!MEM),BitsN.toNat addr,Option.SOME b))
  else raise BadMemAccess addr;

fun mem16 addr = BitsN.@@(mem8(BitsN.+(addr,BitsN.B(0x1,64))),mem8 addr);

fun write'mem16 (w,addr) =
  ( write'mem8(BitsN.bits(w,7,0),addr)
  ; let
      val x = BitsN.+(addr,BitsN.B(0x1,64))
    in
      write'mem8(BitsN.bits(w,15,8),x)
    end
  );
...
fun write'mem32 (w,addr) =
  ( write'mem16(BitsN.bits(w,15,0),addr)
  ; let
      val x = BitsN.+(addr,BitsN.B(0x2,64))
    in
      write'mem16(BitsN.bits(w,31,16),x)
    end
  );
...
fun write'mem64 (w,addr) =
  ( write'mem32(BitsN.bits(w,31,0),addr)
  ; let
      val x = BitsN.+(addr,BitsN.B(0x4,64))
    in
      write'mem32(BitsN.bits(w,63,32),x)
    end
  );
...
fun EA ea =
  case ea of
   ...
   | Zea_r(Z8 have_rex,r) =>
      BitsN.&&
        (if have_rex orelse (not(L3.mem(r,[RSP,RBP,RSI,RDI])))
              ...
              in
                ...
                (Map.update
                   ((!REG),Cast.ZregToNat r,
                    BitsN.bitFieldInsert(w0,BitsN.bits(w,7,0),7,0)))
              end
       else let
            ...
            in
              ...
              (Map.update
                 ((!REG),Cast.ZregToNat x,
                  BitsN.bitFieldInsert(w0,BitsN.bits(w,7,0),15,8)))
            end)
   | Zea_r(Z16,r) =>
      let
        val w0 = Map.lookup((!REG),Cast.ZregToNat r)
      in
        ...
        (Map.update
           ((!REG),Cast.ZregToNat r,
            BitsN.bitFieldInsert(w0,BitsN.bits(w,15,0),15,0)))
      end
   | Zea_r(Z32,r) =>
      REG :=
      (Map.update
         ((!REG),Cast.ZregToNat r,BitsN.zeroExtend 64 (BitsN.bits(w,31,0))))
   | Zea_r(Z64,r) => REG := (Map.update((!REG),Cast.ZregToNat r,w))
   | Zea_m(Z8 _,a) => write'mem8(BitsN.bits(w,7,0),a)
   | Zea_m(Z16,a) => write'mem16(BitsN.bits(w,15,0),a)
   | Zea_m(Z32,a) => write'mem32(BitsN.bits(w,31,0),a)
...
fun write_PF w = write'PF(ByteParity(BitsN.bits(w,7,0)));
    ...
    ( case (size1,size2) of
         (Z8 _,Z16) =>
          ...
          (BitsN.bitFieldInsert
             ((!v),BitsN.signExtend 16 (BitsN.bits(w,7,0)),15,0))
       | (Z8 _,Z32) =>
          v :=
          (BitsN.bitFieldInsert
             ((!v),BitsN.signExtend 32 (BitsN.bits(w,7,0)),31,0))
       | (Z8 _,Z64) => v := (BitsN.signExtend 64 (BitsN.bits(w,7,0)))
       | (Z16,Z32) =>
            v :=
            (BitsN.bitFieldInsert
               ((!v),BitsN.signExtend 32 (BitsN.bits(w,15,0)),31,0))
       | (Z16,Z64) => v := (BitsN.signExtend 64 (BitsN.bits(w,15,0)))
       | (Z32,Z64) => v := (BitsN.signExtend 64 (BitsN.bits(w,31,0)))
...
fun maskShift (size,w) =
  if size = Z64
    then BitsN.toNat(BitsN.bits(w,5,0))
  else BitsN.toNat(BitsN.bits(w,4,0));
...
fun ROL (size,(x,y)) =
  case size of
     Z8 _ =>
      BitsN.fromNat
        (BitsN.toNat
           (BitsN.#<<(BitsN.bits(x,7,0),BitsN.toNat(BitsN.bits(y,4,0)))),
         64)
   | Z16 =>
        BitsN.fromNat
          (BitsN.toNat
             (BitsN.#<<(BitsN.bits(x,15,0),BitsN.toNat(BitsN.bits(y,4,0)))),
           64)
   | Z32 =>
        BitsN.fromNat
          (BitsN.toNat
             (BitsN.#<<(BitsN.bits(x,31,0),BitsN.toNat(BitsN.bits(y,4,0)))),
           64)
   | Z64 => BitsN.#<<(x,BitsN.toNat(BitsN.bits(y,5,0)));
...
fun SAR (size,(x,y)) =
  case size of
     Z8 _ =>
      BitsN.fromNat
        (BitsN.toNat
           (BitsN.>>(BitsN.bits(x,7,0),BitsN.toNat(BitsN.bits(y,4,0)))),64)
   | Z16 =>
        BitsN.fromNat
          (BitsN.toNat
             (BitsN.>>(BitsN.bits(x,15,0),BitsN.toNat(BitsN.bits(y,4,0)))),
  ...
  case bop of
   ...
   | Zsbb =>
        ...
        in
          ( write'CF
              (Nat.<
                 (BitsN.toNat x,Nat.+(BitsN.toNat y,Nat.fromBool carry)))
          ; FlagUnspecified Z_OF
          ; write_arith_result_no_CF_OF(s,(result,ea))
          )
        end
...
fun x64_drop imm =
  ( if not((BitsN.bits(imm,7,0)) = (BitsN.B(0x0,8)))
...
fun dfn'Zcpuid () =
  ( ICACHE := (!MEM)
  ; REG := (Map.update((!REG),Cast.ZregToNat RAX,BitsN.B(0x0,64)))
  ; REG := (Map.update((!REG),Cast.ZregToNat RBX,BitsN.B(0x0,64)))
  ; REG := (Map.update((!REG),Cast.ZregToNat RCX,BitsN.B(0x0,64)))
  ; REG := (Map.update((!REG),Cast.ZregToNat RDX,BitsN.B(0x0,64)))
  );
...
fun dfn'Zdiv (size,rm) =
  let
    ...
    val ea_edx = Zea_r(size,RAX)
    val n = Nat.+(Nat.*(BitsN.toNat(EA ea_eax),w),BitsN.toNat(EA ea_edx))
...
fun Run v0 =
  case v0 of
   ...
   | Zxadd v17 => dfn'Zxadd v17
   | Zxchg v18 => dfn'Zxchg v18;

fun immediate8 strm =
  case strm of
     b :: t => (BitsN.signExtend 64 b,t)
   | _ => (BitsN.B(0x0,64),[]);

fun immediate16 strm =
  case strm of
     b1 :: (b2 :: t) => (BitsN.signExtend 64 (BitsN.@@(b2,b1)),t)
   | _ => (BitsN.B(0x0,64),[]);

fun immediate32 strm =
  case strm of
     b1 :: (b2 :: (b3 :: (b4 :: t))) =>
      (BitsN.signExtend 64 (BitsN.concat[b4,b3,b2,b1]),t)
   | _ => (BitsN.B(0x0,64),[]);

fun immediate64 strm =
  case strm of
     b1 :: (b2 :: (b3 :: (b4 :: (b5 :: (b6 :: (b7 :: (b8 :: t))))))) =>
      (BitsN.concat[b8,b7,b6,b5,b4,b3,b2,b1],t)
   | _ => (BitsN.B(0x0,64),[]);
     ...
     {B = B, R = R, W = W, X = X} =>
      BitsN.modify
        (fn (i,_) =>
           if i = 3
             then W
           else if i = 2 then R else if i = 1 then X else B,BitsN.B(0x0,4));
...
fun readDisplacement (Mod,strm) =
  ...
  else (BitsN.B(0x0,64),strm);

fun readSibDisplacement (w,(Mod,strm)) =
  if (Mod = (BitsN.B(0x0,2))) andalso (not w)
    then (BitsN.B(0x0,64),strm)
  else if Mod = (BitsN.B(0x1,2))
    then immediate8 strm
      ...
      (case (boolify'8 v'0,v'1) of
          ((SS'1,
           ...
           let
             val base =
               RexReg
                 ((#B) REX,BitsN.fromBitstring([Base'2,Base'1,Base'0],3))
             val index =
               RexReg
                 ((#X) REX,
             ...
             val scaled_index =
               if index = RSP
                 then NONE
               else Option.SOME(BitsN.fromBitstring([SS'1,SS'0],2),index)
           in
             if base = RBP
               then let
                      val (displacement,strm2) =
                        readSibDisplacement((#W) REX,(Mod,strm1))
                      val base =
                    ...
                    val (displacement,strm2) = readDisplacement(Mod,strm1)
                  in
                    (Zm(scaled_index,(ZregBase base,displacement)),strm2)
                  end
           end)
   | _ => (Zm(NONE,(ZnoBase,BitsN.B(0x0,64))),strm);
     ...
     v'0 :: v'1 =>
      (case (boolify'8 v'0,v'1) of
        ...
        | ((true,(true,(REG'2,(REG'1,(REG'0,(RM'2,(RM'1,RM'0))))))),strm1) =>
           (RexReg((#R) REX,BitsN.fromBitstring([REG'2,REG'1,REG'0],3)),
            (Zr(RexReg((#B) REX,BitsN.fromBitstring([RM'2,RM'1,RM'0],3))),
             strm1))
        | ((Mod'1,
           ...
           in
             (RexReg
              ...
              (Zm(NONE,
                  (ZregBase
                     (RexReg
                        ((#B) REX,BitsN.fromBitstring([RM'2,RM'1,RM'0],3))),
                   displacement)),strm2))
           end)
   | _ => (RAX,(Zm(NONE,(ZnoBase,BitsN.B(0x0,64))),strm));
...
fun prefixGroup b =
  case b of
   ...
   | BitsN.B(0x66,8) => 3
   | BitsN.B(0x67,8) => 4
   | _ => (if (BitsN.bits(b,7,4)) = (BitsN.B(0x4,4)) then 5 else 0);
     ...
     h :: strm1 =>
      ...
      in
        if group = 0
          ...
          then Option.SOME(p,(true,(rec'REX(BitsN.bits(h,3,0)),strm1)))
        else if L3.mem(group,s)
          then NONE
        else readPrefix((op ::) (group,s),(h :: p,strm1))
      end
   | [] =>
        Option.SOME
...
fun x64_decode strm =
  case readPrefixes strm of
   ...
   | Option.SOME(p,(have_rex,(REX,strm1))) =>
      let
        val prefixes = L3.mkSet p
        val op_size_override = L3.mem(BitsN.B(0x66,8),prefixes)
      in
        if ((#W) REX) andalso op_size_override
          then Zdec_fail("REX.W together with override prefix")
        else if L3.mem(BitsN.B(0x67,8),prefixes)
                 ...
                 (case (boolify'8 v'0,v'1) of
                   ...
                   | ((true,
                      ...
                      let
                        ...
                        val (imm,strm4) = immediate(size,strm3)
                        val binop =
                   ...
                   | ((true,
                      ...
                      let
                        val size =
                          OpSize
                            (false,
                             ((#W) REX,(BitsN.B(0x1,1),op_size_override)))
                        val (opcode,(rm,strm3)) =
                          readOpcodeModRM(REX,strm2)
                        val (imm,strm4) = immediate8 strm3
                        val binop =
                          (Cast.natToZbinop_name o BitsN.toNat) opcode
                      in
                        Zfull_inst
                   ...
                   | ((true,
                      ...
                      in
                        Zfull_inst
                          (p,(Zmov(Z_ALWAYS,(size,src_dst)),strm3))
                      end
                   | ((true,
                   ...
                   | ((true,(false,(false,(true,(false,(r'2,(r'1,r'0))))))),
                      ...
                      let
                        ...
                        val reg =
                          RexReg
                            ((#B) REX,BitsN.fromBitstring([r'2,r'1,r'0],3))
                      in
                        if reg = RAX
                          then Zfull_inst(p,(Znop,strm2))
                        else Zfull_inst
                               (p,(Zxchg(size,(Zr RAX,reg)),strm2))
                      end
                   | ((true,
                      ...
                      in
                        Zfull_inst
                          (p,
                           (Zbinop(Ztest,(size,Zrm_i(Zr RAX,imm))),strm3))
                      end
                   ...
                   | ((true,
                      ...
                      (if (BitsN.fromBitstring([v'0],1)) =
                       ...
                       else Zfull_inst(p,(Zret(BitsN.B(0x0,64)),strm2)))
                   | ((true,
                   ...
                   | ((true,
                      ...
                      strm2) => Zfull_inst(p,(Zleave,strm2))
                   | ((true,
                        ...
                        case opcode of
                           BitsN.B(0x0,3) =>
                              ...
                              in
                                Zfull_inst
                                  (p,
                                   (Zbinop(Ztest,(size,Zrm_i(rm,imm))),
                                    strm4))
                              end
                         | BitsN.B(0x2,3) =>
                   ...
                   | ((true,(true,(true,(true,(true,(true,(true,true))))))),
                      ...
                      in
                        case opcode of
                         ...
                         | BitsN.B(0x4,3) => Zfull_inst(p,(Zjmp rm,strm3))
                         | BitsN.B(0x6,3) =>
                   ...
                   | ((false,
                      ...
                      (case boolify'8 opc of
                        ...
                        | (true,
                             ...
                             Zfull_inst(p,(Zcpuid,strm2))
                        | (true,
                           (false,
                            (true,(true,(false,(false,(false,v'0))))))) =>
                           let
                             val size =
                               OpSize
                                 (have_rex,
                        ...
                        | (true,
                           (true,
                            (false,(false,(false,(false,(false,v'0))))))) =>
                           let
                             val size =
                               OpSize
                                 (have_rex,
                   ...
                   | ((opc'7,
                       (opc'6,
                        (opc'5,(opc'4,(opc'3,(opc'2,(opc'1,opc'0))))))),_) =>
                      Zdec_fail
                        (("Unsupported opcode: ")
                           ^
                           (BitsN.toHexString
...
fun checkIcache n =
  L3.for
    (0,Nat.-(n,1),
     fn i =>
       let
         val addr = BitsN.+((!RIP),BitsN.fromNat(i,64))
       in
         if not((Map.lookup((!MEM),BitsN.toNat addr)) =
                (Map.lookup((!ICACHE),BitsN.toNat addr)))
           then raise FAILURE ("icache miss")
         else ()
       end);

fun x64_next () =
  case x64_decode(x64_fetch ()) of
     Zfull_inst(_,(i,strm1)) =>
      let
        val len = Nat.-(20,List.length strm1)
      in
        ( checkIcache len
        ; RIP := (BitsN.+((!RIP),BitsN.fromNat(len,64)))
        ; Run i
        )
      end
...
fun e_imm8 imm =
  let
    val top = BitsN.bits(imm,63,8)
  in
    if (top = (BitsN.B(0x0,56))) orelse
       ((BitsN.bit(imm,7)) andalso (top = (BitsN.neg(BitsN.B(0x1,56)))))
      then [BitsN.bits(imm,7,0)]
    else []
  end;

fun e_imm16 imm =
  let
    val top = BitsN.bits(imm,63,16)
  in
    if (top = (BitsN.B(0x0,48))) orelse
       ((BitsN.bit(imm,15)) andalso (top = (BitsN.neg(BitsN.B(0x1,48)))))
      then [BitsN.bits(imm,7,0),BitsN.bits(imm,15,8)]
    else []
  end;

fun e_imm32 imm =
  let
    val top = BitsN.bits(imm,63,32)
  in
    if (top = (BitsN.B(0x0,32))) orelse
       ((BitsN.bit(imm,31)) andalso (top = (BitsN.neg(BitsN.B(0x1,32)))))
      then [BitsN.bits(imm,7,0),BitsN.bits(imm,15,8),
            BitsN.bits(imm,23,16),BitsN.bits(imm,31,24)]
    else []
  end;

fun e_imm64 imm =
  [BitsN.bits(imm,7,0),BitsN.bits(imm,15,8),BitsN.bits(imm,23,16),
   BitsN.bits(imm,31,24),BitsN.bits(imm,39,32),BitsN.bits(imm,47,40),
   BitsN.bits(imm,55,48),BitsN.bits(imm,63,56)];
    ...
    if s = 2
      then (4,
            l
              @
              (if BitsN.bit(imm,15)
               then [BitsN.B(0xFF,8),BitsN.B(0xFF,8)]
             else [BitsN.B(0x0,8),BitsN.B(0x0,8)]))
    else (s,l)
  end;
...
fun e_ModRM (r,rm) =
  case rm of
     Zm(NONE,(ZripBase,displacement)) =>
      (BitsN.concat[BitsN.B(0x0,1),BitsN.bits(r,3,3),BitsN.B(0x0,2)],
       (BitsN.concat[BitsN.B(0x0,2),BitsN.bits(r,2,0),BitsN.B(0x5,3)])
         ::
   ...
   | Zr rm =>
      ...
      in
        (BitsN.concat
           [BitsN.B(0x0,1),BitsN.bits(r,3,3),BitsN.B(0x0,1),
            BitsN.bits(rm,3,3)],
         [BitsN.concat
            [BitsN.B(0x3,2),BitsN.bits(r,2,0),BitsN.bits(rm,2,0)]])
      end
   | Zm(NONE,(ZnoBase,imm)) =>
      (case e_imm32 imm of
          [] => (BitsN.B(0x0,4),[])
        | l =>
           (BitsN.concat[BitsN.B(0x0,1),BitsN.bits(r,3,3),BitsN.B(0x0,2)],
            [BitsN.concat[BitsN.B(0x0,2),BitsN.bits(r,2,0),BitsN.B(0x4,3)],
             BitsN.B(0x25,8)]
              @
              l))
   | Zm(Option.SOME(ss,index),(ZnoBase,imm)) =>
      ...
      in
        case e_imm32 imm of
         ...
         | l =>
            (BitsN.concat
               [BitsN.B(0x0,1),BitsN.bits(r,3,3),BitsN.bits(i,3,3),
                BitsN.B(0x0,1)],
             [BitsN.concat
                [BitsN.B(0x0,2),BitsN.bits(r,2,0),BitsN.B(0x4,3)],
              BitsN.concat[ss,BitsN.bits(i,2,0),BitsN.B(0x5,3)]]
               @
               l)
      end
          ...
          then (BitsN.concat
                ...
                [BitsN.concat
                   [case s of
                       0 => BitsN.B(0x0,2)
                     | 1 => BitsN.B(0x1,2)
                     | _ => BitsN.B(0x2,2),BitsN.bits(r,2,0),
                    BitsN.B(0x4,3)],BitsN.concat[ss,BitsN.bits(i,2,0),b20]]
                  @
                  l)
        else (BitsN.B(0x0,4),[])
      end
   | Zm(NONE,(ZregBase base,imm)) =>
      let
        ...
        val base20 = BitsN.bits(base,2,0)
        val (s,l) =
          if (imm = (BitsN.B(0x0,64))) andalso
          ...
          then (BitsN.concat
                ...
                ((BitsN.concat
                    [case s of
                        0 => BitsN.B(0x0,2)
                      | 1 => BitsN.B(0x1,2)
                      | _ => BitsN.B(0x2,2),BitsN.bits(r,2,0),base20])
                   ::
...
fun e_opc (opc1,(opc2,rm)) =
  case e_ModRM(BitsN.fromNat(BitsN.toNat opc2,4),rm) of
     (_,[]) => []
   | (rex,strm) => (rex_prefix rex) @ (opc1 :: strm);
...
fun encode i =
  case i of
     Zbinop(Ztest,(sz,Zrm_r(rm,r))) =>
      e_rm_reg
        (sz,(rm,(BitsN.fromNat(Cast.ZregToNat r,4),([],BitsN.B(0x84,8)))))
   | Zbinop(Ztest,(sz,Zrm_i(Zr RAX,imm))) =>
      e_rax_imm(sz,(imm,BitsN.B(0xA8,8)))
   | Zbinop(Ztest,(sz,Zrm_i(rm,imm))) =>
      e_rm_imm(sz,(rm,(imm,(BitsN.B(0x0,4),BitsN.B(0xF6,8)))))
        ...
        if BitsN.bit(opc,3)
          then if imm = (BitsN.B(0x1,64))
                 then e_rm_reg
                      ...
                      (sz,
                       (rm,
                        (imm,
                         (BitsN.&&(opc,BitsN.B(0x7,4)),BitsN.B(0xC0,8)))))
        else if ((not_byte sz) andalso (not((e_imm8 imm) = []))) andalso
           (not(BitsN.bit(imm,7)))
          ...
          then e_rax_imm
                 (sz,
                  (imm,
                   BitsN.concat
                     [BitsN.B(0x0,2),BitsN.bits(opc,2,0),BitsN.B(0x4,3)]))
               ...
               (sz,
                (rm,
                 (BitsN.fromNat(Cast.ZregToNat r,4),
                  ([],
                   BitsN.concat
                     [BitsN.B(0x0,2),BitsN.bits(opc,2,0),BitsN.B(0x0,3)]))))
      end
               ...
               (sz,
                (rm,
                 (BitsN.fromNat(Cast.ZregToNat r,4),
                  ([],
                   BitsN.concat
                     [BitsN.B(0x0,2),BitsN.bits(opc,2,0),BitsN.B(0x2,3)]))))
      end
   ...
   | Zcpuid => [BitsN.B(0xF,8),BitsN.B(0xA2,8)]
   | Zdiv(sz,rm) =>
      e_rm_reg(sz,(rm,(BitsN.B(0x6,4),([],BitsN.B(0xF6,8)))))
   | Zjcc(cond,imm) =>
      let
        val (s,l) = e_imm_no_16 imm
      in
        case (s,cond) of
           (1,Z_ALWAYS) => (BitsN.B(0xEB,8)) :: l
         | (4,Z_ALWAYS) => (BitsN.B(0xE9,8)) :: l
         | (1,_) =>
            (BitsN.@@
   ...
   | Zlea(sz,Zr_rm(r,Zm m)) =>
      e_rm_reg
        (sz,
         (Zm m,(BitsN.fromNat(Cast.ZregToNat r,4),([],BitsN.B(0x8D,8)))))
   | Zlea _ => []
   ...
   | Zmovzx(sz1,(Zr_rm(r,rm),sz2)) =>
      (if (Nat.<(Zsize_width sz1,Zsize_width sz2)) andalso
          (not(sz1 = Z32))
         then let
                val v =
                  if sz1 = Z16 then BitsN.B(0x1,8) else BitsN.B(0x0,8)
   ...
   | Zpop(Zr reg) =>
      ...
      in
        (if BitsN.bit(r,3) then [BitsN.B(0x49,8)] else [])
          @
          [BitsN.@@(BitsN.B(0xB,5),BitsN.bits(r,2,0))]
      end
   ...
   | Zpush(Zrm(Zr reg)) =>
      ...
      in
        (if BitsN.bit(r,3) then [BitsN.B(0x49,8)] else [])
          @
          [BitsN.@@(BitsN.B(0xA,5),BitsN.bits(r,2,0))]
      end
   ...
   | Zxchg(Z8 h,(rm,r)) =>
      e_rm_reg
        (Z8 h,
         (rm,(BitsN.fromNat(Cast.ZregToNat r,4),([],BitsN.B(0x86,8)))))
   | Zxchg(sz,(Zr RAX,reg)) =>
      let
        val r = BitsN.fromNat(Cast.ZregToNat reg,4)
        val rex =
      ...
      in
        if v = (BitsN.B(0x1,8))
          then prefixes @ [BitsN.@@(BitsN.B(0x12,5),BitsN.bits(r,2,0))]
        else []
      end
   | Zxchg(sz,(Zr reg,RAX)) =>
      let
   ...
   | Zxchg(sz,(rm,r)) =>
      e_rm_reg
        (sz,(rm,(BitsN.fromNat(Cast.ZregToNat r,4),([],BitsN.B(0x86,8)))));
  ...
  case String.explode(stripSpaces s) of
     #"-" :: t =>
      (case p_number(String.implode t) of
          Option.SOME n => Option.SOME(Int.~(Nat.toInt n))
        | NONE => NONE)
   | #"+" :: t =>
      (case p_number(String.implode t) of
...
fun p_imm8 s =
  case p_signed_number s of
     Option.SOME n =>
      (if (Int.<=(Int.~ 128,n)) andalso (Int.<=(n,255))
...
fun p_imm16 s =
  case p_signed_number s of
     Option.SOME n =>
      (if (Int.<=(Int.~ 32768,n)) andalso (Int.<=(n,65535))
...
fun p_imm32 s =
  case p_signed_number s of
     Option.SOME n =>
      (if (Int.<=(Int.~ 2147483648,n)) andalso (Int.<=(n,4294967295))
...
fun p_imm64 s =
  case p_signed_number s of
     Option.SOME n =>
      (if (Int.<=(Int.~ 9223372036854775808,n)) andalso
...
fun p_rip_disp s =
  let
    val (l,r) =
      L3.splitl(fn c => not(L3.mem(c,[#"+",#"-"])),stripLeftSpaces s)
  in
    if l = ("")
      then if r = ("")
             then NONE
  ...
  in
    case (String.explode v'0,(v'1,(v'2,v'3))) of
     ...
     | (#"-" :: t,(si,(b,disp))) =>
        let
          val (l,r) =
            L3.splitl(fn c => not(L3.mem(c,[#"+",#"-"])),String.implode t)
        in
                   ...
                   (case p_scale_index l of
                     ...
                     | Option.SOME(n2,r2) =>
                        (if n1 = (BitsN.B(0x0,2))
                           then p_parts
                                  ((Option.SOME(n2,r2),
                                    (Option.SOME r1,disp)),r)
                         else if n2 = (BitsN.B(0x0,2))
                           then p_parts
                                  ((Option.SOME(n1,r1),
                                    (Option.SOME r2,disp)),r)
                         else (NONE,(NONE,NONE))))
                | _ => (NONE,(NONE,NONE)))
           | (Option.SOME imm2,Option.SOME imm1) =>
              p_parts((si,(b,Option.SOME(BitsN.+(imm2,imm1)))),r)
  ...
  let
    val (si,(b,disp)) =
      case p_parts((NONE,(NONE,NONE)),s) of
         (Option.SOME(BitsN.B(0x0,2),r),(NONE,disp)) =>
          (NONE,(Option.SOME r,disp))
       | (Option.SOME(BitsN.B(0x0,2),RSP),(Option.SOME r,disp)) =>
...
fun p_rm_of_size (sz,s) =
  case p_sz_rm s of
   ...
   | ("",(sz2,rm)) =>
      (if sz = sz2
         then ("",rm)
       else (String.concat["expecting a ",s_sz sz," value"],Zr RAX))
   | (err,_) => (err,Zr RAX);
  ...
  case p_sz_rm a of
     ("",(sz,Zr r)) =>
      (case p_imm_of_size(sz,b) of
          Option.SOME imm =>
           (if ((sz = Z64) andalso nfull_imm) andalso
               (not((BitsN.signExtend 64 (BitsN.bits(imm,31,0))) = imm))
      ...
      (case p_register b of
        ...
        | NONE =>
             (case p_imm_of_size(sz,b) of
                 Option.SOME imm =>
                  (if (sz = Z64) andalso
                      (not((BitsN.signExtend 64 (BitsN.bits(imm,31,0))) =
                           imm))
                     then ("syntax error: bad immediate",NONE)
      ...
      (case v'1 of
          v'4 :: v'5 =>
           (case ((String.explode v'4,v'5),String.explode v'0) of
               ((a,[b]),[#"o",#"r"]) => p_binop(Zor,(String.implode a,b))
             | ((a,[b]),[#"a",#"d",#"d"]) =>
                      ...
                      (case p_imm_rm a of
                        ...
                        | (s,_) =>
                           ...
                           in
                             case p_label b of
                                Option.SOME l =>
                                 PENDING(l,Zjcc(c,BitsN.fromBool 64 far))
                              | NONE => FAIL s
                           end)
             ...
             | ((i,[]),#"l" :: (#"o" :: (#"o" :: (#"p" :: cond)))) =>
                ...
                in
                  case p_cond(String.implode cond) of
                     Option.SOME c =>
                      (if L3.mem(c,[Z_ALWAYS,Z_E,Z_NE])
                         then case p_imm8 i of
                                 Option.SOME imm => OK(Zloop(c,imm))
                               | NONE =>
                                    (case p_label i of
                                        Option.SOME l =>
                                         PENDING
                     ...
                     in
                       if (message = ("")) orelse (message = ("any size"))
                         then if (sz1 = Z32) andalso (sz2 = Z64)
                                then FAIL("movsx used instead of movsxd")
...
fun instructionToString i =
  case i of
   ...
   | Zcall x => ("call",s_imm_rm x)
   | Zcmpxchg(sz,(rm,r)) =>
      ("cmpxchg",String.concat[s_rm(sz,rm),", ",s_register(sz,r)])
   | Zcpuid => ("cpuid","")
   | Zdiv sz_rm => ("div",s_sz_rm sz_rm)
   | Zjcc(Z_ALWAYS,imm) => ("jmp",s_qword imm)
   | Zjcc(cond,imm) => (("j") ^ (s_cond cond),s_qword imm)
   ...
   | Zlea _ => ("?lea?","")
   | Zleave => ("leave","")
   | Zloop(Z_NE,imm) => ("loopne",s_qword imm)
   | Zloop(Z_E,imm) => ("loope",s_qword imm)
   | Zloop(Z_ALWAYS,imm) => ("loop",s_qword imm)
   | Zloop _ => ("?loop?","")
   | Zmonop(mop,sz_rm) =>
      (case mop of
          Zdec => "dec"
   ...
   | Zmovsx(Z32,(_,_)) => ("?movsx?","")
   | Zmovsx(Z64,(_,_)) => ("?movsx?","")
   | Zmovsx(sz1,(Zr_rm(r,rm),sz2)) =>
      ("movsx",String.concat[s_register(sz2,r),", ",s_sz_rm(sz1,rm)])
   | Zmovsx _ => ("?movsx?","")
   | Zmovzx(Z32,(_,_)) => ("?movzx?","")
   | Zmovzx(Z64,(_,_)) => ("?movzx?","")
   | Zmovzx(sz1,(Zr_rm(r,rm),sz2)) =>
