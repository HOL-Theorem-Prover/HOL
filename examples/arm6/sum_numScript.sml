...
val GSUM_1 = store_thm("GSUM_1",
  `!m f. GSUM (m,1) f = f m`,
  REWRITE_TAC [ONE,GSUM_def,ADD_CLAUSES]
);
...
val GSUM_MONO2 = prove(
  ...
  Induct_on `n` THEN1 REWRITE_TAC [prim_recTheory.NOT_LESS_0,GSUM_def]
    ...
    THEN POP_ASSUM (fn th =>
           RULE_ASSUM_TAC (REWRITE_RULE [GSUM_ADD,REWRITE_RULE [DECIDE (Term `!a b. a + (b + 1) = SUC a + b`)] th]))
    THEN DECIDE_TAC
);
...
val GSUM_EQUAL_LEM2 = prove(
  ...
  REPEAT STRIP_TAC
    THEN EQ_TAC
    THENL [
       Cases_on `m = n` THEN1 ASM_REWRITE_TAC []
         THEN IMP_RES_TAC (DECIDE (Term `!(a:num) b. ~(a = b) ==> (a < b) \/ (b < a)`))
         THEN ASM_SIMP_TAC arith_ss []
         THEN SPOSE_NOT_THEN STRIP_ASSUME_TAC
         THEN IMP_RES_TAC GSUM_LESS
         THEN DECIDE_TAC,
       PROVE_TAC [GSUM_EQUAL_LEM]
    ]
);
...
val GSUM_FUN_EQUAL = store_thm("GSUM_FUN_EQUAL",
  `!p n f g. (!x. p <= x /\ x < p + n ==> (f x = g x)) ==> (GSUM (p,n) f = GSUM (p,n) g)`,
