...
fun decode_inst l =
  ...
  | [0,0,0,0,0,0,_,_,_,_,_,_,_,_,_,_,_,_,_,_,1,0,0,1,_,_,_,_] => dec_mla_mul l
...
fun mk_mla_mul (Instruction (x,c)) =
 (case x of
    Mla_mul y =>
      subst [``c:condition`` |-> mk_condition c, ``s:bool`` |-> mk_bool (#S y),
             ``rd:word4`` |-> mk_register (#Rd y),
             ``rm:word4`` |-> mk_register (#Rm y),
             ``rs:word4`` |-> mk_register (#Rs y)]
      (if #A y then
         subst [``rn:word4`` |-> mk_register (#Rn y)] ``MLA c s rd rm rs rn``
       else
         ``instruction$MUL c s rd rm rs``)
   ...
   (case strip_comb t of
      (i, [c,s,rd,rm,rs]) =>
         if term_eq i ``instruction$MUL`` then
           Instruction(Mla_mul {A = false, S = dest_bool s,
    ...
    | (i, [c,s,rd,rm,rs,rn]) =>
         if term_eq i ``instruction$MLA`` then
           Instruction(Mla_mul {A = true, S = dest_bool s,
             Rd = dest_register rd, Rm = dest_register rm,
...
fun term_to_arm t =
  ...
  in
    if eqt ``instruction$SWI`` then
      ...
      dest_br t
    else if eqt ``instruction$MUL`` orelse eqt ``instruction$MLA`` then
      dest_mla_mul t
...
in
  fun mla_mul_to_string a (Instruction(x,c)) =
   (case x of
      Mla_mul y =>
        let val h = mnemonic a ((if #A y then "mla" else "mul") ^
                      condition2string c ^ (if #S y then "s" else ""))
        in
          h ^ reg2string (#Rd y) ^ ", " ^ reg2string (#Rm y) ^ ", " ^
          reg2string (#Rs y) ^ (if #A y then ", " ^ reg2string (#Rn y) else "")
        end
    | _ => raise err)
