(* For interactive use
app load ["gcdTheory", "intLib", "word16Lib","ExtendedEuclidTheory"]; 
app load ["ExtendedEuclidTheory", "gcdTheory", "intLib", "word16Lib"];
quietdec := true;
open gcdTheory dividesTheory arithmeticTheory 
     pairTheory intLib integerTheory
     word16Theory word16Lib
...
val prime_axiom = new_axiom("prime_axiom", ``prime 65537``);
...
("minv_Lemma1",
 ...
 ARW [] THEN `!a b. ~(a <= b) ==> ~(0 < b /\ divides a b)` by METIS_TAC [MONO_NOT,DIVIDES_LE] THEN
 `~(0 < b /\ divides a b) = ~(0 < b) \/ ~(divides a b)` by METIS_TAC [DE_MORGAN_THM] THEN
 `(b < a) ==> (~(0 < b) \/ ~divides a b)`by METIS_TAC [NOT_LESS_EQUAL] THEN METIS_TAC []);
...
("minv_Lemma4",
 `!r1:num r2:num u1:int u2:int v1:int v2:int. gcd r1 r2 = 
   gcd (FST(dec(r1, r2, u1, u2, v1, v2))) (FST(SND(dec(r1, r2, u1, u2,v1, v2))))`,
  ARW [dec_def] THENL [METIS_TAC [LESS_IMP_LESS_OR_EQ, gcd_Lemma2],
  METIS_TAC [NOT_LESS, gcd_Lemma1]]);

val minv_Lemma5 = Q.store_thm
("minv_Lemma5",
`!r1 r2 u1 u2 v1 v2. (~((FST (dec (r1,r2,u1,u2,v1,v2)) = 1) \/ (FST (SND (dec (r1,r2,u1,u2,v1,v2))) = 1)
...
("minv_Lemma6",
 `!r1:num r2:num u1:int u2:int v1:int v2:int. gcd r1 r2 = 
  gcd (FST(inv(r1, r2, u1, u2, v1, v2))) (FST(SND(inv(r1, r2, u1, u2,v1, v2))))`,
  ...
  recInduct inv_ind THEN ARW [] THENL[
    ...
    `inv (r1 - r2,r2,u1 - u2,u2,v1 - v2,v2) = inv (0,r2,u1 - u2,u2,v1 - v2,v2)` by METIS_TAC [] THEN
    ARW [inv_def] THEN METIS_TAC [gcd_Lemma1],
    UNDISCH_TAC ``FST (SND (dec (r1,r2,u1,u2,v1,v2))) = 0`` THEN ARW [dec_def],
    `inv (dec (r1,r2,u1,u2,v1,v2))=inv (dec (dec (r1,r2,u1,u2,v1,v2)))` by ARW [minv_Lemma5] THEN
...
("minv_Lemma8",
 ...
 `((ir1 x) = 1) \/ ((ir1 x) = 0) \/ ((ir2 x) = 1) \/ ((ir2 x) =0)` by ARW [i16_Lemma6] THENL[
   ARW [],
...
`a * (b / c * c) = a * (b / c) * c` by METIS_TAC [INT_MUL_ASSOC] THEN ARW [] THEN
`(a * (b / c) * c) % c = 0` by METIS_TAC [INT_MOD_COMMON_FACTOR] THEN
`(a * b - a * (b / c) * c) / c = a * b / c - a * (b / c) * c / c` by ARW [INT_SUB_CALCULATE, INT_ADD_DIV]
THENL [`~(a * (b / c) * c) % c = 0` by METIS_TAC [mode_Lemma1] THEN
  ARW [INT_ADD_DIV] THEN ARW [INT_NEG_LMUL, INT_DIV_RMUL],
  ARW [INT_DIV_RMUL] THEN ARW [INT_SUB_RDISTRIB] THEN ARW [INT_SUB_CALCULATE] THEN 
  `~(a * b / c * c + ~(a * (b / c) * c)) = ~(a * b / c * c) - ( ~(a * (b / c) * c))` by ARW [INT_SUB_LNEG] THEN
  ARW [] THEN ARW [INT_SUB_RNEG] THEN ARW [INT_ADD_ASSOC] THEN
  `a * b + ~(a * (b / c) * c) + ~(a * b / c * c) + a * (b / c) * c = a * b + (~(a * (b / c) * c) + ~(a * b / c * c) + a * (b / c) * c)`
  by METIS_TAC [INT_ADD_ASSOC] THEN ARW [] THEN
  `~(a * (b / c) * c) + ~(a * b / c * c) + a * (b / c) * c = ~(a * (b / c) * c) + (~(a * b / c * c) + a * (b / c) * c)` 
  by METIS_TAC [INT_ADD_ASSOC] THEN ARW [] THEN
  `~(a * (b / c) * c) + (~(a * b / c * c) + a * (b / c) * c) = (~(a * b / c * c) + a * (b / c) * c) + ~(a * (b / c) * c)`
  by METIS_TAC [INT_ADD_COMM] THEN ARW [] THEN
  `~(a * b / c * c) + a * (b / c) * c + ~(a * (b / c) * c) = ~(a * b / c * c) + (a * (b / c) * c + ~(a * (b / c) * c))`
  by METIS_TAC [INT_ADD_ASSOC] THEN ARW [] THEN
  `a * (b / c) * c + ~(a * (b / c) * c) = 0` by METIS_TAC [INT_ADD_RINV] THEN ARW [] THEN
...
("minv_Lemma10",
 `!x. ((0 < x) /\ (x < 65537)) ==> 
  (((int_of_num x) * (piu1 x)) % 65537 = 1) \/ (((int_of_num x) * (piu2 x)) % 65537 = 1)`,
...
`!x. ((0 < x) /\ (x < 65537)) ==> ((0 <= (minv x)) /\ ((minv x) < 65537))`,
 ...
 `((minv x = piu1 x) \/ (minv x = piu2 x))` by METIS_TAC [minv_def, minv_Lemma10]
...
("encode_Lemma2",
 `!x. (x < 65536) ==> (((int_of_num (encode x)) * (minv (encode x))) % 65537 = 1)`,
...
("encode_Lemma3",
 ...
 STRIP_TAC THEN
 `w2n w < 2 ** WL` by METIS_TAC [w2n_LT] THEN
 `2n ** WL = 65536n` by ARW [WL_def, HB_def] THEN
 `w2n w < 65536` by METIS_TAC [] THEN
 ARW [encode_Lemma1]);
...
("wmul_Lemma1",
 ...
 ARW [] THEN `w2n w < 2 ** WL` by METIS_TAC [w2n_LT] THEN
 `2n ** WL = 65536n` by ARW [WL_def, HB_def] THEN
 `w2n w < 65536` by METIS_TAC [] THEN
...
val winv_def = 
 Define 
   `winv (w:word16) = n2w (decode (Num (minv (encode (w2n w)))))`;
...
("wmul_Lemma2",
 `!x. ((0 < x) /\ (x < 65537)) ==> (int_of_num (encode (w2n (n2w (decode (Num x))))) = x)`,
 ARW [w2n_EVAL] THEN
 ARW [MOD_WL_def, WL_def, HB_def] THEN
 ...
 `decode (Num x) MOD 65536 = decode (Num x)` by RES_TAC THENL [
   `decode (Num x) < 65536n` by METIS_TAC [decode_Lemma1] THEN METIS_TAC [LESS_MOD],
...
("wmul_Theorem",
 ...
 `2n ** WL = 65536n` by ARW [WL_def, HB_def] THEN
 `w2n w < 65536` by METIS_TAC [] THEN
 `(0 < (encode (w2n w))) /\ ((encode (w2n w)) < 65537)` by METIS_TAC [encode_Lemma1] THEN
 `(0 < (minv (encode (w2n w)))) /\ ((minv (encode (w2n w))) < 65537)` by METIS_TAC [minv_Corollary3] THEN
 `($& (encode (w2n (n2w (decode (Num (minv (encode (w2n w))))))))) = (minv (encode (w2n w)))` 
 by METIS_TAC [wmul_Lemma2] THEN ARW [] THEN
 `(($& (encode (w2n w)) * minv (encode (w2n w))) % 65537) = 1` by METIS_TAC [wmul_Lemma1] THEN ARW [] THEN
...
("wmul_Lemma3",
 ...
 `w2n x < 2 ** WL` by METIS_TAC [w2n_LT] THEN
 `2n ** WL = 65536n` by ARW [WL_def, HB_def] THEN
 `w2n x < 65536` by METIS_TAC [] THEN
...
("wmul_Lemma4",
 `!x:word16 y:word16. ~((($& (encode (w2n x)) * $& (encode (w2n y))) % 65537) = 0)`,
 `~(65537i = 0)` by intLib.ARITH_TAC THEN
 SPOSE_NOT_THEN STRIP_ASSUME_TAC THEN
 `($& (encode (w2n x)) * $& (encode (w2n y))) = $&((encode (w2n x)) * (encode (w2n y)))` by METIS_TAC [INT_MUL] THEN
 `($& (encode (w2n x) * encode (w2n y))) % 65537 = 0` by METIS_TAC [] THEN
 `$& (encode (w2n x) * encode (w2n y)) % 65537 = $& ((encode (w2n x) * encode (w2n y)) MOD 65537)` by METIS_TAC [INT_MOD] THEN
 ...
 `divides 65537 (encode (w2n x) * encode (w2n y))` by METIS_TAC [divides_def] THEN
 `((0 < (encode (w2n x))) /\ ((encode (w2n x)) < 65537))` by METIS_TAC [encode_Lemma3] THEN
 `gcd (encode (w2n x)) 65537 = 1` by METIS_TAC [minv_Lemma3] THEN
 `divides 65537 (encode (w2n y))` by METIS_TAC [L_EUCLIDES] THEN
 `((0 < (encode (w2n y))) /\ ((encode (w2n y)) < 65537))` by METIS_TAC [encode_Lemma3] THEN
...
("wmul_ASSOC",
 ...
 `(0 <= (($& (encode (w2n x)) * $& (encode (w2n y))) % 65537)) /\                       
   ((($& (encode (w2n x)) * $& (encode (w2n y))) % 65537) < 65537)` by METIS_TAC [INT_MOD_BOUNDS] THEN
 `(0 <= (($& (encode (w2n y)) * $& (encode (w2n z))) % 65537)) /\                       
   ((($& (encode (w2n y)) * $& (encode (w2n z))) % 65537) < 65537)` by METIS_TAC [INT_MOD_BOUNDS] THEN
 `~((($& (encode (w2n x)) * $& (encode (w2n y))) % 65537) = 0)` by METIS_TAC [wmul_Lemma4] THEN
 `~((($& (encode (w2n y)) * $& (encode (w2n z))) % 65537) = 0)` by METIS_TAC [wmul_Lemma4] THEN
 `0 < (($& (encode (w2n x)) * $& (encode (w2n y))) % 65537)` by METIS_TAC [INT_LE_LT] THEN
 `0 < (($& (encode (w2n y)) * $& (encode (w2n z))) % 65537)` by METIS_TAC [INT_LE_LT] THEN
 `($& (encode (w2n (n2w (decode (Num (($& (encode (w2n x)) * $& (encode (w2n y))) % 65537))))))) = 
 (($& (encode (w2n x)) * $& (encode (w2n y))) % 65537)` by METIS_TAC [wmul_Lemma2] THEN
 ARW [] THEN
 `($& (encode (w2n (n2w (decode (Num (($& (encode (w2n y)) * $& (encode (w2n z))) % 65537))))))) =
 (($& (encode (w2n y)) * $& (encode (w2n z))) % 65537)` by METIS_TAC [wmul_Lemma2] THEN
 ARW [] THEN
 `(($& (encode (w2n x)) * ($& (encode (w2n y)) * $& (encode (w2n z))) % 65537) % 65537) =
 (($& (encode (w2n x)) * ($& (encode (w2n y)) * $& (encode (w2n z)))) % 65537)` by METIS_TAC [mode_Lemma2] THEN
 ARW [] THEN
 `($& (encode (w2n x)) * $& (encode (w2n y))) % 65537 * $& (encode (w2n z)) =
    $& (encode (w2n z)) * ($& (encode (w2n x)) * $& (encode (w2n y))) % 65537` by METIS_TAC [INT_MUL_COMM] THEN
 ARW [] THEN
 `(($& (encode (w2n z)) * ($& (encode (w2n x)) * $& (encode (w2n y))) % 65537) % 65537) = 
 (($& (encode (w2n z)) * ($& (encode (w2n x)) * $& (encode (w2n y)))) % 65537)` by METIS_TAC [mode_Lemma2] THEN
 ARW [] THEN
...
("wmul_Mul1",
 ...
 `($& (encode (w2n w)) * $& (encode (w2n 1w))) = $&((encode (w2n w)) * (encode (w2n 1w)))` 
 by METIS_TAC [INT_MUL] THEN ARW [] THEN
 `$& (encode (w2n w) * encode (w2n 1w)) % 65537 = $& ((encode (w2n w) * encode (w2n 1w)) MOD 65537)` 
 by METIS_TAC [INT_MOD] THEN ARW [] THEN
 `w2n 1w = 1n` by ARW [w2n_EVAL, MOD_WL_def, WL_def, HB_def] THEN ARW [] THEN
 `encode 1 = 1` by ARW [encode_def] THEN ARW [] THEN
 `(encode (w2n w)) < 65537` by METIS_TAC [encode_Lemma3] THEN
 `(encode (w2n w) MOD 65537) = (encode (w2n w))` by METIS_TAC [LESS_MOD] THEN ARW [NUM_OF_INT] THEN
 `w2n w < 2 ** WL` by METIS_TAC [w2n_LT] THEN
 `2n ** WL = 65536n` by ARW [WL_def, HB_def] THEN
 `w2n w < 65536` by METIS_TAC [] THEN
 ARW [decode_Lemma3, w2n_ELIM]);  
