...
open HolKernel Parse boolLib bossLib listTheory rich_listTheory bitsTheory
     markerTheory metisLib pairTheory arithmeticTheory numSyntax word128Theory
     Serpent_Reference_UtilityTheory word128Lib;
...
Induct_on `l1` THEN 
RW_TAC list_ss [selParity_def] THENL [
...
val LT_def = Define
`LT w =  transform LTFun word128$HB w`;


val invLT_def = Define
`invLT w =  transform invLTFun word128$HB w`;
...
Induct_on `d` THEN 
RW_TAC arith_ss [transform_def,LET_THM] THEN
RW_TAC arith_ss [word128Theory.WORD_BIT_BOOLOPS,word128Theory.WORD_BIT_LSL,
                 word128Theory.WORD_BIT_def,word_1_bit128,w2n_zero128,
		 BIT_ZERO,w2n_zero128 ]);
...
RW_TAC arith_ss [transform_def,LET_THM] THENL [
	...
	Cases_on `to <=d`  THEN Cases_on `to=d` THENL [
		...
		FULL_SIMP_TAC arith_ss [word128Theory.WORD_BIT_BOOLOPS,
		                        word128Theory.WORD_BIT_def, 
					w2n_zero128, BIT_ZERO,
					DECIDE ``SUC x<y==>x<y``] THEN
		Cases_on `to<=d` THENL [
			FULL_SIMP_TAC arith_ss [],
			`to>d /\ (to = SUC d)` 
				by METIS_TAC [DECIDE ``~(x<=y) /\ 
					               x <= SUC y=(x=SUC y) /\
						       (x>y)``] THEN
...
`!transFun1 transL2 w.
	...
	==>
	(selParity (transform transFun1 word128$HB w) transL2
	 = selParity w (FLAT (MAP transFun1 transL2)))`,

Induct_on `transL2` THEN 
FULL_SIMP_TAC list_ss [MAP,FLAT,selParity_def,transform_def,selParityAppend,
...
`!i transFun1 transFun2 w.
	...
	==>
	(word128$WORD_BIT i (transform transFun2 word128$HB 
	                               (transform transFun1 word128$HB w))
	 = word128$WORD_BIT i (transform (transCompose transFun1 transFun2)
	                                 word128$HB w))`,
...
`!transFun1 transFun2 w.
	...
	transInRange transFun2  
	==>
	(transform transFun2 word128$HB (transform transFun1 word128$HB w) 
	 = transform (transCompose transFun1 transFun2) word128$HB w)`,
	
RW_TAC std_ss[GSYM word128Theory.WORD_EQ,transformComposeBit]);



val TL128_eq_makeTL128=Q.store_thm(
"TL128_eq_makeTL128",
...
`countEvenL tl 
 = countL (T::T::T::T::T::T::T::T::T::T::T::T::T::T::T::T::T::T::
...
RW_TAC std_ss [GSYM Res_def, transCompose_def,
               GSYM countEvenL_countEven_eq,countEvenL_LT_facts,Res_fact,ALL_EL]);
...
`!i transL w.
	...
	(selParity w transL 
	 = if countEven i transL 
	      ...
	      else  boolXor (word128$WORD_BIT i w) 
	                    (selParity w (FILTER (\x. ~(x =i)) transL)))`,
...
Induct_on `transL` THENL [
	...
	RW_TAC std_ss [selParity_def,FILTER,countEven_def,ALL_EL] THENL [
		...
		`(selParity w transL 
		  = (if countEven h transL 
		        then selParity w (FILTER (\x. ~(x = h)) transL)
			else boolXor (WORD_BIT h w) 
			             (selParity w 
				                (FILTER (\x. ~(x = h))
						        transL))))` 
			by METIS_TAC [] THEN
		...
		`(selParity w transL 
		  = (if countEven h transL 
		        then selParity w (FILTER (\x. ~(x = h)) transL)
						 ...
						 transL))))` 
			by METIS_TAC [] THEN
		FULL_SIMP_TAC std_ss [boolXorAssoc,boolXorFacts,
		                      boolXorComm,boolXor_def]]]);	
...
Cases_on `transL1` THEN Cases_on `transL2` THENL [
	...
	`!j. 
		...
		(countEven j (FILTER (\x. ~(x = h)) (h::t)) 
		 = countEven j (h::t))` by RW_TAC std_ss [] THENL [
		Cases_on `j=h` THEN
		METIS_TAC [countEven_def, countEven_filter2,
		           word128Theory.WL_def, DECIDE ``SUC x = x +1``],
		...
		`LENGTH (FILTER (\x. ~(x = h)) t)<=L` 
			by FULL_SIMP_TAC list_ss [] THENL [
			`LENGTH (FILTER (\x. ~(x = h)) t)<=LENGTH t` 
				by FULL_SIMP_TAC list_ss [LENGTH_FILTER] THEN
			FULL_SIMP_TAC arith_ss [],
			`LENGTH ([]:num list)<=L` 
				by RW_TAC arith_ss [LENGTH] THEN
			`ALL_EL (\x. x < word128$WL) t` 
				by FULL_SIMP_TAC list_ss [ALL_EL] THEN
			`!j. 
				j < word128$WL 
				==> 
				(countEven j [] 
				 = countEven j (FILTER (\x. ~(x = h)) t) )`
				 by METIS_TAC [] THEN
			`ALL_EL (\x. x < word128$WL) (FILTER (\x. ~(x = h)) t)`
			...
			`~countEven h t` 
				by FULL_SIMP_TAC std_ss
				                 [countEven_def,boolXor_def] THEN
			`selParity w t 
			 = if countEven h t
			      then selParity w (FILTER (\x. ~(x=h)) t) 
			      else boolXor (word128$WORD_BIT h w) 
			                   (selParity w 
					              (FILTER (\x. ~(x =h)) t))`
	...
	`!j. 
		...
		(countEven j (FILTER (\x. ~(x = h)) (h::t)) 
		 = countEven j (h::t))` by RW_TAC std_ss [] THENL [
		Cases_on `j=h` THEN
		METIS_TAC [countEven_def, countEven_filter2,word128Theory.WL_def,
		           DECIDE ``SUC x = x +1``],
		`!j:num. countEven j []` by METIS_TAC [countEven_def] THEN
		`!j. 
			j < word128$WL 
	...
	`(h < word128$WL) /\ (h' < word128$WL)` 
		by FULL_SIMP_TAC std_ss [ALL_EL] THEN
	`!j. 
		j < word128$WL 
		==> 
		(countEven j (FILTER (\x. ~(x = h')) (h::t)) 
		 = countEven j (FILTER (\x. ~(x = h')) (h'::t')))` 
		 by METIS_TAC [countEven_filter3,word128Theory.WL_def,
		               DECIDE ``SUC x = x +1``] THEN
	`ALL_EL (\x. x < word128$WL) (FILTER (\x. ~(x = h')) (h::t))` 
		by METIS_TAC [ALL_EL_FILTER] THEN
	`ALL_EL (\x. x < word128$WL) (FILTER (\x. ~(x = h')) (h'::t'))`
		by METIS_TAC [ALL_EL_FILTER] THEN 
	...
	`FILTER (\x. ~(x = h)) (FILTER (\x. ~(x = h')) (h'::t'))
	 = FILTER (\x. ~(x = h)) (FILTER (\x. ~(x = h')) t')` 
		 by  RW_TAC list_ss [FILTER] THEN
	`ALL_EL (\x. x < word128$WL) 
	        (FILTER (\x. ~(x = h)) (FILTER (\x. ~(x = h')) (h'::t')))` 
		by METIS_TAC [ALL_EL_FILTER] THEN
	`ALL_EL (\x. x < word128$WL) 
	        (FILTER (\x. ~(x = h)) (FILTER (\x. ~(x = h')) (h::t)))`
	...
	`LENGTH ( FILTER (\x. ~(x = h)) (FILTER (\x. ~(x = h')) t)) <= L`
		...
		`LENGTH (FILTER (\x. ~(x = h)) 
		                (FILTER (\x. ~(x = h')) t)) 
		 <= LENGTH (FILTER (\x. ~(x = h')) t)` 
			by METIS_TAC [LENGTH_FILTER] THEN
		`LENGTH (FILTER (\x. ~(x = h')) t) <= LENGTH t` 
			by METIS_TAC [LENGTH_FILTER] THEN
		METIS_TAC [LESS_EQ_TRANS],
		`LENGTH ( FILTER (\x. ~(x = h)) 
		                 (FILTER (\x. ~(x = h')) t')) 
		 <= L` by FULL_SIMP_TAC list_ss [] THENL [
			`LENGTH (FILTER (\x. ~(x = h)) 
			                (FILTER (\x. ~(x = h')) t')) 
			 <= LENGTH (FILTER (\x. ~(x = h')) t')` 
				by METIS_TAC [LENGTH_FILTER] THEN
			`LENGTH (FILTER (\x. ~(x = h')) t') 
			 <= LENGTH t'` by METIS_TAC [LENGTH_FILTER] THEN
			METIS_TAC [LESS_EQ_TRANS],
			`selParity w 
			           (FILTER (\x. ~(x = h)) 
				           (FILTER (\x. ~(x = h')) (h::t))) 
			...
			Cases_on `countEven h (h::t)` THEN 
			Cases_on `countEven h' (h'::t')` THEN 
			Cases_on `countEven h (h'::t')` THENL [
...
RW_TAC std_ss [GSYM WORD_EQ,invLT_def,LT_def,transformComposeWord,
               LTFunInRange,invLTFunInRange] THEN
`WORD_BIT x w= selParity w [x]` by RW_TAC std_ss [selParity_def,boolXor_def] THEN
FULL_SIMP_TAC arith_ss [transform_inv2_w128,word128Theory.HB_def,
                        word128Theory.WL_def] THEN
`ALL_EL (\x. x < word128$WL) (transCompose LTFun invLTFun x)` 
	by FULL_SIMP_TAC arith_ss [GSYM Res_def,transCompose_def,
	                           word128Theory.WL_def,word128Theory.HB_def] THENL [
	`!i.
		i < word128$WL
		==>
		ALL_EL (\x. x < word128$WL) (Res i)` 
	...
	FULL_SIMP_TAC arith_ss [word128Theory.WL_def,word128Theory.HB_def],
	 `ALL_EL (\x. x < word128$WL) [x]` 
	 	by RW_TAC list_ss [ALL_EL,word128Theory.WL_def,
