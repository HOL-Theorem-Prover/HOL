...
(* For interactive work
  ...
  app load ["arithmeticTheory","metisLib","word32Theory", "listTheory",
            "MARS_SboxTheory","MARS_keyExpansionTheory"];
...
*)
 ...
 open HolKernel Parse boolLib bossLib
      arithmeticTheory word32Theory pairTheory listTheory
      MARS_SboxTheory MARS_keyExpansionTheory MARS_DataTheory;
...
val  en_f_rnd_def = Define
    `en_f_rnd ((A,B,C,D),i) =
        ...
        let  A = (A #>> 24) +
        ((if (i=5) \/ (i=1) then B else word_0) + 
	(if (i=4) \/ (i=0) then D else word_0))
        in (B,C,D,A)`;
...
val  de_b_rnd_def = Define `
  de_b_rnd ((A,B,C,D),i) =
    ...
    let A = A - ((if (i=5) \/ (i=1) then B else word_0) + 
	(if (i=4) \/ (i=0) then D else word_0)) in
    let A = A #<< 24 in
    let D = D # Sbox1(l8b(A #>> 24)) in
    let C = C - Sbox0(l8b(A #>> 16)) in
...
val Fwd_Mix_Inversion = Q.store_thm
  ...
  SIMP_TAC std_ss [FORALL_BLOCK, en_f_rnd_def, de_b_rnd_def, LET_THM] THEN
  `24 < WL` by ARW_TAC [WL_def, HB_def] THEN
  FULL_SIMP_TAC std_ss [WORD_ADD_SUB,SHIFT_Inversion] THEN
...
("Fwd_Mix_LEMMA",
 `!b:block k:keysched. de_b_mix(en_f_mix(b,k),k) = b`,
   SIMP_TAC std_ss [FORALL_BLOCK] THEN RESTR_EVAL_TAC [en_f_rnd, de_b_rnd] THEN
   RW_TAC std_ss [Fwd_Mix_Inversion] THEN METIS_TAC [WORD_ADD_SUB]);
...
val Core_Inversion = Q.store_thm
  ...
  SIMP_TAC std_ss [FORALL_BLOCK] THEN RW_TAC std_ss [en_core_rnd_def] THEN RW_TAC std_ss [de_core_rnd_def] 
  THEN `13 < WL` by ARW_TAC [WL_def, HB_def] THEN `A #>> 13 = v0` by METIS_TAC [SHIFT_Inversion] THEN 
  FULL_SIMP_TAC std_ss [] THEN CONJ_TAC THENL [Q.UNABBREV_TAC `B` THEN 
  METIS_TAC [WORD_ADD_SUB, WORD_EOR_ASSOC, WORD_EOR_INV, WORD_EOR_ID], 
  CONJ_TAC THENL [Q.UNABBREV_TAC `C` THEN METIS_TAC [WORD_ADD_SUB], 
  Q.UNABBREV_TAC `D` THEN METIS_TAC [WORD_ADD_SUB, WORD_EOR_ASSOC, WORD_EOR_INV, WORD_EOR_ID]]] 
  );
...
("Keyed_Trans_LEMMA",
 `!b:block k:keysched. de_core(en_core(b,k),k) = b`,
   SIMP_TAC std_ss [FORALL_BLOCK] THEN RESTR_EVAL_TAC [en_core_rnd, de_core_rnd] THEN
   RW_TAC std_ss [Core_Inversion]);
...
val Bwd_Mix_Inversion = Q.store_thm
  ...
  `24 < WL` by ARW_TAC [WL_def, HB_def] THEN
  RW_TAC list_ss [SHIFT_Inversion] THEN
  METIS_TAC [WORD_SUB_ADD,WORD_EOR_ASSOC, WORD_EOR_INV, WORD_EOR_ID]
  );

val Whitening_Inversion = Q.store_thm
  ("Whitening_Inversion",
  `!b k. PreWhitening(PostWhitening(b,k),k) = b`,
  SIMP_TAC std_ss [FORALL_BLOCK] THEN RW_TAC std_ss [PreWhitening_def, PostWhitening_def] THEN 
...
("Bwd_Mix_LEMMA",
 `!b:block k:keysched. de_f_mix(en_b_mix(b,k),k) = b`,
   SIMP_TAC std_ss [FORALL_BLOCK] THEN RESTR_EVAL_TAC [en_b_rnd, de_f_rnd] THEN
   RW_TAC std_ss [Whitening_Inversion, Bwd_Mix_Inversion]
   );
