...
open set_sepTheory progTheory ppcTheory;
...
val pPC_def  = Define `pPC x  = pR1 (PPC_PC) x * cond (x && 3w = 0w)`;
...
val PPC_INSTR_def    = Define `PPC_INSTR (a,c:string) = 
  { pMem a      (SOME ((31 >< 24) ((b2w I (hex2bits c)):word32))); 
    pMem (a+1w) (SOME ((23 >< 16) ((b2w I (hex2bits c)):word32))); 
    pMem (a+2w) (SOME ((15 ><  8) ((b2w I (hex2bits c)):word32))); 
    pMem (a+3w) (SOME (( 7 ><  0) ((b2w I (hex2bits c)):word32))) }`;

val PPC_MODEL_def = Define `PPC_MODEL = (ppc2set, PPC_NEXT_REL, PPC_INSTR)`;
...
val PPC_SPEC_SEMANTICS = store_thm("PPC_SPEC_SEMANTICS",
  ...
  SIMP_TAC bool_ss [GSYM RUN_EQ_SPEC,RUN_def,PPC_MODEL_def,STAR_def]
...
val CODE_POOL_ppc2set = store_thm("CODE_POOL_ppc2set",
  ``CODE_POOL PPC_INSTR {(p,c)} (ppc2set' (rs,ms,st) (r,s,m)) =
      ({p;p+1w;p+2w;p+3w} = ms) /\ (rs = {}) /\ (st = {}) /\
      (SOME ((31 >< 24) ((b2w I (hex2bits c)):word32)) = m p) /\ 
      (SOME ((23 >< 16) ((b2w I (hex2bits c)):word32)) = m (p+1w)) /\ 
      (SOME ((15 ><  8) ((b2w I (hex2bits c)):word32)) = m (p+2w)) /\ 
      (SOME (( 7 ><  0) ((b2w I (hex2bits c)):word32)) = m (p+3w))``,
  ...
  \\ Cases_on `SOME (( 7 ><  0) ((b2w I (hex2bits c)):word32)) = m (p + 3w)` \\ ASM_REWRITE_TAC []
  \\ Cases_on `SOME ((15 ><  8) ((b2w I (hex2bits c)):word32)) = m (p + 2w)` \\ ASM_REWRITE_TAC []
  \\ Cases_on `SOME ((23 >< 16) ((b2w I (hex2bits c)):word32)) = m (p + 1w)` \\ ASM_REWRITE_TAC []
  \\ Cases_on `SOME ((31 >< 24) ((b2w I (hex2bits c)):word32)) = m (p     )` \\ ASM_REWRITE_TAC []
