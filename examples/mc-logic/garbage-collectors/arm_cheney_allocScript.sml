...
val arm_alloc_gc_lemma = prove(
  ...
  REWRITE_TAC [arm_alloc_gc,cheney_alloc_gc_def] 
  \\ STRIP_TAC \\ STRIP_TAC
...
val arm_alloc_aux_lemma = prove(
  ...
  STRIP_TAC \\ REWRITE_TAC [arm_alloc_aux,cheney_alloc_aux_def] 
...
val arm_alloc_lemma = prove(
  ...
  REWRITE_TAC [cheney_alloc_def,arm_alloc_mem] \\ STRIP_TAC \\ STRIP_TAC
  \\ `~(i = 0) /\ ~(e = 0)` by 
         (Cases_on `u` \\ FULL_SIMP_TAC bool_ss [ok_state_def,LET_DEF] \\ DECIDE_TAC)
...
val ch_word_alloc = prove(
  ...
  SIMP_TAC std_ss [arm_alloc,LET_DEF]
  ...
  \\ `ch_mem (i,e,rs,l,u,m) (a,x,xs1)` by ALL_TAC THENL [
