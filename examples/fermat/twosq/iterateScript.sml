...
val FUNPOW_LINV_permutes = store_thm(
  "FUNPOW_LINV_permutes",
   ...
   Step: FUNPOW (LINV f s) n x IN s ==> FUNPOW (LINV f s) (SUC n) x IN s
         ...
         and (LINV f s) PERMUTES s              by LINV_permutes
          so (LINV f s) (FUNPOW (LINV f s) n x) IN s
                                                by BIJ_ELEMENT
*)
val FUNPOW_LINV_closure = store_thm(
  "FUNPOW_LINV_closure",
...
*)
val FUNPOW_SUB_LINV1 = store_thm(
  "FUNPOW_SUB_LINV1",
...
*)
val FUNPOW_SUB_LINV2 = store_thm(
  "FUNPOW_SUB_LINV2",
  ``!f s x m n. f PERMUTES s /\ x IN s /\ m <= n ==>
    (FUNPOW f (n - m) x = FUNPOW (LINV f s) m (FUNPOW f n x))``,
  rpt strip_tac >>
...
*)
val FUNPOW_LINV_SUB1 = store_thm(
  "FUNPOW_LINV_SUB1",
  ``!f s x m n. f PERMUTES s /\ x IN s /\ m <= n ==>
    (FUNPOW (LINV f s) (n - m) x = FUNPOW (LINV f s) n (FUNPOW f m x))``,
  rpt strip_tac >>
...
*)
val FUNPOW_LINV_SUB2 = store_thm(
  "FUNPOW_LINV_SUB2",
  ``!f s x m n. f PERMUTES s /\ x IN s /\ m <= n ==>
    (FUNPOW (LINV f s) (n - m) x = FUNPOW f m (FUNPOW (LINV f s) n x))``,
  rpt strip_tac >>
...
(* Proof:
   ...
   Otherwise iterate_period f x k returns NONE,
      ...
      = x                         by FUNPOW_0
*)
val iterate_period_property = store_thm(
  "iterate_period_property",
  ``!f x. FUNPOW f (iterate_period f x) x = x``,
  rpt strip_tac >>
...
val iterate_period_minimal = store_thm(
  "iterate_period_minimal",
  ``!f x n. 0 < n /\ n < (iterate_period f x) ==> FUNPOW f n x <> x``,
  ntac 2 strip_tac >>
  simp[iterate_period_def] >>
...
*)
val iterate_period_thm = store_thm(
  "iterate_period_thm",
  ``!f x n. 0 < n ==> ((iterate_period f x = n) <=>
     (FUNPOW f n x = x) /\ !m. 0 < m /\ m < n ==> FUNPOW f m x <> x)``,
  rw[EQ_IMP_THM] >-
...
*)
val iterate_not_distinct = store_thm(
  "iterate_not_distinct",
  ``!f s x. FINITE s /\ f PERMUTES s /\ x IN s ==>
           ?h k. (FUNPOW f h x = FUNPOW f k x) /\ (h <> k)``,
...
This is because the group is (Z_add, +), which is INFINITE.
...
*)
val iterate_period_exists = store_thm(
  "iterate_period_exists",
  ``!f s x. FINITE s /\ f PERMUTES s /\ x IN s ==>
       ?n. 0 < n /\ (n = iterate_period f x)``,
  rpt strip_tac >>
...
val iterate_period_pos = store_thm(
  "iterate_period_pos",
...
*)
val iterate_mod_period = store_thm(
  "iterate_mod_period",
...
val iterate_period_eq_0 = store_thm(
  "iterate_period_eq_0",
...
*)
val iterate_period_eq_1 = store_thm(
  "iterate_period_eq_1",
...
*)
val iterate_period_eq_2 = store_thm(
  "iterate_period_eq_2",
...
*)
val iterate_period_multiple = store_thm(
  "iterate_period_multiple",
  ``!f x n. (FUNPOW f n x = x) <=> ?k. n = k * (iterate_period f x)``,
  rpt strip_tac >>
  qabbrev_tac `p = iterate_period f x` >>
...
*)
val iterate_period_mod = store_thm(
  "iterate_period_mod",
...
*)
val iterate_period_mod_eq = store_thm(
  "iterate_period_mod_eq",
  ``!f s p x i j. FINITE s /\ f PERMUTES s /\ x IN s /\
                (p = iterate_period f x) ==>
                ((FUNPOW f i x = FUNPOW f j x) <=> (i MOD p = j MOD p))``,
...
*)
val iterate_period_funpow_inj = store_thm(
  "iterate_period_funpow_inj",
...
*)
val iterate_period_upper = store_thm(
  "iterate_period_upper",
  ``!f s p x. FINITE s /\ f PERMUTES s /\ x IN s /\
             (p = iterate_period f x) ==> p <= CARD s``,
...
*)
val iterate_period_inv = store_thm(
  "iterate_period_inv",
  ``!f s x. FINITE s /\ f PERMUTES s /\ x IN s ==>
           (iterate_period (LINV f s) x = iterate_period f x)``,
  rpt strip_tac >>
...
val iterate_period_inv_alt = store_thm(
  "iterate_period_inv_alt",
  ``!f s p x. FINITE s /\ f PERMUTES s /\ x IN s /\ (p = iterate_period f x) ==>
             (iterate_period (LINV f s) x = p)``,
...
*)
val iterate_period_iterate = store_thm(
  "iterate_period_iterate",
