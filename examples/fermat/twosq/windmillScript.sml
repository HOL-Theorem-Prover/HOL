...
*)
val windmill_eq_0 = store_thm(
  "windmill_eq_0",
  ``!x y z. (windmill x y z = 0) <=> ((x = 0) /\ (y * z = 0))``,
  metis_tac[windmill_def, EXP_2, ADD_EQ_0, MULT_EQ_0, DECIDE``4 <> 0``]);
...
(* Proof:
   ...
   = 4 * k + 1                  by arithmetic
*)
val windmill_trivial = store_thm(
  "windmill_trivial",
...
(* Proof:
   ...
   = x ** 2 + (2 * y) ** 2        by EXP_2
*)
val windmill_by_squares = store_thm(
  "windmill_by_squares",
...
*)
val windmill_trivial_prime = store_thm(
  "windmill_trivial_prime",
  ``!p. prime p /\ (p MOD 4 = 1) ==>
   !x z. (p = windmill x x z) <=> ((x = 1) /\ (z = p DIV 4))``,
  rpt strip_tac >>
...
(* Note: y * z <> 0 is a result of n is prime, which is not a perfect square. *)
(* Note: better require x <> 0, so that the windmill has a mind! *)
(* Note: really need both x <> 0 and y * z <> 0!
   If y * z = 0 is allowed, mills is INFINITE for perfect squares, with x ** 2 + 0 * k.
   If any multiple of 4 has no mind to change! *)
(* Note: However, any such condition will add extra condition for this one:
   (x,y,z) IN mills n ==> zagier (x,y,z) IN mills n
   so we have to discard the conditions in general.
   This means (mills n) will be INFINITE for squares, and some no-minds for 4-multiples.
*)
val mills_def = Define`
    mills n = {(x,y,z) | n = windmill x y z}
`;

(* Theorem: (x, y, z) IN mills n <=> (n = windmill x y z) *)
(* Proof: by mills_def. *)
val mills_element = store_thm(
  "mills_element",
...
*)
val mills_element_flip = store_thm(
  "mills_element_flip",
...
*)
val mills_element_trivial = store_thm(
  "mills_element_trivial",
...
(* Theorem: mills 0 = {(0,0,n) | n IN univ(:num)} UNION {(0,n,0) | n IN univ(:num)} *)
(* Proof:
...
*)
val mills_0 = store_thm(
  "mills_0",
  ``mills 0 = {(0,0,n) | n IN univ(:num)} UNION {(0,n,0) | n IN univ(:num)}``,
...
*)
val mills_0_infinite = store_thm(
  "mills_0_infinite",
  ``INFINITE (mills 0)``,
  qabbrev_tac `f = \n:num. (0,0,n)` >>
...
*)
val mills_1 = store_thm(
  "mills_1",
  ``mills 1 = {1} CROSS ({0} CROSS univ(:num)) UNION {1} CROSS (univ(:num) CROSS {0})``,
...
(* Proof:
   ...
   Note INFINITE univ(:num)          by INFINITE_NUM_UNIV
    ==> INFINITE (mills 0)           by INFINITE_INJ
*)
val mills_1_infinite = store_thm(
  "mills_1_infinite",
  ``INFINITE (mills 1)``,
  qabbrev_tac `f = \n:num. (1,0,n)` >>
...
*)
val mills_non_square_bound = store_thm(
  "mills_non_square_bound",
  ``!n x y z. ~square n /\ (x, y, z) IN mills n ==>
             (x < n /\ y < n /\ z < n)``,
...
val mills_non_square_subset = store_thm(
  "mills_non_square_subset",
  ``!n. ~square n ==>
       mills n SUBSET (count n) CROSS (count n CROSS (count n))``,
...
*)
val mills_non_square_card_upper = store_thm(
  "mills_non_square_card_upper",
  ``!n. ~square n ==> CARD (mills n) < n ** 3``,
  rpt strip_tac >>
...
*)
val mills_non_square_finite = store_thm(
  "mills_non_square_finite",
...
*)
val mills_finite_non_square = store_thm(
  "mills_finite_non_square",
...
(* Proof:
   ...
   Only-if part: n MOD 4 = 0 ==> (?y z. (0, y, z) IN mills n)
      ...
      Then n = 0 ** 2 + 4 * n DIV 4     by n MOD 4 = 0
       ==> (0, y, z) IN mills n
*)
val mills_with_no_mind = store_thm(
  "mills_with_no_mind",
...
val mills_with_all_mind = store_thm(
  "mills_with_all_mind",
...
*)
val mills_with_no_arms = store_thm(
  "mills_with_no_arms",
...
*)
val mills_quad_suc_non_empty = store_thm(
  "mills_quad_suc_non_empty",
  ``!n. (n MOD 4 = 1) ==> ~(mills n = EMPTY)``,
  rpt strip_tac >>
...
(* Proof:
   ...
   If y = 0 or z = 0, then n = x ** 2
             which contradicts ~square n
                                      by square_def, EXP_2
*)
val mills_triple_nonzero = store_thm(
  "mills_triple_nonzero",
...
*)
val mills_prime_triple_nonzero = store_thm(
  "mills_prime_triple_nonzero",
...
(* Define the flip function for last two elements of triple. *)
val flip_def = Define`
...
val flip_closure_alt = store_thm(
  "flip_closure_alt",
...
val flip_involute_alt = store_thm(
  "flip_involute_alt",
  ``!t. flip (flip t) = t``,
  rpt strip_tac >>
  qabbrev_tac `x = FST t` >>
...
*)
val flip_involute_mills = store_thm(
  "flip_involute_mills",
...
val zagier_closure_alt = store_thm(
  "zagier_closure_alt",
...
(* Proof:
   ...
   If ~(x < y - z), and ~(x < 2 * y),
      ...
      = (x, y, z)
*)
val zagier_involute = store_thm(
  "zagier_involute",
  ``!x y z. x <> 0 /\ y <> 0 /\ z <> 0 ==> (zagier (zagier (x, y, z)) = (x, y, z))``,
  rw[zagier_def]);

(* Theorem: x * y * z <> 0 ==> (zagier (zagier (x, y, z)) = (x, y, z)) *)
(* Proof: by MULT3_EQ_0, zagier_involute. *)
val zagier_involute_xyz = store_thm(
  "zagier_involute_xyz",
...
val zagier_involute_thm = store_thm(
  "zagier_involute_thm",
...
*)
val zagier_involute_alt = store_thm(
  "zagier_involute_alt",
...
*)
val zagier_involute_mills = store_thm(
  "zagier_involute_mills",
  ``!n. ~square n /\ n MOD 4 <> 0 ==> zagier involute (mills n)``,
...
*)
val zagier_flip_1_1_z = store_thm(
  "zagier_flip_1_1_z",
