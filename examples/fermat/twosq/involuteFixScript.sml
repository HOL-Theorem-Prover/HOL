...
(*
   ...
   fpair_sym     |- !f s x y. f involute s /\ x IN s /\ pair_by f x y ==> pair_by f y x
   fpair_trans   |- !f s x y z. f involute s /\ x IN s /\ pair_by f x y /\ pair_by f y z ==>
                                pair_by f x z
   ...
   equiv_class_fixes_iff   |- !f s x. f endo s ==>
                              (x IN fixes f s <=> (equiv_class (\x y. pair_by f x y) s x = {x}))
...
Proof
  rpt strip_tac >>
  `?x. x IN fixes f s /\ (t = equiv_class (pair_by f) s x)` by rw[GSYM equiv_fixes_element] >>
...
Proof
  rpt strip_tac >>
  `?x. x IN pairs f s /\ (t = equiv_class (pair_by f) s x)` by metis_tac[equiv_pairs_element] >>
  `x IN s /\ f x <> x` by fs[pairs_def] >>
  metis_tac[equiv_class_pairs]
QED
...
Proof
  metis_tac[fixes_finite, mates_finite, mates_fixes_union, mates_fixes_disjoint, CARD_UNION_DISJOINT]
QED
