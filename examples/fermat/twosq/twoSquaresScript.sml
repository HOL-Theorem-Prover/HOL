...
(* Proof:
   ...
   (2) x IN A /\ f x <> x ==> ?e. x IN e /\ e IN orbits (FUNPOW f) Z2 A /\ ~SING e
       ...
       Note {x; f x}
          = orbit (FUNPOW f) Z2 A x          by involute_orbit_not_fixed
...
Proof
 ...
 (`x IN A` by metis_tac[involute_orbits_element_element] >>
  `s = orbit (FUNPOW f) Z2 A x` by rw[involute_orbits_element_is_orbit] >>
  metis_tac[involute_orbit_fixed, SING]) >>
  qexists_tac `{x; f x}` >>
  simp[] >>
  `{x; f x} = orbit (FUNPOW f) Z2 A x` by rw[involute_orbit_not_fixed] >>
...
(* Proof:
   ...
   (3) n < 2 ==> FUNPOW zagier n (1,1,p DIV 4) = (1,1,p DIV 4)
...
Theorem zagier_fixed_points_sing:
  !p. prime p /\ ( p MOD 4 = 1) ==>
       (fixed_points (FUNPOW zagier) Z2 (mills p) = {(1,1,p DIV 4)})
Proof
  rw[fixed_points_def, mills_def, add_mod_property, EXTENSION] >>
  simp[EQ_IMP_THM] >>
  rpt strip_tac >| [
    `~square p` by metis_tac[prime_non_square] >>
