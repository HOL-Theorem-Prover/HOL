...
(* Proof:
   ...
   If ls <> [].
     ...
     Note DROP n ls <> []                   by DROP_NON_NIL, n < LENGTH ls, [1]
        cycle (SUC n) ls
...
val cycle_eq_drop_take = store_thm(
  ...
  rw[cycle_suc] >>
  fs[cycle_def] >>
  rw[DROP_SUC, TAKE_SUC, DROP_1_APPEND, TAKE_1_APPEND, DROP_NON_NIL]);
...
*)
val cycle_1_set_sing = store_thm(
  "cycle_1_set_sing",
...
(* Idea: A list has cycle 1 iff SING set *)
