...
(* the constructor holding a pair used to get a predicate of its own,
   which this specialised away; the principle now states what the pair's
   two components satisfy, and reading that as a pattern says the same *)
Theorem prop_logic_induct =
   SIMP_RULE
     (srw_ss())
