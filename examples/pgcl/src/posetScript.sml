...
(* ------------------------------------------------------------------------- *)
(*
val () = app load ["bossLib", "metisLib", "arithmeticTheory"];
val () = quietdec := true;
*)

open HolKernel Parse boolLib bossLib metisLib;
open combinTheory arithmeticTheory;
...
val carrier_def = Define `carrier ((s,r) : 'a poset) = s`;

val relation_def = Define `relation ((s,r) : 'a poset) = r`;

val top_def = Define
  `top ((s,r) : 'a poset) x = s x /\ !y. s y ==> r y x`;

val bottom_def = Define
...
val poset_trans = store_thm
  ("poset_trans",
   ...
   RW_TAC std_ss [poset_def] ++ RES_TAC);
...
val complete_bottom = store_thm
  ("complete_bottom",
   ...
   ++ RW_TAC std_ss [glb_def]
   ++ Q.EXISTS_TAC `x'`
   ++ RW_TAC std_ss [bottom_def]);

(* ------------------------------------------------------------------------- *)
(* Pointwise lifting of posets                                               *)
(* ------------------------------------------------------------------------- *)

val pointwise_lift_def = Define
...
val complete_pointwise = store_thm
  ("complete_pointwise",
   ...
   GEN_TAC
   ++ Know `?s r. p = (s,r)` >> (Cases_on `p` ++ METIS_TAC [])
   ++ STRIP_TAC
   ++ RW_TAC std_ss [complete_def, pointwise_lift_def] <<
   [Know
    `!y.
       t y ==>
    ...
    ++ RW_TAC std_ss [lub_def, function_def]
    ++ Q.EXISTS_TAC `l`
    ++ CONJ_TAC >> METIS_TAC []
    ++ CONJ_TAC >> METIS_TAC []
    ++ RW_TAC std_ss []
    ...
    ++ DISCH_THEN
       (MP_TAC o CONV_RULE (QUANT_CONV RIGHT_IMP_EXISTS_CONV THENC SKOLEM_CONV))
    ++ RW_TAC std_ss [glb_def, function_def]
    ++ Q.EXISTS_TAC `l`
    ++ CONJ_TAC >> METIS_TAC []
    ++ CONJ_TAC >> METIS_TAC []
    ++ RW_TAC std_ss []
...
val lub_pointwise_push = store_thm
  ("lub_pointwise_push",
   ...
   ++ Know `?s r. p = (s,r)` >> (Cases_on `p` ++ METIS_TAC [])
   ++ STRIP_TAC
   ++ RW_TAC std_ss [lub_def, pointwise_lift_def, carrier_def]
   ++ METIS_TAC []
*)
...
val knaster_tarski_lfp = store_thm
  ("knaster_tarski_lfp",
   ...
   ++ Know `?s r. p = (s,r)` >> (Cases_on `p` ++ PROVE_TAC [])
   ++ RW_TAC std_ss []
   ++ FULL_SIMP_TAC std_ss [function_def, carrier_def]
   ++ Q.UNDISCH_TAC `complete (s,r)`
   ++ SIMP_TAC std_ss [complete_def]
   ...
   ++ STRIP_TAC
   ++ ASM_SIMP_TAC std_ss [lfp_def]
   ++ MATCH_MP_TAC (PROVE [] ``(x ==> y) /\ x ==> x /\ y``)
   ++ REPEAT STRIP_TAC
   >> (Q.PAT_ASSUM `glb X Y Z` MP_TAC ++ ASM_SIMP_TAC std_ss [glb_def])
...
val knaster_tarski_gfp = store_thm
  ("knaster_tarski_gfp",
   ...
   ++ STRIP_TAC
   ++ ASM_SIMP_TAC std_ss [gfp_def]
   ++ MATCH_MP_TAC (PROVE [] ``(x ==> y) /\ x ==> x /\ y``)
   ++ REPEAT STRIP_TAC
   >> (Q.PAT_ASSUM `lub X Y Z` MP_TAC ++ ASM_SIMP_TAC std_ss [lub_def])
   ++ MATCH_MP_TAC poset_antisym
   ++ Q.EXISTS_TAC `s`
   ++ Q.EXISTS_TAC `r`
   ++ ASM_SIMP_TAC std_ss []
