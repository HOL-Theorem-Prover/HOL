...
val arith_dir = HOLDIR ^ "/TNN_tasks/data_arith"
...
(* -------------------------------------------------------------------------
   Training: update to be able to read from sexpr file.
   ------------------------------------------------------------------------- *)

fun train_fixed () =
  let
    val tml = import_terml (arith_dir ^ "/train");
...
fun test_fixed tnn =
  let
    val filel = map (fn x => arith_dir ^ "/" ^ x) ["train","valid","test"]
