...
val pos_simple_fn_integral_mono = store_thm
  ("pos_simple_fn_integral_mono",
   ...
   ++ Q.PAT_ASSUM `!x. x IN m_space m ==> SIGMA (\i. z i * indicator_fn (c i) x) k <=
			SIGMA (\i. z' i * indicator_fn (c i) x) k`
	(MP_TAC o Q.SPEC `t`)
...
val real_le_mult_sustain = lemma
  (``!r y. (!z. 0 < z /\ z < 1 ==> z * r <= y) ==> r <= y``,
   ...
   ++ Q.PAT_ASSUM `!z. P z` (MP_TAC o Q.SPEC `q/r`)
   ...
   ++ `!n. (z * (SIGMA (\i. x' i * measure m (a i INTER B' n)) s') <= x n) /\
	...
	by (STRIP_TAC ++ ONCE_REWRITE_TAC [CONJ_SYM] ++ STRONG_CONJ_TAC
	    ...
	    ++ STRIP_TAC ++ (MP_TAC o Q.SPECL [`m`, `(x:num->real) n`, `(u :num -> 'a -> real) n`]) (GSYM IN_psfis)
   	    ++ RW_TAC std_ss [pos_simple_fn_integral_def]
	    ++ Q.PAT_ASSUM `pos_simple_fn m s s' a x'` MP_TAC
	    ++ RW_TAC std_ss [pos_simple_fn_def]
  ...
  ("nnfis_mon_conv",
   ...
   ++ `(!n. P n (@u. P n u)) ==>
	...
	by (RW_TAC std_ss []
		...
		++ Q.UNABBREV_TAC `u`
		++ METIS_TAC [],
	        Q.PAT_ASSUM `!n. P n @u. P n u` (MP_TAC o Q.SPEC `n`)
	    	++ Q.UNABBREV_TAC `P`
	    	++ Q.UNABBREV_TAC `urev`
	    	++ Q.UNABBREV_TAC `u`
	    	++ METIS_TAC [psfis_nonneg, nonneg_def],
   ...
   ++ `!n. P n @u. P n u`
	by (STRIP_TAC ++ Q.ABBREV_TAC `P' = P n` ++ RW_TAC std_ss [SELECT_THM]
	    ++ Q.UNABBREV_TAC `P'` ++ Q.UNABBREV_TAC `P`
	    ++ RW_TAC std_ss []
	    ++ Q.PAT_ASSUM `!n. x n IN nnfis m (f n)` (MP_TAC o Q.SPEC `n`)
	    ++ RW_TAC std_ss [nnfis_def, GSPECIFICATION]
	    ++ Q.EXISTS_TAC `u'`
	    ++ RW_TAC std_ss []
	    ++ Q.EXISTS_TAC `x' n'`
  ...
  ("borel_measurable_mon_conv_psfis",
   ...
   ++ `!n t i. &i < (&n * 2 pow n) /\ t IN A n i ==>
	...
	by (REPEAT STRIP_TAC
	    ...
	    ++ Q.UNABBREV_TAC `u`
	    ++ Cases_on `i = 0`
	    >> (RW_TAC real_ss [] ++ Q.PAT_ASSUM `!n. FINITE P` (ASSUME_TAC o Q.SPEC `n`)
		++ (MP_TAC o REWRITE_RULE [IN_DELETE, REAL_ENTIRE] 
   ...
   ++ `!t n. t IN m_space m ==>
	...
	by (NTAC 3 STRIP_TAC
		...
		by (Q.UNABBREV_TAC `u` ++ RW_TAC std_ss []
		    ...
		    ++ Q.PAT_ASSUM `y < n * 2 ** n` MP_TAC ++ DECIDE_TAC)
...
val finite_space_POW_integral_reduce = store_thm
  ("finite_space_POW_integral_reduce",
   ...
   REPEAT STRIP_TAC
   ++ `f IN borel_measurable (m_space m, measurable_sets m)`
	by (Q.PAT_ASSUM `P = Q` (MP_TAC o GSYM)
...
val countable_space_integral_reduce = store_thm
  ("countable_space_integral_reduce",
   ...
   >> (Suff `p IN nnfis m (pos_part f)` >> METIS_TAC [nnfis_unique]
       ...
       ++ `BIJ (enumerate(IMAGE (pos_part f) (m_space m))) UNIV (IMAGE (pos_part f) (m_space m))` 
		by (`countable (IMAGE (pos_part f) (m_space m))`
