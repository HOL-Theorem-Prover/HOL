(* ========================================================================= *)(* Create "leakageTheory" setting up the theory of information leakage       *)(* ========================================================================= *)(* ------------------------------------------------------------------------- *)(* Load and open relevant theories                                           *)(* (Comment out "load" and "loadPath"s for compilation)                      *)(* ------------------------------------------------------------------------- *)(*app load ["bossLib", "metisLib", "arithmeticTheory", "pred_setTheory",
	  "pred_setLib", "listTheory",   	  "state_transformerTheory", "probabilityTheory", "formalizeUseful",	  "combinTheory", "pairTheory", "realTheory", "realLib", "extra_boolTheory",	  "jrhUtils", "extra_pred_setTheory", "realSimps", "extra_realTheory", 	  "measureTheory", "numTheory", "simpLib", "seqTheory", "subtypeTheory",	  "transcTheory", "limTheory", "stringTheory", "rich_listTheory",	  "stringSimps", "listSimps", "lebesgueTheory",   	  "informationTheory", "extra_stringTheory", "leakageTheory",	  "extra_stringLib", "leakageLib", "extra_listTheory"];*)open HolKernel Parse boolLib bossLib metisLib arithmeticTheory pred_setTheory     pred_setLib listTheory state_transformerTheory     probabilityTheory formalizeUseful extra_numTheory combinTheory     pairTheory realTheory realLib extra_boolTheory jrhUtils     extra_pred_setTheory realSimps extra_realTheory measureTheory numTheory     simpLib seqTheory subtypeTheory      transcTheory limTheory stringTheory rich_listTheory stringSimps listSimps     lebesgueTheory informationTheory extra_stringTheory leakageTheory     extra_stringLib leakageLib extra_listTheory;     (* ------------------------------------------------------------------------- *)(* Start a new theory called "information"                                   *)(* ------------------------------------------------------------------------- *)val _ = new_theory "dining_cryptos";(* ------------------------------------------------------------------------- *)(* Helpful proof tools                                                       *)(* ------------------------------------------------------------------------- *)infixr 0 ++ << || THENC ORELSEC ORELSER ##;infix 1 >>;val op ++ = op THEN;val op << = op THENL;val op >> = op THEN1;val op || = op ORELSE;val REVERSE = Tactical.REVERSE;val Simplify = RW_TAC arith_ss;val Suff = PARSE_TAC SUFF_TAC;val Know = PARSE_TAC KNOW_TAC;val Rewr = DISCH_THEN (REWRITE_TAC o wrap);val Rewr' = DISCH_THEN (ONCE_REWRITE_TAC o wrap);val Cond =  DISCH_THEN  (fn mp_th =>   let     val cond = fst (dest_imp (concl mp_th))   in     KNOW_TAC cond << [ALL_TAC, DISCH_THEN (MP_TAC o MP mp_th)]   end);val POP_ORW = POP_ASSUM (fn thm => ONCE_REWRITE_TAC [thm]);
...
val dc_prog_space_def = Define   `dc_prog_space (SUC(SUC n)) nsapays =	unif_prog_space		(dc_high_states nsapays (SUC(SUC n)))		dc_low_states 		(dc_random_states (SUC n))`;(* ************************************************************************* *)(* Case Study: The Dining Cryptographers - Basic Lemmas                      *)(* ************************************************************************* *)val set_announcements_alt = store_thm  ("set_announcements_alt",   ``!high low random n i.	(set_announcements high low random n 0 =	 (\s. if (s = (STRCAT "announces" (toString 0))) then			((high (STRCAT "pays" (toString 0)))) xor			((random (STRCAT "coin" (toString 0))) xor (random (STRCAT "coin" (toString n))))		else			low s)) /\	(set_announcements high low random n (SUC i) =	 (\s. if (s = (STRCAT "announces" (toString (SUC i)))) then			((high (STRCAT "pays" (toString (SUC i))))) xor			((random (STRCAT "coin" (toString (SUC i)))) xor (random (STRCAT "coin" (toString i))))		else			(set_announcements high low random n i) s))``,   RW_TAC std_ss [FUN_EQ_THM] ++ RW_TAC std_ss [set_announcements_def]);	val compute_result_alt = store_thm  ("compute_result_alt",   ``!low n. compute_result low n = (\s. if (s = "result") then XOR_announces low n else low s)``,   RW_TAC std_ss [FUN_EQ_THM] ++ RW_TAC std_ss [compute_result_def]);

val dc_high_states_set_finite = store_thm  ("dc_high_states_set_finite",   ``!n. FINITE (dc_high_states_set n)``,   Induct
   ++ FULL_SIMP_TAC safe_set_ss [dc_high_states_set_def]);
val dc_low_states_finite = prove  (``FINITE dc_low_states``,   RW_TAC set_ss [dc_low_states_def]);
val dc_random_states_finite = prove  (``!n. FINITE (dc_random_states n)``,   Induct
   ++ FULL_SIMP_TAC safe_set_ss [dc_random_states_def]);

val IN_dc_high_states_set = store_thm  ("IN_dc_high_states_set",   ``!n x. x IN (dc_high_states_set n) =           (?i. i <= n /\ (x = (\s. s = STRCAT "pays" (toString i))))``,   Induct   ++ RW_TAC set_ss [dc_high_states_set_def]   ++ METIS_TAC [DECIDE ``!(x:num) y. x <= SUC y = x <= y \/ (x = SUC y)``, STRCAT_toString_inj]);

val IN_dc_random_states = store_thm
  ("IN_dc_random_states",
   ``!n s. s IN (dc_random_states n) = !x. (~(?i. i <= n /\					(x = STRCAT "coin" (toString i)))) ==>
					(~(s x))``,
   Induct
...
val dc_high_states_set_not_empty = store_thm
  ("dc_high_states_set_not_empty",   ``!n. ~((dc_high_states_set n)={})``,   Induct ++ RW_TAC set_ss [dc_high_states_set_def]);

val dc_low_states_not_empty = store_thm
  ("dc_low_states_not_empty",   ``~(dc_low_states={})``,   RW_TAC set_ss [dc_low_states_def]);
val coin_out_of_range_eq_zero_dc_random_states = store_thm  ("coin_out_of_range_eq_zero_dc_random_states",   ``!n s. s IN (dc_random_states n) ==>		!i. (n < i) ==> ~ (s (STRCAT "coin" (toString i)))``,   Induct   >> (RW_TAC set_ss [dc_random_states_def]       ++ RW_TAC std_ss [STRCAT_toString_inj]       ++ POP_ASSUM MP_TAC ++ DECIDE_TAC)   ++ RW_TAC set_ss [dc_random_states_def]   ++ RW_TAC std_ss [STRCAT_toString_inj, DECIDE ``!i:num n. SUC n < i ==> ~(i = SUC n)``,					  DECIDE ``!i:num n. SUC n < i ==> n < i``]);

val dc_random_states_not_empty = store_thm
  ("dc_random_states_not_empty",   ``!n. ~((dc_random_states n)={})``,   Induct ++ RW_TAC set_ss [dc_random_states_def]);
...
val dc_states_cross_not_empty = store_thm
  ("dc_states_cross_not_empty",
   ...
   RW_TAC std_ss [CROSS_HLR_NON_EMPTY_IMP, dc_high_states_set_finite, dc_random_states_finite, dc_low_states_finite,
		       dc_high_states_set_not_empty, dc_random_states_not_empty, dc_low_states_not_empty]);
  ...
  ("CARD_dc_random_states",
   ...
   ++ `DISJOINT (IMAGE (\s x. (x = STRCAT "coin" (toString (SUC n))) \/ s x)
		...
		(IMAGE (\s x. ~(x = STRCAT "coin" (toString (SUC n))) /\ s x)
			(dc_random_states n))`
	by (RW_TAC std_ss [DISJOINT_DEF] ++ ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [NOT_IN_EMPTY, IN_INTER, IN_IMAGE] ++ METIS_TAC [])
   ...
   ++ `CARD (IMAGE (\s x. (x = STRCAT "coin" (toString (SUC n))) \/ s x)
	...
	by (MATCH_MP_TAC CARD_IMAGE
	    ++ Q.EXISTS_TAC `(IMAGE (\s x. (x = STRCAT "coin" (toString (SUC n))) \/ s x)
					(dc_random_states n))`
	    ++ RW_TAC set_ss [dc_random_states_finite, INJ_DEF, FUN_EQ_THM, fun_eq_lem] >> (Q.EXISTS_TAC `s'` ++ RW_TAC std_ss [])
...
val dc3_leakage_result = store_thm
  ("dc3_leakage_result",
   ...
   RW_TAC bool_ss [dc_prog_space_F_set_thm, unif_prog_space_leakage_computation_reduce,
		  ...
		  dc_states3_cross_not_empty, CARD_dc_high_states_set, CARD_dc_low_states, CARD_dc_random_states]
   ++ RW_TAC set_ss [dcprog_def, dc_low_states_def, dc_high_states_def, dc_high_states_set_def, dc_random_states_def,
		  CROSS_EQNS, compute_result_alt, XOR_announces_def, set_announcements_def, xor_def, STRCAT_toString_inj]
   ++ CONV_TAC (FIND_CONV ``x UNION y`` (UNION_CONV (SIMP_CONV set_ss [fun_eq_lem])))
   ++ RW_TAC set_ss [CROSS_EQNS, STRCAT_toString_inj]
   ++ CONV_TAC (FIND_CONV ``x UNION y`` (UNION_CONV (SIMP_CONV set_ss [fun_eq_lem])))
   ...
   ++ `(dc_random_witness x (SUC n)) IN (dc_random_states (SUC (SUC n)))`
	by (RW_TAC std_ss [IN_dc_random_states]
		...
		by (STRIP_TAC ++ REVERSE (Cases_on `i <= SUC (SUC n')`) >> ASM_REWRITE_TAC []
		    ++ Q.PAT_ASSUM `!i. ~(i <= SUC (SUC (SUC n'))) \/ ~(x' = STRCAT "coin" (toString i))`
...
val MEM_n_minus_1_announces_list = prove
  (``!n x. MEM x (n_minus_1_announces_list n) =
   ...
   ++ EQ_TAC
   >> (RW_TAC std_ss [] ++ RW_TAC std_ss [] ++ Q.PAT_ASSUM `!s'. b ==> ~s s'` MATCH_MP_TAC ++ STRIP_TAC ++ FULL_SIMP_TAC arith_ss [])
   ...
   >> (DISJ1_TAC ++ Q.EXISTS_TAC `(\x'. if (x' = STRCAT "announces" (toString (SUC n))) then F else x x')`
       ...
       ++ Q.PAT_ASSUM `!s. b ==> ~x s` MATCH_MP_TAC
  ...
  (``!n. dc_valid_outputs (SUC(SUC n)) = set (dc_valid_outputs_list (SUC(SUC(SUC n))))``,
   ...
   >> (RW_TAC std_ss [] ++ Q.EXISTS_TAC `(\s. if s = "result" then F else
       ...
       >> (`x (STRCAT "announces" (toString (SUC (SUC n)))) = ~XOR_announces (\s. if s = "result" then F else
		...
		by (Q.PAT_ASSUM `XOR_announces x (SUC (SUC n))` MP_TAC
       ...
       ++ Q.PAT_ASSUM `!x'. b ==> ~x x'` MATCH_MP_TAC ++ RW_TAC std_ss []
   ...
   ++ ASM_REWRITE_TAC []
   ++ RW_TAC std_ss []
   ++ Q.PAT_ASSUM `!s. b ==> ~l' s` MATCH_MP_TAC ++ STRIP_TAC ++ STRIP_TAC ++ FULL_SIMP_TAC arith_ss []);
...
val valid_coin_assignment_result1 = prove
  (``!x out p n i j. j <= i /\ valid_coin_assignment x out p n i ==> valid_coin_assignment x out p n j``,
   ...
   ++ Q.PAT_ASSUM `!j. b` MATCH_MP_TAC ++ FULL_SIMP_TAC arith_ss [valid_coin_assignment]);
  ...
  ("valid_coin_assignment_eq_2_element_set",
   ...
   >> (STRIP_TAC
	   ...
	   >> (ASM_SIMP_TAC arith_ss [coin_assignment_result1]
	       ...
	       ++ Q.PAT_ASSUM `!x''. b ==> ~x x''` MATCH_MP_TAC
	       ++ STRIP_TAC ++ REVERSE (Cases_on `i <= SUC (SUC n)`) ++ ASM_REWRITE_TAC []
	       ++ FULL_SIMP_TAC arith_ss [])
	   ++ FULL_SIMP_TAC std_ss [] ++ Q.PAT_ASSUM `i <= SUC (SUC n)` MP_TAC
  ...
  ("dc_leakage_result",
   ...
   ++ `(IMAGE (\s. (dcprog (SUC (SUC (SUC n))) s,SND (FST s)))
	...
	by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [dc_low_states_def, dc_high_states_def, IN_CROSS, IN_SING, IN_IMAGE]
		...
		++ FULL_SIMP_TAC std_ss [IN_dc_random_states]
		++ RW_TAC std_ss []
		++ Q.PAT_ASSUM `!x. P ==> ~SND s' x` MATCH_MP_TAC
...
val biased_dc3_leakage_result = store_thm
  ("biased_dc3_leakage_result",
	...
	by (RW_TAC real_ss []
		...
		(biased_high_states foo CROSS biased_low_states CROSS
		 ...
		 by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_INTER, GSPECIFICATION, IN_IMAGE, IN_PREIMAGE, IN_SING,
				    IN_CROSS, L_def]
		     ++ (EQ_TAC ++ RW_TAC std_ss [] ++ RW_TAC std_ss [])
		     >> (Q.PAT_ASSUM `P = (\s:string. s = STRCAT "coin" (toString 0))` (ASSUME_TAC o GSYM)
		...
		(biased_high_states foo CROSS biased_low_states CROSS
		 ...
		 by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_INTER, GSPECIFICATION, IN_IMAGE, IN_PREIMAGE, IN_SING,
				    IN_CROSS, L_def]
		     ++ (EQ_TAC ++ RW_TAC std_ss [] ++ RW_TAC std_ss [])
		     >> (Q.PAT_ASSUM `P = (\s:string. F)` (ASSUME_TAC o GSYM)
			 ++ Q.EXISTS_TAC `(FST(FST x'),SND x')`
			 ++ RW_TAC std_ss [PAIR]
			 ++ Q.EXISTS_TAC `(FST(FST x'),SND x')`
			 ++ RW_TAC std_ss [PAIR_EQ])
	...
	by (RW_TAC real_ss []
		...
		(biased_high_states foo CROSS biased_low_states CROSS
		 ...
		 by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_INTER, GSPECIFICATION, IN_IMAGE, IN_PREIMAGE, IN_SING,
				    IN_CROSS, FST]
		     ++ (EQ_TAC ++ RW_TAC std_ss [] ++ RW_TAC std_ss [])
		     >> (Q.PAT_ASSUM `P = (y,(\s:string. s = STRCAT "coin" (toString 0)))` (ASSUME_TAC o GSYM)
			 ++ Q.EXISTS_TAC `SND x'`
			 ++ RW_TAC std_ss [PAIR])
		     ++ RW_TAC set_ss [biased_low_states_def])
		++ POP_ORW
		...
		(biased_high_states foo CROSS biased_low_states CROSS
		 ...
		 by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_INTER, GSPECIFICATION, IN_IMAGE, IN_PREIMAGE, IN_SING,
				    IN_CROSS, FST]
		     ++ (EQ_TAC ++ RW_TAC std_ss [] ++ RW_TAC std_ss [])
		     >> (Q.PAT_ASSUM `P = (y,(\s:string. F))` (ASSUME_TAC o GSYM)
			 ++ Q.EXISTS_TAC `SND x'`
			 ++ RW_TAC std_ss [PAIR])
		     ++ RW_TAC set_ss [biased_low_states_def])
		++ POP_ORW
   ...
   ++ RW_TAC real_ss [] ++ Q.PAT_ASSUM `!f. f IN P` (K ALL_TAC)
