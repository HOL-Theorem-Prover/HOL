...
val finite_mutual_information_reduce = store_thm
  ("finite_mutual_information_reduce",
   ...
   ++ `prod_space = (s1 CROSS s2, POW (s1 CROSS s2), prod_measure (s1, POW s1, distribution p X)
								  (s2, POW s2, distribution p Y))`
	by (Q.UNABBREV_TAC `prod_space`
	    ++ RW_TAC std_ss [prod_measure_space_def, m_space_def, subsets_def, EXTENSION, subsets_def, sigma_def,
			      prod_sets_def, IN_POW, IN_BIGINTER, measurable_sets_def, SUBSET_DEF, IN_CROSS, GSPECIFICATION]
	    ++ RW_TAC std_ss [GSYM EXTENSION]
	    ++ EQ_TAC
	    >> (RW_TAC std_ss [] ++ Q.PAT_ASSUM `!s. P ==> x IN s` (MP_TAC o Q.SPEC `POW (s1 CROSS s2)`)
   ...
   >> (Q.EXISTS_TAC `\x'. joint_distribution p X Y {x'} / ((distribution p X {FST x'})*(distribution p Y {SND x'}))`
       ++ STRONG_CONJ_TAC
       >> (`(s1 CROSS s2,POW (s1 CROSS s2)) =
	...
	++ ONCE_REWRITE_TAC [(UNDISCH o Q.ISPEC `((s1 :'d -> bool) CROSS (s2 :'e -> bool)) DIFF a`) REAL_SUM_IMAGE_IN_IF]
	...
	++ Q.PAT_ASSUM `a SUBSET s1 CROSS s2` MP_TAC
	++ CONV_TAC (UNBETA_CONV ``(a :'d # 'e -> bool)``)
	++ Q.PAT_ASSUM `FINITE a` MP_TAC
	++ Q.SPEC_TAC (`a`,`a`)
   ...
   ++ `prod_space = (s1 CROSS s2, POW (s1 CROSS s2), prod_measure (s1, POW s1, distribution p X)
	...
	by (Q.UNABBREV_TAC `prod_space`
		...
		++ Suff `(!x''. (?x'. (x'',T) = (\(s,t). (s CROSS t,
                	(!x. x IN s ==> x IN s1) /\ !x. x IN t ==> x IN s2))
              		x') ==> !x. x IN x'' ==> FST x IN s1 /\ SND x IN s2)` >> METIS_TAC []
		++ RW_TAC std_ss [] ++ (MP_TAC o Q.ISPEC `(x''' :('d -> bool) # ('e # 'f -> bool))`) pair_CASES
			++ STRIP_TAC ++ FULL_SIMP_TAC std_ss [] ++ METIS_TAC [IN_CROSS])
	    ++ RW_TAC std_ss []
	    ++ `x = BIGUNION (IMAGE (\a. {a}) x)`
		by (ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_BIGUNION_IMAGE, IN_SING])
  ...
  ("finite_conditional_mutual_information_reduce",
   ...
   ++ `prod_space =
	...
	by (Q.UNABBREV_TAC `prod_space`
	    ...
	    ++ RW_TAC std_ss [GSYM EXTENSION]
	    ++ EQ_TAC
	    >> (ASM_SIMP_TAC std_ss [] ++ STRIP_TAC ++ Q.PAT_ASSUM `!s. P ==> x IN s` (MP_TAC o Q.SPEC `POW (s1 CROSS s2)`)
		...
		++ METIS_TAC [SUBSET_DEF, IN_CROSS])
	     ...
	     ++ ONCE_REWRITE_TAC [EXTENSION] ++ RW_TAC std_ss [IN_CROSS, IN_SING]
	     ++ METIS_TAC [PAIR_EQ, PAIR, FST, SND])
   ++ RW_TAC std_ss [conditional_mutual_information_def, finite_mutual_information_reduce2, PSPACE, EVENTS]
