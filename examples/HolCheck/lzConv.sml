...
structure lzConv :> lzConv =
struct

open HolKernel Parse boolTheory Drule boolSyntax  Abbrev lazyTools Conv Rsyntax
...
fun ABS_CONV conv tm =
    case dest_term tm of
      ...
      in
        ABS Bvar newbody
        handle HOL_ERR _ => (* this can happen if Bvar is now free in the assumptions to newbody *)
               let
		 (* this is buggy:
		  - fun foo_conv tm = List.foldl (fn (th,ath) => ADD_ASSUM (concl th) ath)
						 (REFL tm) (List.map REFL (free_vars tm));
		  > val foo_conv = fn : term -> thm
		  -  foo_conv ``x``;
			 <<HOL message: inventing new type variable names: 'a>>
		  > val it =  [x = x] |- x = x : thm
		  - ABS_CONV foo_conv ``\x.x`` handle ex => Raise ex;
			 <<HOL message: inventing new type variable names: 'a>>
...
fun OR_EXISTS_CONV tm =
   ...
   in
   if (not (eq x y)) then raise ERR "OR_EXISTS_CONV" ""
...
fun EXISTS_AND_CONV tm =
   ...
   handle (e as HOL_ERR{origin_structure = "lzConv",
...
fun AND_EXISTS_CONV tm =
 ...
 in
   if (not(eq x y)) then raise AE_ERR else
...
fun OR_FORALL_CONV tm =
 ...
 in
   if not(eq x y) then raise OF_ERR else
   if (free_in x P orelse free_in x Q)
...
(* ---------------------------------------------------------------------*)
(* lzFORALL_IMP_CONV, implements the following axiom schemes.		*)
(*									*)
...
(*									*)
(* ---------------------------------------------------------------------*)
local val FI_ERR = ERR "lzFORALL_IMP_CONV" "expecting `!x. P ==> Q`"
in
fun lzFORALL_IMP_CONV tm =
   ...
   in
     ...
     then raise ERR "lzFORALL_IMP_CONV"
	      ...
	      val jf = (fn _ => Conv.FORALL_IMP_CONV tm)
	  in mk_lthm (fn _ => (boolSyntax.mk_eq(tm,ltm),jf)) jf  end
   end
   handle (e as HOL_ERR{origin_structure = "lzConv",
			origin_function = "lzFORALL_IMP_CONV",...}) => raise e
        | HOL_ERR _ => raise ERR"lzFORALL_IMP_CONV" ""
end;
...
fun X_FUN_EQ_CONV x tm =
 ...
 handle e => raise (wrap_exn "lzConv" "X_FUN_EQ_CONV" e);
...
fun bool_EQ_CONV tm =
   ...
   in if eq lhs rhs then EQT_INTRO (REFL lhs) else
      if eq lhs T then SPEC rhs Tb else
      if eq rhs T then SPEC lhs bT
      else raise ERR"bool_EQ_CONV" "inapplicable"
   end
   handle e => raise (wrap_exn "lzConv" "bool_EQ_CONV" e)
end;
...
fun COND_CONV tm =
 ...
 in
   ...
   if (eq larm rarm) then SPEC larm (SPEC cond (INST_TYPE' COND_ID)) else
   if (aconv larm rarm)
...
fun AC_CONV(associative,commutative) =
 ...
 handle e => raise (wrap_exn "lzConv" "AC_CONV" e);
