...
val dpfx = "lzpr_"
val ERR = mk_HOL_ERR "lzPairRules"
...
fun lzPABS p th =
    let val cth = concl th
	val _ = if (HOLset.isEmpty(HOLset.intersection(HOLset.addList(HOLset.empty Term.var_compare,free_vars p),hyp_frees th)))
		then () else failwith("lzPABS free")
	val jf = (fn _ => PABS p th)
    in mk_lthm (fn _ => (mk_eq(mk_pabs(p,lhs cth),mk_pabs(p,rhs cth)),jf)) jf  end
(*
    ...
    handle HOL_ERR _ => failwith "lzPABS";

(* ----------------------------------------------------------------------- *)
(* PABS_CONV conv "\p. t[p]" applies conv to t[p]                          *)
(* ----------------------------------------------------------------------- *)

fun lzPABS_CONV conv tm =
    let val (Bvar,Body) = with_exn dest_pabs tm (ERR "lzPABS_CONV" "")
	val bodyth = conv Body
    in
      lzPABS Bvar bodyth handle HOL_ERR _ => failwith "lzPABS_CONV"
    end;
...
(* ------------------------------------------------------------------------- *)
(* ePBETA_CONV "(\p1.t)p2" = (|- (\p1.t)p2 = t[p2/p1])                      *)
(* ------------------------------------------------------------------------- *)
...
let
	...
	let (*FIXME: spped this up. it is slower than pairlike! *)
	    ...
	    val sbm = let val _ = profTools.bgt "epbc_epl_bm"(*PRF*)
		      ...
		      in res end
	    fun mk_pthm bp = let val _ = profTools.bgt "epbc_epl_pth"(*PRF*)(*FIXME: main bottleneck*)
				 val arg = (list_mk_pair(List.map (fn k => Binarymap.find(sbm,k)) (strip_pair bp)))
				 val res = PURE_ONCE_REWRITE_CONV [PAIR] arg
				     handle UNCHANGED => (print_term bp;REFL arg)
				 val _ = profTools.ent "epbc_epl_pth"(*PRF*)
			     in res end
	    val changed = is_changed bp np
	    val pthm = if changed then mk_pthm bp else TRUTH (* |- broken up np = np *)
...
(* ------------------------------------------------------------------------- *)
(* lzPBETA_CONV "(\p1.t)p2" = (|- (\p1.t)p2 = t[p2/p1])                      *)
(* ------------------------------------------------------------------------- *)
local
...
(*bp and np are paired structs (may be vars). Return a list of substs matching up their atomic components using FST and SND*)
...
(* morever, if in the main term, (a,b) occurs as well, then map that to x rather than (FST x, SND x), and so on*)
	...
	then [bp |-> np]
	    ...
	    then pairupvar bp np
		 ...
		 in (pairups bp1 np1) @ (pairups bp2 np2) end
...
fun lzPBETA_CONV tm =
    ...
    in lazyTools.mk_lthm (fn _ =>
			     ...
			     in (mk_eq(tm,subst sb body),fn _ => let val _ = profTools.bgt (dpfx^"epbc")(*PRF*)
								     val res = PairRules.PBETA_CONV tm (*ePBETA_CONV sb tm*)
								     val _ = profTools.ent (dpfx^"epbc")(*PRF*)
								 in res end) end) jf end
	handle _ => failwith ("lzPBETA_CONV")
end

val lzPBETA_RULE = CONV_RULE (DEPTH_CONV lzPBETA_CONV)
and lzPBETA_TAC  = CONV_TAC (DEPTH_CONV lzPBETA_CONV) ;
...
handle HOL_ERR _ => failwith ("lzPETA_CONV:"^(term_to_string tm));
...
let val _ = dbgTools.DEN dpfx "pac" (*DBG*)
    fun new_var v s = if HOLset.member(s,v) then new_var (let val (vnm,vty) = dest_var v in mk_primed_var (prime vnm,vty) end) s else v
    fun new_pair p s = if is_var p then new_var p s else let val (p1,p2) = dest_pair p in mk_pair(new_pair p1 s,new_pair p2 s) end
    val fvtm = (FVL [tm] empty_varset)
    val np' = new_pair np fvtm
    val _ = dbgTools.CBTM (dpfx^"pac_np'") np'
    val _ = dbgTools.CBTM (dpfx^"pac_tm") tm
    val _ = dbgTools.DTM (dpfx^"pac_tm") tm(*DBG*)
    val _ = dbgTools.DTY (dpfx^"pac_tm") (type_of tm)(*DBG*)
    val _ = dbgTools.DTM (dpfx^"pac_np'") np'(*DBG*)
    val _ = dbgTools.DTY (dpfx^"pac_np'") (type_of np')(*DBG*)
    val bod' = rhs(concl (lzPBETA_CONV (mk_comb(tm,np'))))
    val _ = dbgTools.CBTM (dpfx^"pac_bod'") bod'
    val _ = dbgTools.DTM (dpfx^"pac_bod'") bod'(*DBG*)
    val jf = (fn _ => PALPHA_CONV np tm)
    val _ = dbgTools.DEX dpfx "pac" (*DBG*)
    in mk_lthm (fn _ => (mk_eq(tm,mk_pabs(np',bod')),jf)) jf end
...
val (opr,_) = dest_pabs tm
 in if is_var np
    ...
    then let val np' = genvarstruct (type_of np)
             val t1 = lzPBETA_CONV (mk_comb(tm, np'))
             val t2 = lzPABS np' t1
             val th3 = CONV_RULE (RATOR_CONV (RAND_CONV lzPETA_CONV)) t2
         in
            CONV_RULE (RAND_CONV (lzPALPHA_CONV np)) th3
         end
    else (* is_pair opr *)
    let val (np1,np2) = dest_pair np
    in CONV_RULE (RAND_CONV (RAND_CONV (lzPABS_CONV (lzPALPHA_CONV np2))))
                 ((RAND_CONV (lzPALPHA_CONV np1)) tm)
    end
 end
 handle HOL_ERR _ => failwith "lzPALPHA_CONV" ;*)
...
fun lzGEN_PALPHA_CONV p tm =
    if is_pabs tm then
	lzPALPHA_CONV p tm
    else
	AP_TERM (rator tm) (lzPALPHA_CONV p (rand tm))
	handle HOL_ERR _ => failwith "lzGEN_PALPHA_CONV";
