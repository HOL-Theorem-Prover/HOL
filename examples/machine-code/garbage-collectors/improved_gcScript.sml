...
val SET2_TAC = 
  FULL_SIMP_TAC std_ss [EXTENSION,IN_INSERT,IN_UNION,IN_DELETE,SUBSET_DEF,
    DISJOINT_DEF,NOT_IN_EMPTY,EXTENSION,IN_INSERT,IN_INTER,IN_DIFF,IN_UNIV,
    IN_LIST_TO_SET,POINTERS_def,HAS_CHANGED_def,SET_TRANSLATE_def,FDOM_FUPDATE,
...
val abs_step_thm = prove(
  ...
  REPEAT STRIP_TAC \\ Q.PAT_ASSUM `abs_step s1 s1i` ASSUME_TAC
   ...
   (FULL_SIMP_TAC std_ss [abs_gc_inv_def,IN_UNION,IN_DELETE,FDOM_FUPDATE,IN_INSERT,LET_DEF]
    ...
    \\ STRIP_TAC THEN1
     (`a IN gc_reachable (h0,r0)` by SET_TAC
      ...
      \\ DISJ2_TAC \\ Q.PAT_ASSUM `a IN gc_reachable hhh` 
    ...
    \\ STRIP_TAC THEN1 (FULL_SIMP_TAC std_ss [FUN_EQ_THM] \\ SET_TAC)
    \\ Q.PAT_ASSUM `gg = FDOM h0` (ASSUME_TAC o GSYM) \\ ASM_SIMP_TAC std_ss []
    \\ STRIP_TAC THEN1
...
val abs_gc_thm = store_thm("abs_gc_thm",
  ...
  \\ `h2' ' (f q') = (ADDR_MAP f xs,d)` by ALL_TAC THEN1
    (Q.PAT_ASSUM `!d. d IN FDOM h ==> bb` (MP_TAC o GSYM o Q.SPEC `q'`)       
...
val gc_copy_thm = prove(
  ...
  \\ Q.PAT_ASSUM `!d. ff IN FDOM h1 <=> bb IN FDOM h2` ASSUME_TAC
...
val gc_filter_thm = prove(
  ...
  \\ Q.PAT_ASSUM `DRESTRICT h1 (gc_reachable (h1,roots1)) ' b = (zs,d)` MP_TAC
...
val ref_mem_IMP = prove(
  ``(ref_mem (h,f) m /\ (m n = H_BLOCK x) ==> n IN FDOM h /\ (h ' n = x)) /\
    (ref_mem (h,f) m /\ (m n = H_REF i) ==> ~(n IN FDOM h) /\ ~(f n = n) /\ (f n = i)) /\
    (ref_mem (h,f) m /\ (m n = H_EMP) ==> ~(n IN FDOM h) /\ (f n = n))``,
  SIMP_TAC std_ss [ref_mem_def,GSYM AND_IMP_INTRO] 
...
val move_lemma = store_thm("move_lemma",
  ...
  \\ IMP_RES_TAC ref_mem_IMP \\ ASM_SIMP_TAC std_ss [heap_element_11,heap_element_distinct]
  ...
  \\ STRIP_TAC THEN1
   ...
   \\ FULL_SIMP_TAC std_ss [EMP_RANGE_def,IN_DEF]
   \\ REPEAT STRIP_TAC \\ Q.PAT_ASSUM `!k. bbb ==> (m k = H_EMP)` MATCH_MP_TAC 
   \\ RANGE_TAC);
...
val move_thm = store_thm("move_thm",
  ``gc_inv (h1,roots1,h,f) (b,i,j,e,b2,e2,m,z UNION ADDR_SET [x]) /\
    ...
    (move (x,j,m) = (x2,j2,m2)) ==>
      ?h2 f2. gc_inv (h1,roots1,h2,f2) (b,i,j2,e,b2,e2,m2,z UNION (D1(CUT(j,j2)m2))) /\ j <= j2 /\
              ...
              ([x2] = ADDR_MAP f2 [x]) /\ full_heap j j2 m2``,
  CONV_TAC (RATOR_CONV (ONCE_REWRITE_CONV [gc_inv_THM]))
  \\ REVERSE (Cases_on `x`) THEN1
   (SIMP_TAC std_ss [move_def] \\ ONCE_REWRITE_TAC [EQ_SYM_EQ]
  ...
  THEN1
   (Q.LIST_EXISTS_TAC [`h`,`f`] \\ ASM_SIMP_TAC std_ss []
    \\ Q.PAT_ASSUM `move xx = fg` MP_TAC \\ ASM_SIMP_TAC (srw_ss()) [move_def]
  ...
  \\ `full_heap j (j + y + 1) ((j =+ H_BLOCK (x,y,d)) ((n =+ H_REF j) m))` by ALL_TAC THEN1
    (Q.PAT_ASSUM `~(n = j)` (fn th => REWRITE_TAC [MATCH_MP UPDATE_COMMUTES (GSYM th)])
  ...
  \\ REVERSE STRIP_TAC THEN1
   (`~(RANGE(b,j)j) /\ ~(RANGE(b,j)n) /\ ~(RANGE(b2,e2)j)` by RANGE_TAC
    ...
    \\ `f j = j` by METIS_TAC [ref_mem_def,heap_element_distinct]
  ...
  THEN1
   (SIMP_TAC std_ss [ref_mem_def,APPLY_UPDATE_THM,FDOM_FUPDATE,FDOM_DOMSUB,
      IN_INSERT,IN_DELETE,FAPPLY_FUPDATE_THM,DOMSUB_FAPPLY_THM] 
    \\ ALPHA_TAC "k" \\ REPEAT STRIP_TAC 
    \\ Cases_on `j = k` \\ FULL_SIMP_TAC std_ss []
    \\ Cases_on `k = n` \\ FULL_SIMP_TAC std_ss [ref_mem_def])
  \\ FULL_SIMP_TAC std_ss [CUT_UPDATE]
  \\ Q.PAT_ASSUM `~(n = j)` (fn th => REWRITE_TAC [MATCH_MP UPDATE_COMMUTES (GSYM th)] THEN ASSUME_TAC th)
  \\ `~(RANGE(j,j + y + 1)n) /\ ~(RANGE(i,j + y + 1)n)` by RANGE_TAC
  \\ FULL_SIMP_TAC std_ss [CUT_UPDATE]
  \\ REPEAT (Q.PAT_ASSUM `~(RANGE(xx,yy)df)` (K ALL_TAC))
  ...
  \\ Q.PAT_ASSUM `ref_mem (h,f) m` (fn th => FULL_SIMP_TAC std_ss [RW[ref_mem_def]th])
...
val move_list_thm = store_thm("move_list_thm",
  ``!xs z h f m j xs2 j2 (m2:num -> ('a,'b) heap_element).
      ...
      (move_list (xs,j,m) = (xs2,j2,m2)) ==>
        ?h3 f3. gc_inv (h1,roots1,h3,f3) (b,i,j2,e,b2,e2,m2,z UNION (D1(CUT(j,j2)m2))) /\ j <= j2 /\
                DR0 (CUT (b2,e2) m) SUBSET DR0 (CUT (b2,e2) m2) /\
                (CUT(b,j)m = CUT(b,j)m2) /\
                (!a. ~(f a = a) ==> (f3 a = f a)) /\
                (!a. a IN ADDR_SET xs2 ==> ~(f3 a = a)) /\
                (xs2 = ADDR_MAP f3 xs) /\ full_heap j j2 m2``,
  Induct \\ SIMP_TAC std_ss [ADDR_SET_THM,UNION_EMPTY,move_list_def,ADDR_MAP_def,
  ...
  \\ `?x3 j3 m3. move (h,j,m) = (x3,j3,m3)` by METIS_TAC [PAIR]
  \\ `?xs4 j4 m4. move_list (xs,j3,m3) = (xs4,j4,m4)` by METIS_TAC [PAIR]
  \\ FULL_SIMP_TAC std_ss [UNION_ASSOC] \\ ONCE_REWRITE_TAC [EQ_SYM_EQ]
  \\ REPEAT STRIP_TAC
  \\ IMP_RES_TAC move_thm \\ Q.PAT_ASSUM `!z roots1 i. bb` (K ALL_TAC)
  \\ Q.PAT_ASSUM `!z h f.bbb` (MP_TAC o Q.SPECL [
...
val gc_step_lemma = store_thm("gc_step_lemma",
  ...
  SIMP_TAC std_ss [gc_step_def,LET_DEF] \\ REPEAT STRIP_TAC \\ IMP_RES_TAC gc_inv_IMP
  ...
  \\ `i IN FDOM h /\ (h ' i = (xs,n,d))` by METIS_TAC [gc_inv_def,
       ref_mem_def, heap_element_distinct,heap_element_11,PAIR_EQ]
    ...
    \\ Cases_on `x IN FDOM h` THEN1
     (`~(x IN D0 (CUT (b,j) m))` by SET_TAC
      ...
      \\ `m x = H_BLOCK (x1,y1,d1)` by METIS_TAC [ref_mem_def,gc_inv_def]
    ...
    \\ Q.PAT_ASSUM `~(x IN D0 (CUT (b,j) m))` MP_TAC
    \\ SIMP_TAC std_ss [IN_DEF,D0,CUT_EQ]
    \\ FULL_SIMP_TAC std_ss [HAS_CHANGED_def]
    \\ FULL_SIMP_TAC std_ss [gc_inv_def,abs_gc_inv_def,LET_DEF] 
    \\ `m x = H_REF (f x)` by METIS_TAC [ref_mem_def]         
...
val gc_step_thm = store_thm("gc_step_thm",
  ...
  \\ `i < i + n + 1 /\ i <= i + n + 1` by DECIDE_TAC
  \\ FULL_SIMP_TAC std_ss []
  \\ Q.PAT_ASSUM `gc_inv (h1,roots1,h,f) xxx` (fn th => ASSUME_TAC th THEN 
       MP_TAC (Q.INST [`xs2`|->`xs3`,`j2`|->`j3`,`m2`|->`m3`] 
  ...
  \\ REPEAT STRIP_TAC THEN1
   (FULL_SIMP_TAC std_ss [ref_mem_def,APPLY_UPDATE_THM,FDOM_FUPDATE,IN_INSERT]
  ...
  \\ FULL_SIMP_TAC std_ss [SET_TRANSLATE_MAP,SET_TRANSLATE_def]
  \\ Q.PAT_ASSUM `!a. bbb` (MP_TAC o Q.GEN `a` o Q.SPEC `(f3:num->num) a`)
  ...
  THEN1 METIS_TAC [heap_element_distinct,ref_mem_def,heap_element_11,PAIR_EQ]
  THEN1  
...
val gc_loop_thm = store_thm("gc_loop_thm",
  ``!j h f m i2 m2.
      ...
      (gc_loop (i,j,m) = (i2,m2)) ==>
        ?h4 f4. gc_inv (h1,roots1,h4,f4) (b,i2,i2,e,b2,e2,m2,{}) /\ j <= i2 /\
                !a. f a <> a ==> (f4 a = f a)``,
  completeInduct_on `e - i` \\ NTAC 4 STRIP_TAC \\ Cases_on `i = j`
  \\ ONCE_REWRITE_TAC [gc_loop_def] \\ ASM_SIMP_TAC std_ss [CUT_EMPTY] THEN1 SET_TAC
  \\ NTAC 6 STRIP_TAC \\ `?i3 j3 m3. gc_step (i,j,m) = (i3,j3,m3)` by METIS_TAC [PAIR]
...
val ok_full_heap_IMP = store_thm("ok_full_heap_IMP",
  ...
  \\ IMP_RES_TAC ok_heap_IMP \\ REPEAT STRIP_TAC THEN1 RANGE_TAC  
  \\ FULL_SIMP_TAC std_ss [CUT_EMPTY] 
  THEN1 (Q.PAT_ASSUM `!k.bb` MATCH_MP_TAC \\ RANGE_TAC) 
  ...
  \\ FULL_SIMP_TAC std_ss [ref_mem_def,heap_element_distinct]
...
val gc_thm = store_thm("gc_thm",
  ...
  \\ `?i4 m4. gc_loop (b,b3,m3) = (i4,m4)` by METIS_TAC [PAIR] 
  ...
  \\ Q.PAT_ASSUM `gc_inv (h,roots,h3,f3) ttt` MP_TAC
  \\ REPEAT (Q.PAT_ASSUM `gc_inv xxx yyy` (K ALL_TAC)) \\ STRIP_TAC
  \\ IMP_RES_TAC gc_loop_thm 
  \\ Q.PAT_ASSUM `r3 = ADDR_MAP f3 roots` MP_TAC
  \\ REPEAT (Q.PAT_ASSUM `r3 = ADDR_MAP xx roots` (K ALL_TAC)) \\ STRIP_TAC
  \\ FULL_SIMP_TAC std_ss []
  ...
  \\ Q.PAT_ASSUM `gc_inv (h,roots,h4,f4) ttt` MP_TAC
  \\ REPEAT (Q.PAT_ASSUM `gc_inv xxx yyy` (K ALL_TAC))
  ...
  \\ FULL_SIMP_TAC std_ss [ref_mem_def,FUN_EQ_THM,FDOM_DRESTRICT,IN_INTER]
