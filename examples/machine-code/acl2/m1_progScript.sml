...
val m1_2set'_def = Define `
  m1_2set' (ps:unit set, ls: unit set, ss: unit set, is: sexp set, os: unit set) (s:sexp) =
    IMAGE (\a. tPC (pc s)) ps UNION
    IMAGE (\a. tLocals (locals s)) ls UNION
...
val IN_m1_2set = (SIMP_RULE std_ss [oneTheory.one] o prove)(``
  ...
  (!a x s. tPC x IN (m1_2set'' (ps,ls,ss,is,os) s) = (x = pc s) /\ ~(a IN ps)) /\ 
  (!a x s. tLocals x IN (m1_2set s) = (x = locals s)) /\
  (!a x s. tLocals x IN (m1_2set' (ps,ls,ss,is,os) s) = (x = locals s) /\ a IN ls) /\
  (!a x s. tLocals x IN (m1_2set'' (ps,ls,ss,is,os) s) = (x = locals s) /\ ~(a IN ls)) /\ 
...
val m1_2set''_11 = prove(
  ...
  THEN1 (`~((?y. tLocals y IN m1_2set'' (ps,ls,ss,is,os) s) = 
            (?y. tLocals y IN m1_2set'' (ps2,ls2,ss2,is2,os2) s'))` by 
         FULL_SIMP_TAC std_ss [IN_m1_2set,oneTheory.one] THEN1 METIS_TAC [])
...
val DELETE_m1_2set = (SIMP_RULE std_ss [oneTheory.one] o prove)(``
  ...
  (!a s. (m1_2set' (ps,ls,ss,is,os) s) DELETE tLocals (locals s) =
...
val tL_def = Define `tL x = SEP_EQ {tLocals x}`;
...
val STAR_m1_2set = LIST_CONJ (map EXPAND_STAR 
  [``(tP x * p) (m1_2set' (ps,ls,ss,is,os) s)``,
   ``(tL x * p) (m1_2set' (ps,ls,ss,is,os) s)``,
...
val m1_2set''_thm = prove(
  ...
  THEN1 (Q.PAT_ASSUM `!x.bb` (ASSUME_TAC o Q.SPEC `tLocals (locals s1)`)
         \\ METIS_TAC [IN_m1_2set])
...
val IMP_SPEC_M1_MODEL = prove(
  ``(!cs. (nth p1 cs = c) ==>
          (step (make_state p1 l1 s1 cs) = make_state p2 l2 s2 cs)) ==>
    SPEC M1_MODEL (tS s1 * tL l1 * PC p1)
         {(p1,c)} (tS s2 * tL l2 * PC p2)``,
  REPEAT STRIP_TAC  
  ...
  \\ ASM_SIMP_TAC std_ss [make_state_thm]);
...
val M1_TAC = 
  ...
  \\ REPEAT STRIP_TAC
  \\ MATCH_MP_TAC IMP_SPEC_M1_MODEL \\ ONCE_REWRITE_TAC defs2
...
val M1_ICONST = store_thm("M1_ICONST",
  ``SPEC M1_MODEL
       ...
       (tS (push x s) * tL l * PC (add p (nat 1)))``,
  M1_TAC);

val M1_ILOAD = store_thm("M1_ILOAD",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p)
       {(p, List [sym "M1" "ILOAD"; x])}
       (tS (push (nth x l) s) * tL l * PC (add p (nat 1)))``,
  M1_TAC);

val M1_ISTORE = store_thm("M1_ISTORE",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p)
       {(p, List [sym "M1" "ISTORE"; x])}
       (tS (pop s) * tL (update_nth x (top s) l) * PC (add p (nat 1)))``,
  M1_TAC);
...
val M1_IADD = store_thm("M1_IADD",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p)
       {(p, List [sym "M1" "IADD"])}
       (tS (push (add (top (pop s)) (top s)) (pop (pop s))) * tL l * PC (add p (nat 1)))``,
  M1_TAC);
...
val M1_ISUB = store_thm("M1_ISUB",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p)
       {(p, List [sym "M1" "ISUB"])}
       (tS (push (add (top (pop s)) (unary_minus (top s))) (pop (pop s))) * tL l * PC (add p (nat 1)))``,
  M1_TAC);
...
val M1_IMUL = store_thm("M1_IMUL",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p)
       {(p, List [sym "M1" "IMUL"])}
       (tS (push (mult (top (pop s)) (top s)) (pop (pop s))) * tL l * PC (add p (nat 1)))``,
  M1_TAC);
...
val M1_GOTO = store_thm("M1_GOTO",
  ``SPEC M1_MODEL
       ...
       (tS s * tL l * PC (add p x))``,
  M1_TAC);

val M1_IFLE = store_thm("M1_IFLE",
  ``SPEC M1_MODEL
       (tS s * tL l * PC p * precond ((|= (not (less (nat 0) (top s))))))
       {(p, List [sym "M1" "IFLE"; x])}
       (tS (pop s) * tL l * PC (add p x))``,
  M1_TAC);
