...
val ppc_assign2assembly = let
  ...
  fun binop_to_name ASSIGN_BINOP_ADD = ("add","addi")
    ...
    | binop_to_name ASSIGN_BINOP_TIMES = ("mullw","mulli")
  ...
  fun f (ASSIGN_EXP (d, ASSIGN_EXP_REG s)) = ["or " ^ r d ^ ", " ^ r s ^ ", " ^ r s]
    ...
    | f (ASSIGN_EXP (d, ASSIGN_EXP_MEMORY a)) = ["lwz " ^ r d ^ ", " ^ address a]       
    | f (ASSIGN_EXP (d, ASSIGN_EXP_SHIFT_LEFT (ASSIGN_X_REG i,n))) = ["slw " ^ r d ^ ", " ^ r i ^ ", " ^ int_to_string n ]
    ...
    | f (ASSIGN_MEMORY (a,d)) = ["stw " ^ r d ^ ", " ^ address a]
    | f _ = hd []
  in f end  
