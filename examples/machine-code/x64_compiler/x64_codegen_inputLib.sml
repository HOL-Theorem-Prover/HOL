structure x64_codegen_inputLib :> x64_codegen_inputLib =
struct
...
fun dest_n2w tm = if car tm = ``n2w:num->word64`` then numSyntax.dest_numeral (cdr tm) else fail()
...
fun basic_term2guard tm = (* expects input to use "~", "<+", "<" and "=" only *)
  if can dest_neg tm then GUARD_NOT (basic_term2guard (dest_neg tm)) else let
    ...
    in if can (match_term ``(x && y) = 0w:word64``) tm then
       ...
       handle e => let
         val (i,_) = match_term ``(f:word64->word8) a = n2w n`` tm handle e =>
                     match_term ``n2w n = (f:word64->word8) a`` tm
         val addr = dest_address (subst i ``a:word64``)
...
fun basic_term2assign t1 t2 = let
  ...
  fun dest_monop tm =
    ((snd o hd o filter (fn x => fst x = (fst o dest_const o car) tm)) monops,
     (dest_x o cdr) tm)
  fun dest_memory tm = let
    val (i,_) = match_term ``((m:word64->word64) a)`` tm
    val addr = dest_address (subst i ``a:word64``)
    in (ACCESS_WORD,addr) end handle HOL_ERR _ => fail()
  fun dest_byte_memory tm = let
    val (i,_) = match_term ``w2w ((m:word64->word8) a)`` tm
    val addr = dest_address (subst i ``a:word64``)
  ...
  fun dest_memory_update tm = let
    val (i,_) = match_term ``(a =+ w) (m:word64->word64)`` tm
    val addr = dest_address (subst i ``a:word64``)
    val x = dest_x (subst i ``w:word64``)
    val _ = if is_var (subst i ``m:word64->word64``) then () else fail()
    val _ = if is_var (subst i ``w:word64``) then () else fail()
    in (ACCESS_WORD,addr,x) end handle HOL_ERR _ => let
    val (i,_) = match_term ``(a =+ n2w n) (m:word64->word64)`` tm
    val addr = dest_address (subst i ``a:word64``)
    val x = dest_n2w (subst i ``(n2w n):word64``)
    val _ = if is_var (subst i ``m:word64->word64``) then () else fail()
