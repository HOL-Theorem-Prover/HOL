...
val arm_ss = srw_ss() ++ genlist_ss ++ am1_ss ++ am2_ss ++
  ...
  systemTheory.MEM_WRITE_BYTE_def, SHIFT_IMMEDIATE_def,
  IS_MSR_IMMEDIATE_def, IMMEDIATE_def, ADDR_MODE1_def, ADDR_MODE2_def,
  ADDR_MODE3_def, ADDRESS_LIST_def, GSYM ARM_MODE_def, GSYM
  ARM_WRITE_REG_def,ARM_READ_REG_INTRO, ADDR_MODE4_def, UP_DOWN_def,
  WB_ADDRESS_def, FIRST_ADDRESS_def, ZIP, NZCV_def, GSYM
  ARM_READ_MEM_def, GSYM ARM_READ_STATUS_def, CONDITION_PASSED2_def,
  ARM_WRITE_REG_PC_INTRO, ARM_READ_REG_15, ARM_WRITE_MEM_INTRO,
  ARM_REG_READ_INC_PC, ARM_WRITE_STATUS_INTRO, ARM_WRITE_REG_INTRO,
  COLLAPSE_ARM_STATE, ARM_READ_WRITE, ARM_WRITE_UNDEF_INTRO]
...
fun arm_step s = let
  ...
  fun format th = let
    ...
    val g = REWRITE_RULE [WORD_SUB_ONE_MULT,WORD_EQ_SUB_ZERO,GSYM word_sub_def,systemTheory.addr30_def] o SIMP_RULE arm_ss [enc_th]
...
(*
  ...
  val res = arm_step "E5D32000"; (* ldrb r2,[r3] *) 
  val res = arm_step "E5C32000"; (* strb r2,[r3] *)
