...
val Z64_ICACHE_def = Define `
  Z64_ICACHE ((r,e,s,m,i):x64_state) ((r2,e2,s2,m2,i2):x64_state) =
    ?insert delete.
      (r = r2) /\ (e = e2) /\ (s = s2) /\ (m = m2) /\
      (i2 = \addr. if addr IN insert then m addr else
                   if addr IN delete then NONE else i addr)`;

val Z64_ACCURATE_def = Define `
  Z64_ACCURATE a ((r,e,s,m,i):x64_state) = (i a = NONE) \/ (i a = m a)`;
...
val Z64_ICACHE_UPDATE_def = Define `
  Z64_ICACHE_UPDATE x ((r1,e1,s1,m1,i1):x64_state) = (r1,e1,s1,m1,icache x m1 i1)`;


(* theorems *)

val Z64_ICACHE_REFL = store_thm("Z64_ICACHE_REFL",
  ``!s. Z64_ICACHE s s``,
  STRIP_TAC THEN `?r e t m i. s = (r,e,t,m,i)` by METIS_TAC [PAIR]
  THEN ASM_SIMP_TAC std_ss [Z64_ICACHE_def]
  THEN Q.EXISTS_TAC `{}` THEN Q.EXISTS_TAC `{}`
  THEN ASM_SIMP_TAC std_ss [NOT_IN_EMPTY,FUN_EQ_THM]);

val Z64_ICACHE_TRANS = store_thm("Z64_ICACHE_TRANS",
  ``!s t u. Z64_ICACHE s t /\ Z64_ICACHE t u ==> Z64_ICACHE s u``,
  REPEAT STRIP_TAC
  ...
  THEN FULL_SIMP_TAC std_ss [Z64_ICACHE_def,FUN_EQ_THM]
...
val Z64_ICACHE_THM = store_thm("Z64_ICACHE_THM",
  ``Z64_ICACHE (r,e,s,m,i) (r2,e2,s2,m2,i2) =
    ?update.
      (r2,e2,s2,m2,i2) = (r,e,s,m,icache update m i)``,
  SIMP_TAC std_ss [EXISTS_PROD,Z64_ICACHE_def,icache_def,FUN_EQ_THM]
  THEN METIS_TAC []);
...
val ZREAD_CLAUSES = store_thm("ZREAD_CLAUSES",
  ``!s. (ZREAD_REG r (ZWRITE_REG r2 w s) = if r2 = r then w else ZREAD_REG r s) /\
        ...
        (ZREAD_REG r (Z64_ICACHE_UPDATE u s) = ZREAD_REG r s) /\
        ...
        (ZREAD_RIP (Z64_ICACHE_UPDATE u s) = ZREAD_RIP s) /\
        ...
        (ZREAD_EFLAG i (Z64_ICACHE_UPDATE u s) = ZREAD_EFLAG i s) /\
        ...
        (ZREAD_MEM2 a (Z64_ICACHE_UPDATE u s) = ZREAD_MEM2 a s) /\
  ...
  THEN ASM_SIMP_TAC std_ss [ZREAD_REG_def,ZREAD_RIP_def,
         ...
         ZWRITE_EFLAG_def,ZCLEAR_ICACHE_def,CAN_ZWRITE_MEM,Z64_ICACHE_UPDATE_def]
