...
val X64_ICACHE_def = Define `
  X64_ICACHE ((r,e,s,t,m,i):x64_state) ((r2,e2,s2,t2,m2,i2):x64_state) =
    ?insert delete.
      (r = r2) /\ (e = e2) /\ (s = s2) /\ (t = t2) /\ (m = m2) /\
      (i2 = \addr. if addr IN insert then m addr else
                   if addr IN delete then NONE else i addr)`;

val X64_ACCURATE_def = Define `
  X64_ACCURATE a ((r,e,s,t,m,i):x64_state) = (i a = NONE) \/ (i a = m a)`;
...
val X64_ICACHE_UPDATE_def = Define `
  X64_ICACHE_UPDATE x ((r1,e1,s1,t1,m1,i1):x64_state) = (r1,e1,s1,t1,m1,icache x m1 i1)`;
...
val X64_ICACHE_REFL = store_thm("X64_ICACHE_REFL",
  ``!s. X64_ICACHE s s``,
  STRIP_TAC THEN `?r e t t' m i. s = (r,e,t,t',m,i)` by METIS_TAC [PAIR]
...
val X64_ICACHE_TRANS = store_thm("X64_ICACHE_TRANS",
  ...
  REPEAT STRIP_TAC
  THEN `?r1 e1 t1 t1' m1 i1. s = (r1,e1,t1,t1',m1,i1)` by METIS_TAC [PAIR]
  THEN `?r2 e2 t2 t2' m2 i2. t = (r2,e2,t2,t2',m2,i2)` by METIS_TAC [PAIR]
  THEN `?r3 e3 t3 t3' m3 i3. u = (r3,e3,t3,t3',m3,i3)` by METIS_TAC [PAIR]
...
val X64_ICACHE_THM = store_thm("X64_ICACHE_THM",
  ``X64_ICACHE (r,e,s,t,m,i) (r2,e2,s2,t2,m2,i2) =
    ?update.
      (r2,e2,s2,t2,m2,i2) = (r,e,s,t,m,icache update m i)``,
...
val ZREAD_CLAUSES = store_thm("ZREAD_CLAUSES",
  ``!s. (ZREAD_REG r (ZWRITE_REG r2 w s) = if r2 = r then w else ZREAD_REG r s) /\
        ...
        (ZREAD_REG r (ZWRITE_STACK t s) = ZREAD_REG r s) /\
        ...
        (ZREAD_RIP (ZWRITE_STACK t s) = ZREAD_RIP s) /\
        (ZREAD_STACK (ZWRITE_REG r2 w s) = ZREAD_STACK s) /\
        (ZREAD_STACK (ZWRITE_RIP e s) = ZREAD_STACK s) /\
        (ZREAD_STACK (ZWRITE_EFLAG f b s) = ZREAD_STACK s) /\
        (ZREAD_STACK (ZCLEAR_ICACHE s) = ZREAD_STACK s) /\
        (ZREAD_STACK (X64_ICACHE_UPDATE u s) = ZREAD_STACK s) /\
        (ZREAD_STACK (ZWRITE_MEM2 a x s) = ZREAD_STACK s) /\
        (ZREAD_STACK (ZWRITE_STACK t s) = t) /\
        ...
        (ZREAD_EFLAG i (ZWRITE_STACK t s) = ZREAD_EFLAG i s) /\
        ...
        (ZREAD_MEM2 a (ZWRITE_STACK t s) = ZREAD_MEM2 a s)``,
  STRIP_TAC THEN `?r2 e2 s2 t2 m2 i2. s = (r2,e2,s2,t2,m2,i2)` by METIS_TAC [pairTheory.PAIR]
  THEN Cases_on `u`
  THEN ASM_SIMP_TAC std_ss [ZREAD_REG_def,ZREAD_RIP_def,ZREAD_STACK_def,ZWRITE_STACK_def,
