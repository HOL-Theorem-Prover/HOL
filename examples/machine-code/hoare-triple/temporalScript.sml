...
val NOW_def        = Define `NOW sep f seq      = f sep (seq 0)`;
...
val INIT =
  `?to_set next instr less. model = (to_set,next,instr,less)`
  by METIS_TAC [PAIR]

val SPEC_EQ_TEMPORAL = store_thm("SPEC_EQ_TEMPORAL",
  ``SPEC model pre code post <=>
    TEMPORAL model code
      (T_IMPLIES (NOW pre) (EVENTUALLY (NOW post)))``,
  INIT \\ ASM_SIMP_TAC std_ss [SPEC_def,TEMPORAL_def,RUN_def,LET_DEF,
    PULL_FORALL,T_IMPLIES_def,NOW_def,EVENTUALLY_def,rel_sequence_def]
  \\ SIMP_TAC (std_ss) [AC STAR_ASSOC STAR_COMM,GSYM rel_sequence_def]
  \\ METIS_TAC []);

val TEMPORAL_ALWAYS = store_thm("TEMPORAL_ALWAYS",
  ``TEMPORAL model code (ALWAYS p) <=> TEMPORAL model code p``,
  INIT \\ ASM_SIMP_TAC std_ss [SPEC_def,TEMPORAL_def,RUN_def,LET_DEF,
    PULL_FORALL,T_IMPLIES_def,NOW_def,ALWAYS_def]
  \\ REPEAT STRIP_TAC \\ EQ_TAC \\ REPEAT STRIP_TAC THEN1
   (FIRST_X_ASSUM (MP_TAC o Q.SPECL [`state`,`seq`,`r`,`0`])
...
val TEMPORAL_NEXT_IMP_EVENTUALLY = store_thm("TEMPORAL_NEXT_IMP_EVENTUALLY",
  ``TEMPORAL model code (T_IMPLIES p1 (NEXT p2)) ==>
    TEMPORAL model code (T_IMPLIES p1 (EVENTUALLY p2))``,
  INIT \\ ASM_SIMP_TAC std_ss [TEMPORAL_def,LET_DEF,T_IMPLIES_def,NEXT_def,
    EVENTUALLY_def] \\ METIS_TAC []);
