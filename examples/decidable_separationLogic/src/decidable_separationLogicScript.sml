...
val FINITE___LIST_TO_SET = store_thm ("FINITE___LIST_TO_SET",
   ``!l. FINITE (LIST_TO_SET l)``,
   Induct_on `l` THENL [
      SIMP_TAC list_ss [LIST_TO_SET, combinTheory.C_DEF, GSYM EMPTY_DEF, FINITE_EMPTY],

      SIMP_TAC list_ss [LIST_TO_SET, combinTheory.C_DEF] THEN
      GEN_TAC THEN 
      `(\y. (y = h) \/ MEM y l) = h INSERT (LIST_TO_SET l)` by ALL_TAC THEN1 (
         SIMP_TAC std_ss [EXTENSION, IN_INSERT, IN_LIST_TO_SET] THEN
...
val SF_SEM___sf_tree_len_PERM_THM = store_thm ("SF_SEM___sf_tree_len_PERM_THM",
   ``!fL fL' n. 
         PERM fL fL' ==>
         !s h es e.
   ...
   CONJ_TAC THEN1 METIS_TAC[PERM_SYM] THEN
   ...
   Tactical.REVERSE (`?hL'. PERM hL hL' /\
      ...
      REPEAT STRIP_TAC THENL [
         ...
         SIMP_TAC list_ss [PERM_REFL, PERM_CONS_IFF, PERM_SWAP_AT_FRONT,
            ALL_DISJOINT_def, EVERY_MEM, MEM_MAP, GSYM LEFT_FORALL_IMP_THM,
            DISJOINT_DEF, EXTENSION, NOT_IN_EMPTY, IN_INTER] THEN
         REPEAT STRIP_TAC THENL [
            ASM_SIMP_TAC std_ss [GSYM fmap_EQ_THM, EXTENSION, FUNION_DEF, IN_UNION, DISJ_IMP_THM] THEN
            METIS_TAC[],

            `ALL_DISJOINT (MAP FDOM hL')` by METIS_TAC[PERM_MAP, ALL_DISJOINT___PERM] THEN
            FULL_SIMP_TAC std_ss [] THEN
         ...
         REPEAT STRIP_TAC THEN
         `?n. (n < LENGTH fL) /\ ((EL n' fL', EL n' hL') = (EL n fL, EL n hL))` by METIS_TAC[] THEN
         Q.EXISTS_TAC `n''` THEN
...
val LIST_DS_SEM_EVAL1 = prove (``
(!s h. LIST_DS_SEM s h ([], []) = (h = FEMPTY)) /\
(!s h. LIST_DS_SEM s h (pfL, []) = (LIST_PF_SEM s pfL /\ (h = FEMPTY))) /\
(!s h sfL pfL pf1 pf2. LIST_DS_SEM s h ((pf_and pf1 pf2)::sfL, pfL) = LIST_DS_SEM s h (pf1 :: pf2 :: sfL, pfL)) /\

(!s h sfL pfL e1 e2. LIST_DS_SEM s h ((pf_equal e1 e2)::sfL, pfL) = 
(DS_EXPRESSION_EQUAL s e1 e2 /\ LIST_DS_SEM s h (sfL, pfL))) /\

(!s h sfL pfL e1 e2. LIST_DS_SEM s h ((pf_unequal e1 e2)::sfL, pfL) = 
(~DS_EXPRESSION_EQUAL s e1 e2 /\ LIST_DS_SEM s h (sfL, pfL)))``,

SIMP_TAC std_ss [LIST_DS_SEM_def, LIST_PF_SEM_THM, LIST_SF_SEM_THM, PF_SEM_def,
   SF_SEM_def] THEN
METIS_TAC[]);



val LIST_DS_SEM_EVAL2 = prove (``
(!s h sfL pfL. LIST_DS_SEM s h (sfL, sf_emp::pfL) = LIST_DS_SEM s h (sfL, pfL)) /\

(!s h sfL pfL sf1 sf2. LIST_DS_SEM s h (sfL, (sf_star sf1 sf2)::pfL) = LIST_DS_SEM s h (sfL, sf1 :: sf2 :: pfL))``,

SIMP_TAC std_ss [LIST_DS_SEM_def, LIST_PF_SEM_THM, LIST_SF_SEM_THM, PF_SEM_def, SF_SEM_def,
   FUNION_FEMPTY_1, FDOM_FEMPTY, DISJOINT_EMPTY] THEN
REPEAT STRIP_TAC THENL [
   ...
   REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
      ...
      Q.EXISTS_TAC `h2'` THEN
      Q.EXISTS_TAC `h2` THEN
      FULL_SIMP_TAC std_ss [DISJOINT_DEF, EXTENSION, IN_INTER, NOT_IN_EMPTY, FUNION_DEF,
         IN_UNION, GSYM fmap_EQ_THM] THEN
      METIS_TAC[],
...
val LIST_DS_SEM_EVAL3 = prove (``
!s h fL sfL pfL es e. LIST_DS_SEM s h (pfL, (sf_tree fL es e)::sfL) = 
   if (DS_EXPRESSION_EQUAL s e es) then
...
HO_MATCH_MP_TAC (prove (``(!h1 h2. P h1 h2 = Q h1 h2) ==> ((?h1 h2. P h1 h2) = (?h1 h2. Q h1 h2))``,
      METIS_TAC[])) THEN
REPEAT GEN_TAC THEN
...
val SF_EXPRESSION_SET___FINITE = store_thm ("SF_EXPRESSION_SET___FINITE",
   ...
   Induct_on `sf` THEN (
      ASM_SIMP_TAC std_ss [SF_EXPRESSION_SET_def, FINITE_EMPTY, FINITE_INSERT,
         FINITE_UNION, FINITE___LIST_TO_SET]
   ));
...
Cases_on `n` THENL [
   ...
   Induct_on `n'` THENL [
      ...
      ASM_SIMP_TAC std_ss [FDOM_FUPDATE, DISJOINT_DEF, IN_INSERT, EXTENSION, NOT_IN_EMPTY,
         ...
         FUN_FMAP_DEF, FINITE___LIST_TO_SET] THEN
      ...
      ASM_SIMP_TAC list_ss [MAP_MAP_o, combinTheory.o_DEF, FDOM_FEMPTY,
         ...
         FAPPLY_FUPDATE_THM, FUN_FMAP_DEF, FINITE___LIST_TO_SET, EL_IS_EL,
         MEM_MAP] THEN
      ...
      Induct_on `fL` THENL [
         ...
         ASM_SIMP_TAC list_ss [FDOM_FUPDATE, IN_INSERT, DOMSUB_FUPDATE, DOMSUB_FEMPTY,
            ...
            IN_LIST_TO_SET, FUN_FMAP_DEF, FINITE___LIST_TO_SET],
         ...
         REPEAT STRIP_TAC THENL [
            ...
            Tactical.REVERSE (`(MAP (HEAP_READ_ENTRY s h' e) fL) =
               ...
               Q.PAT_ASSUM `h'''' = X` (MP_TAC o GSYM) THEN
               FULL_SIMP_TAC std_ss [DS_EXPRESSION_EVAL_VALUE_def]
            ) THEN
...
REWRITE_TAC[prove(``2 = SUC 1``, DECIDE_TAC)] THEN
...
CONJ_TAC THEN1 (
   ASM_REWRITE_TAC[] THEN
   FULL_SIMP_TAC std_ss [SUBMAP_DEF, FUNION_DEF, IN_UNION, IN_SING, COND_RATOR, COND_RAND, 
      IN_INSERT, SUBMAP_DEF, DS_EXPRESSION_EVAL_VALUE_def, GET_DSV_VALUE_def] THEN
   METIS_TAC[]
) THEN
...
`SF_SEM s h''' sf'''` by ALL_TAC THEN1 (
   ...
   `h1 = (FUNION hl1 h')` by ALL_TAC THEN1 (
      ...
      ASM_SIMP_TAC std_ss [SUBMAP___FUNION___ID] THEN
      Q.PAT_ASSUM `Y = FUNION h1 h2'` (MP_TAC o GSYM) THEN
      Q.PAT_ASSUM `h' SUBMAP X` MP_TAC THEN
      Q.PAT_ASSUM `hl1 SUBMAP X` MP_TAC THEN
      SIMP_TAC std_ss [] THEN
...
Cases_on `sf''` THENL [
   ...
   `(FUNION h2 hl) SUBMAP (FUNION hl1 h')` by ALL_TAC THEN1 (
      ...
      CONJ_TAC THEN1 (
         ...
         Q.PAT_ASSUM `h' SUBMAP Y` MP_TAC THEN
   ...
   `FUNION hl1 h' = FUNION (FUNION h2 hl) h''` by ALL_TAC THEN1 (
      ...
      Q.PAT_ASSUM `FUNION h2 hl SUBMAP Y` MP_TAC THEN
   ...
   Q.PAT_ASSUM `h''' = Y` (K ALL_TAC) THEN
   ...
   REPEAT STRIP_TAC THENL [
      ...
      Q.PAT_ASSUM `DISJOINT (FDOM h) (FDOM h'''')` MP_TAC THEN
      Q.PAT_ASSUM `h = Y` (fn thm => (REWRITE_TAC [thm])) THEN
      SIMP_TAC std_ss [FDOM_FUNION, DISJOINT_UNION_BOTH, DISJOINT_SYM] THEN
      ASM_SIMP_TAC std_ss [],
...
val HEAP_DISTINCT_def = Define 
   ...
   ALL_DISTINCT (MAP (\(e1,e2). DS_EXPRESSION_EVAL_VALUE s e1) 
         (FILTER (\(e1,e2). ~(DS_EXPRESSION_EQUAL s e1 e2)) d)) /\
...
REPEAT STRIP_TAC THEN EQ_TAC THEN STRIP_TAC THENL [
   ...
   `?he. (FDOM he = {c'}) /\ SF_SEM s he (sf_tree fL es e)` by ALL_TAC THEN1 (
      ...
      ASM_SIMP_TAC list_ss [DOMSUB_FUPDATE, EVERY_MEM, MEM_MAP, GSYM LEFT_FORALL_IMP_THM,
         ...
         FUN_FMAP_DEF, FINITE___LIST_TO_SET, DOMSUB_FEMPTY, MAP_MAP_o, combinTheory.o_DEF,
...
val LIST_DS_ENTAILS___ELIM_PRECONDITION_2 = store_thm ("LIST_DS_ENTAILS___ELIM_PRECONDITION_2",
``!c21 c22 c1 pfL pfL' sfL sfL'. 
      LIST_DS_ENTAILS (c1, (c21++c22)) (pfL, sfL) (pfL', sfL') = 
      LIST_DS_ENTAILS (c1, c22) (pfL,(MAP (\(e1,e2). sf_tree [] e2 e1) c21)++sfL) (pfL',(MAP (\(e1,e2). sf_tree [] e2 e1) c21)++sfL')``,
...
Induct_on `c21` THENL [
   ...
   Cases_on `h` THEN
   SIMP_TAC list_ss [] THEN
   ASM_SIMP_TAC list_ss [GSYM INFERENCE_STAR_INTRODUCTION___tree, PERM_REFL] THEN
   POP_ASSUM (ASSUME_TAC o GSYM) THEN
   ASM_SIMP_TAC std_ss [] THEN
   SIMP_TAC std_ss [LIST_DS_ENTAILS_def] THEN 
   REPEAT GEN_TAC THEN
...
val LIST_DS_ENTAILS___ELIM_PRECONDITION_COMPLETE = store_thm ("LIST_DS_ENTAILS___ELIM_PRECONDITION_COMPLETE",
``!c1 c2. ?sfL2. !pfL pfL' sfL sfL'. 
      LIST_DS_ENTAILS (c1,c2) (pfL, sfL) (pfL', sfL') = 
...
Q.ABBREV_TAC `sfL2 = MAP (\(e1,e2). sf_tree [] e2 e1) c2 ++ MAP (\e. sf_points_to e []) c1` THEN
...
val INFERENCE_UNROLL_COLLAPSE_LS___IMPL___EMPTY = prove (
``!e1:('b, 'a) ds_expression e2 f pfL sfL pfL' sfL'. 
      (INFINITE (UNIV:'b set) /\ 
      (LIST_DS_ENTAILS ([], []) ((pf_equal e1 e2)::pfL, sfL) (pfL', sfL') /\ 
       (!x. LIST_DS_ENTAILS ([], []) ((pf_unequal e1 e2)::(pf_unequal (dse_const x) e2)::pfL, 
