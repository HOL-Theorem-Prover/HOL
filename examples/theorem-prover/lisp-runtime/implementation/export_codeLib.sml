...
fun write_code_to_file filename th = let
  ...
  fun int_to_hexstring n = "0x" ^ (Arbnum.toHexString o Arbnum.fromInt) n
  val (m,pre,c,post) = (dest_spec o concl) th
  val model_name = fst (dest_const m)
  val cs = (list_dest pred_setSyntax.dest_insert c)
  val _ = pred_setSyntax.is_empty (last cs) orelse (print ("\n\nCode does not end in empty set. Found:\n\n"^ (term_to_string (last cs)) ^"\n\n"); fail())
  val cs = butlast cs
  val vs = map (pairSyntax.dest_pair) cs
  val vs = map (fn (x,y) => (x,fst (pairSyntax.dest_pair y) handle e => y)) vs
  val vs = map (fn (x,y) => (if is_var x then ``0:num`` else
                             if wordsSyntax.is_word_add x then cdr (cdr x) else
