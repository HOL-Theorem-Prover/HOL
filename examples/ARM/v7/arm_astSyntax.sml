...
fun in_numeral_type ty =
  ...
  of NONE => false
   | SOME {Tyop,...} => mem Tyop ["one","bit0","bit1"];
...
fun inst_list x y =
  map (fn (ty,v) =>
         (if in_numeral_type ty then
            ...
            inst [alpha |-> ty]) v) (zip x y);

(* ------------------------------------------------------------------------- *)

fun mk_ast_const s = prim_mk_const{Name = s, Thy = "arm_ast"}
...
fun mk_Branch_Link_Exchange_Register t =
  mk_comb(Branch_Link_Exchange_Register_tm, inst_word_alpha ``:4`` t)
    |> mk_Branch
  handle HOL_ERR _ => raise ERR "mk_Branch_Link_Exchange_Register" "";

fun mk_Compare_Branch(r,s,t) =
  list_mk_comb(Compare_Branch_tm, inst_list [bool,``:6``,``:3``] [r,s,t])
    |> mk_Branch
  handle HOL_ERR _ => raise ERR "mk_Compare_Branch" "";

fun mk_Table_Branch_Byte(r,s,t) =
  list_mk_comb(Table_Branch_Byte_tm, inst_list [``:4``,bool,``:4``] [r,s,t])
    |> mk_Branch
...
fun mk_Signed_Multiply_Dual(r,s,t,u,v,w) =
  list_mk_comb(Signed_Multiply_Dual_tm,
...
fun mk_Divide(r,s,t,u) =
  list_mk_comb(Divide_tm, inst_list [bool,``:4``,``:4``,``:4``] [r,s,t,u])
    |> mk_DataProcessing
  handle HOL_ERR _ => raise ERR "mk_Divide" "";

fun mk_Signed_Most_Significant_Multiply(r,s,t,u,v) =
  list_mk_comb(Signed_Most_Significant_Multiply_tm,
...
fun mk_Signed_Most_Significant_Multiply_Subtract(r,s,t,u,v) =
  list_mk_comb(Signed_Most_Significant_Multiply_Subtract_tm,
    inst_list [``:4``,``:4``,``:4``,bool,``:4``] [r,s,t,u,v])
    |> mk_DataProcessing
  handle HOL_ERR _ =>
...
fun mk_Set_Endianness t =
  mk_comb(Set_Endianness_tm, inst [alpha |-> bool] t)
    |> mk_StatusAccess
...
fun mk_LoadStore t =
  mk_comb(LoadStore_tm, inst [alpha |-> ``:load_store_instruction``] t)
  handle HOL_ERR _ => raise ERR "mk_LoadStore" "";

fun mk_Load(r,s,t,u,v,w,x,y) =
  list_mk_comb(Load_tm,
...
fun mk_Store_Halfword(r,s,t,u,v,w,x) =
  list_mk_comb(Store_Halfword_tm,
...
fun mk_Coprocessor t =
  mk_comb(Coprocessor_tm, inst [alpha |-> ``:coprocessor_instruction``] t)
  handle HOL_ERR _ => raise ERR "mk_Coprocessor" "";

fun mk_Coprocessor_Load (r,s,t,u,v,w,x,y) =
  list_mk_comb(Coprocessor_Load_tm,
...
fun mk_Coprocessor_Transfer (r,s,t,u,v,w,x) =
  list_mk_comb(Coprocessor_Transfer_tm,
...
fun mk_Coprocessor_Transfer_Two (r,s,t,u,v,w) =
  list_mk_comb(Coprocessor_Transfer_Two_tm,
...
fun dest_op5 c e tm =
  case with_exn strip_comb tm e of
    (t,[t1,t2,t3,t4,t5]) => if same_const t c then (t1,t2,t3,t4,t5) else raise e
  | _ => raise e;
...
val dest_Mode1_immediate =
  dest_monop Mode1_immediate_tm (ERR "dest_Mode1_immediate" "");

val dest_Mode1_register =
  dest_triop Mode1_register_tm (ERR "dest_Mode1_register" "");

val dest_Mode1_register_shifted_register =
  dest_triop Mode1_register_shifted_register_tm
    (ERR "dest_Mode1_register_shifted_register" "");

val dest_Mode2_immediate =
  dest_monop Mode2_immediate_tm (ERR "dest_Mode2_immediate" "");

val dest_Mode2_register =
  dest_triop Mode2_register_tm (ERR "dest_Mode2_register" "");

val dest_Mode3_immediate =
  dest_monop Mode3_immediate_tm (ERR "dest_Mode3_immediate" "");

val dest_Mode3_register =
  dest_binop Mode3_register_tm (ERR "dest_Mode3_register" "");

val dest_Hint_debug =
  dest_monop Hint_debug_tm (ERR "dest_Hint_debug" "");

(* .. *)

val dest_Branch = dest_monop Branch_tm (ERR "dest_Branch" "");

val dest_Branch_Target =
  dest_monop Branch_Target_tm (ERR "dest_Branch_Target" "") o dest_Branch;

val dest_Branch_Exchange =
  dest_monop Branch_Exchange_tm (ERR "dest_Branch_Exchange" "") o dest_Branch;

val dest_Branch_Link_Exchange_Immediate =
  dest_triop Branch_Link_Exchange_Immediate_tm
    (ERR "dest_Branch_Link_Exchange_Immediate" "") o dest_Branch;

val dest_Branch_Link_Exchange_Register =
  dest_monop Branch_Link_Exchange_Register_tm
    (ERR "dest_Branch_Link_Exchange_Register" "") o dest_Branch;

val dest_Compare_Branch =
  dest_triop Compare_Branch_tm (ERR "dest_Compare_Branch" "") o dest_Branch;

val dest_Table_Branch_Byte =
  dest_triop Table_Branch_Byte_tm (ERR "dest_Table_Branch_Byte" "") o
  dest_Branch;

(* .. *)

val dest_DataProcessing =
  dest_monop DataProcessing_tm (ERR "dest_DataProcessing" "");
...
val dest_Move_Halfword =
  dest_triop Move_Halfword_tm (ERR "dest_Move_Halfword" "") o
  dest_DataProcessing;
...
val dest_Count_Leading_Zeroes =
  dest_binop Count_Leading_Zeroes_tm (ERR "dest_Count_Leading_Zeroes" "") o
  dest_DataProcessing;

val dest_Reverse_Bits =
  dest_binop Reverse_Bits_tm (ERR "dest_Reverse_Bits" "") o
  dest_DataProcessing;

val dest_Byte_Reverse_Word =
  dest_binop Byte_Reverse_Word_tm (ERR "dest_Byte_Reverse_Word" "") o
  dest_DataProcessing;
...
val dest_Select_Bytes =
  dest_triop Select_Bytes_tm (ERR "dest_Select_Bytes" "") o
  dest_DataProcessing;
...
val dest_Byte_Reverse_Packed_Halfword =
  dest_binop Byte_Reverse_Packed_Halfword_tm
    (ERR "dest_Byte_Reverse_Packed_Halfword" "") o
  dest_DataProcessing;

val dest_Byte_Reverse_Signed_Halfword =
  dest_binop Byte_Reverse_Signed_Halfword_tm
...
val dest_StatusAccess = dest_monop StatusAccess_tm (ERR "dest_StatusAccess" "");

val dest_Status_to_Register =
  dest_binop Status_to_Register_tm (ERR "dest_Status_to_Register" "") o
  dest_StatusAccess;

val dest_Register_to_Status =
  dest_triop Register_to_Status_tm (ERR "dest_Register_to_Status" "") o
  dest_StatusAccess;

val dest_Immediate_to_Status =
  dest_triop Immediate_to_Status_tm (ERR "dest_Immediate_to_Status" "") o
  dest_StatusAccess;
...
val dest_Set_Endianness =
  dest_monop Set_Endianness_tm (ERR "dest_Set_Endianness" "") o
  dest_StatusAccess;

(* .. *)

val dest_LoadStore = dest_monop LoadStore_tm (ERR "dest_LoadStore" "");
...
val dest_Load_Exclusive =
  dest_triop Load_Exclusive_tm (ERR "dest_Load_Exclusive" "") o
  dest_LoadStore;
...
val dest_Load_Exclusive_Doubleword =
  dest_triop Load_Exclusive_Doubleword_tm
...
val dest_Load_Exclusive_Halfword =
  dest_binop Load_Exclusive_Halfword_tm
    (ERR "dest_Load_Exclusive_Halfword" "") o
  dest_LoadStore;

val dest_Store_Exclusive_Halfword =
  dest_triop Store_Exclusive_Halfword_tm
    (ERR "dest_Store_Exclusive_Halfword" "") o
  dest_LoadStore;

val dest_Load_Exclusive_Byte =
  dest_binop Load_Exclusive_Byte_tm (ERR "dest_Load_Exclusive_Byte" "") o
  dest_LoadStore;

val dest_Store_Exclusive_Byte =
  dest_triop Store_Exclusive_Byte_tm (ERR "dest_Store_Exclusive_Byte" "") o
  dest_LoadStore;
...
val dest_Miscellaneous =
  dest_monop Miscellaneous_tm (ERR "dest_Miscellaneous" "");

val dest_Hint =
  dest_monop Hint_tm (ERR "dest_Hint" "") o
  dest_Miscellaneous;

val dest_Breakpoint =
  dest_monop Breakpoint_tm (ERR "dest_Breakpoint" "") o
  dest_Miscellaneous;

val dest_Data_Memory_Barrier =
  dest_monop Data_Memory_Barrier_tm (ERR "dest_Data_Memory_Barrier" "") o
  dest_Miscellaneous;
...
val dest_Preload_Instruction =
  dest_triop Preload_Instruction_tm (ERR "dest_Preload_Instruction" "") o
  dest_Miscellaneous;

val dest_Supervisor_Call =
  dest_monop Supervisor_Call_tm (ERR "dest_Supervisor_Call" "") o
  dest_Miscellaneous;

val dest_Secure_Monitor_Call =
  dest_monop Secure_Monitor_Call_tm (ERR "dest_Secure_Monitor_Call" "") o
  dest_Miscellaneous;

val dest_If_Then =
  dest_binop If_Then_tm (ERR "dest_If_Then" "") o
  dest_Miscellaneous;

val dest_Data_Synchronization_Barrier =
  dest_monop Data_Synchronization_Barrier_tm
    (ERR "dest_Data_Synchronization_Barrier" "") o
  dest_Miscellaneous;

val dest_Instruction_Synchronization_Barrier =
  dest_monop Instruction_Synchronization_Barrier_tm
...
val dest_Coprocessor =
  dest_monop Coprocessor_tm (ERR "dest_Coprocessor" "");
