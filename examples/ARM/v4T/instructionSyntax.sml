...
val mk_word7 = mk_word ``:7``;
val mk_word8 = mk_word ``:8``;
val mk_word11 = mk_word ``:11``;
...
fun mk_register_ (NReg_ n) = (mk_word3 o Arbnum.fromInt o register2int_) n
  | mk_register_ (VReg_ x) = mk_var (x, ``:word3``);
...
fun mk_opcode2 opc =
  case opc of
    AND_  => ``instruction$AND_``  | EOR_  => ``instruction$EOR_``
  | LSL_2 => ``instruction$LSL_2`` | LSR_2 => ``instruction$LSR_2``
  | ASR_2 => ``instruction$ASR_2`` | ADC_  => ``instruction$ADC_``
  | SBC_  => ``instruction$SBC_``  | ROR_  => ``instruction$ROR_``
  | TST_  => ``instruction$TST_``  | NEG_  => ``instruction$NEG_``
  | CMP_2 => ``instruction$CMP_2`` | CMN_  => ``instruction$CMN_``
  | ORR_  => ``instruction$ORR_``  | MUL_  => ``instruction$MUL_``
  | BIC_  => ``instruction$BIC_``  | MVN_  => ``instruction$MVN_``;

fun mk_opcode3 opc =
  case opc of
    STR_2  => ``instruction$STR_2``  | STRH_2 => ``instruction$STRH_2``
  | STRB_2 => ``instruction$STRB_2`` | LDRSB_ => ``instruction$LDRSB_``
  | LDR_2  => ``instruction$LDR_2``  | LDRH_2 => ``instruction$LDRH_2``
  | LDRB_2 => ``instruction$LDRB_2`` | LDRSH_ => ``instruction$LDRSH_``;

fun mk_opcode4 opc =
  case opc of
    STR_1  => ``instruction$STR_1``  | LDR_1  => ``instruction$LDR_1``
  | STRB_1 => ``instruction$STRB_1`` | LDRB_1 => ``instruction$LDRB_1``
  | STRH_1 => ``instruction$STRH_1`` | LDRH_1 => ``instruction$LDRH_1``;
...
fun mk_bx (Instruction (x,c)) =
 (case x of
    Bx y =>
      subst [``c:condition`` |-> mk_condition c,
             ``rm:word4``  |-> mk_register y]
             ``instruction$BX c rm``
  | _ => raise ERR "mk_bx" "not a bx instruction")
 | mk_bx _ = raise ERR "mk_bx" "not a bx instruction";

fun mk_swi_ex (Instruction (x,c)) =
 (case x of
    Swi_ex y =>
      subst [``c:condition`` |-> mk_condition c,
             ``imm:word24``  |-> mk_word24 (Arbnum.fromInt y)]
             ``instruction$SWI c imm``
  | _ => raise ERR "mk_swi_ex" "not a swi instruction")
 | mk_swi_ex _ = raise ERR "mk_swi_ex" "not a swi instruction";
...
fun arm_to_term (i as Instruction (x,c)) =
   (case x of
    ...
    | Undef       => mk_undef c)
 | arm_to_term (Thumb x) =
   let fun s1 y = subst [``Rd:word3``  |-> mk_register_ (#Rd y),
                         ``Rm:word3``  |-> mk_register_ (#Rm y),
                         ``imm:word5`` |-> mk_word5 (Arbnum.fromInt (#imm y))]
       fun s2 y = subst [``Rd:word3`` |-> mk_register_ (#Rd y),
                         ``Rn:word3`` |-> mk_register_ (#Rn y),
                         ``Rm:word3`` |-> mk_register_ (#Rm y)]
       fun s3 y = subst [``Rd:word3``  |-> mk_register_ (#Rd y),
                         ``Rn:word3``  |-> mk_register_ (#Rn y),
                         ``imm:word3`` |-> mk_word3 (Arbnum.fromInt (#imm y))]
       fun s4 y = subst [``Rd:word3``  |-> mk_register_ (#Rd y),
                         ``imm:word8`` |-> mk_word8 (Arbnum.fromInt (#imm y))]
       fun s5 y = subst [``Rn:word3``  |-> mk_register_ (#Rn y),
                         ``imm:word8`` |-> mk_word8 (Arbnum.fromInt (#imm y))]
       fun s6 y = subst [``f:word3->word3->thumb_instruction``
                                      |-> mk_opcode2 (#opc y),
                         ``Rd:word3`` |-> mk_register_ (#Rd y),
                         ``Rm:word3`` |-> mk_register_ (#Rm y)]
       fun s7 y = subst [``Rd:word4`` |-> mk_register (#Rd y),
                         ``Rm:word4`` |-> mk_register (#Rm y)]
       fun s8 y = subst [``f:word3->word3->word3->thumb_instruction``
                                      |-> mk_opcode3 (#opc y),
                         ``Rd:word3`` |-> mk_register_ (#Rd y),
                         ``Rn:word3`` |-> mk_register_ (#Rn y),
                         ``Rm:word3`` |-> mk_register_ (#Rm y)]
       fun s9 y = subst [``f:word3->word3->word5->thumb_instruction``
                                       |-> mk_opcode4 (#opc y),
                         ``Rd:word3``  |-> mk_register_ (#Rd y),
                         ``Rn:word3``  |-> mk_register_ (#Rn y),
                         ``imm:word5`` |-> mk_word5 (Arbnum.fromInt (#imm y))]
       fun s10 y = subst [``imm:word7`` |-> mk_word7 (Arbnum.fromInt y)]
       fun s11 y = subst [``R:bool``  |-> mk_bool (#R y),
                          ``l:word8`` |-> mk_word8 (Arbnum.fromInt (#list y))]
       fun s12 y = subst [``Rn:word3`` |-> mk_register_ (#Rn y),
                          ``l:word8``  |-> mk_word8 (Arbnum.fromInt (#list y))]
       fun s13 y = subst [``c:condition`` |-> mk_condition (#cond y),
                          ``imm:word8``  |-> mk_word8 (Arbnum.fromInt (#imm y))]
       fun s14 y = subst [``imm:word8`` |-> mk_word8 (Arbnum.fromInt y)]
       fun s15 y = subst [``imm1:11 word`` |-> mk_word11 (ibits 21 11 y),
                          ``imm2:11 word`` |-> mk_word11 (ibits 10 0 y)]
       fun s16 y = subst [``imm:11 word`` |-> mk_word11 (Arbnum.fromInt y)]
   in
   (case x of
      LSL_1  y => s1 y ``instruction$LSL_1 Rd Rm imm``
    | LSR_1  y => s1 y ``instruction$LSR_1 Rd Rm imm``
    | ASR_1  y => s1 y ``instruction$ASR_1 Rd Rm imm``
    | ADD_3  y => s2 y ``instruction$ADD_3 Rd Rn Rm``
    | SUB_3  y => s2 y ``instruction$SUB_3 Rd Rn Rm``
    | ADD_1  y => s3 y ``instruction$ADD_1 Rd Rn imm``
    | SUB_1  y => s3 y ``instruction$SUB_1 Rd Rn imm``
    | MOV_1  y => s4 y ``instruction$MOV_1 Rd imm``
    | CMP_1  y => s5 y ``instruction$CMP_1 Rn imm``
    | ADD_2  y => s4 y ``instruction$ADD_2 Rd imm``
    | SUB_2  y => s4 y ``instruction$SUB_2 Rd imm``
    | DP_    y => s6 y ``(f (Rd:word3) (Rm:word3)):thumb_instruction``
    | ADD_4  y => s7 y ``instruction$ADD_4 Rd Rm``
    | CMP_3  y => s7 y ``instruction$CMP_3 Rn Rm``
    | MOV_3  y => s7 y ``instruction$MOV_3 Rd Rm``
    | BX_    y => subst [``Rm:word4`` |-> mk_register y] ``instruction$BX_ Rm``
    | LDR_3  y => s4 y ``instruction$LDR_3 Rd imm``
    | DT_    y => s8 y
                  ``(f (Rd:word3) (Rn:word3) (Rm:word3)):thumb_instruction``
    | DT_imm y => s9 y
                  ``(f (Rd:word3) (Rn:word3) (imm:word5)):thumb_instruction``
    | STR_3  y => s4 y ``instruction$STR_3 Rd imm``
    | LDR_4  y => s4 y ``instruction$LDR_4 Rd imm``
    | ADD_5  y => s4 y ``instruction$ADD_5 Rd imm``
    | ADD_6  y => s4 y ``instruction$ADD_6 Rd imm``
    | ADD_7  y => s10 y ``instruction$ADD_7 imm``
    | SUB_4  y => s10 y ``instruction$SUB_4 imm``
    | PUSH   y => s11 y ``instruction$PUSH R l``
    | POP    y => s11 y ``instruction$POP R l``
    | LDMIA_ y => s12 y ``instruction$LDMIA_ Rn l``
    | STMIA_ y => s12 y ``instruction$STMIA_ Rn l``
    | B_1    y => s13 y ``instruction$B_1 c imm``
    | UND_     => ``instruction$UND_``
    | SWI_   y => s14 y ``instruction$SWI_ imm``
    | B_2    y => s16 y ``instruction$B_2 imm``
    | BL_    y => s15 y ``(instruction$BL_a imm1, instruction$BL_b imm2)``
    | BL_a   y => s16 y ``instruction$BL_a imm``
    | BL_b   y => s16 y ``instruction$BL_b imm``)
   end
 | arm_to_term (Data n) = mk_word32 n
 | arm_to_term (ThumbData n) = mk_word16 n;
...
fun index_size t = let open Arbnum in
   ...
   if t = ``:7`` then fromInt 128 else
   if t = ``:8`` then fromInt 256 else
   if t = ``:11`` then fromInt 2048 else
...
fun dest_register_ t = NReg_ (int2register_ (dest_wordi t))
  handle HOL_ERR e =>
    let val (x, t) = dest_var t in
      if t = ``:word3`` then VReg_ x else Raise (HOL_ERR e)
    end;
...
local
  val err = ERR "dest_bx" "not a variable-free branch instruction"
in
  fun dest_bx t =
   (case strip_comb t of
      (i, [c, rm]) =>
        if term_eq i ``instruction$BX`` then
          Instruction(Bx (dest_register rm),dest_condition c)
        else
          raise err
    | _ => raise err)
    handle HOL_ERR _ => raise err
...
local
  val err = ERR "dest_swi_ex" "not a variable-free swi instruction"
in
  fun dest_swi_ex t =
   (case strip_comb t of
      (i, [c, imm]) =>
        if term_eq i ``instruction$SWI`` then
          Instruction(Swi_ex (dest_wordi imm),dest_condition c)
        else
          raise err
    | _ => raise err)
    handle HOL_ERR _ => raise err
...
val unary_list =
  [(``instruction$ADD_7``, ADD_7), (``instruction$SUB_4``, SUB_4),
   (``instruction$SWI_``, SWI_), (``instruction$B_2``, B_2),
   (``instruction$BL_a``, BL_a), (``instruction$BL_b``, BL_b)];

val opc2_list =
  [(``instruction$AND_``, AND_), (``instruction$EOR_``, EOR_),
   (``instruction$LSL_2``, LSL_2), (``instruction$LSR_2``, LSR_2),
   (``instruction$ASR_2``, ASR_2), (``instruction$ADC_``, ADC_),
   (``instruction$SBC_``, SBC_), (``instruction$ROR_``, ROR_),
   (``instruction$TST_``, TST_), (``instruction$NEG_``, NEG_),
   (``instruction$CMP_2``, CMP_2), (``instruction$CMN_``, CMN_),
   (``instruction$ORR_``, ORR_) , (``instruction$MUL_``, MUL_),
   (``instruction$BIC_``, BIC_), (``instruction$MVN_``, MVN_)];

val binary_list =
  [(``instruction$MOV_1``, MOV_1), (``instruction$ADD_2``, ADD_2),
   (``instruction$SUB_2``, SUB_2), (``instruction$LDR_3``, LDR_3),
   (``instruction$STR_3``, STR_3), (``instruction$LDR_4``, LDR_4),
   (``instruction$ADD_5``, ADD_5), (``instruction$ADD_6``, ADD_6)];

val opc3_list =
  [(``instruction$STR_2``, STR_2), (``instruction$STRH_2``, STRH_2),
   (``instruction$STRB_2``, STRB_2), (``instruction$LDRSB_``, LDRSB_),
   (``instruction$LDR_2``, LDR_2), (``instruction$LDRH_2``, LDRH_2),
   (``instruction$LDRB_2``, LDRB_2), (``instruction$LDRSH_``, LDRSH_)];

val opc4_list =
  [(``instruction$STR_1``, STR_1), (``instruction$LDR_1``, LDR_1),
   (``instruction$STRB_1``, STRB_1), (``instruction$LDRB_1``, LDRB_1),
   (``instruction$STRH_1``, STRH_1), (``instruction$LDRH_1``, LDRH_1)];

fun term_to_arm t =
  if type_of t = ``:word32`` then
    Data (dest_word t)
  else if type_of t = ``:thumb_instruction`` then
    Thumb
     (case strip_comb t of
        (opc, [])   => if term_eq opc ``instruction$UND_`` then UND_ else
                         raise ERR "term_to_arm"
                                   "not a variable-free Thumb instruction"
      | (opc, [a])  => let val eqt = term_eq opc in
          (if eqt ``instruction$BX_`` then
             BX_ (dest_register a)
           else
            (case List.find (eqt o fst) unary_list of
               SOME y => (snd y) (dest_wordi a)
             | NONE => raise ERR "term_to_arm"
                                 "not a variable-free Thumb instruction"))
        end
      | (opc, [a, b])  => let val eqt = term_eq opc in
          (case List.find (eqt o fst) opc2_list of
             SOME y => DP_ {opc = snd y, Rd = dest_register_ a,
                            Rm = dest_register_ b}
           | NONE =>
              (if eqt ``instruction$CMP_1`` then
                 CMP_1 {Rn = dest_register_ a, imm = dest_wordi b}
               else if eqt ``instruction$ADD_4`` then
                 ADD_4 {Rd = dest_register a, Rm = dest_register b}
               else if eqt ``instruction$CMP_3`` then
                 CMP_3 {Rd = dest_register a, Rm = dest_register b}
               else if eqt ``instruction$MOV_3`` then
                 MOV_3 {Rd = dest_register a, Rm = dest_register b}
               else if eqt ``instruction$PUSH`` then
                 PUSH {R = dest_bool a, list = dest_wordi b}
               else if eqt ``instruction$POP`` then
                 POP {R = dest_bool a, list = dest_wordi b}
               else if eqt ``instruction$LDMIA_`` then
                 LDMIA_ {Rn = dest_register_ a, list = dest_wordi b}
               else if eqt ``instruction$STMIA_`` then
                 STMIA_ {Rn = dest_register_ a, list = dest_wordi b}
               else if eqt ``instruction$B_1`` then
                 B_1 {cond = dest_condition a, imm = dest_wordi b}
               else
                 case List.find (eqt o fst) binary_list of
                   SOME y => (snd y) {Rd = dest_register_ a, imm = dest_wordi b}
                 | NONE => raise ERR "term_to_arm"
                                    "not a variable-free Thumb instruction"))
        end
      | (opc, [a, b, c])  => let val eqt = term_eq opc in
          (case List.find (eqt o fst) opc3_list of
             SOME y => DT_ {opc = snd y, Rd = dest_register_ a,
                            Rn = dest_register_ b, Rm = dest_register_ c}
           | NONE =>
              (case List.find (eqt o fst) opc4_list of
                 SOME y => DT_imm {opc = snd y, Rd = dest_register_ a,
                             Rn = dest_register_ b, imm = dest_wordi c}
               | NONE =>
                  (if eqt ``instruction$LSL_1`` then
                     LSL_1 {Rd = dest_register_ a, Rm = dest_register_ b,
                            imm = dest_wordi c}
                   else if eqt ``instruction$LSR_1`` then
                     LSR_1 {Rd = dest_register_ a, Rm = dest_register_ b,
                            imm = dest_wordi c}
                   else if eqt ``instruction$ASR_1`` then
                     ASR_1 {Rd = dest_register_ a, Rm = dest_register_ b,
                            imm = dest_wordi c}
                   else if eqt ``instruction$ADD_3`` then
                     ADD_3 {Rd = dest_register_ a, Rn = dest_register_ b,
                            Rm = dest_register_ c}
                   else if eqt ``instruction$SUB_3`` then
                     SUB_3 {Rd = dest_register_ a, Rn = dest_register_ b,
                            Rm = dest_register_ c}
                   else if eqt ``instruction$ADD_1`` then
                     ADD_1 {Rd = dest_register_ a, Rn = dest_register_ b,
                            imm = dest_wordi c}
                   else if eqt ``instruction$SUB_1`` then
                     SUB_1 {Rd = dest_register_ a, Rn = dest_register_ b,
                            imm = dest_wordi c}
                   else
                     raise ERR "term_to_arm"
                               "not a variable-free Thumb instruction")))
        end
      | _ => raise ERR "term_to_arm" "not a variable-free Thumb instruction")
  else
    let val opc = fst (strip_comb t)
        val eqt = term_eq opc
  in
    if eqt ``instruction$SWI`` then
      dest_swi_ex t
    else if eqt ``instruction$BX`` then
      dest_bx t
...
val decode_instruction_ = arm_to_term o num_to_thumb;
val decode_instruction_dec_ = decode_instruction_ o Arbnum.fromString;
val decode_instruction_hex_ = decode_instruction_ o Arbnum.fromHexString;
