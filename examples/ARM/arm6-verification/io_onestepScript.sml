  ...
  RW_TAC bool_ss [PACKED_STRM_def]
    ...
    << [
      ...
      IMP_RES_TAC (CONJ lem2 lem4)
        ...
        \\ PAT_ASSUM `!x. P` (SPEC_THEN `x` ASSUME_TAC)
        \\ ASM_SIMP_TAC arith_ss
  ...
  RW_TAC bool_ss [PTCON_def,IS_PMAP_INIT_def]
    << [
      ...
      FULL_SIMP_TAC bool_ss [PMAP_def]
        ...
        \\ PROVE_TAC []]);
(* in HOL4:
val _ = set_trace "meson" 0;
val _ = set_trace "meson" 2;
Saved theorem ADVANCE_ONE
...
Internal goal solved with 4 MESON inferences.
  solved with 4 MESON inferences.
0 inferences so far. Searching with maximum size 0.
0 inferences so far. Searching with maximum size 1.
1 inferences so far. Searching with maximum size 2.
Internal goal solved with 4 MESON inferences.
  solved with 4 MESON inferences.
Definition has been stored under "IMAP_def".
. . .
...
0 inferences so far. Searching with maximum size 1.
1 inferences so far. Searching with maximum size 2.
3 inferences so far. Searching with maximum size 3.
7 inferences so far. Searching with maximum size 4.
16 inferences so far. Searching with maximum size 5.
32 inferences so far. Searching with maximum size 6.
72 inferences so far. Searching with maximum size 7.
137 inferences so far. Searching with maximum size 8.
248 inferences so far. Searching with maximum size 9.
441 inferences so far. Searching with maximum size 10.
717 inferences so far. Searching with maximum size 11.
1132 inferences so far. Searching with maximum size 12.
1786 inferences so far. Searching with maximum size 13.
2694 inferences so far. Searching with maximum size 14.
4106 inferences so far. Searching with maximum size 15.
6310 inferences so far. Searching with maximum size 16.
9542 inferences so far. Searching with maximum size 17.
14425 inferences so far. Searching with maximum size 18.
21911 inferences so far. Searching with maximum size 19.
32428 inferences so far. Searching with maximum size 20.
48244 inferences so far. Searching with maximum size 21.
72412 inferences so far. Searching with maximum size 22.
107642 inferences so far. Searching with maximum size 23.
162292 inferences so far. Searching with maximum size 24.
248071 inferences so far. Searching with maximum size 25.
377287 inferences so far. Searching with maximum size 26.
578369 inferences so far. Searching with maximum size 27.
897921 inferences so far. Searching with maximum size 28.
1377198 inferences so far. Searching with maximum size 29.
2125710 inferences so far. Searching with maximum size 30.
Uncaught exception:
HOL_ERR

1 subgoal:
> val it =
    Pstate_out_state (PNEXT next out (f (t1 + t2) x)) =
    Pstate_out_state (PNEXT next out (f t1 (Pstate_out_state (f t2 x))))
    ------------------------------------
      0.  ∀x. f 0 x = PINIT init out x
      1.  ∀t x. f (SUC t) x = PNEXT next out (f t x)
      2.  ∀t x.
            x.inp ∈ strm ⇒
            ADVANCE t x.inp ∈ strm ∧
            (Pstate_out_state (PINIT init out (Pstate_out_state (f t x))) =
             Pstate_out_state (f t x))
      3.  x.inp ∈ strm
      4.  Pstate_out_state (f (t1 + t2) x) =
          Pstate_out_state (f t1 (Pstate_out_state (f t2 x)))
      5.  ∀:δ.
            ∀out next.
              PNEXT next out (f (t1 + t2) x) =
              PNEXT next out (f t1 (Pstate_out_state (f t2 x)))
     : proof
...
val ths = map NORM_RULE working
val ths' = map (NORM_RULE o TY_SPEC_ALL) working

(* mesonLib.MESON_TAC ths *)
(* mesonLib.MESON_TAC ths' *)

open mesonLib;
(* Scan all of mesonLib.sml *)
...
(* GEN_MESON_TAC 0 (!max_depth) 1 ths *)

val min = 0 and max = !max_depth and step = 1;

e(POP_ASSUM_LIST (K ALL_TAC));

val g = top_goal();

val conv = REWR_CONV(TAUT `p = ~p ==> F`);

e(CONV_TAC conv THEN DISCH_THEN ASSUME_TAC);

open jrhTactics

(*
convert (POLY_ASSUME_TAC (map GEN_ALL ths)
                      THEN PREMESON_CANON_TAC
                      THEN PURE_MESON_TAC min max step)

GEN_ALL (hd ths)
TY_TM_GEN_ALL (hd ths)
*)

val ths1 = map GEN_ALL ths
val ths1' = map GEN_ALL ths'

GEN_MESON_TAC min max step ths g
GEN_MESON_TAC min max step ths' g

val g' = (map ASSUME ## I) g;

val (gs1,jst1) = POLY_ASSUME_TAC ths1 g'

val g1 = hd gs1;

val (gs2,jst2) = PREMESON_CANON_TAC g1;


fun GSPEC th = SPEC (genvar(type_of(fst(dest_forall(concl th))))) th

Look in PREMESON_CANON_TAC:
Need to type-specialize each hyp (RULE_ASSUM_TAC)

RULE_ASSUM_TAC TY_SPEC_ALL g1;


Could just stick a RULE_ASSUM_TAC(liteLib.repeat GTY_SPEC) at the beginning of PREMESON_CANON_TAC,
but it seems better to integrate the type simplification conversions/theorems
with the elements of PREMESON_CANON_TAC, that is,
to revise 
    PRESIMP_CONV
    DELAMB_CONV
    NNFC_CONV
    (not SKOLEM_CONV; or a type-skolemization feature? create a type operator?)
    add TY_CHOOSE_TAC?
    ?? ASM_FOL_TAC
    PRENEX_CONV
    PROP_CNF_CONV

PRESIMP_CONV is run first on each hyp
PRESIMP_CONV THENC DELAMB_CONV THENC NNFC_CONV THENC SKOLEM_CONV
  ...
  RW_TAC bool_ss [IS_PMAP_INIT_def,IMMERSION,PTCON_IMMERSION]
   ...
   << [
     ...
     PAT_ASSUM `!t1:num t2 x. P`
       ...
       \\ PAT_ASSUM `(!x. f 0 x = PINIT init out x) /\ y`
  ...
  REPEAT STRIP_TAC
    ...
    << [
      ...
      FULL_SIMP_TAC bool_ss [PUNIFORM_def,IS_PMAP_INIT_def]
        \\ PAT_ASSUM `PUIMMERSION imm f dur`
...
val lem = prove(
  ...
  RW_TAC bool_ss [PUNIFORM_def,PUIMMERSION_def]
   ...
   \\ PAT_ASSUM `!t1 t2 x. P` (SPECL_THEN [`t`,`0`,`x`] IMP_RES_TAC)
   \\ PAT_ASSUM `!x. imm x 0 = 0`
        (fn th => RULE_ASSUM_TAC (REWRITE_RULE [ADD_CLAUSES,th]))
   \\ POP_ASSUM (SPEC_THEN `t` ASSUME_TAC)
   \\ PAT_ASSUM `imm (Pstate_out_state (f 0 x)) t = imm x t`
        (fn th => RULE_ASSUM_TAC (REWRITE_RULE [th]))
   \\ POP_ASSUM (fn th => REWRITE_TAC [GSYM th]));
...
val PTCON_IMMERSION_COR = prove(
  ...
  REPEAT STRIP_TAC
    ...
    \\ RW_TAC arith_ss []
    \\ FULL_SIMP_TAC bool_ss [PTCON_IMMERSION]
    \\ PAT_ASSUM `!t1 t2 x. P` (SPECL_THEN [`t2`,`t1`,`x`] IMP_RES_TAC)
...
val POUPUT_ONE_STEP_THM = prove(
  ...
  RW_TAC std_ss []
    ...
    \\ PAT_ASSUM `!x. a ==> (POUTPUT spec b 0 = c)`
         (SPEC_THEN `Pstate_out_state (impl (imm x t) x)` ASSUME_TAC)
    \\ PAT_ASSUM `!x. a ==> b` (SPEC_THEN `x` IMP_RES_TAC)
 ...
 REPEAT STRIP_TAC
    ...
    << [
       ...
       SPECL_THEN [`sstrm`,`istrm`,`spec`,`impl`,`imm`,`abs`,`ismpl`,`f`]
         ...
         \\ PAT_ASSUM `IS_PMAP impl` (fn th => FULL_SIMP_TAC bool_ss [th])
         \\ PAT_ASSUM `IS_PMAP spec`
              (fn th => FULL_SIMP_TAC bool_ss [th] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `PUNIFORM imm impl`
              (fn th => FULL_SIMP_TAC bool_ss [th] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `STREAM_ABSTRACTION ismpl sstrm istrm`
              (fn th => FULL_SIMP_TAC bool_ss [th] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `PTCON spec sstrm`
              (fn th => FULL_SIMP_TAC bool_ss [th] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `PTCON_IMMERSION impl imm istrm`
              (fn th => FULL_SIMP_TAC bool_ss [th] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `PTCON_SMPL ismpl imm impl istrm`
              (fn th => FULL_SIMP_TAC std_ss [th,MAP_STRM_def] \\ ASSUME_TAC th)
         \\ PAT_ASSUM `!x. a ==> (POUTPUT spec b 0 = c)`
              (fn th => FULL_SIMP_TAC bool_ss [th])
         \\ PAT_ASSUM `x.inp IN istrm` MP_TAC
         \\ SPEC_TAC (`x`,`x`)
         \\ PAT_ASSUM `a ==> b` MATCH_MP_TAC
         \\ REPEAT STRIP_TAC
    ]
    \\ Induct_on `t` >> ASM_SIMP_TAC bool_ss []
    \\ PAT_ASSUM `!x. x.inp IN istrm ==> !t. P` IMP_RES_TAC
    \\ POP_ASSUM (fn th => REWRITE_TAC [th])
    \\ PAT_ASSUM `PTCON_IMMERSION impl imm istrm`
    ...
    \\ PAT_ASSUM `!t x. P` (fn th => ASM_SIMP_TAC (srw_ss()) [th])
    \\ PAT_ASSUM `IS_PMAP spec` (fn th => ASSUME_TAC (MATCH_MP (GSYM PMAP2) th))
    ...
    \\ PAT_ASSUM `!t1 t2 x. P` (fn th => IMP_RES_TAC
...
val IMAP_TCON_IMMERSION = prove(
  ...
  RW_TAC bool_ss [TCON_IMMERSION,PTCON_IMMERSION]
    ...
    \\ IMP_RES_TAC IMAP_PMAP
    \\ EQ_TAC \\ REPEAT STRIP_TAC
    \\ PAT_ASSUM `!x t. THE_PMAP init next out t x =
...
val TCON_IMMERSION_COR = store_thm("TCON_IMMERSION_COR",
  ...
  RW_TAC bool_ss [IS_IMAP_def]
    ...
    \\ PAT_ASSUM `TCON_IMMERSION impl imm istrm` (K ALL_TAC)
    \\ `PUNIFORM imm (THE_PMAP init next out)` by IMP_RES_TAC IMAP_UNIFORM
    \\ PAT_ASSUM `UNIFORM imm impl` (K ALL_TAC)
...
val ONE_STEP_THM = store_thm("ONE_STEP_THM",
  ...
  RW_TAC bool_ss [IS_IMAP_def,OSMPL] \\ RULE_ASSUM_TAC PairRules.PBETA_RULE
    ...
    \\ PAT_ASSUM `TCON spec sstrm` (K ALL_TAC)
    \\ `PUNIFORM imm (THE_PMAP init' next' out')` by IMP_RES_TAC IMAP_UNIFORM
    \\ PAT_ASSUM `UNIFORM imm impl` (K ALL_TAC)
    ...
    \\ POP_ASSUM IMP_RES_TAC
    \\ PAT_ASSUM `TCON_SMPL ismpl imm impl istrm` (K ALL_TAC)
    \\ ISPECL_THEN [`init`,`next`,`out`] ASSUME_TAC THE_PMAP
    \\ ISPECL_THEN [`init'`,`next'`,`out'`] ASSUME_TAC THE_PMAP
    \\ IMP_RES_TAC IMAP_PMAP
    ...
    \\ NTAC 2 (PAT_ASSUM `!x t. THE_PMAP init next out t x = z`
...
val DATA_ABSTRACTION_I = store_thm("DATA_ABSTRACTION_I",
  ...
  RW_TAC (srw_ss()++boolSimps.LET_ss) [IS_IMAP_INIT_def,IMAP_def,
    ...
    \\ REPEAT STRIP_TAC
    \\ PAT_ASSUM `!x. (f 0 x).state = y` (SPEC_THEN `<|state := a; inp := i|>`
         (ASSUME_TAC o SIMP_RULE (srw_ss()) [] o SYM))
    \\ PAT_ASSUM `!x. (?x'. x = (f 0 x').state) ==>
         ?y. (?x. y = (g 0 x).state) /\ (abs y = x)` (SPEC_THEN `a` IMP_RES_TAC)
    \\ PAT_ASSUM `!x. (g 0 x).state = z` (SPEC_THEN `x` (ASSUME_TAC o SYM))
...
val CORRECT = prove(
  ...
  RW_TAC (srw_ss()++boolSimps.LET_ss) [CORRECT_def]
    ...
    \\ FULL_SIMP_TAC (srw_ss()) []
    << [
      PAT_ASSUM `!x. P` (SPEC_THEN `<| state := a; inp := i |>`
        (IMP_RES_TAC o SIMP_RULE (srw_ss()) [])),
      PAT_ASSUM `!x. P` (SPEC_THEN `<| state := a; inp := i |>`
        (IMP_RES_TAC o SIMP_RULE (srw_ss()) [])),
      PAT_ASSUM `!a i. P` (SPECL_THEN [`x.state`,`x.inp`]
...
val STATE_FUNPOW_INIT4 = store_thm("STATE_FUNPOW_INIT4",
  ...
  REPEAT STRIP_TAC
    ...
    \\ PAT_ASSUM `!x t. P` (fn th => ASM_SIMP_TAC (srw_ss()++boolSimps.LET_ss)
