...
val sorry : tactic = fn g => ([],fn _ => mk_thm g)
...
val MAP_gtpm_eqr = store_thm(
"MAP_gtpm_eqr",
``!t u. (t = MAP (gtpm pi) u) = (u = MAP (gtpm (REVERSE pi)) t)``,
Induct >- srw_tac [][] >>
gen_tac >>
Cases >- srw_tac [][] >>
srw_tac [][gtpm_eqr] >>
...
val pm_sing_o = prove(
``is_perm pm ==> (pm [xy] o pm [xy] = I)``,
srw_tac [][FUN_EQ_THM,is_perm_sing_inv]);
