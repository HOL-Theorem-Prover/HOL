...
Theorem extension_chain:
  !subst source z.
    ...
    (!n. z < n ==>
         FDOM (subst n) UNION (source n) = FDOM (subst z) UNION (source z))
   ==>
...
`!m. z <= m ==> ?n.m < n /\ CARD(source n) < CARD(source m)`
   suffices_by (
    DISCH_THEN
      (STRIP_ASSUME_TAC o
       SIMP_RULE (srw_ss()) [SKOLEM_THM,GSYM RIGHT_EXISTS_IMP_THM]) THEN
    `?ff.!n.ff (SUC n) < ff n`
    ...
    METIS_TAC [WF_LESS,WF_IFF_WELLFOUNDED,wellfounded_def]
   ) THEN
