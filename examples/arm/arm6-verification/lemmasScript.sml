...
Theorem ARITHMETIC_THM2:
   !i:word32. ~(i %% 23) /\ ~(i %% 22) \/ i %% 24 /\ i %% 23 <=>
         ~ARITHMETIC ((24 >< 21) i)
Proof RW_TAC bool_ss [ARITHMETIC_THM]
QED
...
Theorem REG_READ6_TO_READ_SUB8:
...
Theorem DECODE_INST_NOT_UNEXEC:
   !n. ~(DECODE_INST n = unexec)
Proof RW_TAC std_ss [DECODE_INST_def]
QED
...
Theorem DATA_PROC_IMP_NOT_BIT4:
   !i. (DECODE_INST i = data_proc) /\ ~(i %% 25) ==>
         ~(((11 >< 0) i):word12 %% 4)
Proof tac
QED

Theorem REG_SHIFT_IMP_BITS:
   !i. (DECODE_INST i = reg_shift) ==>
        ~(i %% 25) /\ ((11 >< 0) i):word12 %% 4
Proof tac
QED

Theorem LDR_IMP_BITS:
   !i. (DECODE_INST i = ldr) ==> (i %% 20)
Proof
  RW_TAC arith_ss [DECODE_INST_def]
QED

Theorem STR_IMP_BITS:
   !i. (DECODE_INST i = str) ==> ~(i %% 20)
Proof
  RW_TAC arith_ss [DECODE_INST_def]
QED

Theorem DECODE_INST_LDM:
   !i. (DECODE_INST i = ldm) ==> i %% 20
Proof
  RW_TAC arith_ss [DECODE_INST_def]
QED

Theorem DECODE_INST_STM:
...
Theorem ALUOUT_ALU_logic:
   !a. SND (ALU_logic a) = a
Proof
  SIMP_TAC std_ss [ALU_logic_def]
QED

Theorem NZ_ALU_logic:
