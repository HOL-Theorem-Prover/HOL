...
(* ------------------------------------------------------------------------- *)
(* Rewrites consumed by numContext.sml.  Kept here rather than in            *)
(* arithmeticTheory so we don't have to touch the kernel to feed the         *)
(* miller SIMPLIFY context.                                                  *)
(* ------------------------------------------------------------------------- *)
...
Theorem NUM_LT_0_MUL:
  !m n : num. 0 < m * n <=> 0 < m /\ 0 < n
Proof
  Cases_on `m` >> Cases_on `n` >> RW_TAC arith_ss []
QED
