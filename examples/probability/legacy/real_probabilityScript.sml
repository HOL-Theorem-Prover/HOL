...
Theorem PROB_SPACE_COUNTABLY_ADDITIVE:
...
Theorem PROB_SPACE_INCREASING:   !p. prob_space p ==> increasing p
Proof
...
Theorem INDEP_EMPTY:   !p s. prob_space p /\ s IN events p ==> indep p {} s
Proof
    RW_TAC real_ss [indep_def, EVENTS_EMPTY, PROB_EMPTY, INTER_EMPTY]
QED

Theorem INTER_PSPACE:
...
Theorem INDEP_SPACE:   !p s. prob_space p /\ s IN events p ==> indep p (p_space p) s
Proof
...
Theorem PROB_FINITELY_ADDITIVE:
    !p s f n.
...
Theorem INDEP_SYM:   !p a b. prob_space p /\ indep p a b ==> indep p b a
Proof
    RW_TAC std_ss [indep_def]
...
Theorem PROB_REAL_SUM_IMAGE_FN:
...
Theorem PROB_REAL_SUM_IMAGE_SPACE:
...
Theorem marginal_joint_zero:   !p X Y s t. prob_space p /\ (events p = POW (p_space p)) /\
...
Theorem distribution_prob_space:
...
Theorem joint_distribution_sym:  !p X Y a b. prob_space p  ==>
...
Theorem joint_distribution_pos:  !p X Y a. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem joint_distribution_le_1:  !p X Y a. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem joint_distribution_le:  !p X Y a b. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem joint_distribution_le2:  !p X Y a b. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem joint_conditional:  !p X Y a b. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem distribution_pos:  !p X a. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem conditional_distribution_pos:  !p X Y a b. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem conditional_distribution_le_1:  !p X Y a b. prob_space p /\ (events p = POW (p_space p)) ==>
...
Theorem marginal_distribution1:  !p X Y a. prob_space p /\ FINITE (p_space p) /\
...
Theorem marginal_distribution2:  !p X Y b. prob_space p /\ FINITE (p_space p) /\
                                       (events p = POW (p_space p))
    ==> (distribution p Y b =
