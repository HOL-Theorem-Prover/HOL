...
val CHINESE = store_thm("CHINESE",
                        ...
                        THEN POP_ASSUM MP_TAC THEN ARW[divides_def]
                        THEN `divides p (q'*q)` by PROVE_TAC[divides_def]
...
val RSA_CORRECT = store_thm("RSA_CORRECT",
                     ...
                     ARW[POWER_POWER]
                     THEN `1 <= e*d` by PROVE_TAC[POWER_LE,CONGRUENT_LE_1,PHI_GT_1]
                     THEN `0 < e*d` by ARW[]
                     THEN `divides ((p-1)*(q-1)) ((e*d)-1)` by PROVE_TAC[CONGRUENT_DIVIDES]
                     ...
                     THENL [
                      ...
                      Cases_on `divides p w`
                        THENL [
                          PROVE_TAC[CONGRUENT_TRANS,CONGRUENT_SYM,POWER_0,CONGRUENT_POWER,DIVIDES_CONGRUENT],ALL_TAC
                        ] 
                        THEN `congruent (power w (p-1)) 1 p` by PROVE_TAC[FERMAT]
                        THEN `congruent (power (power w (p-1)) (q' * (q-1))) 1 p` by PROVE_TAC[CONGRUENT_POWER,POWER_1]
                        THEN `congruent (power w (e*d-1)) 1 p` by PROVE_TAC[POWER_POWER,MULT_ASSOC,MULT_SYM]
                        THEN `congruent (w * power w (e*d -1)) w p` by PROVE_TAC[CONGRUENT_TIMES,MULT_SYM,MULT_LEFT_1]
                        THEN PROVE_TAC[SUB_ADD,power_def,ADD1]
                      ,
                      Cases_on `divides q w`
                        THENL [
                          PROVE_TAC[CONGRUENT_TRANS,CONGRUENT_SYM,POWER_0,CONGRUENT_POWER,DIVIDES_CONGRUENT],ALL_TAC
                        ] 
                        THEN `congruent (power w (q-1)) 1 q` by PROVE_TAC[FERMAT]
                        THEN `congruent (power (power w (q - 1)) (q' * (p - 1))) 1 q` by PROVE_TAC[CONGRUENT_POWER,POWER_1]
                        THEN `congruent (power w (e*d-1)) 1 q` by PROVE_TAC[POWER_POWER,MULT_ASSOC,MULT_SYM]
                        THEN `congruent (w * power w (e*d -1)) w q` by PROVE_TAC[CONGRUENT_TIMES,MULT_SYM,MULT_LEFT_1]
                        THEN PROVE_TAC[SUB_ADD,power_def,ADD1]
                     ] 
                  );
