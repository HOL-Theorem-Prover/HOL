...
val word_index = METIS_PROVE [word_index_n2w]
  ``!i n. i < dimindex (UNIV:'a->bool) ==> ((n2w n):'a word %% i = BIT i n)``;
...
fun eval_nop t = SYMBOLIC_EVAL_CONV NOP_ss (nop_cntxt
  (subst [``f:condition -> bool -> word4 ->
   ...
   ``enc ((f:condition -> bool -> word4 ->
             word4 -> addr_mode1 -> arm_instruction) c x Rd Rm Op2)``));
...
val ARM_UMULL_NOP = SYMBOLIC_EVAL_CONV NOP_ss (nop_cntxt
  ``enc (instruction$UMULL c x RdHi RdLo Rs Rm)``);

val ARM_UMLAL_NOP = SYMBOLIC_EVAL_CONV NOP_ss (nop_cntxt
  ``enc (instruction$UMLAL c x RdHi RdLo Rs Rm)``);

val ARM_SMULL_NOP = SYMBOLIC_EVAL_CONV NOP_ss (nop_cntxt
  ``enc (instruction$SMULL c x RdHi RdLo Rs Rm)``);

val ARM_SMLAL_NOP = SYMBOLIC_EVAL_CONV NOP_ss (nop_cntxt
  ``enc (instruction$SMLAL c x RdHi RdLo Rs Rm)``);
...
fun eval_op t = SYMBOLIC_EVAL_CONV DP_ss (cntxt [abbrev_imm]
  (subst [``f:condition -> bool -> word4 ->
              word4 -> addr_mode1 -> arm_instruction`` |-> t]
   ``enc ((f:condition -> bool -> word4 ->
             word4 -> addr_mode1 -> arm_instruction)
...
val ARM_UMULL = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  ...
  ``enc (instruction$UMULL c F RdHi RdLo Rm Rs)``);

val ARM_UMULLS = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$UMULL c T RdHi RdLo Rm Rs)``);

val ARM_UMLAL = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$UMLAL c F RdHi RdLo Rm Rs)``);

val ARM_UMLALS = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$UMLAL c T RdHi RdLo Rm Rs)``);

val ARM_SMULL = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$SMULL c F RdHi RdLo Rm Rs)``);

val ARM_SMULLS = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$SMULL c T RdHi RdLo Rm Rs)``);

val ARM_SMLAL = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$SMLAL c F RdHi RdLo Rm Rs)``);

val ARM_SMLALS = SYMBOLIC_EVAL_CONV MLA_MUL_ss (cntxt
  [``~(RdHi = 15w:word4)``,``~(RdLo = 15w:word4)``,``~(RdHi = RdLo:word4)``,
   ``~(RdHi = Rm:word4)``,``~(RdLo = Rm:word4)``]
  ``enc (instruction$SMLAL c T RdHi RdLo Rm Rs)``);
...
val _ = save_thm("ARM_UMULLS", ARM_UMULLS);
val _ = save_thm("ARM_UMLALS", ARM_UMLALS);
val _ = save_thm("ARM_SMULL", ARM_SMULL);
val _ = save_thm("ARM_SMLAL", ARM_SMLAL);
val _ = save_thm("ARM_SMULLS", ARM_SMULLS);
val _ = save_thm("ARM_SMLALS", ARM_SMLALS);
