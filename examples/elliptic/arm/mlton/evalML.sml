...
val _ = loadPath := !loadPath @ ".." :: map (fn x => HOLDIR ^ x)
        ["src/theoryML", "src/portableML", "tools/mlyacc/mlyacclib"];
...
val exc_mem = list_assemble armML.empty_memory
  ["0x0: movs pc, #32",
   "label: b label",
   "movs pc, r14",
   "subs pc, r14, #4",
...
val reg = armML.REG_WRITE armML.empty_registers
  armML.usr (fromNum4 "15") (fromNum32 "20");
...
val final = time (STATE_ARMe printer max)
  (armML.state_arme (reg,psr,prog,false));
...
val final = time (STATE_ARMe printer (* combinML.I *) max)
  (armML.state_arme (reg,psr,prog,false));

val _ = print ("count: " ^ Int.toString (!count) ^ "\n");
*)

(* 1469 instructions *)
(* HOL: 7200s, 0.2 ips *)
(* DIV2, DIV_2EXP and MOD_2EXP not numeral forms: 77.2s, 19 ips *)
(* Numeral form: 7.98s, which is 902 times faster and 184 ips *)
(* Decode optimised: 5.70s, 258 ips *)
(* MLton: 0.90s, 1632 ips *)
(* MLton + LargeInt: 0.37s, 3970 ips *)
(*  + red-black memory: 0.33s, 4451 ips *)
(*  + red-black memory + bespoke DIV/MOD_2EXP: 0.28s, 5246 ips *)
(*  + no monitoring: 0.23s, 6386 ips *)
(*  + red-black memory + no monitoring: 0.20s, 7345 ips *)
(*  + red-black memory + no monitoring + bespoke DIV/MOD_2EXP:
        0.14s, 10492 ips *)
(*  + red-black registers: 0.20s, 6995 ips *)
