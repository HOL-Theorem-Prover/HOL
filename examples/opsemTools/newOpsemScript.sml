...
val _ = 
 Hol_datatype
  ...
  | AnWhile   of bexp => pred => program             (* annotated while loop *)
  | Local     of string => program                   (* local variable block *)
  | AnProc    of pred => pred => vars => sfun`;      (* annotated procedure  *)
...
(* Unannotated while loop *)

val While_def =
 Define
  `While b c = AnWhile b ARB c`;

(* Unannotated procedure call *)

val Proc_def =
 Define
  `Proc f = AnProc ARB ARB ARB f`;
...
val ANPROC_THM = store_thm
 ("ANPROC_THM",
  ``!P Q xs f s1 s2 . EVAL (AnProc P Q xs f) s1 s2 = (s2 = f s1)``,
  RW_TAC std_ss [EQ_IMP_THM,Once ecases,While_def] THEN METIS_TAC rulel);
...
val lemma1 = Q.prove
(`!c s1 s2. EVAL c s1 s2 ==> !b' R' c'. (c = AnWhile b' R' c') ==> ~beval b' s2`,
...
("ANWHILE_RULE",
 ``!P b R c. SPEC (\s. P(s) /\ beval b s) c P ==>
             SPEC P (AnWhile b R c) (\s. P s /\ ~beval b s)``,
   ...
   REPEAT STRIP_TAC
    ...
    THEN TRY (METIS_TAC
          [SMALL_SKIP_THM,SMALL_GEN_ASSIGN_THM,SMALL_DISPOSE_THM,SMALL_IF_THM,SMALL_SEQ_THM,
           SMALL_LOCAL_THM,SMALL_ANPROC_THM,SMALL_ANWHILE_THM,outcome_distinct,outcome_11,
    ...
    THEN FULL_SIMP_TAC (srw_ss()) [SMALL_ANPROC_THM]
    THEN TRY (METIS_TAC
          [SMALL_SKIP_THM,SMALL_GEN_ASSIGN_THM,SMALL_DISPOSE_THM,SMALL_IF_THM,SMALL_SEQ_THM,
           SMALL_LOCAL_THM,SMALL_ANPROC_THM,SMALL_ANWHILE_THM,outcome_distinct,outcome_11,
 ...
 store_thm
  ("STEP",
   ``(STEP n ([],s) = ([], RESULT s))
     ...
     /\
     (STEP (SUC n) (AnProc P Q xs f :: l, s) = (l, RESULT(f s)) >>= STEP n)``,
   Induct_on `n`
...
val VARL_def =
 Define
  `(VARL Skip = [])
   ...
   /\
   (VARL (AnProc P Q xs f) = SET_TO_LIST xs)`;
...
val SIMPLE_def =
 Define
  `(SIMPLE Skip = T)
   ...
   /\
   (SIMPLE (AnProc P Q xs f) = FINITE xs)`;
...
val ANWHILE_SP =
 store_thm
  ("ANWHILE_SP",
   ``SP P (AnWhile b R c) =
      \s'. SP (SP (\s. P s /\ beval b s) c) (AnWhile b R c) s' \/ (P s' /\ ~(beval b s'))``,
   CONV_TAC FUN_EQ_CONV
...
val WVC_def =
 Define
  `(WVC(Skip, Q) = (Q, \s. T))
   ...
   /\
   (WVC(AnProc P Q' xs f, Q) = (P, \s. (Q' s ==> Q s) /\ (P s ==> Q'(f s))))`;
...
val WLVC =
 store_thm
  ...
  ``(WLVC Skip Q = T)
    ...
    /\
    (WLVC (AnProc P Q' xs f) Q = (!s. Q' s ==> Q s) /\ (!s. P s ==> Q'(f s)))``,
    ...
    THENL
     [SIMP_TAC std_ss 
      ...
      FULL_REWRITE_TAC [WVC_def,VALID_def]
       THEN RW_TAC std_ss []
       THEN `SPEC (FST (WVC (c,f))) c f` by METIS_TAC[]
    ...
    THENL
     [RW_TAC std_ss [SPEC_def,VALID_def,ASP_SVC_def,SKIP_THM],
      ...
      FULL_SIMP_TAC std_ss [ASP_SVC_def,VALID_def]
       ...
       THEN METIS_TAC[],
      FULL_SIMP_TAC std_ss [SPEC_def,VALID_def,ASP_SVC_def,ANPROC_THM]]);
*)
    ...
    THENL
     [RW_TAC std_ss [SPEC_def,ASP_SVC_def,SKIP_THM],
      ...
      FULL_SIMP_TAC std_ss [ASP_SVC_def]
       ...
       THEN `SPEC (\s. f s /\ beval b s) c (FST (ASP_SVC ((\s. f s /\ beval b s),c)))` by METIS_TAC[]
       THEN IMP_RES_TAC(SIMP_RULE std_ss [VALID_def]POST_DERIVED_ANWHILE_RULE),
      FULL_SIMP_TAC std_ss [SPEC_def,ASP_SVC_def,ANPROC_THM]]);
...
val VC_CHECK_def =
 Define
  `(VC_CHECK xl Skip = T)
   ...
   /\
   (VC_CHECK xl (AnWhile b R c) =
     VC_CHECK xl c /\ ?l. (MAP FST l = xl) /\ R (FEMPTY |++ l) /\ ~(beval b (FEMPTY |++ l)))
   /\
   (VC_CHECK xl (Local v c) = F)
   /\
   (VC_CHECK xl (AnProc P Q xs f) = F)`;
...
val VC_RUN_def =
 Define
  `(VC_RUN Skip lP = (lP, T))
   ...
   /\
   (VC_RUN (AnWhile b R c) lP =
       let newlist l' = (MAP FST (FST lP) = MAP FST l') /\ R(FEMPTY |++ l') /\ ~(beval b (FEMPTY |++ l'))
       in
 ...
 store_thm
  ("VC_RUN",
   ``(VC_RUN Skip (l,P) = ((l,P), T))
     ...
     /\
     (VC_RUN (AnWhile b R c) (l,P) =
       let newlist l' = (MAP FST l = MAP FST l') /\ R(FEMPTY |++ l') /\ ~(beval b (FEMPTY |++ l'))
       in
       (((@l'. newlist l'), \s. R s /\ ~(beval b s)),
        (?l'. newlist l')
        /\
...
val XP_EXEC_ANWHILE =
 store_thm
  ("XP_EXEC_ANWHILE",
   ``!xl (f:(string#value)list->(string#value)list) P c R b.
      ALL_DISTINCT xl
      ==>
      (XP xl (\s. ?l. (MAP FST l = xl) /\ P l /\ (s = FEMPTY |++ (f l))) (AnWhile b R c) =
        \s. ?l. (MAP FST l = xl) /\ (ANWHILE_PRED xl P R b o ($|++ FEMPTY)) l /\ (s = FEMPTY |++ (I l)))``,
...
val XP_EXEC_ANPROC =
 store_thm
  ("XP_EXEC_ANPROC",
   ``!xl (f:(string#value)list->(string#value)list) P P' Q xs f.
      ALL_DISTINCT xl
      ==>
      (XP xl (\s. ?l. (MAP FST l = xl) /\ P l /\ (s = FEMPTY |++ (f l))) (AnProc P' Q xs f') =
        \s. ?l. (MAP FST l = xl) /\ (ANPROC_PRED xl P Q o ($|++ FEMPTY)) l /\ (s = FEMPTY |++ (I l)))``,
   RW_TAC std_ss [XP_def,ANPROC_PRED_def]
    THEN CONV_TAC(TOP_DEPTH_CONV(X_FUN_EQ_CONV ``s:state``))
    THEN RW_TAC std_ss []
    THEN EQ_TAC
    THEN RW_TAC std_ss []
...
val FVC_def =
 Define
  `(FVC xl P Skip = T)
   ...
   /\
   (FVC xl P (AnWhile b R c) = (!s. P s ==> R s) /\ SPEC (\s. R s /\ beval b s) c R /\ FVC xl P c)
   /\
   (FVC xl P (Local v c) = !val. FVC xl (\s. P s /\ (val = s ' v)) c)
   /\
   (FVC xl P (AnProc P' Q xs f) = (!s. P s ==> P' s) /\ (!s. P' s ==> Q(f s)) /\ (!s:state. xs SUBSET FDOM s ==> (FDOM(f s) = FDOM s)))`;
    ...
    THENL
     [RW_TAC std_ss [SPEC_def,XP_def,ORF_def,FVC_def,SKIP_THM],
      ...
      FULL_SIMP_TAC std_ss [XP_def,ORF_def,FVC_def]
       THEN RW_TAC std_ss []
       THEN IMP_RES_TAC(SIMP_RULE std_ss [VALID_def]POST_DERIVED_ANWHILE_RULE)
       THEN `SPEC P (AnWhile b f c) (\s. f s /\ ~beval b s)`by METIS_TAC[]
      ...
      FULL_SIMP_TAC std_ss [XP_def,FVC_def,LOCAL_THM,VARS_def,SPEC_def,INSERT_SUBSET,PDOM_def]
       ...
       THEN METIS_TAC[],
      FULL_SIMP_TAC std_ss [XP_def,ORF_def,FVC_def,SPEC_def,ANPROC_THM,PDOM_def,VARS_def]
       THEN RW_TAC std_ss []
       THEN METIS_TAC[]]);
