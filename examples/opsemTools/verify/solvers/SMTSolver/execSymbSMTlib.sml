...
open HolKernel Parse boolLib
     ...
     simpTools stateTools HolSmtLib;
...
(* Global variable to select the tactic to use when calling
   external SMT solver *)
...
(* Functions quantify a term *)
(* ----------------------------------------- *)
fun existQuantify tm =
  list_mk_exists(!programVars,tm);


fun forallQuantify tm =
  list_mk_forall(!programVars,tm);
...
fun testPath name pre path st =
 ...
 let    val conj = mk_conj(pre,path)
   ...
   (print "======================\n";
    ...
    handle HOL_ERR s =>
       (print "======================\n";
        print "Yices fails to shows that path is unfeasable\n";
...
(*     
       ...
       let  val existsConj = existQuantify conj
         ...
         val resSimp=  getThm thSimp
       in
          (print "path has been shown feasable with HOL\n";
           pathInfo resSimp path 0.0)
       end
       handle UNCHANGED  =>
        (* It was not possible to show the theorem in HOL
...
(*    val np =  takeNegPost po
    ...
    val tm = forallQuantify(mk_imp(conj,po))
    in
      (print "======================\n";
       ...
       (print ("HOL_ERR when calling SMT solver\n");
        print (origin_structure ^ "." ^ origin_function ^ ", message: " ^ message ^"\n" );
...
-  SMTnam is the name of the external SMT solver to be called
   (either Z3 or YICES)
   The symbolic state is built from variables in the program
 *)
fun execSymbWithSMT name spec n solverName =
