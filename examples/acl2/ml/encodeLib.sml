...
val IF_THMS = prove(``(acl2_if t a b = a) /\ (acl2_if nil a b = b)``,RW_TAC std_ss [acl2_if_def,t_def,nil_def]);
...
let	val mp_thm = prove(``(X = acl2_if (bool P) a b) ==> (P ==> (a = X))``,Cases_on `P` THEN RW_TAC std_ss [acl2_if_def,nil_def,t_def,bool_def])
...
local
	val acl2_if_thm = prove(``(bool p = P) /\ (p ==> (f (a:'a) = A)) /\ (~p ==> (f b = B)) ==> (f (if p then a else b) = acl2_if P A B)``,
		RW_TAC std_ss [bool_def,acl2_if_def,nil_def,t_def])
...
local
val acl2_if_thm = prove(``(X = acl2_if P (f a) (f b)) /\ (!a. Q (f a)) ==> (!a. Q X)``,RW_TAC std_ss [acl2_if_def,nil_def,t_def])
...
local 
val rewr1 = prove(``(A = acl2_if P B C) = ((P = nil) ==> (A = C)) /\ (~(P = nil) ==> (A = B))``,RW_TAC std_ss [acl2_if_def,nil_def,t_def])
...
val every_acl2_thm = prove(``bool (sexp_EVERY P a) = acl2_if (consp a) (acl2_if (bool (P (car a))) (bool (sexp_EVERY P (cdr a))) nil) (equal a nil)``,
	Induct_on `a` THEN 
	RW_TAC std_ss [sexp_EVERY_def,acl2_if_def,equal_def,bool_def,consp_def,car_def,cdr_def,prove(``~(t = nil)``,RW_TAC std_ss [nil_def,t_def])] THEN
	Cases_on `P a` THEN FULL_SIMP_TAC std_ss [bool_def,prove(``~(t = nil)``,RW_TAC std_ss [nil_def,t_def]),equal_def,acl2_if_def]);
