open preamble;
open dfaTheory scottreTheory lexer_runtimeTheory;

val _ = new_theory "lexer_spec_to_dfa"

val is_error_state_def = Define `
is_error_state lex_spec =
  EVERY (λ(regexp,action). regexp = CharSet charset_empty) lex_spec`;
...
`n < LENGTH start_st' ∧
 ...
 rw [EL_MAP] >>
 cases_on `EL n start_st` >>
 fs [GSYM regexp_matches_smart_deriv, smart_deriv_matches_def]);
...
val lemma = Q.prove  (
`!s. ~regexp_matches (CharSet charset_empty) s`,
rw [regexp_matches_def, charset_mem_empty]);
 ...
 `∀n'. n' < n ⇒
         ¬smart_deriv_matches (FST (EL n' (MAP (λ(regexp,action). (smart_deriv h regexp,action)) lex_spec))) l`
               by (rw [] >>
                   ...
                   fs [smart_deriv_matches_def, GSYM regexp_matches_smart_deriv] >>
     ...
     [fs [is_error_state_def, EVERY_MAP] >>
          ...
          fs [] >>
          rw [] >>
          metis_tac [GSYM regexp_matches_smart_deriv, lemma],
      res_tac >>
          rw [] >>
