 ...
 store_thm
  ("ACCEPT_COND___VAR_RENAMING",
   ...
   INDUCT_THEN acceptance_condition_induct ASSUME_TAC THENL [
      ...
      Know `(i t UNION  P_USED_VARS p) SUBSET (PATH_USED_VARS i UNION P_USED_VARS p)` THEN1 (
      ...
      Q.EXISTS_TAC `(PATH_USED_VARS i ∪ P_USED_VARS p)` THEN
      Q.EXISTS_TAC `univ(:'b)` >> ASM_REWRITE_TAC [SUBSET_REFL],
      ...
      PROVE_TAC[INJ_SUBSET, SUBSET_UNION, UNION_COMM, UNION_ASSOC,SUBSET_REFL],
...
(* TODO
   ...
   Suff `x' IN IMAGE f (A_USED_INPUT_VARS a)` THEN1 (
      DISCH_TAC THEN
      `INJ f (A_USED_VARS a) UNIV` by METIS_TAC[INJ_SUBSET, SUBSET_UNION, SUBSET_REFL] THEN
  ...
  ("STATE_VARDISJOINT_AUTOMATON_FORMULA___VAR_RENAMING",
   ...
   INDUCT_THEN automaton_formula_induct STRIP_ASSUME_TAC THENL [
      ...
      BINOP_TAC THENL [
         ...
         PROVE_TAC[SUBSET_UNION, INJ_SUBSET, SUBSET_REFL]
      ],
...
val EXISTS_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING =
 store_thm
  ("EXISTS_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING",
   ...
   PROVE_TAC[AUTOMATON_FORMULA___STATE_VAR_RENAMING, INJ_SUBSET, SUBSET_UNIV, SUBSET_REFL]);
 ...
 store_thm
  ("UNIQUE_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING",
   ...
   EQ_TAC THEN REPEAT STRIP_TAC THENL [
     ...
     METIS_TAC[A_SEM_AUTOMATON_RUN___STATE_VAR_RENAMING, SEMI_AUTOMATON___STATE_VAR_RENAMING, IN_UNION, SUBSET_UNIV, INJ_SUBSET, SUBSET_REFL],
 ...
 store_thm
  ("ACCEPT_COND_RESTN_SEM",
   ...
   INDUCT_THEN acceptance_condition_induct ASSUME_TAC THENL [
      ...
      REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
         `t'' + t2 >= t1 + t2` by DECIDE_TAC THEN (* was `t' + t2 >= ...` *)
         PROVE_TAC[],
