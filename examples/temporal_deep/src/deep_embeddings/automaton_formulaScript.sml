 ...
 store_thm
  ("A_USED_INPUT_VARS_INTER_SUBSET_THM",
   ...
   INDUCT_THEN automaton_formula_induct STRIP_ASSUME_TAC THENL [
      ...
      BINOP_TAC THENL [
         Know `!n. ((INPUT_RUN_STATE_UNION p_1 (i n) (w n)) INTER (S UNION p_1.S)) =
  ...
  ("A_AND___A_NDET",
   ...
   REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
      ...
      REPEAT STRIP_TAC THENL [
         ...
         Know `(DISJOINT (A_USED_INPUT_VARS f1) A2.S) /\
         ...
         Know `(DISJOINT (A_USED_INPUT_VARS f2) A1.S) /\
...
val PRODUCT_SEMI_AUTOMATON___EXISTS_RUN_WITH_PROPERTIES =
 store_thm
  ("PRODUCT_SEMI_AUTOMATON___EXISTS_RUN_WITH_PROPERTIES",
   ...
   Know `AUTOMATON_EQUIV (A_AND (A_NDET (A1, A_BIGAND C1),A_NDET (A2, A_BIGAND C2)))
 ...
 store_thm
  ("PRODUCT_SEMI_AUTOMATON___UNIQUE_RUN_WITH_PROPERTIES",
   ...
   REPEAT STRIP_TAC THENL [
      ...
      Suff `(PATH_RESTRICT x A1.S = PATH_RESTRICT y A1.S) /\
      ...
      Suff `(IS_SYMBOLIC_RUN_THROUGH_SEMI_AUTOMATON A1 i (PATH_RESTRICT x A1.S) /\ IS_SYMBOLIC_RUN_THROUGH_SEMI_AUTOMATON A2 i (PATH_RESTRICT x A2.S) /\
      ...
      Know `((PATH_DIFF (INPUT_RUN_PATH_UNION (PRODUCT_SEMI_AUTOMATON A1 A2) i x) A1.S) =
 ...
 store_thm
  ("A_NDET___FLATTENING",
   ...
   EQ_TAC THEN REPEAT STRIP_TAC THENL [
      ...
      Know `!n. (((w n UNION w' n UNION (v n DIFF (A1.S UNION A2.S)))) =
      ...
      Know `!n. (((w n UNION (v n DIFF (A1.S UNION A2.S))) INTER (COMPL A2.S)) =
      ...
      REPEAT STRIP_TAC THENL [
        ...
        EXISTS_TAC ``(PATH_RESTRICT w A2.S)`` THEN
        SIMP_TAC std_ss [PATH_RESTRICT_def, PATH_MAP_def, IN_INTER] THEN
        Know `!n. (w n INTER A2.S UNION (w n INTER A1.S UNION (v n DIFF A1.S) DIFF A2.S)) =
 ...
 store_thm
  ("ACCEPT_COND___VAR_RENAMING",
   ...
   INDUCT_THEN acceptance_condition_induct ASSUME_TAC THENL [
      ...
      `(i t UNION  P_USED_VARS p) SUBSET (PATH_USED_VARS i UNION P_USED_VARS p)` by (
   ...
   INDUCT_THEN automaton_formula_induct ASSUME_TAC THENL [
      ...
      EQ_TAC THEN REPEAT STRIP_TAC THENL [
         ...
         REPEAT STRIP_TAC THENL [
            ...
            Know `(INPUT_RUN_PATH_UNION (SEMI_AUTOMATON_VAR_RENAMING f p_1) (PATH_VAR_RENAMING f i) (PATH_VAR_RENAMING f w)) =
            ...
            Suff `INJ f (PATH_USED_VARS (INPUT_RUN_PATH_UNION p_1 i w) UNION A_USED_VARS a) UNIV` THEN1 (
              PROVE_TAC[]
            ) THEN
         ...
         Know `?w'. PATH_SUBSET w' p_1.S /\ (w = PATH_VAR_RENAMING f w')` THEN1 (
      ...
      REPEAT STRIP_TAC THEN
      Suff `INJ f (PATH_USED_VARS i UNION A_USED_VARS a) UNIV /\
...
val AUTOMATON_FORMULA___STATE_VAR_RENAMING =
 store_thm
  ("AUTOMATON_FORMULA___STATE_VAR_RENAMING",
   ...
   REPEAT STRIP_TAC THEN
   Suff `(PATH_RESTRICT (PATH_VAR_RENAMING f i) (A_USED_INPUT_VARS (A_VAR_RENAMING f a))) =
   ...
   Know `x' IN IMAGE f (A_USED_INPUT_VARS a)` THEN1 (
      `INJ f (A_USED_VARS a) UNIV` by METIS_TAC[INJ_SUBSET_DOMAIN, SUBSET_UNION] THEN
...
val A_SEM_AUTOMATON_RUN___STATE_VAR_RENAMING =
 store_thm
  ("A_SEM_AUTOMATON_RUN___STATE_VAR_RENAMING",
   ...
   REPEAT STRIP_TAC THEN
   Suff `(PATH_RESTRICT (PATH_VAR_RENAMING f (INPUT_RUN_PATH_UNION A i w)) (A_USED_INPUT_VARS (A_VAR_RENAMING f a))) =
   ...
   REPEAT STRIP_TAC THEN CONJ_EQ_STRIP_TAC THEN
   `x'' IN IMAGE f (A_USED_INPUT_VARS a)` by PROVE_TAC[INJ_SUBSET_DOMAIN, SUBSET_UNIV, A_VAR_RENAMING___USED_INPUT_VARS] THEN
 ...
 store_thm
  ("VARDISJOINT_AUTOMATON_FORMULA___VAR_RENAMING",
   ...
   REPEAT GEN_TAC THEN DISCH_TAC THEN BINOP_TAC THENL[
      PROVE_TAC[STATE_VARDISJOINT_AUTOMATON_FORMULA___VAR_RENAMING, INJ_SUBSET_DOMAIN, SUBSET_UNION,
                SUBSET_REFL],
...
val EXISTS_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING =
 store_thm
  ("EXISTS_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING",
   ...
   PROVE_TAC[AUTOMATON_FORMULA___STATE_VAR_RENAMING, INJ_SUBSET_DOMAIN, SUBSET_UNIV, SUBSET_REFL]);
...
val UNIQUE_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING =
 store_thm
  ("UNIQUE_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING",
   ...
   EQ_TAC THEN REPEAT STRIP_TAC THENL [
     Suff `PATH_VAR_RENAMING f x = PATH_VAR_RENAMING f y` THEN1 (
       UNDISCH_HD_TAC THEN
       ONCE_REWRITE_TAC [FUN_EQ_THM] THEN
     ...
     METIS_TAC[A_SEM_AUTOMATON_RUN___STATE_VAR_RENAMING, SEMI_AUTOMATON___STATE_VAR_RENAMING, IN_UNION, SUBSET_UNIV, INJ_SUBSET_DOMAIN, SUBSET_REFL],


     Know `(?x'. x = PATH_VAR_RENAMING f x') /\ (?y'. y = PATH_VAR_RENAMING f y')` THEN1 (
 ...
 store_thm
  ("ACCEPT_COND_RESTN_SEM",
   ...
   INDUCT_THEN acceptance_condition_induct ASSUME_TAC THENL [
      ...
      REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
         rename1 `t3 >= t1` THEN
         `t3 + t2 >= t1 + t2` by DECIDE_TAC THEN
         PROVE_TAC[],

         rename1 `t3 >= t1 + t2` THEN
         `?t. t = (t3 - t2)` by METIS_TAC[] THEN
         `(t3 = t + t2) /\ (t >= t1)` by DECIDE_TAC THEN
         PROVE_TAC[]
      ]
   ]);
...
EQ_TAC THEN REPEAT STRIP_TAC THENL [
  ...
  REPEAT STRIP_TAC THENL [
    SIMP_ALL_TAC std_ss [PATH_SUBSET_def, SUBSET_DEF, IN_UNION, IN_IMAGE, IN_INTER, SUBSET_REFL] THEN
    METIS_TAC[],
    ...
    SIMP_ALL_TAC std_ss [IS_SIMPLIFIED_SYMBOLIC_SEMI_AUTOMATON_def, IN_UNION,
      ...
      IN_INTER, PATH_SUBSET_def, SUBSET_DEF, SUBSET_REFL] THEN
    METIS_TAC[],
    ...
    SIMP_ALL_TAC std_ss [IS_SIMPLIFIED_SYMBOLIC_SEMI_AUTOMATON_def,
      PATH_SUBSET_def, SUBSET_DEF, SUBSET_REFL, DISJOINT_DISJ_THM] THEN
...
``!A A' f ac. IS_SIMPLIFIED_SYMBOLIC_SEMI_AUTOMATON A' A f /\
  ...
  Know `!A ac i. A_SEM i (A_NDET (A, A_NOT (ACCEPT_COND ac))) =
