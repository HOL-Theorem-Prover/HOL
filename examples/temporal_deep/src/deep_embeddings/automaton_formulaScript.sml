...
val ACCEPT_COND_USED_VARS_INTER_SUBSET_THM =
 store_thm
  ("ACCEPT_COND_USED_VARS_INTER_SUBSET_THM",
   ``!a S t v. ((ACCEPT_COND_USED_VARS a) SUBSET S) ==> (ACCEPT_COND_SEM_TIME t v a = ACCEPT_COND_SEM_TIME t (PATH_RESTRICT v S) a)``,
...
val ACCEPT_COND_USED_VARS_INTER_THM =
 store_thm
  ("ACCEPT_COND_USED_VARS_INTER_THM",
   ``!a t v. (ACCEPT_COND_SEM_TIME t v a = ACCEPT_COND_SEM_TIME t (PATH_RESTRICT v (ACCEPT_COND_USED_VARS a)) a)``,
  ...
  ("PRODUCT_SEMI_AUTOMATON___UNIQUE_RUN_WITH_PROPERTIES",
   ...
   rpt STRIP_TAC THENL [
      ...
      SUBGOAL_TAC `((PATH_DIFF (INPUT_RUN_PATH_UNION (PRODUCT_SEMI_AUTOMATON A1 A2) i x) A1.S) =
        ...
        UNDISCH_NO_TAC 7 THEN UNDISCH_NO_TAC 9 THEN rpt WEAKEN_HD_TAC THEN
        ONCE_REWRITE_TAC [FUN_EQ_THM] THEN
...
val AUTOMATON_FORMULA___STATE_VAR_RENAMING =
 store_thm
  ("AUTOMATON_FORMULA___STATE_VAR_RENAMING",
   ...
   rpt STRIP_TAC THEN CONJ_EQ_STRIP_TAC THEN
...
val VARDISJOINT_AUTOMATON_FORMULA___VAR_RENAMING =
 store_thm
  ("VARDISJOINT_AUTOMATON_FORMULA___VAR_RENAMING",
   ...
   rpt GEN_TAC THEN DISCH_TAC THEN BINOP_TAC THENL[
...
val UNIQUE_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING =
 store_thm
  ("UNIQUE_RUN_WITH_PROPERTIES___STATE_VAR_RENAMING",
   ...
   rpt STRIP_TAC THEN SIMP_TAC std_ss [FORALL_AND_THM] THEN
...
val A_NDET___INITIAL_ACCEPTANCE_SYM =
 store_thm
  ("A_NDET___INITIAL_ACCEPTANCE_SYM",
   ...
   rpt GEN_TAC THEN EXISTS_EQ_STRIP_TAC THEN rpt BOOL_EQ_STRIP_TAC);
...
(* References:
 ...
 [2] Tuerk, T., Schneider, K.: A hierarchy for Accellera's property specification language, (2005).
 ...
 [4] Schneider, K., Hoffmann, D.W.: A HOL Conversion for Translating Linear Time
     Temporal Logic to ω-Automata. In: LNCS 1690 - Theorem Proving in Higher Order
