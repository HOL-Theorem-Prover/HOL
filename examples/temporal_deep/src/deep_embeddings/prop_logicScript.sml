...
val prop_logic =
 Hol_datatype
  `prop_logic = P_PROP   of 'a                         (* atomic proposition       *)
        | P_TRUE                                       (* true                     *)
        | P_NOT of prop_logic                          (* negation                 *)
        | P_AND of prop_logic # prop_logic`;           (* conjunction              *)

val prop_logic_11 = DB.fetch "-" "prop_logic_11";


val prop_logic_induct =
 save_thm
  ("prop_logic_induct",
   Q.GEN
    `P`
...
val IS_POSITIVE_NEGATIVE_PROP_FORMULA_SUBSET_def =
Define
`(IS_POSITIVE_PROP_FORMULA_SUBSET S P_TRUE = T) /\
  (IS_POSITIVE_PROP_FORMULA_SUBSET S (P_PROP a) = T) /\
  (IS_POSITIVE_PROP_FORMULA_SUBSET S (P_NOT p') = IS_NEGATIVE_PROP_FORMULA_SUBSET S p') /\
  ...
  (IS_NEGATIVE_PROP_FORMULA_SUBSET S (P_NOT p') = IS_POSITIVE_PROP_FORMULA_SUBSET S p') /\
  (IS_NEGATIVE_PROP_FORMULA_SUBSET S (P_AND(p', p'')) = (
    IS_NEGATIVE_PROP_FORMULA_SUBSET S p' /\
    IS_NEGATIVE_PROP_FORMULA_SUBSET S p''))`;


val IS_POSITIVE_PROP_FORMULA_def =
Define
`IS_POSITIVE_PROP_FORMULA p =
  IS_POSITIVE_PROP_FORMULA_SUBSET UNIV p`;


val IS_NEGATIVE_PROP_FORMULA_def =
Define
`IS_NEGATIVE_PROP_FORMULA p =
  IS_NEGATIVE_PROP_FORMULA_SUBSET UNIV p`;
...
val P_PROP_DISJUNCTION_def =
Define
`(P_PROP_DISJUNCTION [] = P_FALSE) /\
  (P_PROP_DISJUNCTION (s::S) = P_OR (P_PROP s, P_PROP_DISJUNCTION S))`;


val P_PROP_CONJUNCTION_def =
Define
`(P_PROP_CONJUNCTION [] = P_TRUE) /\
  (P_PROP_CONJUNCTION (s::S) = P_AND (P_PROP s, P_PROP_CONJUNCTION S))`;


val IS_PROP_DISJUNCTION_def =
Define
`(IS_PROP_DISJUNCTION p = (?S. p = P_PROP_DISJUNCTION S))`;


val IS_PROP_CONJUNCTION_def =
Define
`(IS_PROP_CONJUNCTION p = (?S. p = P_PROP_CONJUNCTION S))`;


val P_MODEL_DISJUNCTION_def =
Define
    `P_MODEL_DISJUNCTION S p =
      P_PROP_DISJUNCTION (SET_TO_LIST ({S' | P_SEM S' p} INTER (POWER_SET S)))`;


val P_MIN_MODEL_DISJUNCTION_def =
Define
    `P_MIN_MODEL_DISJUNCTION S p =
    P_PROP_DISJUNCTION (SET_TO_LIST ({S' | P_SEM_MIN S' p} INTER (POWER_SET S)))`;


val P_PROP_SET_MODEL_def = Define
  `(P_PROP_SET_MODEL S S' =
        P_AND(P_PROP_CONJUNCTION (SET_TO_LIST (S INTER S')), P_NOT(
        P_PROP_DISJUNCTION (SET_TO_LIST (S' DIFF S)))))`;


val P_NEGATE_VARS_def =
 Define
...
val P_IS_CONTRADICTION_def =
 Define
   `P_IS_CONTRADICTION p = (!P. ~(P_SEM P p))`;

val P_IS_TAUTOLOGY_def =
 Define
   `P_IS_TAUTOLOGY p = (!P. P_SEM P p)`;



val P_ASSIGN_TRUE_def =
Define
`(P_ASSIGN_TRUE V (P_PROP p) = (if p IN V then P_TRUE else P_PROP p)) /\
  ...
  (P_ASSIGN_TRUE V (P_AND(b1,b2)) = P_AND (
        P_ASSIGN_TRUE V b1,
        P_ASSIGN_TRUE V b2))`;


val P_ASSIGN_FALSE_def =
Define
...
Define
`(P_EXISTS [] p = p) /\
  (P_EXISTS (v::l) p = P_EXISTS l (P_OR(P_ASSIGN_TRUE {v} p, P_ASSIGN_FALSE {v} p)))`;
...
val VAR_RENAMING_HASHTABLE_def =
Define
`(VAR_RENAMING_HASHTABLE S f =
   P_BIGOR (SET_TO_LIST (IMAGE (\s. P_AND(P_PROP_SET_MODEL s S, P_PROP(f s))) (POWER_SET S))))`;
 ...
 store_thm
  ("P_SEM_THM",
   ...
   ((P_SEM s (P_TRUE)) /\ ~(P_SEM s (P_FALSE)) /\
    ...
    (P_SEM s (P_COND(c, b1,b2)) = (P_SEM s c /\ P_SEM s b1) \/ (~P_SEM s c /\ P_SEM s b2)) /\
...
val P_USED_VARS_INTER_SUBSET_THM =
 store_thm
  ("P_USED_VARS_INTER_SUBSET_THM",
   ...
   INDUCT_THEN prop_logic_induct ASSUME_TAC THENL [
      SIMP_TAC arith_ss [P_SEM_def, P_USED_VARS_def, INTER_DEF, SUBSET_DEF, IN_SING] THEN
...
val P_USED_VARS_EVAL =
 store_thm
  ...
  REWRITE_TAC[P_USED_VARS_def, P_FALSE_def, P_OR_def, P_IMPL_def, P_EQUIV_def] THEN
 ...
 store_thm
  ("P_VAR_RENAMING_EVAL",
   ``!p f b b1 b2. (
    ...
    (P_VAR_RENAMING f (P_NOT b) = P_NOT(P_VAR_RENAMING f b)) /\
    (P_VAR_RENAMING f (P_AND(b1, b2)) = P_AND(P_VAR_RENAMING f b1, P_VAR_RENAMING f b2)) /\
    (P_VAR_RENAMING f (P_OR(b1, b2)) = P_OR(P_VAR_RENAMING f b1, P_VAR_RENAMING f b2)) /\
    (P_VAR_RENAMING f (P_IMPL(b1, b2)) = P_IMPL(P_VAR_RENAMING f b1, P_VAR_RENAMING f b2)) /\
    (P_VAR_RENAMING f (P_EQUIV(b1, b2)) = P_EQUIV(P_VAR_RENAMING f b1, P_VAR_RENAMING f  b2))
   )``,
...
val IS_POSITIVE_NEGATIVE_PROP_FORMULA_SUBSET_SEM =
 store_thm
  ("IS_POSITIVE_NEGATIVE_PROP_FORMULA_SUBSET_SEM",
   ``!p V V' S. ((IS_POSITIVE_PROP_FORMULA_SUBSET V p /\ P_SEM S p /\ V' SUBSET V) ==>
        (P_SEM (S UNION V') p)) /\
      ((IS_NEGATIVE_PROP_FORMULA_SUBSET V p /\ P_SEM ((S UNION V')) p /\ V' SUBSET V) ==>
        (P_SEM S p))``,
...
val P_PROP_DISJUNCTION_MIN_SEM =
 store_thm
  ("P_PROP_DISJUNCTION_MIN_SEM",
    ...
    REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
        FULL_SIMP_TAC std_ss [SOME_EL_IS_EL] THEN
        ...
        PROVE_TAC[SOME_EL___IMPL],

        FULL_SIMP_TAC std_ss [SOME_EL_IS_EL, PSUBSET_DEF] THEN
...
val P_SEM_MIN_MODEL_EXISTS_FINITE =
    prove (
    ``!n p S. ((FINITE S) /\ (CARD S < n)) ==> (P_SEM S p ==> (?S'. S' SUBSET S /\ P_SEM_MIN S' p))``,
...
val P_PROP_SET_MODEL_SEM =
 store_thm
  ("P_PROP_SET_MODEL_SEM",
    ...
    P_PROP_CONJUNCTION_SEM, SOME_EL_IS_EL, EVERY_MEM] THEN
...
val P_SEM___VAR_RENAMING___NOT_INJ =
  store_thm (
    ...
    INDUCT_THEN prop_logic_induct ASSUME_TAC THEN (
      ASM_SIMP_TAC std_ss [P_VAR_RENAMING_def, P_SEM_def, IN_BETA_THM]
    ));
...
val P_MIN_MODEL_DISJUNCTION_SEM =
 store_thm
  ("P_MIN_MODEL_DISJUNCTION_SEM",
    ...
    REPEAT STRIP_TAC THEN
    REWRITE_TAC[P_MIN_MODEL_DISJUNCTION_def, P_PROP_DISJUNCTION_SEM, SOME_EL_IS_EL] THEN
    `FINITE (POWER_SET S2)` by PROVE_TAC[POWER_SET_FINITE] THEN
    `?M. M = ({S' | P_SEM_MIN S' p} INTER POWER_SET S2)` by PROVE_TAC[] THEN
    `M SUBSET (POWER_SET S2)` by PROVE_TAC[INTER_SUBSET] THEN
    `FINITE M` by PROVE_TAC[SUBSET_FINITE] THEN
    FULL_SIMP_TAC list_ss [MEM_SET_TO_LIST] THEN
    `!e. (IS_EL e (SET_TO_LIST ({S' | P_SEM_MIN S' p} INTER POWER_SET S2))) =
        (e IN  ({S' | P_SEM_MIN S' p} INTER POWER_SET S2))` by PROVE_TAC[MEM_SET_TO_LIST] THEN
    ASM_SIMP_TAC std_ss [IN_POWER_SET_SUBSET_EQUIV, IN_INTER, GSPECIFICATION] THEN
    PROVE_TAC[]);
...
val P_MODEL_DISJUNCTION_SEM =
 store_thm
  ("P_MODEL_DISJUNCTION_SEM",
    ...
    REPEAT STRIP_TAC THEN
    REWRITE_TAC[P_MODEL_DISJUNCTION_def, P_PROP_DISJUNCTION_SEM, SOME_EL_IS_EL] THEN
    `FINITE (POWER_SET S2)` by PROVE_TAC[POWER_SET_FINITE] THEN
    `?M. M = ({S' | P_SEM S' p} INTER POWER_SET S2)` by PROVE_TAC[] THEN
    `M SUBSET (POWER_SET S2)` by PROVE_TAC[INTER_SUBSET] THEN
    `FINITE M` by PROVE_TAC[SUBSET_FINITE] THEN
    FULL_SIMP_TAC list_ss [MEM_SET_TO_LIST] THEN
    `!e. (IS_EL e (SET_TO_LIST ({S' | P_SEM S' p} INTER POWER_SET S2))) =
        (e IN  ({S' | P_SEM S' p} INTER POWER_SET S2))` by PROVE_TAC[MEM_SET_TO_LIST] THEN
    ASM_SIMP_TAC std_ss [IN_POWER_SET_SUBSET_EQUIV, IN_INTER, GSPECIFICATION] THEN
    PROVE_TAC[]);
...
val P_MIN_MODEL_DISJUNCTION_MIN_SEM =
 store_thm
  ("P_MIN_MODEL_DISJUNCTION_MIN_SEM",
    ...
    REPEAT STRIP_TAC THEN
    REWRITE_TAC[P_MIN_MODEL_DISJUNCTION_def, P_PROP_DISJUNCTION_MIN_SEM, SOME_EL_IS_EL] THEN
    `FINITE (POWER_SET S2)` by PROVE_TAC[POWER_SET_FINITE] THEN
    `?M. M = ({S' | P_SEM_MIN S' p} INTER POWER_SET S2)` by PROVE_TAC[] THEN
    `M SUBSET (POWER_SET S2)` by PROVE_TAC[INTER_SUBSET] THEN
    `FINITE M` by PROVE_TAC[SUBSET_FINITE] THEN
...
val P_MODEL_DISJUNCTION_MIN_SEM =
 store_thm
  ("P_MODEL_DISJUNCTION_MIN_SEM",
    ...
    REPEAT STRIP_TAC THEN
    REWRITE_TAC[P_MODEL_DISJUNCTION_def, P_PROP_DISJUNCTION_MIN_SEM, SOME_EL_IS_EL] THEN
    `FINITE (POWER_SET S2)` by PROVE_TAC[POWER_SET_FINITE] THEN
    `?M. M = ({S' | P_SEM S' p} INTER POWER_SET S2)` by PROVE_TAC[] THEN
    `M SUBSET (POWER_SET S2)` by PROVE_TAC[INTER_SUBSET] THEN
    `FINITE M` by PROVE_TAC[SUBSET_FINITE] THEN
 ...
 store_thm
  ("VAR_RENAMING_HASHTABLE_SEM",
    ...
    ASM_SIMP_TAC std_ss [VAR_RENAMING_HASHTABLE_def, P_BIGOR_SEM,
        POWER_SET_FINITE, IMAGE_FINITE, MEM_SET_TO_LIST, IN_IMAGE,
        GSYM LEFT_EXISTS_AND_THM, IN_POWER_SET_SUBSET_EQUIV, P_SEM_THM,
...
Induct_on `l` THENL [
  SIMP_TAC std_ss [LIST_TO_SET_THM,  VAR_RENAMING_HASHTABLE_def, VAR_RENAMING_HASHTABLE_LIST_def, P_SEM_def,
  P_BIGOR_SEM, POWER_SET_IND_THM, IMAGE_SING, MEM_SET_TO_LIST,
  ...
  ASM_SIMP_TAC std_ss [LIST_TO_SET_THM, P_SEM_THM,
    ...
    FINITE_LIST_TO_SET, POWER_SET_FINITE, FINITE_INSERT, IMAGE_FINITE,
  ...
  REPEAT STRIP_TAC THEN EQ_TAC THEN REPEAT STRIP_TAC THENL [
    Cases_on `h IN s` THEN ASM_REWRITE_TAC [] THENL [
      Cases_on `MEM h l` THENL [
        ...
        Know `h INSERT LIST_TO_SET l = LIST_TO_SET l` THEN1 (
          SIMP_TAC std_ss [EXTENSION, IN_INSERT,
            LIST_TO_SET] THEN
          PROVE_TAC[]
        ) THEN DISCH_TAC THEN
        Know `s' UNION {h} = s'` THEN1 (
        ...
        REPEAT STRIP_TAC THEN (
          SIMP_ALL_TAC std_ss [IN_POWER_SET_SUBSET_EQUIV, SUBSET_DEF, IN_INSERT, IN_DELETE, LIST_TO_SET, EXTENSION, IN_INTER] THEN
          METIS_TAC[]
        )
      ],
    ...
    SIMP_ALL_TAC std_ss [IN_POWER_SET_SUBSET_EQUIV, SUBSET_DEF, IN_INSERT, IN_DELETE, LIST_TO_SET, EXTENSION, IN_INTER,
    UNION_SING] THEN
    METIS_TAC[],
...
val VAR_RENAMING_HASHTABLE_LIST___P_USED_VARS =
  store_thm ("VAR_RENAMING_HASHTABLE_LIST___P_USED_VARS",

``!l S f.
P_USED_VARS (VAR_RENAMING_HASHTABLE_LIST l S f) =
(LIST_TO_SET l) UNION (IMAGE (\x. f (x UNION S)) (POWER_SET (LIST_TO_SET l)))``,
...
Induct_on `l` THENL [
  ...
  ASM_SIMP_TAC std_ss [VAR_RENAMING_HASHTABLE_LIST_def, P_USED_VARS_EVAL,
    ...
    POWER_SET_IND_THM, IMAGE_UNION] THEN
  REPEAT STRIP_TAC THEN
