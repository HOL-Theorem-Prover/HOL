...
\begin{alltt}
\( \vdash \DEF{FactIter} (n,acc) =                                                               \)
\(     (\texttt{if} n = 0 \texttt{then} (n,acc) \texttt{else} \DEF{FactIter} (n-1,n{\times}acc)) \)
\end{alltt}
...
\DEF{SEQ},
\DEF{PAR}, \DEF{ITE} and \DEF{REC} that build had shaking devices and, for easy access, are defined
at the end of this paper in the appendix.  The key property of these constructors
...
If $e_c$ is an expression built using \DEF{Seq},
\DEF{Par}, \DEF{Ite} and \DEF{Rec} then, by suitably instantiating the predicate variables $P_1$, $P_2$ and $P_3$,  
these theorems allow us to construct an expression $e_C$
built from circuit constructos \DEF{SEQ},
\DEF{PAR}, \DEF{ITE} and \DEF{REC} such that $\vdash e_C \FIMP \DEF{Dev}~e_c$. From Step~1 we have
$\vdash f=e_c$, hence $\vdash e_C \FIMP \DEF{Dev}~f$


A function $f$ which is combinational (i.e.~can be implemented directly with logic
gates without using registers) can be packaged as a handshaking device
using a contructor \DEF{ATM} which create a simple handshake interface and satisfies:
...
\noindent The circuit constructor \DEF{ATM} is defined with the other constructors in Section~\ref{CircuitConstructors}.
...
\noindent $\DEF{PRECEED}~f~d$ processes inputs with $f$ before sending them to $d$ and
$\DEF{FOLLOW}~d~f$ processes outputs of $d$ with
$f$. The construction $\DEF{SEQ}~d_1~d_2$ introduces a handshake between the executions
of $d_1$ and $d_2$, but $\DEF{PRECEED}~f~d$ and $\DEF{FOLLOW}~d~f$
just `wire' f before or after $d$, respectively, without introducing a
handshake. The definitions are:
...
However, we can easily implement \DEF{DEL} and \DEF{DFF} in terms of off-the-shelf
...
Dtypes. We implement \DEF{DEL} and \DEF{DFF} in terms of a standard
