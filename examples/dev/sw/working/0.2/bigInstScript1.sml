...
val valid_dom_def = Define `
    valid_dom ((regs,mem):DSTATE) = 9 IN FDOM regs`;
...
val push_one_lem = Q.store_thm (
    ...
    `!st e. valid_exp e /\ valid_dom st ==>
     ...
     Cases_on `e` THEN
     FULL_SIMP_TAC std_ss [valid_exp_def, valid_regs_lem, valid_dom_def, push_one_def,CFL_SEMANTICS_BLK, 
             reade_def, read_thm] THENL [
       NTAC 2 tac1 THEN FULL_SIMP_TAC finmap_ss [valid_regs_lem] THEN
           METIS_TAC [DECIDE ``~(9 = 13)``, FUPDATE_COMMUTES, FUPDATE_REFL],
       NTAC 3 tac1 THEN METIS_TAC [],
       NTAC 3 tac1 THEN METIS_TAC []
     ]
  );
...
val legal_push_exp_def = Define `
    legal_push_exp st e k = 
       ~(addr_in_range st e (w2n (read st SP), w2n (read st SP) - k)) /\
       valid_exp e`;

val LEGAL_PUSH_EXP_NOT_FP_SP = Q.store_thm (
    "LEGAL_PUSH_EXP_NOT_FP_SP",
    `!e st k. legal_push_exp st e k ==>
        ~(e = isR 9) /\ ~(e = isR 13)`,
    SIMP_TAC std_ss [legal_push_exp_def] THEN
    Cases_on `e` THEN
    RW_TAC std_ss [valid_exp_def, valid_regs_def]
   );

val PUSH_LIST_SAME_DOM = Q.store_thm (
    "PUSH_LIST_SAME_DOM",
    `!st l. valid_dom st ==> 
          valid_dom (run_cfl (BLK (push_list l)) st)`,
    Induct_on `l` THENL [
      RW_TAC std_ss [CFL_SEMANTICS_BLK, push_list_def, valid_dom_def],
      RW_TAC list_ss [RUN_CFL_BLK_APPEND, push_list_def] THEN
         ...
         FULL_SIMP_TAC std_ss [valid_dom_def] THEN
         Cases_on `h` THEN
         RW_TAC std_ss [push_one_def, CFL_SEMANTICS_BLK, read_thm] THENL [
           NTAC 2 tac1 THEN RW_TAC set_ss [valid_dom_def, FDOM_FUPDATE],
           NTAC 3 tac1 THEN RW_TAC set_ss [valid_dom_def, FDOM_FUPDATE],
           NTAC 3 tac1 THEN RW_TAC set_ss [valid_dom_def, FDOM_FUPDATE],
           tac1 THEN RW_TAC set_ss [valid_dom_def, FDOM_FUPDATE]
         ]
     ]
  );
...
val PUSH_ONE_SANITY = Q.store_thm (
    "PUSH_ONE_SANITY",
    `!st h l. valid_exp h /\ valid_dom st /\ locate_ge (read st SP) (SUC (LENGTH l))
                ==>
              let st1 = run_cfl (BLK (push_one h)) st in
              valid_dom st1 /\ locate_ge (read st1 SP) (LENGTH l)`,
     ...
     FULL_SIMP_TAC set_ss [valid_dom_def, FDOM_FUPDATE] THEN
...
val PUSH_LIST_SANITY = Q.store_thm (
    "PUSH_LIST_SANITY",
    `!l st x. EVERY valid_exp l /\ locate_ge (read st SP) (LENGTH l) /\ valid_dom st
             ==> !e. legal_push_exp st e (LENGTH l) ==>                
    ...
    Induct_on `l` THENL [
        ...
        RW_TAC list_ss [RUN_CFL_BLK_APPEND, push_list_def] THEN
          ...
          RES_TAC THEN POP_ASSUM (K ALL_TAC) THEN
          IMP_RES_TAC PUSH_LIST_SAME_DOM THEN
          POP_ASSUM (ASSUME_TAC o Q.SPEC `l`) THEN
          ...
          IMP_RES_TAC push_one_lem THEN
          RW_TAC std_ss [sp_def] THEN
          NTAC 8 (POP_ASSUM (K ALL_TAC)) THEN
          `(!st. ~(eq_exp st (isR 9) e)) /\ (!st.~(eq_exp st (isR 13) e))` by 
                METIS_TAC [NOT_EQ_isR_LEM, REWRITE_RULE [legal_push_exp_def] LEGAL_PUSH_EXP_NOT_FP_SP] THEN
...
val PUSH_LIST_FUNCTIONALITY = Q.store_thm (
    ...
    `!l st. EVERY valid_exp l /\ locate_ge (read st SP) (LENGTH l) /\ valid_dom st ==> 
        ...
        RW_TAC list_ss [RUN_CFL_BLK_APPEND, push_list_def] THEN
          ...
          Cases_on `i` THENL [
             FULL_SIMP_TAC list_ss [] THEN
               ...
               Q.PAT_ASSUM `w2n i = w2n j - k` (ASSUME_TAC o GSYM) THEN
               ASM_REWRITE_TAC [] THEN
               `~(is_C (isM (w2n (read st1 SP))))` by RW_TAC std_ss [is_C_def] THEN
               Q.UNABBREV_TAC `st1` THEN
               RW_TAC std_ss [READE_WRITEE, PUSH_LIST_SANITY],
...
val pop_one_lem = Q.store_thm (
    ...
    `!st e. valid_exp e /\ valid_dom st ==>
     ...
     RW_TAC finmap_ss [reade_def, writee_def, read_thm, write_thm, SP_def, sp_def] THEN
     Cases_on `e` THEN
     FULL_SIMP_TAC std_ss [valid_exp_def, valid_regs_lem, valid_dom_def, pop_one_def,CFL_SEMANTICS_BLK, 
       ...
       NTAC 3 tac1 THEN RW_TAC std_ss [fp_def] THEN METIS_TAC []
     ]
  );
...
val POP_ONE_SANITY = Q.store_thm (
    "POP_ONE_SANITY", 
    `!st e h l. valid_exp e /\ valid_exp h /\ valid_dom st /\ grow_lt (read st SP) (SUC (LENGTH l)) /\ 
              ~eq_exp st e h ==>
           let st1 = run_cfl (BLK (pop_one h)) st in  
              valid_dom st1 /\ grow_lt (read st1 SP) (LENGTH l) /\ (st1 '' e = st '' e)
    `,
    SIMP_TAC std_ss [FORALL_DSTATE] THEN
    REPEAT STRIP_TAC THEN
    ...
    (fn g => (NTAC 3 tac1 THEN FULL_SIMP_TAC finmap_ss [LET_THM, read_thm, valid_regs_lem, valid_dom_def] THEN
         FULL_SIMP_TAC set_ss [FDOM_FUPDATE] THEN
         FULL_SIMP_TAC arith_ss [grow_lt_def] THEN
         Cases_on `e` THEN 
         IMP_RES_TAC valid_regs_lem THEN
         FULL_SIMP_TAC finmap_ss [eq_exp_def, eq_addr_def, addr_of_def, valid_exp_def, is_C_def, reade_def, 
...
val POP_ONE_SANITY_2 = Q.store_thm (
    "POP_ONE_SANITY_2", 
    `!st h l. valid_exp e /\ valid_exp h /\ valid_dom st /\ grow_lt (read st SP) (SUC (LENGTH l)) ==>
           let st1 = run_cfl (BLK (pop_one h)) st in  
              valid_dom st1 /\ grow_lt (read st1 SP) (LENGTH l)
    `,
    SIMP_TAC std_ss [FORALL_DSTATE] THEN
    REPEAT STRIP_TAC THEN
    IMP_RES_TAC grow_lt_thm THEN
    ...
    (fn g => (NTAC 3 tac1 THEN FULL_SIMP_TAC finmap_ss [LET_THM, read_thm, valid_regs_lem, valid_dom_def] THEN
         FULL_SIMP_TAC set_ss [FDOM_FUPDATE] THEN
         FULL_SIMP_TAC arith_ss [grow_lt_def] THEN
         FULL_SIMP_TAC finmap_ss [eq_exp_def, eq_addr_def, addr_of_def, valid_exp_def, is_C_def, 
                       read_thm, addr_in_range_def, in_range_def, fp_def, FP_def] THEN
         FULL_SIMP_TAC arith_ss [valid_regs_lem]
         ) g)
  );
...
val POP_ONE_ADDR_IN_RANGE = Q.store_thm (
    ...
    `!st e h l. EVERY valid_exp l /\ valid_exp h /\ valid_dom st /\ grow_lt (read st SP) (SUC (LENGTH l)) /\ 
...
val POP_LIST_SAME_DOM = Q.store_thm (
    "POP_LIST_SAME_DOM",
    `!st l. valid_dom st ==> 
          valid_dom (run_cfl (BLK (pop_list l)) st)`,

    Induct_on `l` THENL [
      RW_TAC std_ss [CFL_SEMANTICS_BLK, pop_list_def, valid_dom_def],

      SIMP_TAC std_ss [FORALL_DSTATE] THEN
         RW_TAC list_ss [RUN_CFL_BLK_APPEND, pop_list_def] THEN
         `!f1 f2 n a b c d. valid_dom ((f1,f2):DSTATE) ==> valid_dom (f1 |+ (a,b), f2 |+ (c,d)) /\
             valid_dom (f1 |+ (a,b), f2) /\ valid_dom (f1, f2 |+ (c,d))` 
                   by RW_TAC set_ss [valid_dom_def, FDOM_FUPDATE] THEN
         Cases_on `h` THEN
         RW_TAC std_ss [pop_one_def, CFL_SEMANTICS_BLK, read_thm] THENL [
           NTAC 2 tac1 THEN RW_TAC finmap_ss [],
           NTAC 3 tac1 THEN RW_TAC finmap_ss [],
           NTAC 3 tac1 THEN RW_TAC finmap_ss [],
           tac1 THEN RW_TAC finmap_ss []
        ]
      ]
  );
...
val POP_LIST_SANITY = Q.store_thm (
    "POP_LIST_SANITY",
    `!l st x. EVERY valid_exp l /\ grow_lt (read st SP) (LENGTH l) /\ valid_dom st
             ==> !e. legal_pop_exp st e l ==>                
    ...
    Induct_on `l` THENL [
        ...
        RW_TAC list_ss [RUN_CFL_BLK_APPEND, pop_list_def] THEN
          FULL_SIMP_TAC list_ss [legal_pop_exp_def] THEN
          IMP_RES_TAC POP_ONE_SANITY THEN NTAC 19 (POP_ASSUM (K ALL_TAC)) THEN
...
val POP_LIST_FUNCTIONALITY = Q.store_thm (
    ...
    `!l st. EVERY valid_exp l /\ grow_lt (read st SP) (LENGTH l) /\ valid_dom st /\
          ...
          Cases_on `i` THENL [
            ...
            FULL_SIMP_TAC list_ss [] THEN
              `(run_cfl (BLK (pop_one h)) st) '' h = st '' isM (w2n (read st SP) + 1)` by ALL_TAC THENL [
                IMP_RES_TAC pop_one_lem THEN ASM_REWRITE_TAC [] THEN NTAC 2 (POP_ASSUM (K ALL_TAC)) THEN
              ...
              IMP_RES_TAC POP_ONE_SANITY_2 THEN
                IMP_RES_TAC POP_ONE_EXP_INTACT THEN
                FULL_SIMP_TAC std_ss [LET_THM] THEN
...
val PUSH_LIST_ADDR_IN_RANGE = Q.store_thm (
    ...
    `!st l1 l2. EVERY valid_exp l1 /\ locate_ge (read st SP) (LENGTH l1) /\ valid_dom st /\
...
val  ADDR_IN_RANGE_LEGAL_EXP = Q.store_thm (
    ...
    FULL_SIMP_TAC std_ss [addr_in_range_def, in_range_def, addr_of_def, valid_exp_def, locate_ge_def] THEN
...
val COPY_LIST_SANITY = Q.store_thm (
    ...
    `!st dstL srcL. EVERY valid_exp dstL /\ EVERY valid_exp srcL /\ (LENGTH srcL = LENGTH dstL) /\
         locate_ge (read st SP) (LENGTH srcL) /\ valid_dom st
    ...
    `valid_dom (run_cfl (BLK (push_list srcL)) st)` by METIS_TAC [PUSH_LIST_SAME_DOM] THEN
    RW_TAC std_ss [POP_LIST_SANITY] THEN
    METIS_TAC [PUSH_LIST_SANITY]
  );
...
val COPY_LIST_FUNCTIONALITY = Q.store_thm (
    ...
    `!st dstL srcL. EVERY valid_exp srcL /\ EVERY ($~ o is_C) srcL /\ EVERY ($~ o is_C) dstL /\
         ...
         locate_ge (read st SP) (LENGTH srcL) /\ valid_dom st /\ unique_exp_list st dstL /\
    ...
    NTAC 9 (POP_ASSUM (K ALL_TAC)) THEN
    ...
    METIS_TAC [PUSH_LIST_FUNCTIONALITY]
  );
