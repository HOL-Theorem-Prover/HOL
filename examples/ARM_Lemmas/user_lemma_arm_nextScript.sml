...
e(ASSUME_TAC reflex_priv_mode_constraints_v2a_thm);
...
(* data abort *)
e(`(ARM_MODE s1'' = 16w) /\ (ARM_MODE s2'' = 16w)` by (UNDISCH_TAC ``priv_mode_constraints_v2a g s1' s1''`` THEN UNDISCH_TAC ``priv_mode_constraints_v2a g s2' s2''`` THEN RW_TAC (srw_ss()) [LET_DEF, priv_mode_constraints_v2a_def, priv_mode_constraints_v1_def]));
e(IMP_RES_TAC untouched_mmu_setup_lem);
...
val mmu_arm_next_svc_thm = store_thm(
    ...
    MP_TAC mmu_arm_next_thm
       ...
       THEN `(s1'.psrs (0,SPSR_svc)) =  (s1.psrs (0,CPSR))` by FULL_SIMP_TAC (srw_ss()) [priv_mode_constraints_v2a_def]
       THEN `(s2'.psrs (0,SPSR_svc)) =  (s2.psrs (0,CPSR))` by FULL_SIMP_TAC (srw_ss()) [priv_mode_constraints_v2a_def]
       THEN IMP_RES_TAC untouched_mmu_setup_lem
