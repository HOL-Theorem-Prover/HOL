...
g `preserve_relation_mmu (read_memA_with_priv <|proc:=0|> (add, n, p)) (assert_mode 16w) (assert_mode 16w) empty_unt empty_sim`;
go_on 1;
...
*)
val IT_advance_empty_thm = MATCH_MP (#1 (EQ_IMP_RULE (SPEC_ALL empty_extras_lem))) (IT_advance_thm);
val IT_advance_thm = save_thm("IT_advance_thm", MATCH_MP extras_lem2 (SPEC_ALL IT_advance_empty_thm));

(*
val IT_advance_thm =
   |- preserve_relation_mmu (IT_advance <|proc := 0|>) (assert_mode 16w)
     (assert_mode 16w) empty_unt empty_sim ∧
   preserve_relation_mmu (IT_advance <|proc := 0|>) (assert_mode 16w)
     (assert_mode 16w) priv_mode_constraints priv_mode_similar ∧
   preserve_relation_mmu (IT_advance <|proc := 0|>) (assert_mode 16w)
     (assert_mode 16w) priv_mode_constraints
     (fix_flags xI xF priv_mode_similar) ∧
   preserve_relation_mmu (IT_advance <|proc := 0|>) (assert_mode 16w)
     (assert_mode 16w) empty_unt (fix_flags xI xF empty_sim)
*)
