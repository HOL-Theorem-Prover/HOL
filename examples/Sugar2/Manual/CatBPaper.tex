...
higher order logic supported by the \Hol system and are contemplating
a variety of applications. For example, we want to `sanity check' our
embedding by subjecting it to `formal challenges', and we hope to
exploit existing work to build a Sugar model checker inside \Hol. In
the longer term we want to investigate the use of theorem proving to
reason about models with infinite state spaces, which might involve
developing extensions of \Sugar.
\end{abstract}
...
I teach both semantic embedding and property specification as part of
...
Cindy Eisner patiently answered several email questions
