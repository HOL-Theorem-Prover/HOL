...
An invocation `match_terml avoid_tys avoid_tms pat ob (tmS,tyS)`, if it
does not raise an exception, returns a pair of substitutions `(S,T)`
such that
...
The arguments `avoid_tys` and `avoid_tms` specify type and term
variables in `pat` that are not allowed to become `redex`es in `S` and
`T`.
...
but elements of `avoid_tys` would appear as redexes in `T` or elements
of `avoid_tms` would appear as redexes in `S`, then `match_terml` will
also fail.
...
[`Term.raw_match`](#Term.raw_match), [`Term.subst`](#Term.subst),
[`Term.inst`](#Term.inst), [`Type.match_typel`](#Type.match_typel),
