...
fun Mfail nm tm =
  ...
  in
    require (check_HOL_ERR (fn _ => true)) TAC_PROOF (([], tm), MESON_TAC [])
  end
...
val _ = temp_remove_termtok {term_name = "O", tok = "O"};
val _ = app (ignore o hide) ["irreflexive", "transitive", "one"];
...
(* hol90 - no *)
val P49 = Mfail "P49" “(?x y. !z:'a. (z = x) \/ (z = y)) /\
                            P a /\ P b /\ ~(a = b) ==> !x:'a. P x”;
...
fun MESON _ thms tm = (tm,mesonLib.MESON_TAC (ASSUME BOOL :: thms) ([], tm));
val BOOL_FACTS = true
...
val GRP130_1_002 = Mfail "GRP130_1_002" $
 “(group_element(e_1:'a)) /\
   (group_element(e_2)) /\
   (~equal(e_1,e_2)) /\
   (~equal(e_2,e_1)) /\
...
val SYN034_1 = M "SYN034_1" $
 “(!A:'a. p(A,a) \/ p(A,f(A))) /\
   (!A. p(A,a) \/ p(f(A),A)) /\
   (!A B. ~(p(A,B) /\ p(B,A) /\ p(B,a))) ==> F”;


val SYN071_1 = M "SYN071_1" $
 “(!X:'a. equal(X,X)) /\
   (!Y X. equal(X,Y) ==> equal(Y,X)) /\
   (!Y X Z. equal(X,Y) /\ equal(Y,Z) ==> equal(X,Z)) /\
   (equal(a,b) \/ equal(c,d)) /\
