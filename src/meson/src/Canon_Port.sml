...
val RIGHT_AND_TY_EXISTS_THM = GSYM RIGHT_TY_EXISTS_AND_THM;
val LEFT_AND_TY_EXISTS_THM  = GSYM LEFT_TY_EXISTS_AND_THM;
val OR_TY_EXISTS_THM        = GSYM TY_EXISTS_OR_THM;
val AND_TY_FORALL_THM       = GSYM TY_FORALL_AND_THM;
val LEFT_OR_TY_FORALL_THM   = GSYM LEFT_TY_FORALL_OR_THM;
val RIGHT_OR_TY_FORALL_THM  = GSYM RIGHT_TY_FORALL_OR_THM;
val LEFT_IMP_TY_FORALL_THM  = GSYM LEFT_TY_EXISTS_IMP_THM;
val RIGHT_IMP_TY_FORALL_THM = GSYM RIGHT_TY_FORALL_IMP_THM;
val LEFT_IMP_TY_EXISTS_THM  = boolTheory.LEFT_TY_EXISTS_IMP_THM;
val RIGHT_IMP_TY_EXISTS_THM = GSYM RIGHT_TY_EXISTS_IMP_THM;

fun freesl tml = itlist (op_union eq o free_vars) tml [];;

fun is_eqc tm = same_const equality tm

local fun get_heads lconsts tm (sofar as (cheads,vheads)) =
        let val (v,bod) = dest_forall tm
        in get_heads (op_subtract eq lconsts [v]) bod sofar
        end
        handle HOL_ERR _ =>
        ...
        handle HOL_ERR _ =>
            let val (hop,args) = strip_comb tm
                val len = length args
                val newheads =
                  if is_const hop orelse op_mem eq hop lconsts
                  then (op_insert (pair_cmp eq equal) (hop,len) cheads, vheads)
                  else if len > 0
                       then (cheads,op_insert (pair_cmp eq equal) (hop,len) vheads)
                       else sofar
        in
    ...
    if is_forall tm then
      ...
      if is_conj tm orelse is_disj tm then
        ...
        let
          ...
          val n = length args - op_assoc eq opn hddata
                  handle HOL_ERR _ => 0
        in
          if n = 0 then th
...
in
  fun GEN_FOL_CONV (cheads,vheads) =
    let val hddata =
          if null vheads
          then let val hops = op_mk_set eq (map fst cheads)
                   fun getmin h =
                    let val ns = mapfilter
                          (fn (k,n) => if eq k h then n else fail()) cheads
               ...
               in mapfilter getmin hops
                   ...
                   (op_mk_set eq (map fst (vheads @ cheads)))
    in FOL_CONV hddata
    end
end
...
local
  val NOT_EXISTS_UNIQUE_THM = Tactical.prove(
    ...
    REWRITE_TAC [EXISTS_UNIQUE_THM, DE_MORGAN_THM,NOT_EXISTS_THM,NOT_TY_EXISTS_THM]
     THEN CONV_TAC (REDEPTH_CONV (NOT_FORALL_CONV ORELSEC NOT_TY_FORALL_CONV))
...
val NNFC_CONV = let
  fun SINGLE_SWEEP_CONV conv tm = let
    fun continue tm = if is_abs tm orelse is_tyabs tm then NNFC_CONV0 tm
...
val DELAMB_CONV =
  let val pth = prove(
        ...
        REWRITE_TAC [])
      val qconv =
          TOP_DEPTH_CONV (TY_BETA_CONV ORELSEC BETA_CONV) THENC
          REPEATC (QCHANGED_CONV (GEN_REWRITE_CONV ONCE_DEPTH_CONV [pth]) THENC
                   TOP_DEPTH_CONV (TY_BETA_CONV ORELSEC BETA_CONV))
...
val PRESIMP_CONV =
  GEN_REWRITE_CONV DEPTH_CONV
   [NOT_CLAUSES, AND_CLAUSES, OR_CLAUSES, IMP_CLAUSES, EQ_CLAUSES,
    ...
    TY_FORALL_SIMP, TY_EXISTS_SIMP, TY_EXISTS_OR_THM, TY_FORALL_AND_THM,
    LEFT_EXISTS_AND_THM, RIGHT_EXISTS_AND_THM,
    LEFT_FORALL_OR_THM, RIGHT_FORALL_OR_THM,
    LEFT_TY_EXISTS_AND_THM, RIGHT_TY_EXISTS_AND_THM,
    LEFT_TY_FORALL_OR_THM, RIGHT_TY_FORALL_OR_THM];
...
val SKOLEM_CONV =
  GEN_REWRITE_CONV REDEPTH_CONV
     [RIGHT_AND_EXISTS_THM,
      ...
      OR_TY_EXISTS_THM,
      RIGHT_OR_TY_EXISTS_THM,
      LEFT_OR_TY_EXISTS_THM,
      SKOLEM_THM];
