...
val REAL_1 = prove_thm("REAL_1",
  (--`real_1 = &1`--),
  REWRITE_TAC[num_CONV (--`1`--), real_of_num, REAL_ADD_LID]);
...
val REAL_ADD_SYM =
    save_thm("REAL_ADD_SYM",reeducate(REAL_ADD_SYM));
val REAL_ADD_ASSOC =
    save_thm("REAL_ADD_ASSOC",reeducate(REAL_ADD_ASSOC));
val REAL_ADD_LID =
    save_thm("REAL_ADD_LID",reeducate(REAL_ADD_LID));
val REAL_ADD_LINV =
    save_thm("REAL_ADD_LINV",reeducate(REAL_ADD_LINV));
val REAL_LDISTRIB =
    save_thm("REAL_LDISTRIB",reeducate(REAL_LDISTRIB));
val REAL_LT_TOTAL =
    save_thm("REAL_LT_TOTAL",reeducate(REAL_LT_TOTAL));
val REAL_LT_REFL =
    save_thm("REAL_LT_REFL",reeducate(REAL_LT_REFL));
val REAL_LT_TRANS =
    save_thm("REAL_LT_TRANS",reeducate(REAL_LT_TRANS));
val REAL_LT_IADD =
    save_thm("REAL_LT_IADD",reeducate(REAL_LT_IADD));
val REAL_SUP_ALLPOS =
    save_thm("REAL_SUP_ALLPOS",reeducate(REAL_SUP_ALLPOS));
val REAL_MUL_SYM =
    save_thm("REAL_MUL_SYM",reeducate(REAL_MUL_SYM));
val REAL_MUL_ASSOC =
    save_thm("REAL_MUL_ASSOC",reeducate(REAL_MUL_ASSOC));
val REAL_MUL_LID =
    save_thm("REAL_MUL_LID",reeducate(REAL_MUL_LID));
val REAL_MUL_LINV =
    save_thm("REAL_MUL_LINV",reeducate(REAL_MUL_LINV));
val REAL_LT_MUL =
    save_thm("REAL_LT_MUL",reeducate(REAL_LT_MUL));
val REAL_INV_0 =
    save_thm("REAL_INV_0",reeducate REAL_INV_0);
...
val REAL_LT_MULTIPLE = prove_thm("REAL_LT_MULTIPLE",
  (--`!n d. 1 < n ==> (d |<| (&n |*| d) = &0 |<| d)`--),
  let val LESS_LEMMA1 = LESS_LEMMA1 in
...
val REAL_POW_LT2 = prove_thm("REAL_POW_LT2",
 ...
 INDUCT_TAC THEN REWRITE_TAC[NOT_SUC, pow] THEN REPEAT STRIP_TAC THEN
  ASM_CASES_TAC (Term `n = 0`) THEN ASM_REWRITE_TAC[pow, REAL_MUL_RID] THEN
 ...
 Term`!m n x. &1 |<| x /\ m < n ==> x pow m |<| x pow n`,
  ...
  MATCH_MP_TAC REAL_LT_LMUL_IMP THEN CONJ_TAC THENL
   [SPEC_TAC(Term`d:num`,Term`d:num`) THEN
    ...
    ASM_REWRITE_TAC[REAL_LT,prim_recTheory.LESS_0, num_CONV (Term`1`)]]);
   ...
   [X_GEN_TAC (--`y:num`--) THEN DISCH_TAC THEN
    ...
    CONJ_TAC THENL
     [DISJ_CASES_TAC(SPECL [(--`x:num`--), (--`k:num`--)] LESS_CASES) THENL
      ...
      CONV_TAC(ONCE_DEPTH_CONV EXISTS_UNIQUE_CONV) THEN
      DISCH_THEN(MP_TAC
                 o SPECL [(--`if x1 < k then x1 else SUC x1`--),
                          (--`if x2 < k then x2 else SUC x2`--)]
                 o CONJUNCT2) THEN
      ASM_REWRITE_TAC[] THEN
      W(C SUBGOAL_THEN MP_TAC o funpow 2 (fst o dest_imp) o snd) THENL
...
val real_lt = prove_thm ("real_lt",
  ``!y x. x |<| y = ~(y |<=| x)``,
  let
    val th1 = TAUT_PROVE ``!t u. (t = ~u) ==> (u = ~t)``
    val th2 = SPEC ``x |<| y`` (SPEC ``y |<=| x`` th1)
    val th3 = SPEC ``x:real`` (SPEC ``y:real`` |<=|)
  in
    ACCEPT_TAC (GEN ``y:real`` (GEN ``x:real`` (MP th2 th3)))
  end);
