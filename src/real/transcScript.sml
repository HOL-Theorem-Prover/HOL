...
val SQRT_POS_LT = prove_thm("SQRT_POS_LT",
 Term`!x. &0 |<| x ==> &0 |<| sqrt(x)`,
 REWRITE_TAC [sqrt,num_CONV (Term`2`)] THEN REPEAT STRIP_TAC
...
val POW_2_SQRT = prove_thm("POW_2_SQRT",
 ...
 REWRITE_TAC [sqrt,num_CONV (Term`2`),POW_ROOT_POS]);

val SQRT_POS_UNIQ = prove_thm("SQRT_POS_UNIQ",
 Term`!x y. &0 |<=| x /\ &0 |<=| y /\ (y pow 2 = x)
           ==> (sqrt x = y)`,
  REWRITE_TAC[sqrt, num_CONV (Term`2`), ROOT_POS_UNIQ]);

val SQRT_MUL = prove_thm("SQRT_MUL",
 Term`!x y. &0 |<=| x /\ &0 |<=| y
           ==> (sqrt(x |*| y) = sqrt x |*| sqrt y)`,
  REWRITE_TAC[sqrt, num_CONV (Term`2`), ROOT_MUL]);;

val SQRT_INV = prove_thm("SQRT_INV",
 Term`!x. &0 |<=| x ==> (sqrt (inv x) = inv(sqrt x))`,
  REWRITE_TAC[sqrt, num_CONV (Term`2`), ROOT_INV]);;

val SQRT_DIV = prove_thm("SQRT_DIV",
 Term`!x y. &0 |<=| x /\ &0 |<=| y
           ==> (sqrt (x / y) = sqrt x / sqrt y)`,
  REWRITE_TAC[sqrt, num_CONV (Term`2`), ROOT_DIV]);;

val SQRT_MONO_LE = prove_thm("SQRT_MONO_LE",
 Term`!x y. &0 |<=| x /\ x |<=| y ==> sqrt(x) |<=| sqrt(y)`,
  REWRITE_TAC[sqrt, num_CONV (Term`2`), ROOT_MONO_LE]);;

val lem = prove(Term`0<2`, REWRITE_TAC[num_CONV(Term`2`),LESS_0]);
...
val COS_SIN_SQRT = prove_thm("COS_SIN_SQRT",
  ...
  REWRITE_TAC[sqrt, num_CONV (Term`2`)] THEN
...
val DIVISION_1 = prove_thm("DIVISION_1",
  ...
  REWRITE_TAC[NOT_SUC] THEN EQ_TAC THENL
   [DISCH_TAC THEN MATCH_MP_TAC LESS_EQUAL_ANTISYM THEN CONJ_TAC THENL
     [POP_ASSUM(MP_TAC o SPEC (Term`1`) o CONJUNCT1) THEN
      REWRITE_TAC[num_CONV(Term`1`), GSYM SUC_NOT] THEN
      REWRITE_TAC[REAL_LT_REFL, NOT_LESS],
      POP_ASSUM(MP_TAC o SPEC (Term `2`) o CONJUNCT2) THEN
      REWRITE_TAC[num_CONV (Term `2`), GSYM SUC_NOT, GREATER_EQ] THEN
      CONV_TAC CONTRAPOS_CONV THEN
      REWRITE_TAC[num_CONV (Term `1`), NOT_SUC_LESS_EQ, CONJUNCT1 LE] THEN
    ...
    DISCH_THEN SUBST1_TAC THEN CONJ_TAC THENL
     [GEN_TAC THEN REWRITE_TAC[num_CONV (Term `1`),LESS_THM, NOT_LESS_0] THEN
      DISCH_THEN SUBST1_TAC THEN ASM_REWRITE_TAC[],
      X_GEN_TAC (Term `n:num`) THEN REWRITE_TAC[GREATER_EQ,num_CONV(Term`1`)]
      THEN ASM_CASES_TAC (Term `n = 0`) THEN
...
val DIVISION_SINGLE = prove_thm("DIVISION_SINGLE",
  ...
  POP_ASSUM(DISJ_CASES_TAC o REWRITE_RULE[REAL_LE_LT]) THENL
   [EXISTS_TAC (Term `1`) THEN CONJ_TAC THEN X_GEN_TAC (Term `n:num`) THENL
     [REWRITE_TAC[LESS_1] THEN DISCH_THEN SUBST1_TAC THEN
      ASM_REWRITE_TAC[NOT_SUC],
      REWRITE_TAC[GREATER_EQ] THEN
      COND_CASES_TAC THEN ASM_REWRITE_TAC[num_CONV (Term`1`)] THEN
      REWRITE_TAC[LE, NOT_SUC]],
    EXISTS_TAC (Term `0`) THEN REWRITE_TAC[NOT_LESS_0] THEN
...
val DIVISION_THM = prove_thm("DIVISION_THM",
 Term `!D a b. 
  ...
  SUBGOAL_THEN (Term `dsize D = N`) (fn th => ASM_REWRITE_TAC[th]) THEN
  REWRITE_TAC[dsize] THEN MATCH_MP_TAC SELECT_UNIQUE THEN
  X_GEN_TAC (Term `M:num`) THEN BETA_TAC THEN EQ_TAC THENL
   [ALL_TAC, DISCH_THEN SUBST1_TAC THEN ASM_REWRITE_TAC[] THEN
    MP_TAC(SPEC (Term `N:num`) (ASSUME (Term `!n. n >= N ==> (D n:real = b)`)))
...
val DIVISION_LT_GEN = prove_thm("DIVISION_LT_GEN",
  Term`!D a b m n. division(a,b) D /\
               m < n /\
               n <= (dsize D) ==> D(m) |<| D(n)`,
  REPEAT STRIP_TAC THEN UNDISCH_TAC (Term`m < n`) THEN
...
val DIVISION_LT = prove_thm("DIVISION_LT",
  ...
  ASM_REWRITE_TAC[] THEN UNDISCH_TAC (Term`D(0):real = a`) THEN
...
val DIVISION_LE = prove_thm("DIVISION_LE",
  ...
  UNDISCH_TAC (Term`D(0):real = a`) THEN DISCH_THEN(SUBST1_TAC o SYM) THEN
 ...
 Term`!D a b. division(a,b) D ==> !r. D(r) |<=| b`,
  ...
  SUBST1_TAC THENL
   [ALL_TAC,
    ...
    DISCH_THEN SUBST_ALL_TAC THEN
    UNDISCH_TAC (Term`r < 0`) THEN REWRITE_TAC[NOT_LESS_0]] THEN
   ...
   [REWRITE_TAC[tdiv] THEN CONJ_TAC THENL
    ...
    THENL
     [DISCH_THEN(ASSUME_TAC o MATCH_MP DIVISION_1) THEN
      ASM_REWRITE_TAC[num_CONV (Term`1`), LESS_THM, NOT_LESS_0] THEN
      DISCH_THEN SUBST1_TAC THEN ASM_REWRITE_TAC[],
...
val FTC1 = prove_thm("FTC1",
 Term `!f f' a b. 
  ...
  SUBST1_TAC THENL
   [MP_TAC(SPECL [Term`\n:num. (f:real->real)(D(n))`, Term`0`, Term`dsize D`]
...
val MCLAURIN = prove_thm("MCLAURIN",
 Term
  ...
  SUBGOAL_THEN (Term`(g(&0) = &0) /\ (g(h) = &0)`) ASSUME_TAC THENL
   [EXPAND_TAC "g" THEN BETA_TAC THEN ASM_REWRITE_TAC[REAL_SUB_REFL] THEN
    ...
    MP_TAC(GEN (Term`j:num->real`)
     (SPECL [Term`j:num->real`, Term`r:num`, Term`1`] SUM_OFFSET)) THEN
  ...
  SUBGOAL_THEN (Term`difg(0):real->real = g`) ASSUME_TAC THENL
   ...
   [REPEAT GEN_TAC THEN DISCH_TAC THEN EXPAND_TAC "difg" THEN BETA_TAC THEN
    ...
    (MP_TAC o C SPECL DIFF_SUM)
     [Term`\p x. (diff((p + 1) + m)(&0) / &(FACT(p + 1)))
      ...
      Term`0`, Term`d:num`, Term`t:real`] THEN BETA_TAC THEN
  ...
  SUBGOAL_THEN (Term`!m. m < n ==> (difg(m)(&0) = &0)`) ASSUME_TAC THENL
   [X_GEN_TAC (Term`m:num`) THEN EXPAND_TAC "difg" THEN
    ...
    THEN ONCE_REWRITE_TAC[ADD_SYM] THEN REWRITE_TAC[ADD_SUB] THEN
    MP_TAC(GEN (Term`j:num->real`)
     (SPECL [Term`j:num->real`, Term`d:num`, Term`1`] SUM_OFFSET)) THEN
    REWRITE_TAC[ADD1, REAL_EQ_SUB_LADD] THEN
    DISCH_THEN(fn th => REWRITE_TAC[GSYM th]) THEN BETA_TAC THEN
  ...
  SUBGOAL_THEN (Term`!m. m < n ==> ?t. &0 |<| t /\ t |<| h /\
   ...
   [DISCH_TAC THEN MATCH_MP_TAC ROLLE THEN ASM_REWRITE_TAC[] THEN
    ...
    MP_TAC THENL
     [GEN_TAC THEN DISCH_TAC THEN REWRITE_TAC[differentiable] THEN
      ...
      SUBST1_TAC(SYM(ASSUME (Term`difg(0):real->real = g`))) THEN
      FIRST_ASSUM MATCH_MP_TAC THEN ASM_REWRITE_TAC[], ALL_TAC] THEN
    DISCH_TAC THEN CONJ_TAC THENL
     [GEN_TAC THEN DISCH_TAC THEN MATCH_MP_TAC DIFF_CONT THEN
      ...
      CONJ_TAC THEN MATCH_MP_TAC REAL_LT_IMP_LE THEN ASM_REWRITE_TAC[]],
    DISCH_TAC THEN SUBGOAL_THEN (Term`m < n`)
 ...
 Term`!diff n x. (x = &0) /\ 0 < n 
  ...
  DISCH_THEN(DISJ_CASES_THEN2 (SUBST1_TAC o SYM) MP_TAC) THENL
   [REWRITE_TAC[sum, ADD_CLAUSES] THEN BETA_TAC THEN 
    ...
    THEN REWRITE_TAC [GSYM REAL_INV_1OVER,  GSYM (num_CONV(Term`1`))]
    ...
    UNDISCH_TAC (Term`0 < n`) THEN SPEC_TAC(Term`n:num`,Term`n:num`) THEN
...
val LET_CASES = prove(Term`!m n. m <= n \/ n < m`,
...
val REAL_POW_EQ_0 = prove
 (Term`!x n. (x pow n = &0) = (x = &0) /\ ~(n = 0)`,
  ...
  ASM_REWRITE_TAC[NOT_SUC, pow, REAL_ENTIRE] THENL
   [REWRITE_TAC [REAL_OF_NUM_EQ, num_CONV(Term`1`),NOT_SUC],
      ...
      ==> !x n. ?t. abs t  |<=| abs x /\
                   (f(x) = sum(0,n)
                           ...
                           |+|
                            (diff n t / &(FACT n)) |*| x pow n)`,
  REPEAT STRIP_TAC THEN
  DISJ_CASES_THEN MP_TAC(SPECL [Term`n:num`, Term`0`] LET_CASES) THENL
