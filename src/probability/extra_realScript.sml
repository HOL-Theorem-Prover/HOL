...
val exp_convex_lemma5 = store_thm
  ("exp_convex_lemma5",
   ...
   ++ POP_ASSUM (fn thm => ONCE_REWRITE_TAC [thm])
   ++ Q.PAT_ASSUM `!x. i < x /\ x < j ==> 0 < f' x` MATCH_MP_TAC ++ RW_TAC std_ss []);
  ...
  ("jensen_convex_SIGMA",
   ...
   >> (FULL_SIMP_TAC real_ss []
	...
	by (MATCH_MP_TAC FINITE_INDUCT
	    ...
	    ++ Q.PAT_ASSUM `!g. (SIGMA g s' = 0) /\ (!x. x IN s' ==> 0 <= g x /\ g x <= 1) ==>
				!x. x IN s' ==> (g x = 0)`
		(MP_TAC o Q.SPEC `g'`)
	    ++ PROVE_TAC [REAL_SUM_IMAGE_POS])
	...
	++ POP_ORW
	++ POP_ASSUM (ASSUME_TAC o UNDISCH_ALL o REWRITE_RULE [GSYM AND_IMP_INTRO] o Q.SPEC `g` o UNDISCH o Q.SPEC `s`)
   ...
   >> (RW_TAC std_ss [] ++ FULL_SIMP_TAC std_ss [convex_fn, GSPECIFICATION]
       ...
       ++ RW_TAC std_ss []
       ++ Q.PAT_ASSUM `!x y t. P` (MP_TAC o Q.SPECL [`g' e`, `SIGMA (\x. g x / (1 - g e) * g' x) s`, `g e`])
       ++ RW_TAC std_ss []
       ++ MATCH_MP_TAC REAL_LE_TRANS
       ...
       ++ Know `0 < 1 - g e`
       >> (Q.PAT_ASSUM `g e <= 1` MP_TAC ++ Q.PAT_ASSUM `~ (g e = 1)` MP_TAC ++ REAL_ARITH_TAC)
       ++ STRIP_TAC
       ++ Suff `f (SIGMA (\x. g x / (1 - g e) * g' x) s) <=
		SIGMA (\x. g x / (1 - g e) * f (g' x)) s`
       >> PROVE_TAC [REAL_LE_LMUL]
       ++ Q.PAT_ASSUM `(SIGMA (\x. g x / (1 - g e)) s = 1) /\
	   ...
	   ++ Q.PAT_ASSUM `g e + SIGMA g s = 1` MP_TAC
	   ++ REAL_ARITH_TAC)
       ...
       ++ Q.PAT_ASSUM `g e + SIGMA g s = 1` (fn thm => ONCE_REWRITE_TAC [GSYM thm])
   ...
   >> (RW_TAC std_ss [] ++ FULL_SIMP_TAC std_ss [pos_convex_fn, GSPECIFICATION]
       ...
       ++ `0 < SIGMA (\x. g x / (1 - g e) * g' x) s`
		...
		by (REPEAT (POP_ASSUM (K ALL_TAC))
			...
			>> (FULL_SIMP_TAC real_ss [REAL_ENTIRE] ++ PROVE_TAC [REAL_LT_IMP_NE, REAL_LT_LE])
       ...
       ++ Suff `f (SIGMA (\x. g x / (1 - g e) * g' x) s) <=
		SIGMA (\x. g x / (1 - g e) * f (g' x)) s`
       >> PROVE_TAC [REAL_LE_LMUL]
       ++ Q.PAT_ASSUM `(!x. x IN s ==> 0 <= g x / (1 - g e) /\ g x / (1 - g e) <= 1) /\
...
val REAL_ADD_POW_2 = store_thm
  ("REAL_ADD_POW_2", ``!a b. (a + b) pow 2 = a pow 2 + b pow 2 + 2 * a * b``,
    RW_TAC real_ss [POW_2,REAL_ADD_LDISTRIB,REAL_ADD_RDISTRIB,REAL_ADD_ASSOC]
    ++ REAL_ARITH_TAC);

val REAL_SUB_POW_2 = store_thm
  ("REAL_SUB_POW_2", ``!a b. (a - b) pow 2 = a pow 2 + b pow 2 - 2 * a * b``,
...
val REAL_SUP_SEQ = store_thm
  ("REAL_SUP_SEQ",``!P:real->bool. (?x. P x) /\ (?z. !x. P x ==> x <= z) ==>
	...
	++ RW_TAC std_ss []
	>>    ( RW_TAC std_ss [seq_sup_def]
		++ Suff `(\r. seq_sup P n <= r) (@r. r IN P /\ max (sup P - (1 / 2) pow SUC (n)) (seq_sup P n) < r /\ r < sup P)`
		>> METIS_TAC []
		++ MATCH_MP_TAC SELECT_ELIM_THM
		++ RW_TAC std_ss []
		>> ( `sup P - (1 / 2) pow (SUC n) < sup P` by METIS_TAC [REAL_LT_SUB_RADD,REAL_LT_ADDR,POW_HALF_POS]
	...
	++ `!n. sup P - (1/2) pow n  <= seq_sup P n` 
			...
			++ RW_TAC std_ss [seq_sup_def]
			++ Suff `(\r. sup P - (1 / 2) pow (SUC n) <= r) (@r. r IN P /\ max (sup P - (1 / 2) pow (SUC n)) (seq_sup P n) < r /\ r < sup P)`
			>> METIS_TAC []
			++ MATCH_MP_TAC SELECT_ELIM_THM
			++ RW_TAC std_ss []
			>> ( `sup P - (1 / 2) pow (SUC n) < sup P` by METIS_TAC [REAL_LT_SUB_RADD,REAL_LT_ADDR,POW_HALF_POS]
			...
			++ `sup P - (1 / 2) pow (SUC n) <= max (sup P - (1 / 2) pow (SUC n)) (seq_sup P n)` by RW_TAC real_ss [max_def]
			++ METIS_TAC [REAL_LET_TRANS,REAL_LT_IMP_LE]
		      )
...
val REAL_SUP_SEQ_IMAGE = store_thm
  ("REAL_SUP_SEQ_IMAGE", ``!P P'. (?x. P x) /\ (?z. !x. P x ==> x <= z) /\ (P = IMAGE f P') /\ 
	...
	++ Cases_on `y (SUC n) = y n`
	>> RW_TAC std_ss [REAL_LE_REFL]
	++ `y n < y (SUC n)` by METIS_TAC [REAL_LE_LT]
...
val max_fn_seq_inc = store_thm
  ("max_fn_seq_inc", ``!g n x. max_fn_seq g n x <= max_fn_seq g (SUC n) x``,
   RW_TAC real_ss [max_fn_seq_def,max_def]
  );
