...
val MAP_SND_FILTER_NEQ = store_thm("MAP_SND_FILTER_NEQ",
  ``MAP SND (FILTER (λ(x,y). y ≠ z) ls) =
    FILTER ($<> z) (MAP SND ls)``,
  Q.ISPECL_THEN[`$<> z`,`SND:('b#'a)->'a`,`ls`]MP_TAC FILTER_MAP >> rw[] >>
...
val TAKE_EL_SNOC = store_thm("TAKE_EL_SNOC",
  ...
  Cases_on`n = LENGTH ls` THEN1 (
    rw[EL_LENGTH_SNOC,TAKE_SNOC,TAKE_APPEND1,EL_APPEND1,EL_APPEND2,TAKE_APPEND2]
    >> FULL_SIMP_TAC arith_ss []) THEN
  `n < LENGTH ls` by FULL_SIMP_TAC arith_ss [ADD1] THEN
  rw[TAKE_SNOC,TAKE_APPEND1,EL_APPEND1] >>
  FULL_SIMP_TAC arith_ss [ADD1, LENGTH_SNOC, TAKE_APPEND1, SNOC_APPEND])
...
val REVERSE_DROP = store_thm("REVERSE_DROP",
  ...
  Cases_on`n = SUC (LENGTH ls)` THEN1 (
    rw[DROP_LENGTH_NIL_rwt,ADD1,LASTN] ) THEN
  `n <= LENGTH ls` by RW_TAC arith_ss [] >>
  rw[DROP_APPEND1,LASTN_APPEND1] >>
  `LENGTH [x] ≤ LENGTH ls + 1 − n` by RW_TAC arith_ss [LENGTH] >>
  RW_TAC arith_ss [LASTN_APPEND1, LENGTH]);
...
val EVERY2_REVERSE1 = store_thm("EVERY2_REVERSE1",
  ...
  simp[EVERY2_EVERY] >> REPEAT STRIP_TAC >>
