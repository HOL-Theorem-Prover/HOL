...
Theorem TAKE_cons:  0 < n ==> (TAKE n (x::xs) = x::(TAKE (n-1) xs))
Proof
  SRW_TAC[][TAKE_def]
QED
...
Theorem DROP_cons: 0 < n ==> (DROP n (x::xs) = DROP (n-1) xs)
Proof
  SRW_TAC[][DROP_def]
QED
...
Theorem ALL_DISTINCT_SET_TO_LIST:
