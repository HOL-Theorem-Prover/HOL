...
in
 val list_rws = computeLib.add_thms
    (true,[ APPEND,APPEND_NIL, FLAT, HD, TL, LENGTH, MAP, MAP2,
	    NULL_DEF, CONS_11, NOT_CONS_NIL,
	    NOT_NIL_CONS, EQT_INTRO(REFL(--`[]:'a list`--)),
            MEM_DEF', EXISTS_DEF', EVERY_DEF',
            FILTER, FOLDR, FOLDL, EL_compute,
	    computeLib.lazyfy_thm (SPEC_ALL list_case_compute)])
end;
