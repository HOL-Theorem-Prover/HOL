...
val IN_DEF = new_infixr_definition ("IN_DEF",
...
val INSERT_DEF = new_infixr_definition(
  "INSERT_DEF",
   --`INSERT (x:'a) s = {y | (y = x) \/ y IN s}`--,
   490);
...
val INSERT_UNION = store_thm("INSERT_UNION",
     --`!x:'a. !s t.
      (x INSERT s) UNION t = if x IN t then s UNION t
                             else x INSERT (s UNION t)`--,
...
val INSERT_INTER = store_thm("INSERT_INTER",
     --`!(x:'a). !s t.
      (x INSERT s) INTER t = (if x IN t then x INSERT (s INTER t)
                              else s INTER t)`--,
     REPEAT GEN_TAC THEN COND_CASES_TAC THEN
      ...
      [REPEAT GEN_TAC THEN
       ...
       [EXISTS_TAC (--`CARD (s:'a set)`--) THEN
        ...
        FILTER_ASM_REWRITE_TAC (fn t => (not (lhs t = (--`s:'a set`--)))
                                        handle _ => true) [],
        SUBST1_TAC(ASSUME (--`x:'a = if n < CARD (s:'a set) then f n else e`--)) THEN
