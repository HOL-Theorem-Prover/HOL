...
let fol_unify_lit offset (h1, l1) (h2, l2) sofar =
  if h1 <> h2 then failwith "" else
  List.fold_left (fun sf (t1, t2) -> fol_unify offset t1 t2 sf) sofar (zip l1 l2)
;;
...
let print_var i = try print_string (String.uppercase (fst (Fusion.dest_var (hol_of_var i)))) with _ -> print_char 'V'; print_int i;;

let rec print_foft = function
    Fvar i -> print_var i
  | Fnapp (i, l) -> (try print_string (String.lowercase (fst (Fusion.dest_const (hol_of_const i))))
...
let print_fof_clause l =
  print_char '{'; List.iter (fun i -> print_fofl i; print_char ' ') l; print_char '}';;

let print_fol_subst s =
  print_char '{'; List.iter (fun (t,v) -> print_var v; print_string ":="; print_foft t; print_char ' ') s; print_char '}'
;;
