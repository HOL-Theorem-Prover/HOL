		  ...
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                     ...
                     val guess2_opt = SOME (first (fn guess' => let val (i',fvL',_) = guess_extract guess' in
						 (i = i') andalso (fvL = fvL') end) gL42)  handle HOL_ERR _ => NONE
...
*)
   ...
   val resL = append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                     val guess2_opt = SOME (first (fn guess' => let val (i',fvL',_) = guess_extract guess' in
						 (i = i') andalso (fvL = fvL') end) gL52) handle HOL_ERR _ => NONE				   
                     val thm2_opt = if isSome guess2_opt then #3 (guess_extract (valOf guess2_opt)) else NONE
                  in
		     if (not (isSome guess2_opt)) then
                        guess___do_not_trust t1 guess
...
*)
   ...
   val resL = if (mem v (free_vars t2)) then resL else
              append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                  in
   ...
   (*if i the the only possibility for t2 and v does not occur in t1 then
     i the the only possibility for (t1 \/ t2)*)
   val resL = if (mem v (free_vars t1)) then resL else
              append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                  in
   ...
   val resL = append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                     val guess2_opt = SOME (first (fn guess' => let val (i',fvL',_) = guess_extract guess' in
						 (i = i') andalso (fvL = fvL') end) gL62) handle HOL_ERR _ => NONE				   
                     val thm2_opt = if isSome guess2_opt then #3 (guess_extract (valOf guess2_opt)) else NONE
                  in
   ...
   val resL = append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                     ...
                     val guess2_opt = SOME (first (fn guess' => let val (i',fvL',_) = guess_extract guess' in
						 (i = i') andalso (fvL = fvL') end) gL72)  handle HOL_ERR _ => NONE
                     val thm2_opt = if isSome guess2_opt then #3 (guess_extract (valOf guess2_opt)) else NONE
                  in
		     if (not (isSome guess2_opt)) then
                        guess___do_not_trust t1 guess
...
*)
   ...
   val resL = if mem x (free_vars tx) then resL else
              append (map (fn guess =>
		  let
		     val (i,fvL,thm_opt) = guess_extract guess
                  in
