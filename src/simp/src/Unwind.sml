...
fun check_var var conj =
  if is_eq conj then let
    val (arg1, arg2) = dest_eq conj in
      if (op_mem eq arg1 (free_vars arg2) orelse
          op_mem eq arg2 (free_vars arg1)) then
         failwith "check_var - Duplicate values" else
      if (eq arg1 var) then arg2 else
      if (eq arg2 var) then arg1
      else failwith "check_var - No value" end
  else if is_neg conj andalso eq (dest_neg conj) var then false_tm
  else if eq var conj then truth_tm
...
fun ENSURE_EQ_CONV var tm =
   if (is_eq tm)
   then if (eq (lhs tm) var) then REFL tm else SYM_CONV tm
   else if is_neg tm
