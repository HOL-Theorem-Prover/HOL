...
val resq_SS =
  ...
  {name=SOME"resq",
   ...
   [{conv = K (K RES_FORALL_CONV),
     key = SOME (empty_tmset, Term `RES_FORALL (p:'a -> bool) m`),
     name = "RES_FORALL_CONV", trace = 2},
    {conv = K (K RES_EXISTS_CONV),
     key = SOME (empty_tmset, Term `RES_EXISTS (p:'a -> bool) m`),
     name = "RES_EXISTS_CONV", trace = 2},
    {conv = K (K RES_SELECT_CONV),
     key = SOME (empty_tmset, Term `RES_SELECT (p:'a -> bool) m`),
     name = "RES_SELECT_CONV", trace = 2},
    {conv = K (K RES_EXISTS_UNIQUE_CONV),
     key = SOME (empty_tmset, Term `RES_EXISTS_UNIQUE (p:'a -> bool) m`),
