...
fun labcmp (p as (l1, l2)) =
    ...
    | (Lam _, _) => LESS
    | (_, Lam _) => GREATER
    | (TyLam n1, TyLam n2) => Int.compare (n1, n2)
    | (TyLam _, _) => LESS
    | (_, TyLam _) => GREATER
    | (TyCmb n1, TyCmb n2) => Int.compare (n1, n2)
    | (TyCmb _, _) => LESS
    | (_, TyCmb _) => GREATER
...
in
  ...
  | TYLAMB(bv, bod) => (TyLam (length args), (fvs, bod) :: args')
  | TYCOMB(tm, ty)  => (TyCmb (length args), (fvs, tm ) :: args')
