...
in
  Parse_support.make_preterm (absyn_to_preterm_in_env (oinfo,kcs) absyn)
end;
...
fun Preterm q =
 ...
 in
    Parse_support.make_preterm
            (absyn_to_preterm_in_env (oinfo,kcs) absyn)
 end
...
fun set_known_constants sl =
  case partition (not o null o Term.decls) sl
   of (ok_names, [])
       => the_term_grammar
             := fupdate_known_constants (K ok_names) (!the_term_grammar)
    | (_, bad_names)
...
fun parent_grammars () =
 ...
 in case parents ct
     of [] => raise ERROR "parent_grammars"
