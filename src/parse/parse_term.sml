...
The design structure and strategy of term parsing for HOL.
(Derived from an email by Michael Norrish, November 26, 2007).

The basic idea is that the term parser implements operator-precedence 
parsing.  This is a simplistic shift-reduce framework.  Between any 
given pair of tokens there is a <. =. or >. relation.

The algorithm is

  compare top of stack to next symbol;
  case result of
    >.     ->  perform reduction, looking backwards in stack until
               finding the first <. relation between a pair of symbols
               on the stack; this marks the left extent of the
               reduction.
    o/wise ->  shift the symbol onto the stack

  when doing a reduction, replace the right hand side of the rule with
  a "term" non-terminal.

The operator-precedence matrix is generated from the user grammar in
the function mk_prec_matrix.  The reduction rules are stored in
another hash-table.  This is generated by mk_ruledb.  The next big
functions implement the basic algorithm as above.

(After the declaration of val recupd_errstring, there is a little code
for massaging the Absyn result of parsing into a more correct form.  
This could be elsewhere.)

Complexities arise in the basic algorithm because of the way the type
parser has to be called at the drop-of-a-hat, because of the way
parsing vstructs (things that appear after binders, like x (x,y) and
(z::P)) is different (this is marked by the invstruct flag and the
vs_state value).   Most of the complexity is in the perform_reduction
function, which handles all sorts of special cases: things like the
vstructs, and also the special treatment of (:'a) etc.
...
and 'a preterm
  ...
  | TYCOMB of ('a preterm locn.located * Pretype.pretype locn.located)
  | VAR of string
  | QIDENT of string * string
  | ABS of ('a varstruct locn.located * 'a preterm locn.located)
  | TYABS of (Pretype.pretype locn.located * 'a preterm locn.located)
  | AQ of 'a
...
fun all_tokens strlist =
  map STD_HOL_TOK strlist @ [BOS, EOS, Id, TypeColon, TypeTok, TypeListTok, EndBinding, TypeBracket]
...
fun left_grabbing_elements rule =
    ...
    | SUFFIX TYPE_application => [TypeBracket]
    | _ => []
...
fun mk_prec_matrix G = let
  ...
  val {lambda, type_lambda, endbinding, type_intro,
       type_lbracket, type_rbracket, restr_binders, ...} = specials G
  ...
  val rule_elements:rule_record -> rule_element list = term_grammar.rule_elements o #elements
  ...
  val all_lhs =
    TypeColon::BOS::VS_cons::ResquanOpTok::TypeBracket::Id::
    map STD_HOL_TOK (fnapp_special::(map_rule first_tok Grules))
  val all_rhs =
    TypeTok::EndBinding::EOS::VS_cons::ResquanOpTok::Id::
    map STD_HOL_TOK (fnapp_special::type_rbracket::(map_rule last_tok Grules))
...
in
  ...
  app (fn l => insert ((STD_HOL_TOK l, true), EndBinding) EQUAL') type_lambda;
  ...
  (* these next equality pairs will never have terms interfering between
     ...
     the above *)
  app (fn b => insert ((STD_HOL_TOK b,false), TypeListTok) EQUAL') (type_lambda @ type_binders G);
  insert ((TypeListTok,false), EndBinding) EQUAL';
  insert ((TypeBracket,false), TypeListTok) EQUAL';
  insert ((TypeListTok,false), STD_HOL_TOK type_rbracket) EQUAL';
  insert ((TypeColon,false), TypeTok) EQUAL';
  app (fn b => insert ((STD_HOL_TOK b,true), EndBinding) EQUAL') (binders G); (* type & term *)
  insert_rhs_relns () ;
  insert_lhs_relns () ;
...
datatype 'a lookahead_item =
  Token of 'a term_token |
  PreType of Pretype.pretype |
  PreTypeList of Pretype.pretype list |
...
fun parse_term (G : grammar) typeparser type_var_parser = let
  val Grules = grammar_rules G
  val {type_intro, type_lbracket, type_rbracket, lambda, type_lambda,
       endbinding, restr_binders, res_quanop} = specials G
  ...
  val _ =
      isSome (findpos (fn (SUFFIX TYPE_application) => true | _ => false)
                      Grules)
  ...
  fun lifted_type_list_parser (qb, ps) =
     let open type_tokens
         fun recurse tys =
             let val (adv, (t,locn)) = typetok_of qb
             in case t of
                   Comma => (adv(); recurse (typeparser qb :: tys))
                 | _ => List.rev tys
             end
     in ((qb, ps), SOME (recurse [typeparser qb]))
     end
  fun lifted_type_var_list_parser (qb, ps) =
     let open type_tokens
         fun recurse tys =
             let val (adv, (t,locn)) = typetok_of qb
             in case t of
                   Period => List.rev tys
                 | _ => (recurse (type_var_parser qb :: tys))
             end
     in ((qb, ps), SOME (recurse [type_var_parser qb]))
     end
  ...
  fun transform (in_vs as (vs_state, nparens))
      ...
      in
        case x of
          Ident s =>
            ...
            else if s = type_lbracket then (TypeBracket, locn, SOME tt)
    ...
    | SOME (tt as PreTypeList tys,locn) => (TypeListTok, locn, SOME tt)
  ...
  in
    ...
    | (((Terminal (STD_HOL_TOK type_rbracket),rlocn), _)::
       ...
       ((NonTerminal t,llocn), _)::rest) => let
        fun list_mk_tycomb(tm,tys) =
           List.foldl (fn(ty,tm) => TYCOMB((tm,llocn),(ty,tlocn))) tm tys
      in
        repeatn 4 pop >>
        push ((NonTerminal (list_mk_tycomb(t,tys)),
    ...
    | (((Terminal TypeTok,rlocn), PreType ty)::((Terminal TypeColon,_), _)::
       ...
       in
         repeatn 3 pop >> push ((NonTermVS (map (fn (v as (_,locn)) => (TYPEDV(v,(ty,rlocn)),locn)) vsl),
                                 locn.between llocn rlocn),
                                XXX)
       end
    | [((Terminal TypeTok,rlocn), PreType ty), ((Terminal TypeColon,_), _)] =>
    ...
    | TypeBracket => let
       ...
       *)
        fun action_on_la la =
          case la of
            [x as (_,locn)] =>
              lifted_type_list_parser >-  (fn tys =>
...
fun remove_specials t =
  ...
  | TYCOMB (tm, (ty,_)) => Absyn.TYAPP(#2 t, remove_specials tm, ty)
  | ABS(v, t2) => Absyn.LAM(#2 t, to_vabsyn v, remove_specials t2)
  | TYABS((vty,_), t2) => Absyn.TYLAM(#2 t, vty, remove_specials t2)
