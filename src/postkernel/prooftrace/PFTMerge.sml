...
(* Describes the layout of arguments following the result id varint.
   Used in pass 1 to extract produced/consumed ID pairs without storing
   full arguments, and in pass 2 to skip excluded commands. *)
...
fun lookup_desc descs opc =
  case List.find (fn (o', _) => o' = opc) descs of
    SOME (_, d) => d
  | NONE => raise Fail ("PFTMerge: unknown opcode 0x" ^
