...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=2, n_tm=1, n_th=1}
  val got = readFile f
  val expected =
    ...
    \{\"cmd\":\"REFL\",\"id\":0,\"tm\":0}\n\
    \{\"cmd\":\"LIMITS\",\"n_ty\":2,\"n_tm\":1,\"n_th\":1}\n"
in
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=5, n_tm=0, n_th=0}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"hol4\"}\n\
    \{\"cmd\":\"TYVAR\",\"id\":0,\"name\":\"\"}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":5,\"n_tm\":0,\"n_th\":0}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got ^ "expected:\n" ^ expected);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=1, n_tm=2, n_th=7}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"hol4\"}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":1,\"n_tm\":2,\"n_th\":7}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=2, n_tm=2, n_th=4}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"hol4\"}\n\
    \{\"cmd\":\"TYOP\",\"id\":0,\"name\":\"bool\",\"args\":[]}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":2,\"n_tm\":2,\"n_th\":4}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=1, n_tm=1, n_th=2}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"hol4\"}\n\
    \{\"cmd\":\"TYOP\",\"id\":0,\"name\":\"bool\",\"args\":[]}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":1,\"n_tm\":1,\"n_th\":2}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=1, n_tm=1, n_th=5}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"hol4\"}\n\
    \{\"cmd\":\"TYOP\",\"id\":0,\"name\":\"bool\",\"args\":[]}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":1,\"n_tm\":1,\"n_th\":5}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=2, n_tm=2, n_th=10}
  val got = readFile f
  val expected =
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":2,\"n_tm\":2,\"n_th\":10}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=2, n_tm=1, n_th=3}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"candle\"}\n\
    ...
    \{\"cmd\":\"LIMITS\",\"n_ty\":2,\"n_tm\":1,\"n_th\":3}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
  cleanup f
...
val () = let
  ...
  val () = PFTWriter.Candle.compute_init out [0, 1, 2]
  val () = PFTWriter.Candle.compute out 4 0 [0]
  val () = PFTWriter.closeOut out {n_ty=1, n_tm=1, n_th=5}
  val got = readFile f
  val expected =
    "{\"cmd\":\"PFT\",\"version\":\"0.1.0\",\"ruleset\":\"candle\"}\n\
    ...
    \{\"cmd\":\"COMPUTE_INIT\",\"ths\":[0,1,2]}\n\
    \{\"cmd\":\"COMPUTE\",\"id\":4,\"tm\":0,\"ths\":[0]}\n\
    \{\"cmd\":\"LIMITS\",\"n_ty\":1,\"n_tm\":1,\"n_th\":5}\n"
in
  if got = expected then OK()
  else die ("got:\n" ^ got);
...
val () = let
  ...
  val () = PFTWriter.closeOut out {n_ty=1, n_tm=1, n_th=4}
...
val () = let
  ...
  val expected = String.implode (map Char.chr [
    ...
    (* footer: 0xFF n_ty=2 n_tm=1 n_th=1, then uint16le length=4 *)
    0xFF, 0x02, 0x01, 0x01,
    0x04, 0x00
  ])
in
