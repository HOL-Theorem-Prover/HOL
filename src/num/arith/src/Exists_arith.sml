...
fun NUM_REDUCE_CONV tm =
   ...
   else if (is_plus tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC ADD_CONV) tm
   else if (is_mult tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC MUL_CONV) tm
   else ALL_CONV tm;
...
fun INEQ_REDUCE_CONV tm =
   if (is_eq tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC NEQ_CONV) tm
   else if (is_leq tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC LE_CONV) tm
   else if (is_less tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC LT_CONV) tm
   else if (is_geq tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC GE_CONV) tm
   else if (is_greater tm) then ((BINOP_CONV NUM_REDUCE_CONV) THENC GT_CONV) tm
   else ALL_CONV tm;
...
fun BOOL_REDUCE_CONV tm =
   ...
   else if (is_conj tm) then ((BINOP_CONV BOOL_REDUCE_CONV) THENC AND_CONV) tm
   else if (is_disj tm) then ((BINOP_CONV BOOL_REDUCE_CONV) THENC OR_CONV) tm
   else if (is_imp tm) then ((BINOP_CONV BOOL_REDUCE_CONV) THENC IMP_CONV) tm
   else if (is_eq tm) then ((BINOP_CONV BOOL_REDUCE_CONV) THENC BEQ_CONV) tm
...
fun WITNESS binding tm =
 ...
 ) handle _ => EQT_ELIM (QCONV BOOL_REDUCE_CONV tm);
...
fun EXISTS_ARITH_CONV tm =
 (reset_multiplication_theorems ();
  let val th = QCONV ARITH_FORM_NORM_CONV
