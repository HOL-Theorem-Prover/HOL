...
fun AND_CONV tm =
 ...
 in if eq xn T then SPEC yn c1 else
    if eq yn T then SPEC xn c2 else
    if eq xn F then SPEC yn c3 else
    if eq yn F then SPEC xn c4 else
    if eq xn yn then SPEC xn c5 else
    if aconv xn yn
...
fun COND_CONV tm =
 ...
 in if eq b T then SPECL [t1,t2] (INST_TYPE[alpha |-> type_of t1] c1) else
    if eq b F then SPECL [t1,t2] (INST_TYPE[alpha |-> type_of t1] c2) else
    if eq t1 t2 then SPECL [b,t1]  (INST_TYPE[alpha |-> type_of t1] c3) else
    if aconv t1 t2
