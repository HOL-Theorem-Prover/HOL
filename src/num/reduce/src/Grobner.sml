...
fun mlcm ((c1,m1) :pterm) ((c2,m2) :pterm) :pterm =
   (Arbrat.one,map2 (curry Arbnum.max) m1 m2)
...
fun grobner (pols :poly list) :basis = let
  ...
  val prs3 = filter (fn (l,(p,q)) => not(orthogonal l (fst p) (fst q))) prs2
in
...
fun resolve_proof (vars :term list) (prf :history) :cert =
  ...
  | Mult(pol,lin) => let
    val lis = resolve_proof vars lin
    in map (fn (n,p) => (n,grob_cmul pol p)) lis end
  | Add(lin1,lin2) => let
    val lis1 = resolve_proof vars lin1
    and lis2 = resolve_proof vars lin2
    val dom = setify_int (union (map fst lis1) (map fst lis2))
    in
      map (fn n => let
        val a = assoc n lis1 handle HOL_ERR _ => []
        and b = assoc n lis2 handle HOL_ERR _ => []
        in (n,grob_add a b) end) dom
    end
...
fun grobner_weak (vars :term list) (pols :poly list) :rat * cert = let
  val cert = resolve_proof vars (grobner_refute pols)
  val l = Arbrat.fromNat
    (itlist (itlist
      (curry Arbnum.lcm o Arbrat.denominator o fst) o snd) cert num_1)
  in (l,map (fn (i,p) => (i,map (fn (d,m) => (Arbrat.* (l,d),m)) p)) cert) end
...
fun grobner_ideal (vars :term list) (pols :poly list) (pol :poly) :cert =
  let val (pol',h) = reduce (grobner pols) (grob_neg pol,Start NONE) in
...
fun grobner_strong (vars :term list)
  ...
  if null(pol) then
    ...
    let
      ...
      val d = itlist (itlist (curry Arbnum.max o hd o snd) o snd) cert num_0
...
fun RING_AND_IDEAL_CONV (
  ...
  fun grobvars (tm :term) (acc :term list) :term list =
    ...
    else if can ring_dest_pow tm andalso is_numeral (rand tm) then
      grobvars (lhand tm) acc
    else if can ring_dest_add tm orelse
            can ring_dest_sub tm orelse can ring_dest_mul tm then
      grobvars (lhand tm) (grobvars (rand tm) acc)
    else if can ring_dest_inv tm then let
      val gvs = grobvars (rand tm) []
      in if null(gvs) then acc else tm::acc end
    else if can ring_dest_div tm then let
      val lvs = grobvars (lhand tm) acc
      and gvs = grobvars (rand tm) []
      in if null(gvs) then lvs else tm::acc end
    else tm::acc
  ...
  fun grobify_term (vars :term list) (tm :term) : poly =
    ...
    handle HOL_ERR _ => let
      val x = ring_dest_const tm
      in if x = Arbrat.zero then [] else [(x,map (fn v => num_0) vars)] end
    handle HOL_ERR _ =>
      grob_neg(grobify_term vars (ring_dest_neg tm))
    handle HOL_ERR _ =>
      grob_inv(grobify_term vars (ring_dest_inv tm))
    handle HOL_ERR _ => let
      val (l,r) = ring_dest_add tm
      in grob_add (grobify_term vars l) (grobify_term vars r) end
    handle HOL_ERR _ => let
      val (l,r) = ring_dest_sub tm
      in grob_sub (grobify_term vars l) (grobify_term vars r) end
    handle HOL_ERR _ => let
      val (l,r) = ring_dest_mul tm
      in grob_mul (grobify_term vars l) (grobify_term vars r) end
    handle HOL_ERR _ => let
      val (l,r) = ring_dest_div tm
      in grob_div (grobify_term vars l) (grobify_term vars r) end
    handle HOL_ERR _ => let
      val (l,r) = ring_dest_pow tm
      in grob_pow vars (grobify_term vars l) (dest_numeral r) end
    handle HOL_ERR _ =>
      failwith "grobify_term: unknown or invalid term"

  fun grobify_equation vars tm = let
    val (l,r) = dest_eq tm
    in grob_sub (grobify_term vars l) (grobify_term vars r) end
  ...
  local
    ...
    fun holify_monomial vars (c,m) = let
      val xps = map holify_varpow
        (filter (fn (_,n) => n <> num_0) (zip vars m))
      in end_itlist (curry ring_mk_mul) (ring_mk_const c :: xps) end
  in
    fun holify_polynomial (vars:term list) (p:(rat * num list) list): term =
      if p = [] then ring_mk_const Arbrat.zero
      else end_itlist (curry ring_mk_add) (map (holify_monomial vars) p)
  end

  val (pth_idom,pth_ine) = CONJ_PAIR(MATCH_MP RING_pth_step RING_INTEGRAL)
  ...
  fun MUL_RULE (vars :term list) (m :rat * num list) th =
    CONV_RULE (BINOP_CONV RING_NORMALIZE_CONV) $
    AP_TERM (mk_comb(ring_mul_tm,holify_polynomial vars [m])) th

  fun execute_proof vars eths prf = let
    val initpols =
      map (CONV_RULE(BINOP_CONV RING_NORMALIZE_CONV) o SUB_EQ_RULE) eths
    val th = if is_eq $ snd $ strip_forall $ concl RABINOWITSCH_THM then let
      val _ = print_verbose "Generating HOL version of proof\n"
      val execache = ref []
      fun memoize prf x = (execache := (prf,x)::(!execache); x)
      fun run_proof vars prf = assoceq prf (!execache) handle HOL_ERR _ =>
        (case prf of
      ...
      val ans = run_proof vars prf
      in (execache := []; ans) end
    else let
      val _ = print_verbose "Generating HOL version of scaled proof\n"
      ...
      fun run_scaled_proof vars prf = assoceq prf (!execache) handle HOL_ERR _ =>
        ...
        | Add(p1,p2) => let
          val (d1,th1) = run_scaled_proof vars p1
          and (d2,th2) = run_scaled_proof vars p2
          val d = lcm_num d1 d2
          val r1 = Arbrat./ (Arbrat.fromNat d, Arbrat.fromNat d1)
          val r2 = Arbrat./ (Arbrat.fromNat d, Arbrat.fromNat d2)
          in
            memoize prf (d, ADD_RULE
              (MUL_RULE vars (r1,km) th1) (MUL_RULE vars (r2,km) th2))
          end
        | Mult((c,xs),p2) => let
          val e = Arbrat.denominator c
          val (d,th) = run_scaled_proof vars p2
          val r = Arbrat.* (c, Arbrat.fromNat e)
          in memoize prf (Arbnum.* (d, e), MUL_RULE vars (r,xs) th) end)
      val (_,ans) = run_scaled_proof vars prf
      in (execache := []; ans) end
    in CONV_RULE RING_EQ_CONV th end (* of execute_proof *)

  fun REFUTE tm = if tm ~~ false_tm then ASSUME tm else let
    val (nths0,eths0) = partition (is_neg o concl) (CONJUNCTS(ASSUME tm))
    val nths = filter (is_eq o rand o concl) nths0
    and eths = filter (is_eq o concl) eths0
    in
      if null(eths) then let
        val th1 = end_itlist (fn th1 => fn th2 => IDOM_RULE(CONJ th1 th2)) nths
        val th2 = CONV_RULE(RAND_CONV(BINOP_CONV RING_NORMALIZE_CONV)) th1
        val (l,r) = dest_eq(rand(concl th2))
        in EQ_MP (EQF_INTRO th2) (REFL l) end
      else if null(nths) andalso not(is_var ring_neg_tm) then let
        val (vars,pols) = grobify_equations $ list_mk_conj $ map concl eths
        in execute_proof vars eths (grobner_refute pols) end
      else let
        val (vars,l,cert,noteqth) = if null(nths) then let
          val (vars,pols) = grobify_equations(list_mk_conj(map concl eths))
        ...
        val cert_pos = map (fn (i,p) =>
          (i, filter (fn (c,m) => Arbrat.> (c,Arbrat.zero)) p)) cert
        and cert_neg = map (fn (i,p) =>
          (i, map (fn (c,m) => (Arbrat.~ c,m)) $
        ...
        fun thm_fn pols =
          ...
          end_itlist MK_ADD (map (fn (i,p) =>
            AP_TERM(mk_comb(ring_mul_tm,p)) (List.nth (eths,i))) pols)
  ...
  in
    if is_forall bod then failwith "RING: non-universal formula" else let
      val th1a = DNF_CONV bod (* NOTE: it was WEAK_DNF_CONV here *)
      ...
      val (finbod,th3) = let
        ...
        in CHOOSES evs (bod,th2) end
    in
      SPECL avs (MATCH_MP (MATCH_MP RING_FINAL_pth (DISCH finbod th3)) th1)
    end
  end (* of RING *)
...
val RING = fst o RING_AND_IDEAL_CONV
val ideal_cofactors = snd o RING_AND_IDEAL_CONV
...
local
  ...
  fun NUM_MULTIPLY_CONV pos tm =
    if is_forall tm orelse is_exists tm orelse is_uexists tm then
      ...
      val t = find_term (fn t => is_pre t andalso free_in t tm) tm
      val ty = type_of t
      val v = genvar ty
      ...
      in CONV_RULE (RAND_CONV (NUM_MULTIPLY_CONV pos)) th2 end
    handle HOL_ERR _ => let
      val t = find_term (fn t => is_sub t andalso free_in t tm) tm
      val ty = type_of t
      val v = genvar ty
      val p = mk_abs(v,subst [t |-> v] tm)
      ...
      in CONV_RULE(RAND_CONV (NUM_MULTIPLY_CONV pos)) th2 end
    handle HOL_ERR _ => let
      val t = find_term (fn t => is_divmod t andalso free_in t tm) tm
...
in
  val NUM_SIMPLIFY_CONV =
    NUM_REDUCE_CONV THENC
    CONDS_CELIM_CONV THENC
    NNF_CONV THENC
    NUM_MULTIPLY_CONV true THENC
    NUM_REDUCE_CONV THENC
    GEN_REWRITE_CONV ONCE_DEPTH_CONV empty_rewrites
      [NUM_SIMPLIFY_CONV_pth_evenodd]
end
