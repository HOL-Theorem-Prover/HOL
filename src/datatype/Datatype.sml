...
fun bool_list () = mk_type ("list", [bool]);
  
fun bool_nil () = mk_thy_const {Thy = "list", Name = "NIL", Ty = bool_list ()};
...
local
  fun booll _ _ Nil 0 = [Nil]
    ...
    in map ConsT l @ map ConsF l
    end;
in
  fun bool_lists n =
    List.take
    (booll (curry mk_bool_cons T) (curry mk_bool_cons F) (bool_nil ()) (n - 1),
     n);
end;
  ...
  fun insert_boolify {def, const_tyopl} tyinfol =
    (case tyinfol of [] => raise ERR "build_tyinfos" "empty tyinfo list"
     | tyinfo::rst =>
       let
         val first_tyname = TypeBasePure.ty_name_of tyinfo
         fun insert_boolify info boolify_eqs =
           let
             val tyname = TypeBasePure.ty_name_of info
           in
             case assoc2 tyname const_tyopl of SOME(c,tyop)
       ...
       in
         insert_boolify tyinfo (TypeBasePure.ORIG def) ::
         map (C insert_boolify (TypeBasePure.COPY (first_tyname,def))) rst
       end
       handle HOL_ERR _ => tyinfol);
in
  fun build_tyinfos db {induction,recursion} =
    ...
    in
      ...
      | SOME s =>
          let
            val tyinfol = insert_size s tyinfol
          in
