...
fun to_tyspecs ASTs =
 let val _ = List.app check_fields ASTs
     ...
     fun mk_hol_ty (dAQ ty) = ty
       ...
       | mk_hol_ty (dTyRankConstr{Ty,...}) = mk_hol_ty Ty
       | mk_hol_ty (dTyApp(opr as dContype{Tyop = s, ...},arg)) =
            if Lib.mem s new_type_names
            then raise ERR "to_tyspecs"
                     ("Omit arguments to new type:"^Lib.quote s)
            else mk_app_type(mk_hol_ty opr, mk_hol_ty arg)
       | mk_hol_ty (dTyApp(opr,arg)) =
            mk_app_type(mk_hol_ty opr, mk_hol_ty arg)
       | mk_hol_ty (dContype{Thy,Tyop = s,Kind}) =
