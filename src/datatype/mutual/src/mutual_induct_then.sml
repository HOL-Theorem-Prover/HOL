...
structure Mutual_induct_then : Mutual_induct_then_sig =
struct
(* open CoreHol; *)
open Lib;
open Parse;
...
*)
       ...
       val ind = GENL vs (Rsyntax.SUBST [{var = bool, thm = GALPHA asm}]
...
*)
       val find = Lib.first;
       fun findt ts v =
           find (fn x => type_of (#Rand(dest_comb x)) = type_of v) ts
           handle _ => let val ty = type_of v
                           val ty' = hd(#Args(dest_type ty))
                           val {Tyop=Tyop',Args=Args'} = dest_type ty'
                           val nm = implode [hd (explode Tyop')]
                           val v' = mk_var{Name = nm, Ty = ty'}
                       in
                       mk_forall{Bvar = v', Body = (--`T`--)}
                       end
   ...
   fn ttac => fn (A,t) => (* t is the current goal *)
      let val ts = strip_conj t
          val lams = map (#Rand o dest_comb) ts
...
*)
          ...
          val spec = SPECL lams' (itlist2 (fn v => fn lam =>
                     INST_TYPE (Lib.snd(Match.match_term v lam))
                     handle _ => Lib.I
                     ) vs lams' ind)
(*
...
(*
          ...
          val _ = print_string "\n"
*)
          val bvars = map (#Bvar o dest_abs) lams'
...
end; (* Mutual_induct_then *)

(*
use "mutual_induct_then.sml";

open Mutual_induct_then;

e(MUTUAL_INDUCT_THEN avexp_induct ASSUME_TAC);
*)
