Theory bnfFixBNF[bare]
Ancestors
  bnfInitial bnfPrelims pred_set cardinalityCore
Libs
...
(* ----------------------------------------------------------------------
    ...
    parameters constrained by the equations they satisfy: bnfFIXMAP for the
    map, bnfFIXSET for a set function.  Second, F's map is *bundled*: a
    ...
    below needs them related — which is exactly how bnfMapComp already
...
Definition FIXMAP_def:
  bnfFIXMAP (cn : 'fn -> 'n) (cm : 'fm -> 'm) mp M ⇔
...
val fix_induct =
    qpat_assum ‘bnfFIXIND _ _’ (ho_match_mp_tac o MATCH_MP FIXIND_thm)

(* bnfMapCong comes first in every hypothesis list below because it fixes
   both the map and the set function, so drule can resolve it and know
   what the rest means *)
Theorem FIXMAP_ID:
  bnfMapCong mp stn ∧ bnfMapId mp ∧ bnfFIXIND cn stn ∧ bnfFIXMAP cn cn mp M ⇒
  ∀n. M n = n
Proof
...
Theorem FIXMAP_O:
  bnfMapCong mp_np stn ∧ bnfMapComp mp_nm mp_mp mp_np ∧ bnfFIXIND cn stn ∧
  bnfFIXMAP cn cm mp_nm M1 ∧ bnfFIXMAP cm cp mp_mp M2 ∧ bnfFIXMAP cn cp mp_np M12 ⇒
  ∀n. M2 (M1 n) = M12 n
Proof
...
Theorem FIXSET_NATURAL:
  bnfNatural mp stn stm ∧ bnfNaturalP mp sbn sbm p ∧ bnfFIXIND cn stn ∧
  bnfFIXMAP cn cm mp M ∧ bnfFIXSET cn stn sbn Sn ∧ bnfFIXSET cm stm sbm Sm ⇒
...
Theorem FIXMAP_CONG:
  bnfMapCongP mp1 mp2 stn sbn p q ∧ bnfFIXIND cn stn ∧ bnfFIXSET cn stn sbn Sn ∧
  bnfFIXMAP cn cm mp1 M1 ∧ bnfFIXMAP cn cm mp2 M2 ⇒
...
Theorem FIXSET_CARDLEQ:
  bnfFIXSET cn stn sbn Sn ∧ INFINITE B ∧ (∀af. sbn af ≼ B) ∧
  (∀af. stn af ≼ B) ∧ bnfFIXIND cn stn ⇒
  ∀n. Sn n ≼ B
Proof
  strip_tac >> fix_induct >> rpt strip_tac >>
...
Theorem FIXSET_EMPTY:
  bnfFIXSET cn stn sbn Sn ∧ stn w = ∅ ⇒ Sn (cn w) = sbn w
Proof
  simp[FIXSET_def]
QED

Theorem FIXSET_IN:
  bnfFIXSET cn stn sbn Sn ⇒ ∀af b. b ∈ sbn af ⇒ b ∈ Sn (cn af)
Proof
  simp[FIXSET_def]
QED
