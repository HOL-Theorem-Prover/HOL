...
void bdd_fprintall(FILE *ofile)
{
   ...
   for (n=0 ; n<bddnodesize ; n++)
   {
      if (LOW(n) != -1)
      {
	 fprintf(ofile, "[%5d] %3d/%3d :",
		 n, bddlevel2var[LEVEL(n)], LEVEL(n));
...
void bdd_fprinttable(FILE *ofile, BDD r)
{
   ...
   for (n=0 ; n<bddnodesize ; n++)
   {
	 ...
	 fprintf(ofile, "[%5d] %3d/%3d :", n, bddlevel2var[node->level],
		 node->level);
...
DESCR   {* Prints all the truth assignments for {\tt r} that would yield
	   ...
	   BDD. All variables not shown are don't cares.
ALSO    {* bdd\_printall, bdd\_printtable, bdd\_printdot *}
*/
   ...
   if (r == 1)
   {
	 ...
	 if (set[n] > 0)
	 {
	    if (!first)
	       fprintf(ofile, ", ");
	    first = 0;
	    fprintf(ofile, "%d/%d:%d", bddlevel2var[n],n, (set[n]==2 ? 1 : 0));
	 }
      }
...
static void bdd_fprintdot_rec(FILE* ofile, BDD r)
{
   ...
   fprintf(ofile, "%d [label=\"%d/%d\"];\n",
	   r, bddlevel2var[LEVEL(r)], LEVEL(r));
