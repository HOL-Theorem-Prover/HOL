...
static void checkreorder(int node)
{
   INCREF(node);
...
static int simplify_rec(int f, int d)
{
   BddCacheData *entry;
   int res;
...
BDD bdd_appex(BDD l, BDD r, int opr, BDD var)
{
   ...
   quantid = (appexid << 2) | CACHEID_APPEX;  /* FIXME: is _APPEX needed? */
...
/* FIXME: ... */
/* This function is still experimental and is therefore NOT mentioned in the
   documentation.  Jorn nov-25-1998 */

static int simplex_rec(int l, int r);

BDD bdd_simplex(BDD l, BDD r, BDD var)
{
   BDD res;
   ...
   bddrefstacktop = bddrefstack;
   applyop = bddop_or;
   appexid = (var << 5) | (bddop_simplify << 1); /* FIXME: range! */
...
static int simplex_rec(int l, int r)
{
   ...
   if (LEVEL(l) > quantlast  &&  LEVEL(r) > quantlast)
   {
      res = simplify_rec(l,r);
   }
   else
   {
      entry = BddCache_lookup(&appexcache, PAIR(l,r));
      if (entry->a == l  &&  entry->b == r  &&  entry->c == appexid)
	 return entry->r.res;
	 ...
	 if (LOW(r) == 0)
	 {
	    res = simplex_rec(HIGH(l), HIGH(r));
	 }
	 else
	 if (HIGH(r) == 0)
	 {
	    res = simplex_rec(LOW(l), LOW(r));
	 }
	 else
#endif
	 {
	    PUSHREF( simplex_rec(LOW(l), LOW(r)) );
	    PUSHREF( simplex_rec(HIGH(l), HIGH(r)) );
	    if (INVARSET(LEVEL(l)))
	       res = apply_rec(READREF(2), READREF(1));
	    else
	       res = bdd_makenode(LEVEL(l), READREF(2), READREF(1));
      ...
      if (LEVEL(l) < LEVEL(r))
      {
	 PUSHREF( simplex_rec(LOW(l), r) );
	 PUSHREF( simplex_rec(HIGH(l), r) );
	 if (INVARSET(LEVEL(l)))
	    res = apply_rec(READREF(2), READREF(1));
	 else
	    res = bdd_makenode(LEVEL(l), READREF(2), READREF(1));
	 POPREF(2);
      ...
      {
	 PUSHREF( simplex_rec(l, LOW(r)) );
	 PUSHREF( simplex_rec(l, HIGH(r)) );
	 if (INVARSET(LEVEL(r)))
	    res = apply_rec(READREF(2), READREF(1));
	 else
	    res = bdd_makenode(LEVEL(r), READREF(2), READREF(1));
