...
(* constants and a theorem fixing the equivalence relation.  That        *)
(* theorem must be a hypothesis-free `QUOTIENT R abs rep`, which is what *)
(* [quotient.define_quotient_type] returns - not the extensionality      *)
(* theorem handed to it; anything else is rejected with "equivalence     *)
(* theorem has an unsupported shape".  Refute always uses the sound      *)
(* partial encoding, so the legacy [partial] field is ignored.           *)
(* --------------------------------------------------------------------- *)
