...
(*                                                                       *)
(* Native narrowing wins this race, so the tree it reports is partial:   *)
(* the [_] marks a subtree the search never had to look at.              *)
(* --------------------------------------------------------------------- *)

refute_def ``SORTED (<) (flatten t)``;

(* ==> Refute found a counterexample                                     *)
(*     (backend: narrowing, substrate: native, size 2):                  *)
(*       t = Branch Leaf 0 (Branch Leaf 0 _)                             *)
...
(* Sets are functions, and no QC substrate can evaluate the set          *)
(* operations, so every candidate the QC backends produce comes back     *)
(* "evaluation stuck during testing".  By default each such candidate is *)
(* reported before the search resumes, which floods the screen;          *)
(* [upd_genuine_only true] tells Refute to retry silently instead.  The  *)
(* model finder is what actually settles this goal — it is the backend   *)
(* 09_model_finder.sml is about, and the one call in this file that      *)
(* needs the Kodkodi component (without it the answer is Unknown).       *)

refute (upd_genuine_only true (!the_config)) ``(s : num set) UNION t = s``;

(* ==> Refute found a counterexample (backend: kodkod, substrate:        *)
(*     kodkod).  The scope it settles on varies between runs; the        *)
(*     witness is an empty [s] against a nonempty [t] either way, e.g.   *)
(*       Scope: card num = 3                                             *)
(*       s = {}                                                          *)
(*       t = {2}                                                         *)
(*     Evaluated terms:                                                  *)
...
(* Narrowing does find [xs = [0]], but certifying an existential goal    *)
(* replays the recorded case tree, and that replay currently fails here, *)
(* so the candidate is demoted to Potential and the run ends with the    *)
(* "within the tested finite bounds" verdict rather than a               *)
(* counterexample.                                                       *)
...
(*       existential narrowing case-tree replay failed:                  *)
(*         Fail "quantified domain metadata is incomplete"               *)
...
(* Opting out of certification accepts narrowing's own verdict: the same *)
(* [xs = [0]] comes back Genuine, but flagged uncertified.  That         *)
(* trade-off is the subject of 02_verdicts_and_config.sml.               *)
