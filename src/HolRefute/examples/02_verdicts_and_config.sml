...
(*                                                                       *)
(*   Genuine       - a real counterexample (with cert, unless you opted  *)
(*                   out above).                                         *)
(*   QuasiGenuine  - a counterexample under stated caveats.              *)
(*   Potential     - a candidate Refute could not confirm; the report    *)
(*                   lists why and the search continues.  Model-finder   *)
(*                   reconstructions that carry no HOL certificate stay  *)
(*                   here; section 4 below produces one, and             *)
(*                   09_model_finder.sml is the full story.              *)
(*                                                                       *)
...
(* finite scopes and reconstructs a model there.  That model is an       *)
(* untrusted Kodkodi artefact rather than a HOL theorem, so the verdict  *)
(* is Potential and the race keeps looking for a genuine one.  (This     *)
(* needs a Kodkodi component; without one the call reports               *)
(* Unknown ["no configured backend"] — see 09_model_finder.sml.)         *)

refute_def ``(f : ind -> ind) x = x``;
(* ==> Potential ["untrusted Kodkodi model; no HOL certificate"],        *)
(*     scope: card ind = 2                                               *)

(* [upd_genuine_only true] from section 3 discards that candidate.       *)
(* Nothing on this goal reaches Genuine, so the call ends in Unknown —   *)
(* it never falls back to the weaker verdict it just rejected.           *)

refute (upd_genuine_only true (!the_config)) ``(f : ind -> ind) x = x``;
(* ==> Unknown, last reason: "kodkod: every model found was discarded    *)
(*     after checking 1 of 10 scopes"                                    *)
