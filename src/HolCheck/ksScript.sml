...
(* well formed ks. *)
val wfKS_def = save_thm("wfKS_def",Define `wfKS (ks:('state,'prop) KS) = 
                             (ks.S0 SUBSET ks.S)
 			 /\  (ks.S = (UNIV:('state -> bool)))`);
...
val KS_TRANSITION_def = save_thm("KS_TRANSITION_def",Define 
    `KS_TRANSITION (p:'state) (ks:('state,'prop) KS) a (q:'state) = (ks.T a)(p,q)`);
...
val wfKS_UNIV = save_thm("wfKS_UNIV",prove(``!ks. (ks.S0 SUBSET UNIV) /\ (ks.S = UNIV) ==> wfKS ks``,PROVE_TAC [wfKS_def]));
