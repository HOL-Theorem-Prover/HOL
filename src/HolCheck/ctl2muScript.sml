...
(*app load ["bossLib","stringTheory","stringLib","HolBddLib","pairTheory","pred_setLib","listLib","boolSimps","pairSimps","combinSimps",
          ...
          "muTheory","intLib","res_quanLib","unwindLib","ctlTheory","ctl2muTools"];
*)
...
val ELEM_REST = prove(``!(p:'state path) (n:num). ELEM (REST p) n = ELEM p (n+1)``,
...
THEN REPEAT STRIP_TAC THENL [
 ...
 THEN Induct_on `p` THENL [
  ...
  SIMP_TAC std_ss [LENGTH_def,xnum_to_def,IS_INFINITE_def]
...
THEN FULL_SIMP_TAC resq_ss [IN_DEF,xnum_to_def,LENGTH_def]
THEN SIMP_TAC arith_ss []
THEN REPEAT STRIP_TAC THENL [
...
THEN FULL_SIMP_TAC arith_ss [LENGTH_def,xnum_to_def]
...
THEN CONJ_TAC THENL [
 ...
 THEN Induct_on `n` THENL [
  SIMP_TAC arith_ss [Nf2,ELEM_def,RESTN_def,REST_def,HEAD_def,listTheory.HD,listTheory.TL,
		     LENGTH_def,GT_xnum_num_def,listTheory.LENGTH]
 ...
 SIMP_TAC resq_ss [IN_DEF,LENGTH_def]
 ...
 THEN SIMP_TAC std_ss [IS_INFINITE_def,IN_DEF,xnum_to_def,LENGTH_def]
 THEN SIMP_TAC arith_ss []
...
recInduct (theorem "CTL2MU_ind") THEN REPEAT CONJ_TAC THENL [
 ...
 SIMP_TAC std_ss ([ctl2mu,IMF_def,NNF_def,MU_SUB_def,RVNEG_def,IMF_CTL_LEM8]@(tsimps "mu")), (* C_EG *)
 SIMP_TAC std_ss ([ctl2mu,IMF_def,NNF_def,MU_SUB_def,RVNEG_def,IMF_CTL_LEM8]@(tsimps "mu"))  (* C_EU *)
]);
...
FULL_SIMP_TAC std_ss [STATES_def,ctl2mu], (* /\ *)
FULL_SIMP_TAC std_ss [STATES_def,ctl2mu], (* EX *)
FULL_SIMP_TAC std_ss [STATES_def,ctl2mu]
THEN REPEAT STRIP_TAC
...
THEN Induct_on `n` THENL [
 FULL_SIMP_TAC std_ss [CTL_GFP,STATES_def,ENV_EVAL,wfKS_def,ctl2muks_def], (* 0 *)
 REWRITE_TAC [CTL_GFP]
...
THEN POP_ASSUM (fn t => ASSUME_TAC (REWRITE_RULE [ctl2mu] (SPEC ``C_EG (f:'prop ctl)`` t)))
...
val P1 = Define `P1 P R s = R(ARB,s) /\ P s`;
...
val INF_PFX = Define `
(INF_PFX f 0 = []) /\
(INF_PFX f (SUC n) = (f n)::(INF_PFX f n))`;

val PREFIX = Define `
(PREFIX (FINITE l) n = FIN_PFX l n) /\
(PREFIX (INFINITE f) n = INF_PFX f n)`;

val INF_PREFIX_LENGTH = prove(``!f k. LENGTH (PREFIX (INFINITE f) k) = k``,
REWRITE_TAC [PREFIX]
THEN Induct_on `k` THENL [
REWRITE_TAC [INF_PFX,LENGTH],
REWRITE_TAC [INF_PFX]
THEN FULL_SIMP_TAC arith_ss [LENGTH]]);
...
THEN FULL_SIMP_TAC std_ss [B_SEM_def,bexp2mu,STATES_def,ctl2muks_def,wfKS_def]
THEN FULL_SIMP_TAC std_ss [ctl2muks_def,KS_accfupds,KS_accessors,KS_fn_updates]
THEN FULL_SIMP_TAC std_ss [SET_SPEC]
THEN PROVE_TAC [IN_UNIV],
FULL_SIMP_TAC std_ss [ctl2muks_def,KS_accfupds,KS_accessors,KS_fn_updates,B_SEM_def,bexp2mu,STATES_def,wfKS_def,IN_UNIV,DIFF_DEF,SET_SPEC],
FULL_SIMP_TAC std_ss [ctl2muks_def,KS_accfupds,KS_accessors,KS_fn_updates,B_SEM_def,bexp2mu,STATES_def,wfKS_def,IN_UNIV,INTER_DEF,SET_SPEC]], (* C_BOOL *)
FULL_SIMP_TAC std_ss [ctl2muks_def,KS_accfupds,KS_accessors,KS_fn_updates,B_SEM_def,bexp2mu,STATES_def,wfKS_def,IN_UNIV,DIFF_DEF,
		      SET_SPEC,MU_SAT_def,STATES_def,ctl2mu,C_SEM_def,B_SEM_def], (* C_NOT *)
...
THEN EQ_TAC THENL [
 ...
 THEN Induct_on `n` THENL [
  ...
  PAT_ASSUM ``!(f:'prop ctl). IMF (CTL2MU f)`` (fn t => ASSUME_TAC (REWRITE_RULE [ctl2mu] (SPEC ``C_EG (f:'prop ctl)`` t)))
...
THEN EQ_TAC THENL [
 ...
 THEN Induct_on `n` THENL [
  ...
  PAT_ASSUM ``!(f:'prop ctl). IMF (CTL2MU f)`` (fn t => ASSUME_TAC (REWRITE_RULE [ctl2mu] (SPEC ``C_EU(f:'prop ctl,g:'prop ctl)`` t)))
