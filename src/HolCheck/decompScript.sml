...
val AP_EXT = save_thm("AP_EXT",prove(list_mk_forall ([``f:'prop mu``,``ks1:('prop,'state1) KS``,``ks2:('prop,'state2) KS``,``(e:string -> 'state1 -> bool)``,``(e':string -> 'state2 -> bool)``,``s1:'state1``,``s2:'state2``],
list_mk_imp([
``wfKS (ks1:('prop,'state1) KS)``,
``wfKS (ks2:('prop,'state2) KS)``,
