...
``!p (s1:'state1) (s2:'state2). p IN ks1.ap ==> (p IN (ks1:('state1,'prop) KS).L s1 = p IN (ks2:('state2,'prop) KS).L s2)``,
``!a s1 s1' s2 s2'. ((s1>--(ks1:('state1,'prop) KS)/a-->s1' = s2>--(ks2:('state2,'prop) KS)/a-->s2'))``,
mk_forall(``Q:string``,mk_pforall(``s1:'state1``,mk_forall(``s2:'state2``,
					      ``s1 IN ((e:string -> 'state1 -> bool) Q) = s2 IN ((e':string -> 'state2 -> bool) Q)``))),
``IMF (f:'prop mu)``,
``!p. SUBFORMULA (AP p) f ==> p IN (ks1:('state1,'prop) KS).ap``
],
``(s1 IN STATES (f:'prop mu) (ks1:('state1,'prop) KS) (e:string -> 'state1 -> bool) = s2 IN STATES f (ks2:('state2,'prop) KS) (e':string -> 'state2 -> bool))``)),
...
THEN (SUBGOAL_THEN (list_mk_forall([``n:num``,``s1:'state1``,``s2:'state2``],``s1 IN FP f s (ks1:('state1,'prop) KS) e[[[s<--{}]]] n = s2 IN FP f s (ks2:('state2,'prop) KS) e'[[[s<--{}]]] n``)) ASSUME_TAC) THENL [
Induct_on `n` THENL [
 RW_TAC std_ss [STATES_def,ENV_EVAL,NOT_IN_EMPTY], (* 0 *)
 REWRITE_TAC [STATES_def,ENV_UPDATE]
 THEN  (PAT_ASSUM ``!q1 q2 q3 q4 q5 q6. t`` (fn t => ASSUME_TAC (ISPECL [``(ks1:('state1,'prop) KS)``,``(ks2:('state2,'prop) KS)``,``e[[[s <-- FP f s (ks1:('state1,'prop) KS) e[[[s<--{}]]] n]]]``, ``e'[[[s<--FP f s (ks2:('state2,'prop) KS) e'[[[s<--{}]]] n]]]``] t)))
  THEN RES_TAC
...
THEN (SUBGOAL_THEN (list_mk_forall([``n:num``,``s1:'state1``,``s2:'state2``],``s1 IN FP f s (ks1:('state1,'prop) KS) e[[[s<--ks1.S]]] n = s2 IN FP f s (ks2:('state2,'prop) KS) e'[[[s<--ks2.S]]] n``)) ASSUME_TAC) THENL [
Induct_on `n` THENL [
  FULL_SIMP_TAC std_ss [STATES_def,ENV_EVAL,IN_UNIV,wfKS_def], (* 0 *)
  REWRITE_TAC [STATES_def,ENV_UPDATE]
  THEN (PAT_ASSUM ``!q1 q2 q3 q4 q5 q6. t`` (fn t => ASSUME_TAC (ISPECL [``(ks1:('state1,'prop) KS)``,``(ks2:('state2,'prop) KS)``,``e[[[s <-- FP f s (ks1:('state1,'prop) KS) e[[[s<--ks1.S]]] n]]]``, ``e'[[[s<--FP f s (ks2:('state2,'prop) KS) e'[[[s<--ks2.S]]] n]]]``] t)))
  THEN RES_TAC
  THEN NTAC 2 (POP_ASSUM (K ALL_TAC))
  THEN POP_ASSUM (fn t => ASSUME_TAC t THEN SUBGOAL_THEN (fst(dest_imp(concl t))) ASSUME_TAC) THENL [
  REPEAT GEN_TAC
...
val lem3a = REWRITE_RULE [GSYM IMF_MU_NNF] (ISPECL [``ks:('state,'prop) KS``,``(NNF f):'prop mu``,``e:string->'state->bool``,``Y:'state->bool``,``X:'state->bool``,``Q:string``] (REWRITE_RULE [SUBSET_DEF] (GEN_ALL (SIMP_RULE std_ss [] (REWRITE_RULE [SUBSET_REFL] (ISPECL [``f:'prop mu``,``ks:('state,'prop) KS``,``e:string -> 'state -> bool``,``e:string -> 'state -> bool``,``Q:string``,``X:'state->bool``,``Y:'state->bool``] STATES_MONO))))));

(* gen decomp proof *)
val DECOMP = save_thm("DECOMP",SIMP_RULE std_ss [STATES_NNF_ID] (prove(list_mk_pforall ([``(f:'prop mu)``,``s:'state``,``(e:string -> 'state -> bool)``,``Q:string``,``ks1:('state,'prop) KS``,``ks2:('state,'prop) KS``],list_mk_imp([``IMF mu (Q:string) .. (f:'prop mu)``,``!a g. ~SUBFORMULA (<<a>> g) (NNF (f:'prop mu))``,``wfKS (ks1:('state,'prop) KS)``,``wfKS (ks2:('state,'prop) KS)``,``(ks1:('state,'prop) KS).ap=(ks2:('state,'prop) KS).ap``,``(ks1:('state,'prop) KS).L=(ks2:('state,'prop) KS).L``,``!a s s'. ((ks2:('state,'prop) KS).T a)(s,s') ==> ((ks1:('state,'prop) KS).T a)(s,s')``],``s IN STATES (NNF (f:'prop mu)) (ks1:('state,'prop) KS) e ==> s IN STATES (NNF f) (ks2:('state,'prop) KS) e``)),
...
THEN RES_TAC
THEN (SUBGOAL_THEN (mk_forall(``n:num``,list_mk_forall([``s:'state``],mk_imp(``s IN FP (NNF f) Q (ks1:('state,'prop) KS) e[[[Q<--{}]]] n``,``s IN FP (NNF f) Q (ks2:('state,'prop) KS) e[[[Q<--{}]]] n``)))) ASSUME_TAC) THENL [
 Induct_on `n` THENL [
  ...
  THEN SPEC_TAC (``FP (NNF f) Q  (ks1:('state,'prop) KS) e[[[Q<--{}]]] n``,``X:'state -> bool``)
  THEN SPEC_TAC (``FP (NNF f) Q  (ks2:('state,'prop) KS)e[[[Q<--{}]]] n``,``Y:'state->bool``)
  THEN REPEAT GEN_TAC
  THEN STRIP_TAC
...
THEN RES_TAC
THEN (SUBGOAL_THEN (mk_forall(``n:num``,mk_forall(``s:'state``,mk_imp(``s IN FP (NNF f) Q (ks1:('state,'prop) KS) e[[[Q<--ks1.S]]] n``,``s IN FP (NNF f) Q (ks2:('state,'prop) KS) e[[[Q<--ks2.S]]] n``)))) ASSUME_TAC) THENL [
 Induct_on `n` THENL [
  SIMP_TAC std_ss [STATES_def,ENV_EVAL]
  THEN FULL_SIMP_TAC std_ss [wfKS_def],
  SIMP_TAC std_ss [STATES_def,ENV_UPDATE]
  THEN POP_ASSUM (fn t => ASSUME_TAC t THEN UNDISCH_TAC (concl t))
  THEN SPEC_TAC (``FP (NNF f) Q (ks1:('state,'prop) KS) e[[[Q<--ks1.S]]] n``,``X:'state -> bool``)
  THEN SPEC_TAC (``FP (NNF f) Q (ks2:('state,'prop) KS) e[[[Q<--ks2.S]]] n``,``Y:'state ->bool``)
  THEN REPEAT GEN_TAC
  THEN STRIP_TAC
  THEN IMP_RES_TAC lem3a
  THEN FULL_SIMP_TAC std_ss []
...
THEN RES_TAC
THEN (SUBGOAL_THEN (mk_forall(``n:num``,mk_forall(``s:'state``,mk_imp(``s IN FP (NNF (RVNEG Q ~f)) Q (ks1:('state,'prop) KS) e[[[Q<--ks1.S]]] n``,``s IN FP (NNF (RVNEG Q ~f)) Q (ks2:('state,'prop) KS) e[[[Q<--ks2.S]]] n``)))) ASSUME_TAC) THENL [
 Induct_on `n` THENL [
  FULL_SIMP_TAC std_ss [STATES_def,ENV_EVAL,wfKS_def],
  SIMP_TAC std_ss [STATES_def,ENV_UPDATE]
  THEN POP_ASSUM (fn t => ASSUME_TAC t THEN UNDISCH_TAC (concl t))
  THEN SPEC_TAC (``FP (NNF (RVNEG Q ~f)) Q (ks1:('state,'prop) KS) e[[[Q<--ks1.S]]] n``,``X:'state -> bool``)
  THEN SPEC_TAC (``FP (NNF (RVNEG Q ~f)) Q (ks2:('state,'prop) KS) e[[[Q<--ks2.S]]] n``,``Y:'state->bool``)
  THEN REPEAT GEN_TAC
  THEN STRIP_TAC
  THEN IMP_RES_TAC lem3a
  THEN FULL_SIMP_TAC std_ss []
...
THEN POP_ASSUM (fn t => ASSUME_TAC (ONCE_REWRITE_RULE [IMF_MU_INV_RVNEG] t))
THEN RES_TAC
THEN (SUBGOAL_THEN (mk_forall(``n:num``,mk_forall(``s:'state``,mk_imp(``s IN FP (NNF (RVNEG Q ~f)) Q (ks1:('state,'prop) KS) e[[[Q<--{}]]] n``,``s IN FP (NNF (RVNEG Q ~f)) Q (ks2:('state,'prop) KS) e[[[Q<--{}]]] n``)))) ASSUME_TAC) THENL [
 Induct_on `n` THENL [
  SIMP_TAC std_ss [STATES_def,ENV_EVAL],
  SIMP_TAC std_ss [STATES_def,ENV_UPDATE]
  THEN POP_ASSUM (fn t => ASSUME_TAC t THEN UNDISCH_TAC (concl t))
  THEN SPEC_TAC (``FP (NNF (RVNEG Q ~f)) Q (ks1:('state,'prop) KS) e[[[Q<--{}]]] n``,``X:'state -> bool``)
  THEN SPEC_TAC (``FP (NNF (RVNEG Q ~f)) Q (ks2:('state,'prop) KS) e[[[Q<--{}]]] n``,``Y:'state ->bool``)
  THEN REPEAT GEN_TAC
  THEN STRIP_TAC
  THEN IMP_RES_TAC lem3a
  THEN FULL_SIMP_TAC std_ss []
