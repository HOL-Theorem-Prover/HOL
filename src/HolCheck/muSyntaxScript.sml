...
recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN BETA_TAC THEN SIMP_TAC std_ss ([NNF_def,RVNEG_def,IMF_def,MU_SUB_def]@(tsimps "mu")) THENL [
REPEAT GEN_TAC THEN DISCH_TAC 
THEN FULL_SIMP_TAC std_ss ([NNF_def,RVNEG_def,IMF_def,MU_SUB_def]@(tsimps "mu"))
THEN Cases_on `Q''=Q` THENL [
FULL_SIMP_TAC std_ss ([NNF_def,RVNEG_def,IMF_def,MU_SUB_def]@(tsimps "mu")),
FULL_SIMP_TAC std_ss ([NNF_def,RVNEG_def,IMF_def,MU_SUB_def]@(tsimps "mu"))],
...
Induct_on `f` THEN FULL_SIMP_TAC std_ss ([IMF_def,MU_SUB_def,RVNEG_def]@(tsimps "mu")) THENL [
...
val STATES_MONO_NEG_MU_LEM1 =  save_thm("STATES_MONO_NEG_MU_LEM1",prove(``!Q' Q (f:'prop mu) . SUBFORMULA (~RV Q') (NNF ~mu Q.. f) = SUBFORMULA (~RV Q') (NNF (RVNEG Q ~f))``,SIMP_TAC std_ss ([NNF_def,MU_SUB_def]@(tsimps "mu"))))
...
recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN BETA_TAC THEN (SIMP_TAC std_ss ([NNF_def,MU_SUB_def,RVNEG_def]@(tsimps "mu"))) THENL [
PROVE_TAC [], (* /\ *)
PROVE_TAC [], (* \/ *)
REPEAT GEN_TAC THEN DISCH_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu"))], (* RV *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu"))], (* mu *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu"))], (* nu *)
PROVE_TAC [], (* ~/\ *)
PROVE_TAC [], (* ~\/ *)
REPEAT GEN_TAC THEN DISCH_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
...
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")) 
 THEN FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def,RVNEG_SYM]@(tsimps "mu"))], (* ~ mu *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@(tsimps "mu")) 
 THEN FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def,RVNEG_SYM]@(tsimps "mu"))]])) (* ~ nu *)

val NNF_RVNEG_DUALITY = save_thm("NNF_RVNEG_DUALITY",prove(``!(f:'prop mu) Q. NNF (RVNEG Q (RVNEG Q f)) = NNF f``,
recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN BETA_TAC THEN FULL_SIMP_TAC std_ss (NNF_def::MU_SUB_def::RVNEG_def::(tsimps "mu")) THENL [
REPEAT GEN_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu"))], (* RV *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu"))], (* mu *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu"))], (* nu *)
REPEAT GEN_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu"))], (* ~RV *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_SYM::RVNEG_def::NNF_def::(tsimps "mu"))], (* ~mu *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss (RVNEG_def::NNF_def::(tsimps "mu")),
 FULL_SIMP_TAC std_ss (RVNEG_SYM::RVNEG_def::NNF_def::(tsimps "mu"))]])) (* ~nu *)
...
val CLOSED_AND = save_thm("CLOSED_AND",prove(``!f g. CLOSED (f /\ g) = CLOSED f /\ CLOSED g``,Induct_on `f` THEN FULL_SIMP_TAC std_ss ([CLOSED_def,FV_def,UNION_EMPTY,EMPTY_UNION]@tsimps "mu")))

val CLOSED_OR = save_thm("CLOSED_OR",prove(``!f g. CLOSED (f \/ g) = CLOSED f /\ CLOSED g``,Induct_on `f` THEN FULL_SIMP_TAC std_ss ([CLOSED_def,FV_def,UNION_EMPTY,EMPTY_UNION]@tsimps "mu")))

val CLOSED_AP = save_thm("CLOSED_AP",prove(``!p. CLOSED (AP p)``,FULL_SIMP_TAC std_ss ([CLOSED_def,FV_def,UNION_EMPTY,EMPTY_UNION]@tsimps "mu")))

val CLOSED_BOX = save_thm("CLOSED_BOX",prove(``!f a. CLOSED ([[a]] f) = CLOSED f``,Induct_on `f` THEN FULL_SIMP_TAC std_ss ([CLOSED_def,FV_def,UNION_EMPTY,EMPTY_UNION]@tsimps "mu")))

val CLOSED_DMD = save_thm("CLOSED_DMD",prove(``!f a. CLOSED (<<a>> f) = CLOSED f``,Induct_on `f` THEN FULL_SIMP_TAC std_ss ([CLOSED_def,FV_def,UNION_EMPTY,EMPTY_UNION]@tsimps "mu")))
...
THEN REPEAT STRIP_TAC THEN (((DISJ1_TAC THEN RES_TAC) THEN NO_TAC) ORELSE ((DISJ2_TAC THEN RES_TAC) THEN NO_TAC) ORELSE (DISJ2_TAC THEN DISJ1_TAC THEN RES_TAC THEN NO_TAC) ORELSE ASM_REWRITE_TAC [SUBF_REFL])))
...
Induct_on `f` THEN SIMP_TAC std_ss (MU_SUB_def::tsimps "mu")));
...
Induct_on `f` THEN FULL_SIMP_TAC std_ss (RVNEG_def::MU_SUB_def::tsimps "mu")
THEN REPEAT GEN_TAC THEN (TRY EQ_TAC) THEN RW_TAC std_ss [] 
THEN FULL_SIMP_TAC std_ss (RVNEG_def::MU_SUB_def::tsimps "mu") THEN ASSUM_LIST PROVE_TAC));

val SUB_AP_NEG_MU = save_thm("SUB_AP_NEG_MU",prove(``!f p Q . SUBFORMULA (AP p) ~(mu Q.. f) = SUBFORMULA (AP p) (RVNEG Q ~f)``, 
Induct_on `f` THEN FULL_SIMP_TAC std_ss ([SUB_AP_RVNEG,NNF_def,RVNEG_def,MU_SUB_def]@(tsimps "mu"))
THEN REPEAT GEN_TAC THEN (TRY EQ_TAC) THEN RW_TAC std_ss [] 
THEN FULL_SIMP_TAC std_ss (RVNEG_def::MU_SUB_def::tsimps "mu") THEN ASSUM_LIST PROVE_TAC
));
...
THEN REPEAT (RW_TAC std_ss ([IMF_def,MU_SUB_def,NNF_def,RVNEG_def,IMF_INV_RVNEG]@(tsimps "mu")))))
...
val SUB_DMD_DMD = save_thm("SUB_DMD_DMD",prove (``!f a. ~(!a' g . ~SUBFORMULA <<a'>> g <<a>> f)``,SIMP_TAC std_ss [] THEN REPEAT GEN_TAC THEN MAP_EVERY Q.EXISTS_TAC [`a`,`f`] THEN SIMP_TAC std_ss (MU_SUB_def::tsimps "mu")))
...
val SUB_DMD_NEG_CONJ  = save_thm("SUB_DMD_NEG_CONJ",prove(``!f f1 g a. ~SUBFORMULA (<<a>> g) ~(f /\ f1) = (~SUBFORMULA (<<a>> g) ~f) /\ (~SUBFORMULA (<<a>> g) ~f1)``,Induct_on `f` THEN SIMP_TAC std_ss (MU_SUB_def::tsimps "mu")))

val SUB_DMD_NEG_DISJ  = save_thm("SUB_DMD_NEG_DISJ",prove(``!f f1 g a. ~SUBFORMULA (<<a>> g) ~(f \/ f1) = (~SUBFORMULA (<<a>> g) ~f) /\ (~SUBFORMULA (<<a>> g) ~f1)``,Induct_on `f` THEN SIMP_TAC std_ss (MU_SUB_def::tsimps "mu")))
...
REPEAT GEN_TAC THEN Cases_on `Q'=Q` THENL [
 ...
 THEN FULL_SIMP_TAC std_ss (tsimps "mu"),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@ (tsimps "mu"))], (* RV *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@ (tsimps "mu")),
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@ (tsimps "mu")) 
...
REPEAT GEN_TAC THEN Cases_on `Q'=Q` THENL [
 ...
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@ (tsimps "mu"))], (* ~RV *)
REPEAT STRIP_TAC THEN Cases_on `Q'=Q` THENL [
 FULL_SIMP_TAC std_ss ([NNF_RVNEG_DUALITY,MU_SUB_def,NNF_def,RVNEG_def]@ (tsimps "mu")), 
 FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def,RVNEG_def]@ (tsimps "mu"))
...
val STATES_MONO_LEM6 = save_thm("STATES_MONO_LEM6",prove (``!Q Q' (f:'prop mu). SUBFORMULA (~RV Q') (NNF mu Q.. f) = SUBFORMULA (~RV Q') (NNF f)``,FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@ (tsimps "mu"))))

val STATES_MONO_LEM11 = save_thm("STATES_MONO_LEM11",prove(``!Q Q' (f:'prop mu). SUBFORMULA (~RV Q') (NNF nu Q.. f) = SUBFORMULA (~RV Q') (NNF f)``,FULL_SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@ (tsimps "mu"))))


val STATES_MONO_LEM8 = save_thm("STATES_MONO_LEM8",prove(``!Q. ~SUBFORMULA (~RV Q) (NNF (RV:(string -> 'prop mu) Q))``,SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@tsimps "mu")))

val STATES_MONO_LEM9 = save_thm("STATES_MONO_LEM9",prove(``!Q. SUBFORMULA (~RV Q) (NNF (~(RV:(string -> 'prop mu)) Q))``,SIMP_TAC std_ss ([MU_SUB_def,NNF_def]@tsimps "mu")))

(* thms about IMF *)

val NNF_IDEM = save_thm("NNF_IDEM",prove(``!f. NNF (NNF f) = NNF f``,recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (NNF_def::MU_SUB_def::(tsimps "mu")) THEN ASSUM_LIST (fn l => TRY (PROVE_TAC l))))

val IMF_NNF = save_thm("IMF_NNF",prove(``!f. IMF f = IMF (NNF f)``,
recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (NNF_IDEM::IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN
...
val IMF_MU_CONJ = save_thm("IMF_MU_CONJ",prove(``!f g Q. IMF (mu Q.. f /\ g) = IMF (mu Q .. f) /\ IMF (mu Q .. g)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC)) 

val IMF_MU_NEG_CONJ = save_thm("IMF_MU_NEG_CONJ",prove(``!f g Q. IMF (mu Q.. ~(f /\ g)) = IMF (mu Q .. ~f) /\ IMF (mu Q .. ~g)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC)) 

val IMF_MU_DISJ = save_thm("IMF_MU_DISJ",prove(``!f g Q. IMF (mu Q.. f \/ g) = IMF (mu Q .. f) /\ IMF (mu Q .. g)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_NEG_DISJ = save_thm("IMF_MU_NEG_DISJ",prove(``!f g Q. IMF (mu Q.. ~(f \/ g)) = IMF (mu Q .. ~f) /\ IMF (mu Q .. ~g)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_DMD = save_thm("IMF_MU_DMD",prove(``!f a Q. IMF (mu Q.. <<a>> f ) = IMF (mu Q .. f)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_NEG_DMD = save_thm("IMF_MU_NEG_DMD",prove(``!f a Q. IMF (mu Q.. ~<<a>> f ) = IMF (mu Q .. ~f)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_BOX = save_thm("IMF_MU_BOX",prove(``!f a Q. IMF (mu Q.. [[a]] f) = IMF (mu Q .. f)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_NEG_BOX = save_thm("IMF_MU_NEG_BOX",prove(``!f a Q. IMF (mu Q.. ~[[a]] f) = IMF (mu Q .. ~f)``,SIMP_TAC std_ss [IMF_def] THEN Induct_on `f` THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST PROVE_TAC))

val IMF_MU_MU = save_thm("IMF_MU_MU",prove(``!f Q Q'. IMF (mu Q.. mu Q' .. f) ==> IMF (mu Q' .. f)``,SIMP_TAC std_ss [IMF_def] THEN recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST (fn l => TRY (PROVE_TAC l))))

val IMF_MU_NEG_MU = save_thm("IMF_MU_NEG_MU",prove(``!f Q Q'. IMF (mu Q.. ~mu Q' .. f) ==> IMF (mu Q' .. f)``,SIMP_TAC std_ss [IMF_def] THEN recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST (fn l => TRY (PROVE_TAC l))))

val IMF_MU_NU = save_thm("IMF_MU_NU",prove(``!f Q Q'. IMF (mu Q.. nu Q' .. f) ==> IMF (mu Q' .. f)``,SIMP_TAC std_ss [IMF_def] THEN recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST (fn l => TRY (PROVE_TAC l))))

val IMF_MU_NEG_NU = save_thm("IMF_MU_NEG_NU",prove(``!f Q Q'. IMF (mu Q.. ~nu Q' .. f) ==> IMF (mu Q' .. f)``,SIMP_TAC std_ss [IMF_def] THEN recInduct NNF_IND_def THEN REPEAT CONJ_TAC THEN SIMP_TAC std_ss (IMF_def::MU_SUB_def::NNF_def::(tsimps "mu")) THEN ASSUM_LIST (fn l => TRY (PROVE_TAC l))))
