...
local open HolKernel basicHol90Lib clauses
in

exception CANNOT_HAPPEN of string;

fun dead_code s = raise CANNOT_HAPPEN s;
...
fun match_list bds (pat::pats) (arg::args) =
  ...
  | match_list bds []          []          = bds
  | match_list _   _           _           = dead_code "match_list"
...
fun comb_ct {Rws=(0,_,_)::l,...} arg =
      dead_code "comb_ct: yet rules to match"
...
(*
val st = ref((--`0`--,[],[]):
      Term.term * (Term.term * clauses.ft) list *
      (int * Type.hol_type * clauses.rewrite list) list);
*)


(* TODO: match type in from_term, but it is currently not possible, since in
 * from_term all the rules are not necessarily known.
 *)
fun reduce_cst {Head, Args, Rws=(0,ty,rws)::arw} =
      (let (*val _ = st :=(Head,cargs,(0,ty,rws)::arw)*)
 	   val inst =
