...
has a programming language, and \ml{CBV\_CONV} is an evaluator which
...
\end{eqnarray*}
Patterns must be linear, that is there cannot be two identical pattern
variables (same name and same type). This corresponds to the usual
definition scheme by pattern-matching.
...
evaluated strictly. If the boolean is \ml{true}, then the theorems are
not preprocessed. If it is \ml{false}, the last variable patterns are
...
theorems can be combined using \ml{from\_list}.
...
\ml{numeral\_distrib}, \ml{numeral\_eq}, \ml{numeral\_suc},
\ml{numeral\_iisuc}, \ml{numeral\_add}, \ml{numeral\_mult},
\ml{iDUB\_removal}.
