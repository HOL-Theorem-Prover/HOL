...
fun op_assoc cmp i alist =
   case Lib.op_assoc1 cmp i alist
     of NONE => raise HOL88_ERR {function = "op_assoc", message = ""}
      | (SOME x) => x;

fun op_rev_assoc cmp i alist =
   case Lib.op_assoc2 cmp i alist
     of NONE => raise HOL88_ERR {function = "op_rev_assoc", message = ""}
      | (SOME x) => x;
...
fun GEN_ALL th =
  Lib.itlist Thm.GEN
    (Lib.op_set_diff Term.eq (frees (Thm.concl th)) (freesl (Thm.hyp th))) th
