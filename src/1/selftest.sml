...
(* ----------------------------------------------------------------------
    Context.with_context pins a thread's ambient reads

    `mk_const` and `mk_type` resolve a name against the live context, and
    no amount of context-passing removes that.  So a proof running on one
    thread while another rewinds the live context -- which is exactly
    what the LSP's proof pool and its compile thread do -- can fail to
    find a constant its own theory defines, and be reported as a failed
    proof when nothing is wrong with it.

    `with_context` answers that thread's ambient reads from a pinned
    context instead.  Measured before it existed: 6 spurious failures in
    16000 attempts against a concurrent restore, varying run to run.
    With the pin: none.  This asserts the pin, not the race -- the race
    is timing-dependent and a test for it would be flaky.
   ---------------------------------------------------------------------- *)
val _ = let
  val before_def = Context.snapshot ()
  val c = new_definition ("ctxpin_def", Term`ctxpin (b:bool) <=> b`)
  val with_def = Context.snapshot ()
  val n = 2000
  (* Naming the constant is the point: this is the read that a rewind
     would otherwise break. *)
  fun buildsOK () =
      (ignore (Term`ctxpin T`); true) handle _ => false
  val fails = ref 0
  val done = ref false
  fun body () = let
    fun go 0 = ()
      | go k = (if buildsOK () then () else fails := !fails + 1; go (k - 1))
    in go n end
  val () = tprint "with_context pins ambient reads against a restore"
  val t = Thread.fork (fn () => (Context.with_context with_def body ();
                                 done := true),
                       [Thread.InterruptState Thread.InterruptDefer])
  fun churn 0 = ()
    | churn k = (Context.restore before_def; Context.restore with_def;
                 churn (k - 1))
  val () = churn n
  fun wait 0 = ()
    | wait k = if !done then ()
               else (OS.Process.sleep (Time.fromMilliseconds 20); wait (k - 1))
  val () = wait 250
  (* leave the context as the rest of the file expects, whatever happened *)
  val () = Context.restore with_def
  in
    if !done andalso !fails = 0 then OK ()
    else die ("FAILED: " ^ Int.toString (!fails) ^ "/" ^ Int.toString n ^
              " builds failed" ^ (if !done then "" else " (worker hung)"))
  end
