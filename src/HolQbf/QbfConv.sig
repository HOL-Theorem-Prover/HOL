signature QbfConv = sig
  ...
  *)
  ...
  (* Simplifies clauses (specialisation of SIMP_CONV).
     In particular, does the following rewrites:
       (∀x. x ∨ P) = P, (∀x. ¬x ∨ P) = P, (∀x. x) = F, (∀x. ¬x) = F,
     and associativity/commutativity normalisation for conjunction and
     disjunction *)
  val simp_clauses : conv

  (* conversion that takes [!x:bool. t] where t is in CNF and may contain x and
     removes the quantifier and all occurrences of x. also simplifies clauses
     (as above). *)
  ...
  (* [last_quant_seq_conv cq cb] applies cb under the quantifier prefix and
    then cq before each quantifier. That is:
      Q1 x1. Q2 x2. ... Qn xn. P -->
      cq (Q1 x1. cq (Q2 x2. ... cq (Qn. xn. cb P))))
  *)
