...
fun tdistinct [] = true
  | tdistinct (x::xs) = not (tmem x xs) andalso tdistinct xs
...
fun check_tyop_quotient th =
   let val (taus, ksis, Rs, abss, reps, uncond_tm) =
       ...
       val _ = assert tdistinct (append (append Rs abss) reps)
   in
       null (Term.free_vars (concl th)) andalso
...
(* The RAISE_ONE_RSP rule raises the order of the given wf theorem by one,
   ...
   Fails when order cannot be raised any further, i.e., when arity is 0. *)
        ...
        fun RAISE_ONE_RSP th =
          ...
          let val (left,right) = ((rand ## I) o Psyntax.dest_comb o concl) th
              val lx = rand left and rx = rand right
              val _ = assert (aconv lx) rx
              val _ = assert (not o is_rep_ty o type_of) rx
              val tm = concl th and asl = hyp th
              val free =
                  op_mk_set aconv (free_vars tm @ flatten (map free_vars asl))
...
*)
        fun LAMBDA_RSP_TAC (asl,gl) =
            let val {Rator=Rf, Rand=g} = dest_comb gl
                ...
                val free =
                    op_mk_set aconv (free_vars f @ flatten (map free_vars asl))
...
(* ------------------------------------------------------------------------- *)
        ...
        val LIFT_RULE =
              (fn th => let val thr = (REGULARIZE_RULE o
                            ...
                            val _ = check_quotient_tys (tys_quot_ths @ tyop_ty_ths) tys
                            val ops = op_mk_set aconv (findops tm)
...
fun check_respects_tm tm =
   let val (vrs,body) = strip_forall tm
       ...
       val _ = assert tdistinct (uncurry append (unzip ant_args))
       val ((Rc,t1),t2) = ((Psyntax.dest_comb ## I) o Psyntax.dest_comb) base
       val (c1,args1) = strip_comb t1
       val (c2,args2) = strip_comb t2
       val _ = assert (aconv c1) c2
       val _ = assert tdistinct args1
       val _ = assert tdistinct args2
       val argpairs = zip args1 args2
       fun check_arg (a12 as (a1,a2)) =
               ((op_mem tmp_eq a12 ant_args andalso tmem a1 vrs andalso
                 tmem a2 vrs
                 orelse a1 ~~ a2)
               andalso type_of a1 = type_of a2)
       fun check_ant (ant12 as (ant1,ant2)) =
                (op_mem tmp_eq ant12 argpairs
                 andalso tmem ant1 vrs andalso tmem ant2 vrs
                 andalso type_of ant1 = type_of ant2)
       fun check_var vr = tmem vr args1 orelse tmem vr args2
   in
...
fun check_poly_preserves th =
   let val (taus, ksis, Rs, abss, reps, uncond_tm) =
       ...
       val _ = assert (all (C tmem args1')) vrs
       val _ = assert (all (C tmem vrs)) args1'
...
fun dest_con_inhab c =
      let open Psyntax
          ...
          val ants = if Teq ant then [] else strip_conj ant
      in
          (vs, ants, conseq)
      end
...
fun check_con_inhab th =
   let val (vs, ants, conseq) = dest_con_inhab (concl th)
       ...
       val _ = assert tdistinct vs
       val _ = assert tdistinct ants
   in
