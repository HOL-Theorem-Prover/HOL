...
\\
http://www.cis.upenn.edu/\symbol{126}homeier
}
...
%%In particular,
%The conditional quotient theorem for function types
...
HOL \cite{GoMe93}.
%and then expressed here in normal notation.
...
%of the corresponding equivalence class of $x$ from set theory.
...
(\forall r:\tau
.\ 
\lfloor \, \lceil \, [ r ]_E \rceil_c \rfloor_c = [ r ]_E ) .
$
\end{theorem}
Proof: 
The left conjunct comes directly from theorem \ref{abs_rep_classes}.
...
\begin{lemma}[$\lfloor \_ {\rfloor}_c$ maps to equivalence classes]
\label{ty_REP_REL}
$\forall a.\ \exists r.\ \lfloor a {\rfloor}_c = [ r ]_E$
\end{lemma}
Proof:
By the left clause of theorem \ref{abs_rep_classes},
$ \lceil \, \lfloor a {\rfloor}_c {\rceil}_c = a$, so taking
the $\lfloor \_ {\rfloor}_c$ of both sides,
$\lfloor \, \lceil \, \lfloor a {\rfloor}_c {\rceil}_c {\rfloor}_c
= \lfloor a {\rfloor}_c$.
By the right clause of theorem \ref{abs_rep_classes}, this implies
$P\ \lfloor a {\rfloor}_c$.
By the definition of $P$, this is the goal.
$\Box$

\begin{lemma}[$\lceil \_ {\rceil}_c$ is one-to-one]
\label{ty_ABS_11}
$\forall x\ y.\ 
\lceil \, [ x ]_E {\rceil}_c = \lceil \, [ y ]_E {\rceil}_c
\ \Leftrightarrow \ [ x ]_E = [ y ]_E$
\end{lemma}
Proof: We will prove this as a biconditional.

%{\it Case 1.}
$\Rightarrow$:
Assume $\lceil \, [ x ]_E {\rceil}_c = \lceil \, [ y ]_E {\rceil}_c$.
Applying the function $\lfloor \_ {\rfloor}_c$ to
%Taking the representative ($\lfloor \_ {\rfloor}_c$) of
both sides gives us
$\lfloor \, \lceil \, [ x ]_E {\rceil}_c {\rfloor}_c =
\lfloor \, \lceil \, [ y ]_E {\rceil}_c {\rfloor}_c$.
By the right clause of theorem \ref{abs_rep_classes_simple},
$[ x ]_E = [ y ]_E $.
% which is the goal.
...
$\Leftarrow$:
Assume $[ x ]_E = [ y ]_E $. 
%Then the goal is immediate.
Then
applying
$\lceil \_ {\rceil}_c$
to both sides yields the goal,
$\lceil \, [ x ]_E {\rceil}_c = \lceil \, [ y ]_E {\rceil}_c$.
$\Box$
...
$E\ r\ (\mbox{\tt \$@} \ (E\ r))$.
%By lemma
%\ref{ty_REL_SELECT} and symmetry, $E\ (\mbox{\tt \$@} \ (E\ r))\ r$.
Then
...
%& \ \Leftrightarrow \ &
% \lceil E\ r {\rceil}_c = \lceil E\ r' {\rceil}_c 
%& \mbox{definition \ref{equiv_class}}  \\
& \ \Leftrightarrow \ &
\lceil r \rceil = \lceil r' \rceil 
& \mbox{definition \ref{abs_rep_def}}  \\
...
% and to relate it properly to the original type. 
...
as indicated below
%in the obvious way
%either in standard HOL or
in
{\tt quotientTheory},
%an HOL theory provided.
...
% (see sections \ref{newquotienttype} and \ref{aggregates}).
% The exception is 
% the function operator, {\tt ===>},
% which takes two \quotient{} relations (\S\ref{newquotienttype}).
% The function operator
% % requires its arguments to be
% tests if the two functions are
% also
% respectful (see \S\ref{respectfulness});
% hence, unlike the others,
% the relation that {\tt ===>} forms is {\it not\/} an equivalence relation.
% It is listed here for comparison with the other operators, but is
% only used to form \quotient{} relations, not equivalence relations
% (see sections \ref{newquotienttype} and \ref{aggregates}).
...
%$\wedge$ \ 
%|- ($\forall$a. {\it abs}({\it rep} a) = a) $\wedge$
%} \\
& {\tt ...} \\
...
% In this case
%We also say that
%{\tt qty} is also called a quotient (type).
...
They are created in theory {\tt quotientTheory} or in standard HOL. 
...
%analogeous to that for the main design,
...
%
%\begin{center}
%{\tt |- $\forall$x y. {\it R} x y = ({\it R} x = {\it R} y)}
%\end{center}
...
%{\tt \{name="qty", equiv=R\_EQUIV\}}
%automatically declares a new type {\tt qty}
...
At the lower level, we can prove the theorem {\tt |- \verb+~+(3 = 5)}. 
But this theorem does not lift to the abstract level, where it would be
{\tt |- \verb+~+(ODD = ODD)}, which is obviously false.
So not every theorem should or will lift!

At the lower level, we can prove the theorem {\tt |- \verb+~+(0 = 1)}.
For this theorem, there {\it is\/}
a corresponding theorem at the abstract level,
{\tt |- \verb+~+(EVEN = ODD)}, which should be true.  However, there is just not
enough information in the lower theorem {\tt |- \verb+~+(0 = 1)}
to prove the lifted version, because the lower theorem
only states an inequality.  What is actually needed is
{\tt |- \verb+~+(0 $\sim$ 1)}, which states that the two values are
not equivalent, which is stronger. 
{\tt |- \verb+~+(0 $\sim$ 1)} implies that
{\tt |- \verb+~+(0 = 1)},
but not the reverse.
{\tt |- \verb+~+(0 $\sim$ 1)} is provable at the lower level,
and it will successfully lift to {\tt |- \verb+~+(EVEN = ODD)}.
...
%This package is widely applicabile, consistent with
%the use of quotients in the literature.
%
%For example, bags may be modeled as lists where the order is irrelevant,
%and sets may be modeled as bags where the number of
%equal elements is unimportant.
%
...
%This is useful for the study of systems which
Some systems
...
%For an extended example of the use of
%a prior version of
%this package in a significant proof,
%and for more information on these and other new HOL tools, please see 
%\cite{HoW01}.
%\begin{center}
%{\tt http://www.cis.upenn.edu/\verb+~+hol/lamcr}.
%\end{center}
%{\centering {\tt http://www.cis.upenn.edu/\verb+~+hol/lamcr}.
%
%}

%{\sc DEDICATION:}  This paper is dedicated to David F. Martin, Professor of
%Computer Science at UCLA, who passed away December 22, 1996.  Without his
%encouragement, none of these results would have happened.
