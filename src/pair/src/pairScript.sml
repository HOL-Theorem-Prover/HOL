...
S "        | check (h::t) = is_var h andalso not(op_mem eq h t) andalso check t"; NL();
S "in"; NL();
...
S "           val (tyS,kdS,rkS) = align_types [alpha |-> t1, beta |-> t2, gamma |-> t3]"; NL();
S "           val iDEF = INST_RK_KD_TY (tyS,kdS,rkS) DEF"; NL();
...
NL();
S "fun inter s1 [] = []"; NL();
S "  | inter s1 (h::t) = case op_intersect eq s1 h of [] => inter s1 t | X => X"; NL();
NL();
S "fun joint_vars []  = []"; NL();
S "  | joint_vars [_] = []"; NL();
