...
structure Z3 = struct
  ...
  fun ERR f =
    Feedback.mk_HOL_ERR "Z3" f
      "Set $HOL4_Z3_EXECUTABLE to point to the Z3 executable file."

  val is_configured = Option.isSome (OS.Process.getEnv "HOL4_Z3_EXECUTABLE")
  ...
  val Z3_SMT_Oracle = case OS.Process.getEnv "HOL4_Z3_EXECUTABLE" of
    SOME file =>
      SolverSpec.make_solver
        ...
        (Lib.K is_sat_file)
  | NONE =>
      (fn _ => raise ERR "Z3_SMT_Oracle")

  (* Z3 (Linux/Unix), SMT-LIB file format, with proofs *)
  val Z3_SMT_Prover = case OS.Process.getEnv "HOL4_Z3_EXECUTABLE" of
    SOME file =>
      SolverSpec.make_solver
        (fn goal =>
          let
