...
(* Various theorems for HolSmtLib *)
  ...
  val _ = s ("r035", T ``(T ==> p) <=> p``)
  val _ = s ("r036", T ``(p ==> T) <=> T``)
  val _ = s ("r037", T ``(F ==> p) <=> T``)
  val _ = s ("r038", T ``(p ==> p) <=> T``)
  val _ = s ("r039", T ``((p <=> q) ==> r) <=> (r \/ (q <=> ~p))``)
  ...
  val _ = s ("r044", T ``p \/ T <=> T``)
  val _ = s ("r045", T ``p \/ ~p <=> T``)
  val _ = s ("r046", T ``~p \/ p <=> T``)
  val _ = s ("r047", T ``T \/ p <=> T``)
  val _ = s ("r048", T ``p \/ F <=> p``)
  val _ = s ("r049", T ``F \/ p <=> p``)

  val _ = s ("r050", T ``p /\ q <=> q /\ p``)
  val _ = s ("r051", T ``p /\ T <=> p``)
  val _ = s ("r052", T ``T /\ p <=> p``)
  val _ = s ("r053", T ``p /\ F <=> F``)
  val _ = s ("r054", T ``F /\ p <=> F``)
  val _ = s ("r055", T ``p /\ q <=> ~(~p \/ ~q)``)
  val _ = s ("r056", T ``~p /\ q <=> ~(p \/ ~q)``)
  val _ = s ("r057", T ``p /\ ~q <=> ~(~p \/ q)``)
  val _ = s ("r058", T ``~p /\ ~q <=> ~(p \/ q)``)
  val _ = s ("r059", T ``p /\ q <=> ~(~q \/ ~p)``)
  val _ = s ("r060", T ``~p /\ q <=> ~(~q \/ p)``)
  val _ = s ("r061", T ``p /\ ~q <=> ~(q \/ ~p)``)
  val _ = s ("r062", T ``~p /\ ~q <=> ~(q \/ p)``)

  val _ = s ("r063", U [combinTheory.APPLY_UPDATE_ID] ``(x =+ f x) f = f``)

  val _ = s ("r064", S ``ALL_DISTINCT [x; x] <=> F``)
  val _ = s ("r065", S ``ALL_DISTINCT [x; y] <=> x <> y``)
  val _ = s ("r066", S ``ALL_DISTINCT [x; y] <=> y <> x``)

  val _ = s ("r067", A ``((x :int) = y) <=> (x + -1 * y = 0)``)
  val _ = s ("r068", A ``((x :int) = y + z) <=> (x + -1 * z = y)``)
  val _ = s ("r069", A ``((x :int) = y + -1 * z) <=> (x + (-1 * y + z) = 0)``)
  val _ = s ("r070", A ``((x :int) = -1 * y + z) <=> (y + (-1 * z + x) = 0)``)
  val _ = s ("r071", A ``((x :int) = y + z) <=> (x + (-1 * y + -1 * z) = 0)``)
  val _ = s ("r072", A ``((x :int) = y + z) <=> (y + (z + -1 * x) = 0)``)
  val _ = s ("r073", A ``((x :int) = y + z) <=> (y + (-1 * x + z) = 0)``)
  val _ = s ("r074", A ``((x :int) = y + z) <=> (z + -1 * x = -y)``)
  val _ = s ("r075", A ``((x :int) = -y + z) <=> (z + -1 * x = y)``)
  val _ = s ("r076", A ``(-1 * (x :int) = -y) <=> (x = y)``)
  val _ = s ("r077", A ``(-1 * (x :int) + y = z) <=> (x + -1 * y = -z)``)
  val _ = s ("r078", A ``((x :int) + y = 0) <=> (y = -x)``)
  val _ = s ("r079", A ``((x :int) + y = z) <=> (y + -1 * z = -x)``)
  val _ = s ("r080", A
    ``((a :int) + (-1 * x + (v * y + w * z)) = 0) <=> (x + (-v * y + -w * z) = a)``)

  val _ = s ("r081", A ``0 + (x :int) = x``)
  val _ = s ("r082", A ``(x :int) + 0 = x``)
  val _ = s ("r083", A ``(x :int) + y = y + x``)
  val _ = s ("r084", A ``(x :int) + x = 2 * x``)
  val _ = s ("r085", A ``(x :int) + y + z = x + (y + z)``)
  val _ = s ("r086", A ``(x :int) + y + z = x + (z + y)``)
  val _ = s ("r087", A ``(x :int) + (y + z) = y + (z + x)``)
  val _ = s ("r088", A ``(x :int) + (y + z) = y + (x + z)``)
  val _ = s ("r089", A ``(x :int) + (y + (z + u)) = y + (z + (u + x))``)

  val _ = s ("r090", A ``(x :int) >= x <=> T``)
  val _ = s ("r091", A ``(x :int) >= y <=> x + -1 * y >= 0``)
  val _ = s ("r092", A ``(x :int) >= y <=> y + -1 * x <= 0``)
  val _ = s ("r093", A ``(x :int) >= y + z <=> y + (z + -1 * x) <= 0``)
  val _ = s ("r094", A ``-1 * (x :int) >= 0 <=> x <= 0``)
  val _ = s ("r095", A ``-1 * (x :int) >= -y <=> x <= y``)
  val _ = s ("r096", A ``-1 * (x :int) + y >= 0 <=> x + -1 * y <= 0``)
  val _ = s ("r097", A ``(x :int) + -1 * y >= 0 <=> y <= x``)

  val _ = s ("r098", A ``(x :int) > y <=> ~(y >= x)``)
  val _ = s ("r099", A ``(x :int) > y <=> ~(x <= y)``)
  val _ = s ("r100", A ``(x :int) > y <=> ~(x + -1 * y <= 0)``)
  val _ = s ("r101", A ``(x :int) > y <=> ~(y + -1 * x >= 0)``)
  val _ = s ("r102", A ``(x :int) > y + z <=> ~(z + -1 * x >= -y)``)

  val _ = s ("r103", A ``x <= (x :int) <=> T``)
  val _ = s ("r104", A ``0 <= (1 :int) <=> T``)
  val _ = s ("r105", A ``(x :int) <= y <=> y >= x``)
  val _ = s ("r106", A ``0 <= -(x :int) + y <=> y >= x``)
  val _ = s ("r107", A ``-1 * (x :int) <= 0 <=> x >= 0``)
  val _ = s ("r108", A ``(x :int) <= y <=> x + -1 * y <= 0``)
  val _ = s ("r109", A ``(x :int) <= y <=> y + -1 * x >= 0``)
  val _ = s ("r110", A ``-1 * (x :int) + y <= 0 <=> x + -1 * y >= 0``)
  val _ = s ("r111", A ``-1 * (x :int) + y <= -z <=> x + -1 * y >= z``)
  val _ = s ("r112", A ``-(x :int) + y <= z <=> y + -1 * z <= x``)
  val _ = s ("r113", A ``(x :int) + -1 * y <= z <=> x + (-1 * y + -1 * z) <= 0``)
  val _ = s ("r114", A ``(x :int) <= y + z <=> x + -1 * z <= y``)
  val _ = s ("r115", A ``(x :int) <= y + z <=> z + -1 * x >= -y``)
  val _ = s ("r116", A ``(x :int) <= y + z <=> x + (-1 * y + -1 * z) <= 0``)

  val _ = s ("r117", A ``(x :int) < y <=> ~(y <= x)``)
  val _ = s ("r118", A ``(x :int) < y <=> ~(x >= y)``)
  val _ = s ("r119", A ``(x :int) < y <=> ~(y + -1 * x <= 0)``)
  val _ = s ("r120", A ``(x :int) < y <=> ~(x + -1 * y >= 0)``)
  val _ = s ("r121", A ``(x :int) < y + -1 * z <=> ~(x + -1 * y + z >= 0)``)
  val _ = s ("r122", A ``(x :int) < y + -1 * z <=> ~(x + (-1 * y + z) >= 0)``)
  val _ = s ("r123", A ``(x :int) < -y + z <=> ~(z + -1 * x <= y)``)
  val _ = s ("r124", A ``(x :int) < -y + (z + u) <=> ~(z + (u + -1 * x) <= y)``)
  val _ = s ("r125", A
    ``(x :int) < -y + (z + (u + v)) <=> ~(z + (u + (v + -1 * x)) <= y)``)

  val _ = s ("r126", A ``-(x :int) + y < z <=> ~(y + -1 * z >= x)``)
  val _ = s ("r127", A ``(x :int) + y < z <=> ~(z + -1 * y <= x)``)
  val _ = s ("r128", A ``0 < -(x :int) + y <=> ~(y <= x)``)

  val _ = s ("r129", A ``(x :int) - 0 = x``)
  val _ = s ("r130", A ``0 - (x :int) = -x``)
  val _ = s ("r131", A ``0 - (x :int) = -1 * x``)
  val _ = s ("r132", A ``(x :int) - y = -y + x``)
  val _ = s ("r133", A ``(x :int) - y = x + -1 * y``)
  val _ = s ("r134", A ``(x :int) - y = -1 * y + x``)
  val _ = s ("r135", A ``(x :int) - 1 = -1 + x``)
  val _ = s ("r136", A ``(x :int) + y - z = x + (y + -1 * z)``)
  val _ = s ("r137", A ``(x :int) + y - z = x + (-1 * z + y)``)

  val _ = s ("r138", R ``(0 = -u * (x :real)) <=> (u * x = 0)``)
  val _ = s ("r139", R ``(a = -u * (x :real)) <=> (u * x = -a)``)
  val _ = s ("r140", R ``((a :real) = x + (y + z)) <=> (x + (y + (-1 * a + z)) = 0)``)
  val _ = s ("r141", R ``((a :real) = x + (y + z)) <=> (x + (y + (z + -1 * a)) = 0)``)
  val _ = s ("r142", R ``((a :real) = -u * y + v * z) <=> (u * y + (-v * z + a) = 0)``)
  val _ = s ("r143", R ``((a :real) = -u * y + -v * z) <=> (u * y + (a + v * z) = 0)``)
  val _ = s ("r144", R ``(-(a :real) = -u * x + v * y) <=> (u * x + -v * y = a)``)
  val _ = s ("r145", R
  ...
  val _ = s ("r152", R ``(-(a :real) = -u * x + (-v * y + w * z)) <=> (u * x + (v * y + -w * z) = a)``)
  val _ = s ("r153", R ``(-(a :real) = -u * x + (-v * y + -w * z)) <=> (u * x + (v * y + w * z) = a)``)
  val _ = s ("r154", R ``((a :real) = -u * x + (-1 * y + w * z)) <=> (u * x + (y + -w * z) = -a)``)
  val _ = s ("r155", R ``((a :real) = -u * x + (-1 * y + -w * z)) <=> (u * x + (y + w * z) = -a)``)
  val _ = s ("r156", R ``(-u * (x :real) + -v * y = -a) <=> (u * x + v * y = a)``)
  val _ = s ("r157", R ``(-1 * (x :real) + (-v * y + -w * z) = -a) <=> (x + (v * y + w * z) = a)``)
  val _ = s ("r158", R ``(-u * (x :real) + (v * y + w * z) = -a) <=> (u * x + (-v * y + -w * z) = a)``)
  val _ = s ("r159", R ``(-u * (x :real) + (-v * y + w * z) = -a) <=> (u * x + (v * y + -w * z) = a)``)
  val _ = s ("r160", R ``(-u * (x :real) + (-v * y + -w * z) = -a) <=> (u * x + (v * y + w * z) = a)``)

  val _ = s ("r161", R ``(x :real) + -1 * y >= 0 <=> y <= x``)
  val _ = s ("r162", R ``(x :real) >= y <=> x + -1 * y >= 0``)

  val _ = s ("r163", R ``(x :real) > y <=> ~(x + -1 * y <= 0)``)

  val _ = s ("r164", R ``(x :real) <= y <=> x + -1 * y <= 0``)
  val _ = s ("r165", R ``(x :real) <= y + z <=> x + -1 * z <= y``)
  val _ = s ("r166", R ``-u * (x :real) <= a <=> u * x >= -a``)
  val _ = s ("r167", R ``-u * (x :real) <= -a <=> u * x >= a``)
  val _ = s ("r168", R ``-u * (x :real) + v * y <= 0 <=> u * x + -v * y >= 0``)
  val _ = s ("r169", R ``-u * (x :real) + v * y <= a <=> u * x + -v * y >= -a``)
  val _ = s ("r170", R ``-u * (x :real) + v * y <= -a <=> u * x + -v * y >= a``)
  val _ = s ("r171", R ``-u * (x :real) + -v * y <= a <=> u * x + v * y >= -a``)
  val _ = s ("r172", R ``-u * (x :real) + -v * y <= -a <=> u * x + v * y >= a``)
  val _ = s ("r173", R
  ...
  val _ = s ("r186", R ``(x :real) < y <=> ~(x >= y)``)
  val _ = s ("r187", R ``-u * (x :real) < a <=> ~(u * x <= -a)``)
  val _ = s ("r188", R ``-u * (x :real) < -a <=> ~(u * x <= a)``)
  val _ = s ("r189", R ``-u * (x :real) + v * y < 0 <=> ~(u * x + -v * y <= 0)``)
  val _ = s ("r190", R ``-u * (x :real) + -v * y < 0 <=> ~(u * x + v * y <= 0)``)
  val _ = s ("r191", R ``-u * (x :real) + v * y < a <=> ~(u * x + -v * y <= -a)``)
  val _ = s ("r192", R ``-u * (x :real) + v * y < -a <=> ~(u * x + -v * y <= a)``)
  val _ = s ("r193", R ``-u * (x :real) + -v * y < a <=> ~(u * x + v * y <= -a)``)
  val _ = s ("r194", R ``-u * (x :real) + -v * y < -a <=> ~(u * x + v * y <= a)``)
  val _ = s ("r195", R
    ``-u * (x :real) + (v * y + w * z) < a <=> ~(u * x + (-v * y + -w * z) <= -a)``)
  val _ = s ("r196", R
    ``-u * (x :real) + (v * y + w * z) < -a <=> ~(u * x + (-v * y + -w * z) <= a)``)
  ...
  val _ = s ("r215", R ``0 + (x :real) = x``)
  val _ = s ("r216", R ``(x :real) + 0 = x``)
  val _ = s ("r217", R ``(x :real) + y = y + x``)
  val _ = s ("r218", R ``(x :real) + x = 2 * x``)
  val _ = s ("r219", R ``(x :real) + y + z = x + (y + z)``)
  val _ = s ("r220", R ``(x :real) + y + z = x + (z + y)``)
  val _ = s ("r221", R ``(x :real) + (y + z) = y + (z + x)``)
  val _ = s ("r222", R ``(x :real) + (y + z) = y + (x + z)``)

  val _ = s ("r223", R ``0 - (x :real) = -x``)
  val _ = s ("r224", R ``0 - u * (x :real) = -u * x``)
  val _ = s ("r225", R ``(x :real) - 0 = x``)
  val _ = s ("r226", R ``(x :real) - y = x + -1 * y``)
  val _ = s ("r227", R ``(x :real) - y = -1 * y + x``)
  val _ = s ("r228", R ``(x :real) - u * y = x + -u * y``)
  val _ = s ("r229", R ``(x :real) - u * y = -u * y + x``)
  val _ = s ("r230", R ``(x :real) + y - z = x + (y + -1 * z)``)
  val _ = s ("r231", R ``(x :real) + y - z = x + (-1 * z + y)``)
  val _ = s ("r232", R ``(x :real) + y - u * z = -u * z + (x + y)``)
  val _ = s ("r233", R ``(x :real) + y - u * z = x + (-u * z + y)``)
  val _ = s ("r234", R ``(x :real) + y - u * z = x + (y + -u * z)``)

  val _ = s ("r235", R ``0 * (x :real) = 0``)
  val _ = s ("r236", R ``1 * (x :real) = x``)

  val _ = s ("r237", W ``0w + x = x``)
  val _ = s ("r238", W ``(x :'a word) + y = y + x``)
  val _ = s ("r239", W ``1w + (1w + x) = 2w + x``)
  val _ = s ("r240", Drule.EQT_ELIM
  ...
  val _ = s ("r242", Drule.UNDISCH (simpLib.SIMP_PROVE bossLib.std_ss
  ...
  val _ = s ("r246", Drule.UNDISCH_ALL (bossLib.PROVE
    [wordsTheory.word_concat_0_eq] ``FINITE univ(:'a) ==>
      dimindex(:'b) <= dimindex(:'c) ==> y < dimword(:'b) ==>
      (((n2w y :'c word) = (0w :'a word) @@ (x :'b word)) <=> (n2w y = x))``))

  val _ = s ("r247", W ``x && y = y && x``)
  val _ = s ("r248", W ``x && y && z = y && x && z``)
  val _ = s ("r249", W ``x && y && z = (x && y) && z``)
  val _ = s ("r250", W ``(1w = (x :word1) && y) <=> (1w = x) /\ (1w = y)``)
  val _ = s ("r251", W ``(1w = (x :word1) && y) <=> (1w = y) /\ (1w = x)``)
  val _ = s ("r252", W ``(7 >< 0) (x :word8) = x``)
  val _ = s ("r253", W ``x <+ y <=> ~(y <=+ x)``)
  val _ = s ("r254", W ``(x :'a word) * y = y * x``)
  val _ = s ("r255", W ``(0 >< 0) (x :word1) = x``)
  val _ = s ("r256", W ``(x && y) && z = x && y && z``)
  val _ = s ("r257", W ``0w || x = x``)
