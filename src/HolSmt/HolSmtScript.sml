...
struct
  ...
  val _ = s ("d014", D ``~(~(~(p <=> q) <=> r)) \/ p \/ ~q \/ ~r``)
  val _ = s ("d015", D ``~(~(~(p <=> q) <=> r)) \/ p \/ q \/ r``)
  val _ = s ("d016", D ``~(~(p <=> q) <=> r) \/ ~p \/ ~q \/ ~r``)
  val _ = s ("d017", D ``~(~(p <=> q) <=> r) \/ ~p \/ q \/ r``)
  val _ = s ("d018", D ``~(~(p <=> q) <=> r) \/ p \/ ~q \/ r``)
  val _ = s ("d019", D ``~(~(p <=> q) <=> r) \/ p \/ q \/ ~r``)
  val _ = s ("d020", D ``~((p /\ r) \/ (p /\ s) \/ (q /\ t)) \/ p \/ q``)
  val _ = s ("d021", D ``~((p /\ r) \/ (p /\ s) \/ (t /\ q)) \/ p \/ q``)
  val _ = s ("d022", D ``(~p /\ ~q) \/ p \/ q``)
  val _ = s ("d023", D ``(~p /\ q) \/ p \/ ~q``)
  val _ = s ("d024", D ``(p /\ ~q) \/ ~p \/ q``)
  val _ = s ("d025", D ``(p /\ q) \/ ~p \/ ~q``)
  val _ = s ("d026", D ``((p /\ q) \/ r) \/ ~p \/ ~q``)
  val _ = s ("d027", D ``(r \/ (p /\ q) \/ s) \/ ~p \/ ~q``)
  val _ = s ("d028", D ``(r \/ s \/ (p /\ q)) \/ ~p \/ ~q``)
  val _ = s ("d029", P ``p \/ (x = if p then y else x)``)
  val _ = s ("d030", P ``~p \/ (x = if p then x else y)``)
  val _ = s ("d031", P ``p \/ q \/ ~(if p then r else q)``)
  val _ = s ("d032", P ``~p \/ q \/ ~(if p then q else r)``)
  val _ = s ("d033", P ``(if p then q else r) \/ ~p \/ ~q``)
  val _ = s ("d034", P ``(if p then q else r) \/ p \/ ~r``)
  val _ = s ("d035", P ``~(if p then q else r) \/ ~p \/ q``)
  val _ = s ("d036", P ``~(if p then q else r) \/ p \/ r``)
