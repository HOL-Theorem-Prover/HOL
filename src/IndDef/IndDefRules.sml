...
fun MK_CONJ_THEN Fn tm =
 let val (conj1,conj2) = dest_conj tm
     val tcl1 = if eq (fst(strip_comb conj1)) Fn
                  then fn t1 => fn t2 => t1
                  else fn t1 => fn t2 => t2
 ...
 handle HOL_ERR _ => if eq (fst(strip_comb tm)) Fn then K else C K;
...
local fun mkred Fn (c::cs) =
        let val cfn = if eq (fst(strip_comb c)) Fn then LIST_BETA_CONV else REFL
...
fun residue_assoc itm =
   let fun assc ([]:(term,term)subst) = NONE
         | assc ({redex,residue}::rst) =
             if eq itm residue then SOME redex else assc rst
   in assc
   end;
...
fun prove_conj ths tm =
 ...
 handle HOL_ERR _ => first (eq tm o concl) ths;
...
fun reduce vs ths res subf =
 if null ths then (rev res, subf)
          ...
          val (sth,pai) =
             if op_mem eq lhs vs then (hd ths,(lhs |-> rhs))
...
local fun chfn v (a,th) =
      ...
      fun efn ss v (pat,th) =
        let val wit = case subst_assoc (eq v) ss
                       of NONE => v
