...
val ERR = mk_HOL_ERR "String_rules";
...
local val Estr = prim_mk_const{Name="emptystring",Thy="string"}
      ...
      fun check c = fst(dest_type(type_of c)) = "string"
      val Tand = CONJUNCT1 (SPEC b AND_CLAUSES)
      val mkC = AP_THM o AP_TERM boolSyntax.conjunction
fun conv l r =
     if l=Estr
     then let
	      ...
	      val (Rator, S) = (dest_comb (rand(concl thm)))
              val A = rand Rator
          in
	  (* result of SUBST below is a thm: |- ("" = <r>) = F *)
          ...
          then let val thm = String_conv.string_CONV l
		   ...
		   val (Rator, s1) = dest_comb(rand(concl th1))
		   val a1 = rand Rator
                   val (Rator, s2) = dest_comb(rand(concl th2))
		   val a2 = rand Rator
...
fun string_EQ_CONV tm =
   ...
   handle HOL_ERR _ => raise ERR "string_EQ_CONV" ""
end;
