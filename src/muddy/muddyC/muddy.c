...
/* I don't want to adjust the GC so I've made my own alloc_final,
   stolen from alloc.c
   FIXME: Now I understand how to adjust the GC
*/
...
static int printgc = 0; /* Invariant: if printgc != 0 then will the
                           three strings above point to valid strings */
static int joingc = 1;
...
/* ML type: bool -> unit */
value mlbdd_joingc(value join) /* ML */
{
  if(Bool_val(join)) joingc = 1;
  else               joingc = 0;
  return Val_unit;
}
...
/* ML type: bool -> string -> string -> string -> unit */
value mlbdd_setprintgc(value print, value pre, value post1, value post2) /* ML */
{
  mlbdd_freegcstrings();

  if(Bool_val(print)) {
    mlbdd_copy2cstring(pre, pregc);
    mlbdd_copy2cstring(post1, postmosgc);
...
/* ML type: string -> bdd -> unit */
value mlbdd_bdd_fnsave(value filename, value r) /* ML */
{
...
/* ML type: string -> bdd */
value mlbdd_bdd_fnload(value filename) /* ML */
{
  BDD res;
...
/* ML type: int -> int */
value mlbdd_bdd_setmaxincrease(value n) /* ML */
{
  return Val_int(bdd_setmaxincrease(Int_val(n)));
}

/* ML type: int -> int */
value mlbdd_bdd_setcacheratio(value n) /* ML */
{
