...
DESCR   {* Build a boolean vector with {\tt bitnum} elements copied from
	   ...
	   {\tt bitnum} then the most significant bits a removed, otherwise
...
PROTO   {* bvec bvec_mul(bvec v, int c) *}
DESCR   {* Builds a new boolean vector representing the integer multiplication
           of {\tt v} and {\tt c}. It is not possible to do multiplication in
	   general as this will result in an exponential blowup in the size
	   of the resulting BDDs. *}
RETURN  {* The result of the multiplication (which is already reference counted) *}
ALSO    {* bvec\_add, bvec\_div, bvec\_shl *}
*/
bvec bvec_mul(bvec e, int c)
{
   bvec res, next, rest;
   int n;
...
static void bvec_div_rec(bvec divisor, bvec *remainder, bvec *result, int step)
{
   ...
   bvec newResult = bvec_shl( *result, 1, isSmaller );
   ...
   newRemainder = bvec_shl(tmp, 1, result->bitvec[divisor.bitnum-1]);
...
PROTO   {* int bvec_div(bvec v, int c, bvec *res, bvec *rem) *}
DESCR   {* Builds a new boolean vector representing the integer division
           of {\tt v} with {\tt c}. The result of the division will be stored
	   in {\tt res} and the remainder of the division will be stored in
	   {\tt rem}. Both vectors should {\em not} contain any data, as
	   this will be overwritten by {\tt bvec\_div}.
	   
	   It is not possible to do division in
	   general as this will result in an exponential blowup in the size
	   of the resulting BDDs. *}
...
int bvec_div(bvec e, int c, bvec *res, bvec *rem)
{
   if (c > 0)
   {
      ...
      bvec tmpremainder = bvec_shl(tmp, 1, e.bitvec[e.bitnum-1]);
      bvec result = bvec_shl(e, 1, bddfalse);
      bvec remainder;
      
      bvec_div_rec(divisor, &tmpremainder, &result, divisor.bitnum);
      remainder = bvec_shr(tmpremainder, 1, bddfalse);
...
NAME    {* bvec\_shl *}
SECTION {* bvec *}
SHORT   {* shift left operation *}
PROTO   {* bvec bvec_shl(bvec v, int pos, BDD c) *}
...
ALSO    {* bvec\_add, bvec\_mul, bvec\_shr *}
*/
bvec bvec_shl(bvec e, int pos, BDD c)
{
   bvec res;
...
PROTO   {* bvec bvec_shr(bvec v, int pos, BDD c) *}
DESCR   {* Builds a boolean vector that represents {\tt v} shifted {\tt pos}
           times to the right. The new empty elements will be set to {\tt c}.*}
RETURN  {* The result of the operation (which is already reference counted) *}
ALSO    {* bvec\_add, bvec\_mul, bvec\_shl *}
*/
bvec bvec_shr(bvec e, int pos, BDD c)
{
   bvec res;
