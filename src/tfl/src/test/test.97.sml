...
val Rfunction = fn q1 => fn q2 =>
   let open Thm
       ...
       val {rules,induction,tcs} = tflLib.Rfunction q1 q2
       val {ABS, ASSUME, BETA_CONV, DISCH, INST_TYPE, MP, REFL, SUBST, 
            axiom,definition, drule,from_disk, oracle, valid_tac} = thm_count()
   in {rules=rules, induction=induction, 
       thms = ABS+ASSUME+BETA_CONV+DISCH+INST_TYPE+MP+REFL+SUBST
              + axiom+definition+drule+from_disk+oracle+valid_tac,
       tcs = tcs}
   end;

new_theory"test";

val fact_cond_def = Rfunction `^(tflLib.pred)`
...
val partition_def = 
  PoD.newDefinition `!(P:'a->bool). partition(P,L) = part(P,L,[],[])`
        "partition" Term.Prefix;
...
val list_pred_def = 
   PoD.newDefinition `list_pred l1 l2 = ?h:'a. l2 = CONS h l1` 
   "list_pred" Term.Prefix;
...
val fdef = (hd o map #1 o #extracta)
          (tflLib.wfrec_eqns (Term`f x = num_case 1 (\m. SUC m * f m) x`));
