...
open HolKernel Parse Rules Drule Conv tflUtils;
...
(*---------------------------------------------------------------------------
    Some simple theorems are used in the following.

      val eqT       = |- !x. (x = T) ==> x
      val thm_eq    = |- !x y. x ==> y ==> (x = y)
      val rev_eq_mp = |- !x y. (x = y) ==> y ==> x
      val simp_thm  = |- !x x' y. (x==>y) ==> (x = x') ==> (x' ==> y)
      val imp_elim  = |- !P Q R. P ==> (P==>Q = P==>R) = (P==>Q = P==>R)

 ---------------------------------------------------------------------------*)

val eqT = 
 let val x = mk_var{Name="x",Ty=Type.bool}
     val x_eq_T = mk_eq{lhs=x,rhs=mk_const{Name="T",Ty=Type.bool}}
 in 
    GEN x (DISCH x_eq_T (EQT_ELIM (ASSUME x_eq_T)))
 end;

val thm_eq = 
 let val x = mk_var{Name="x",Ty=Type.bool}
     val y = mk_var{Name="y",Ty=Type.bool}
     val th = CONJ (ASSUME x) (ASSUME y)
     val th1 = DISCH y (CONJUNCT1 th)
     val th2 = DISCH x (CONJUNCT2 th)
 in GENL [x,y]
      (DISCH x (DISCH y (IMP_ANTISYM_RULE th2 th1)))
 end;
 
val rev_eq_mp = 
 let val x = mk_var{Name="x",Ty=Type.bool}
     val y = mk_var{Name="y",Ty=Type.bool}
     val tm = mk_eq{lhs=x,rhs=y}
     val th = EQ_MP (SPECL[x,y] boolTheory.EQ_IMP_THM) (ASSUME tm)
 in GENL [x,y] 
       (DISCH tm (CONJUNCT2 th))
 end;
...
val simp_thm =
 let val x  = mk_var{Name="x",Ty=Type.bool}
     ...
     val y  = mk_var{Name="y",Ty=Type.bool}
     val tm1 = mk_imp{ant=x,conseq=y}
     val tm2 = mk_eq{lhs=x,rhs=x'}
     val th1 = ASSUME tm1
     val th2 = ASSUME tm2
 in GENL [x,x',y] 
       (DISCH tm1 (DISCH tm2 (SUBS [th2] th1)))
 end;
...
val imp_elim = 
 ...
 in GENL [P,Q,R] 
       (DISCH tm1 (IMP_ANTISYM_RULE th4b th5b))
 end;
...
fun mk_case ty_info ty_match FV range_ty =
 ...
 val fresh_var = tflUtils.vary FV 
...
fun mk_functional thy eqs =
 let val clauses = strip_conj eqs
     ...
     val FV = a::fvs
     val ty_info = match_info thy
     val ty_match = match_type thy
...
fun wfrec_eqns thy eqns =
 let val {functional,pats} = mk_functional thy eqns
     ...
     val tmp = fst(tflUtils.strip_imp(concl WFREC_THM))
...
fun mk_case ty_info FV thy =
 let 
 val divide:divide_ty = ipartition (tflUtils.vary FV)  (* do not eta-expand!! *)
...
fun build_ih f (P,R,SV) (pat,TCs) = 
 let val pat_vars = free_vars_lr pat
     ...
     fun dest_TC tm = 
         let val (cntxt,R_y_pat) = tflUtils.strip_imp(#2(strip_forall tm))
             val (R,y,_) = tflUtils.dest_relation R_y_pat
 ...
 DISCH antc
   (if is_imp(concl thm') (* recursive calls in this clause *)
    then let val th1     = ASSUME antc
             ...
             val ylist   = map (#2 o dest_relation o #2 o tflUtils.strip_imp o 
...
let val Sinduction = UNDISCH (ISPEC R relationTheory.WF_INDUCTION_THM)
    ...
    val vars = map (tflUtils.vary[P]) (strip_prod_type Parg_ty)
...
fun simplify_induction thy hth ind = 
  let val (asl,_) = Thm.dest_thm ind
      val (_,tc_eq_tc') = Thm.dest_thm hth
      val tc = lhs tc_eq_tc'
      fun loop [] = ind
        | loop (asm::rst) = 
           let val hypcore = #2 (strip_forall asm)
           in if can (match_term thy hypcore) tc
              then let val thm = DISCH asm ind
                       open Rewrite
...
let 
   ...
   val (rules1,induction1)  = 
     let val thm = Tactical.prove(Lib.trye hd (#1(Thm.dest_thm rules)),WFtac)
     in 
   ...
   fun simplify_tc tc (r,ind) =
       let val tc_eq = simplifier (#2 (strip_forall tc))
       in 
         elim_tc (MATCH_MP eqT tc_eq) (r,ind)
         handle HOL_ERR _ 
         => (elim_tc (MATCH_MP(MATCH_MP rev_eq_mp tc_eq)
                       (Tactical.prove(rhs(concl tc_eq),terminator))) (r,ind)
            handle HOL_ERR _ 
        ...
        GEN_ALL
         (MATCH_MP eqT tc_eq
          handle HOL_ERR _ => (MATCH_MP(MATCH_MP rev_eq_mp tc_eq)
                        (Tactical.prove(rhs(concl tc_eq),terminator))
    ...
    *-------------------------------------------------------------------*)
   val f = tflUtils.func_of_cond_eqn (hd (strip_conj(concl rules)))
   fun nested tm = can(find_term (aconv f)) tm handle HOL_ERR _ => false
   fun loop ([],extras,R,ind) = (rev R, ind, extras)
     | loop ((r,ftcs)::rst, nthms, R, ind) =
        let val tcs = map gen_all (#1(tflUtils.strip_imp (concl r)))
...
fun mutual_function thy name eqns =
  let open Psyntax
      ...
      fun define_subfn fvar thy =
         ...
         in 
           make_definition thy fvarname (mk_eq(dlhs,drhs))
         end
      val (defns,theory2) = 
