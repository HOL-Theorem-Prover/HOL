...
in
	...
	{name = "int", equiv = TINT_EQ_EQUIV,
	 defs = [mk_def ("int_0", Term `tint_0`,     "int_0", Prefix),
		 ...
		 mk_def ("int_add",Term `$tint_add`, "+_",    Infix 500),
		 mk_def ("int_mul",Term `$tint_mul`, "*_",    Infix 600),
		 mk_def ("int_lt",Term `$tint_lt`,   "<_",    Infix 450)],
...
val int_sub = 
    new_infix_definition("int_sub",
			 Term `$-_ x y = x +_ (-- y)`,
			 500);

val int_le = 
    new_infix_definition("int_le",
			 Term `$<=_ x y = ~(y <_ x)`,
			 450);

val int_gt = 
    new_infix_definition("int_gt",
			 Term `$>_ x y = y <_ x`,
			 450);

val int_ge = 
    new_infix_definition("int_ge",
			 Term `$>=_ x y = y <=_ x`,
			 450);
...
val INT_1 = 
    prove_thm("INT_1",
	      Term `int_1 = & 1`,
	      REWRITE_TAC[num_CONV (Term `1`), int_of_num, INT_ADD_LID]);
...
val INT_MUL_LZERO = 
    prove_thm("INT_MUL_LZERO",
	      ...
	      (SYM(SPECL [Term `&0 *_ x`, Term `&0 *_ x`] INT_ADD_LID_UNIQ))
...
val INT_INJ = 
    prove_thm("INT_INJ",
	      Term `!m n. (&m = &n) = (m = n)`,
	      let val th = PROVE(Term `(m = n) = m <= n /\ n <= m`,
				 EQ_TAC 
...
val INT_DOUBLE = 
    prove_thm("INT_DOUBLE",
	      Term `!x. x +_ x = &2 *_ x`,
	      GEN_TAC THEN REWRITE_TAC[num_CONV (Term `2`), INT] THEN
...
val NUM_LEMMA = 
    prove_thm("NUM_LEMMA",
	      Term `!i. &0 <=_ i ==> ?n. i = mk_int($tint_eq (n,0))`,
		  GEN_TAC THEN
		  ...
		  DISCH_TAC THEN EXISTS_TAC (Term `m - n`)
...
val NUM_DECOMPOSE = 
    prove_thm("NUM_DECOMPOSE",
	      ...
	      [AP_TERM_TAC THEN REWRITE_TAC[GSYM TINT_EQ_EQUIV, 
	       ...
	       REWRITE_TAC[num_CONV (Term `1`), ADD_CLAUSES]]);
