...
in
  case op_intersect aconv lvars rvars of
    [] => NO_CONV
...
in
  case List.find (fn lst => length lst = 1) div_varsets of
    SOME vs => let
      ...
      val v = hd vs
    in
      push_nthvar_to_bot (index (aconv v) vars) THENC
  ...
  | NONE => let
    ...
    in
      ...
      | NONE => let
        ...
        in
          push_nthvar_to_bot (index (aconv minv) vars) THENC
...
in
  if null vars then REDUCE_CONV
    ...
    val next_var =
      ...
      | SOME v => v
  in
      push_nthvar_to_bot (index (aconv next_var) vars) THENC
