 ...
 *---------------------------------------------------------------------------*)
(*---------------------------------------------------------------------------
 * This constant family stuff is completely horrible. It will get removed 
 * when Mike Norrish numbers come along. The code takes advantage of the 
 * fact that the types of strings and numbers won't get re-defined. The 
 * witness for a family literal should be an equation involving its 
 * predecessor, e.g., `1 = SUC 0` for "mk_const 1"  but I'll just make it be
 * the base element of the family (0 for nums and "" for strings).
 *---------------------------------------------------------------------------*)
...
(*
  handle HOL_ERR _ =>  (* handle the family cases, if necessary *)
    (case (Globals.nums_defined(), Globals.strings_defined())
     of (false,false) => raise e
      | (false, true) => if (Lexis.is_string_literal s) 
                         then mkX (mk String thy e) s else raise e
      | (true,false)  => if (Lexis.is_num_literal s) 
                         then mkX (mk Num thy e) s else raise e
      | (true,true)   => 
         if (Lexis.is_num_literal s) then mkX (mk Num thy e) s else 
         if (Lexis.is_string_literal s) then mkX (mk String thy e) s 
         else raise e)
end;
*)
