...
Theorem BITWISE_ONE_COMP_THM[local]:
   !wl a b. 0 < wl ==>
...
Theorem sw2sw_sw2sw_lem2[local]:
   !w:'a word. dimindex(:'b) < dimindex(:'a) /\
               dimindex(:'b) < dimindex(:'c) ==>
...
Theorem MIN_lem2[local]:
   MIN a (MIN b (MIN (c + a) (c + b))) = MIN a b
Proof
  RW_TAC arith_ss [MIN_DEF]
QED

Theorem MIN_FST[local]:
   !x y. x <= y ==> (MIN x y = x)
Proof RW_TAC arith_ss [MIN_DEF]
QED
...
Theorem TWO_EXP_DIMINDEX[local]:
   2 <= 2 ** ^WL
Proof
...
Theorem WORD_EXTRACT_BITS_COMP =
...
Theorem EXTRACT_OVER_ADD_lem[local]:
    !h1 h2 a b.
       h1 <= h2 ==>
...
Theorem EXTRACT_OVER_MUL_lem[local]:
    !h1 h2 a b.
       h1 <= h2 ==>
       (BITS h1 0 (BITS h2 0 a * BITS h2 0 b) = BITS h1 0 (a * b))
Proof
...
Theorem bitwise_log_left[local]:
...
Theorem word_or_n2w_alpha[local]:
...
Theorem word_and_n2w_alpha[local]:
...
Theorem word_and_1comp_n2w_alpha[local]:
   !n m. n2w n && ~(n2w m) =
...
Theorem word_and_1comp_n2w_alpha2[local]:
   !n m. ~(n2w n) && ~(n2w m) =
...
Theorem word_or_1comp_n2w_alpha[local]:
   !n m. n2w n || ~(n2w m) =
...
Theorem word_or_1comp_n2w_alpha2[local]:
   !n m. ~(n2w n) || ~(n2w m) =
...
Theorem GENLIST_FCP_INDEX[local]:
   !n.
...
Theorem GENLIST_UINT_MAXw[local]:
...
Theorem WORD_REDUCE_LIFT[local]:
...
Theorem NOT_EVERY_HD_F[local]:
   !l. ~(FOLDL (/\) F l)
Proof Induct \\ SRW_TAC [listSimps.LIST_ss] []
QED

Theorem EXISTS_HD_T[local]:
   !l. (FOLDL (\/) T l)
Proof Induct \\ SRW_TAC [listSimps.LIST_ss] []
QED
...
Theorem WORD_RIGHT_SUB_DISTRIB =
...
Theorem word_extract_mask1[local]:
    !h l a.
        (h >< l) a =
...
Theorem word_extract_w2w_mask1[local]:
    !h l a.
        (h >< l) a =
...
Theorem SPEC_LESS_EXP_SUC_MONO[local]:
   2 ** ^HB < 2 ** dimindex (:'a)
Proof
  SRW_TAC [][DIMINDEX_GT_0]
QED

Theorem SPLIT_2_EXP_WL[local]:
...
Theorem TWO_COMP_NEG_lem[local]:
...
Theorem SUB_EQUAL_WORD_L_INT_MIN[local]:
...
Theorem NOT_INT_MIN_ZERO =
  METIS_PROVE [WORD_L_NEG, WORD_0_POS] ``~(INT_MINw = 0w)``

Theorem ZERO_LO_INT_MIN =
...
Theorem INT_MIN_LT_dimword[local]:
   INT_MIN (:'a) < dimword (:'a)
Proof
...
Theorem FST_ADD_WITH_CARRY[local]:
...
Theorem word_reverse_reverse[local]:
...
Theorem word_reverse_lsl[local]:
...
Theorem word_reverse_lsr[local]:
   !w n. word_reverse (w >>> n) = (word_reverse w << n):'a word
Proof
  METIS_TAC [word_reverse_lsl,word_reverse_reverse]
QED

Theorem word_reverse_EQ_ZERO[local]:
...
Theorem word_reverse_EQ_ONE[local]:
...
Theorem triv_exp[local]:
  !m. 0 < 2 **  m
Proof
  Induct THEN RW_TAC arith_ss [EXP]
QED

Theorem ONE_LESS_TWO_EXP[local]:
  !m. 0<m ==> 1 < 2 ** m
Proof
...
Theorem ZERO_LE_TOP_FALSE[local]:
...
Theorem WORD_ZERO_LE_SUB_LEMMA[local]:
...
Theorem WORD_ZERO_LE_SUB[local]:
...
Theorem WORD_ZERO_LT_SUB[local]:
   !x:'a word y. 0w < y /\ y < x ==> 0w < x - y
Proof
  REPEAT STRIP_TAC
...
Theorem WORD_LE_SUB_UPPER[local]:
   !x:'a word y. 0w <= y /\ y <= x ==> x - y <= x
Proof
  REPEAT STRIP_TAC
...
(* ------------------------------------------------------------------------- *)
(* Lemmas wordsLib formerly proved as it loaded.                             *)
(* ------------------------------------------------------------------------- *)

Theorem w2n_n2w_compute:
  !n. w2n ((n2w n) : 'a word) =
      if n < dimword(:'a) then n else n MOD dimword(:'a)
Proof
  BasicProvers.PRIM_SRW_TAC arith_ss [boolSimps.LET_ss] [w2n_n2w]
QED

Theorem word_2comp_compute:
  !n. word_2comp (n2w n) : 'a word =
        let x = n MOD dimword (:'a) in
          if x = 0 then 0w else n2w (dimword (:'a) - x)
Proof
  BasicProvers.PRIM_SRW_TAC arith_ss [boolSimps.LET_ss] [word_2comp_n2w,n2w_11]
QED

Theorem word_lsl_compute:
  !n m. (n2w m : 'a word) << n =
        if dimindex(:'a) - 1 < n then 0w
        else n2w ((m * 2 ** n) MOD dimword(:'a))
Proof
  REWRITE_TAC [word_lsl_n2w, n2w_mod]
QED

Theorem ROL_ROR_MOD_RWT:
  !n w:'a word. dimindex (:'a) <= n ==>
    (word_rol w n = word_rol w (n MOD dimindex (:'a))) /\
    (word_ror w n = word_ror w (n MOD dimindex (:'a)))
Proof
  BasicProvers.PRIM_SRW_TAC arith_ss [] [Once (GSYM ROL_MOD), Once (GSYM ROR_MOD)]
QED

Theorem ASR_ROR_ROL_UINT_MAX:
...
Theorem word_eq_imp_num_eq:
  !m n. (n2w m = n2w n : 'a word) /\
        m < dimword(:'a) /\ n < dimword(:'a) ==> (m = n)
Proof SRW_TAC [] [n2w_11] THEN FULL_SIMP_TAC arith_ss []
QED

Theorem word_lt_imp_num_lt:
  !m n. (n2w m) <+ (n2w n : 'a word) /\
        m < dimword(:'a) /\ n < dimword(:'a) ==> (m < n)
Proof SRW_TAC [] [word_lo_n2w] THEN FULL_SIMP_TAC arith_ss []
QED

Theorem word_ls_imp_num_ls:
  !m n. (n2w m) <=+ (n2w n : 'a word) /\
        m < dimword(:'a) /\ n < dimword(:'a) ==> (m <= n)
Proof SRW_TAC [] [word_ls_n2w] THEN FULL_SIMP_TAC arith_ss []
QED

Theorem word_n2w_le:
  !a. w2n (n2w a :'a word) <= a MOD dimword(:'a)
Proof SIMP_TAC std_ss [w2n_n2w]
QED

Theorem word_n2w_le2:
  !a. w2n (n2w a :'a word) <= a
Proof SIMP_TAC std_ss [w2n_n2w, bitTheory.MOD_LEQ, ZERO_LT_dimword]
QED

Theorem word_extract_le:
...
Proof
  ...
  SIMP_TAC std_ss [bitTheory.MOD_LEQ, word_add_def, w2n_n2w, ZERO_LT_dimword]
QED
...
Proof
  Cases THEN Cases THEN
  SIMP_TAC std_ss [bitTheory.MOD_LEQ, word_mul_def, w2n_n2w, ZERO_LT_dimword]
QED
...
Theorem word_div_le:
  !a:'a word b. 0 < b MOD dimword (:'a) ==>
                w2n (a // n2w b) <= w2n a DIV (b MOD dimword (:'a))
Proof
  Cases THEN STRIP_TAC THEN Cases_on `b MOD dimword (:'a) = 1`
  THENL
...
Proof
  ...
  THEN ASM_SIMP_TAC arith_ss [arithmeticTheory.DIV_LE_MONOTONE,
         word_div_le |> SPEC_ALL |> Q.DISCH `b < dimword (:'a)`
                     |> SIMP_RULE arith_ss []]
QED

Theorem word_div_order2:
  !a:'a word b n.
    0 < b /\ b < dimword (:'a) /\ w2n a < n ==>
    w2n (a // n2w b) <= n DIV b
Proof
...
Theorem word_div_order3:
  !a:'a word b n. ODD b /\ w2n a <= n ==> w2n (a // n2w b) <= n
Proof
  REPEAT STRIP_TAC
...
Theorem EXISTS_NUMBER:
  !P. (?w:'a word. P (w2n w)) <=> (?n. n < dimword(:'a) /\ P n)
Proof
