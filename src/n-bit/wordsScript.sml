...
val w2w = store_thm("w2w",
  `!w:bool ** 'a i. i < dimindex (UNIV:'b->bool) ==>
      ((w2w w):bool ** 'b %% i = i < ^WL /\ w %% i)`,
...
val w2w_id = store_thm("w2w_id",
  `!w:bool ** 'a. w2w w:bool ** 'a = w`, FIELD_WORD_TAC);

val w2w_w2w = store_thm("w2w_w2w",
  `!w:bool ** 'a. (w2w ((w2w w):bool ** 'b)):bool ** 'c =
        w2w ((dimindex (UNIV:'b->bool) - 1 -- 0) w)`,
  FIELD_WORD_TAC
    \\ Cases_on `i < ^WL` \\ FIELD_WORD_TAC
    \\ Cases_on `i < dimindex (UNIV:'b->bool)` \\ FIELD_WORD_TAC
...
val word_bits_w2w = store_thm("word_bits_w2w",
  `!w. (h -- l) (w2w (w:bool ** 'a)):bool ** 'b =
       w2w ((MIN h (dimindex (UNIV:'b->bool) - 1) -- l) w)`,
...
val WORD_BITS_LSR = store_thm("WORD_BITS_LSR",
  ...
  FIELD_WORD_TAC \\ Cases_on `i + n < dimindex (UNIV:'a->bool)`
    \\ ASM_SIMP_TAC (fcp_ss++ARITH_ss) []);
...
val WORD_BITS_OVER_BITWISE = store_thm("WORD_BITS_OVER_BITWISE",
  `(!h l v:bool ** 'a w:bool ** 'a.
   ...
   (!h l v:bool ** 'a w:bool ** 'a.
      (h -- l) v ?? (h -- l) w = (h -- l) (v ?? w))`,
  FIELD_WORD_TAC
    \\ Cases_on `i + l <= h /\ i + l <= dimindex (UNIV:'a->bool) - 1`
...
val WORD_NEG_1 = store_thm("WORD_NEG_1",
  ...
  REWRITE_TAC [word_T_def,word_2comp_def,w2n_n2w]
    \\ Cases_on `2 ** dimindex (UNIV:'a->bool) = 1`
...
val LSL_TRUE = store_thm("LSL_TRUE",
  ...
  RW_TAC arith_ss [n2w_11,word_T_def,word_lsl_n2w]
    << [
      `^WL <= n` by DECIDE_TAC \\ IMP_RES_TAC TWOEXP_MONO2
        \\ ASM_SIMP_TAC std_ss [LESS_MOD,ZERO_MOD,ZERO_LT_TWOEXP,
             DECIDE ``a <= b ==> (a - b = 0)``],
...
val WORD_NEG_L = store_thm("WORD_NEG_L",
  ...
  SIMP_TAC bool_ss [word_2comp_n2w,word_L_def,n2w_11,
    ...
    \\ REWRITE_TAC [SPLIT_2_EXP_WL]
    \\ SIMP_TAC arith_ss [GSYM EXP,LESS_MOD,
...
val TWO_COMP_POS = store_thm("TWO_COMP_POS",
  ...
  Cases_word
    ...
    \\ `2 ** SUC m - BITS (m - 1) 0 (w2n ((n2w n):bool ** 'a)) < 2 ** SUC m /\
