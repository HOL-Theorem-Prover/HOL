...
fun mk_index (w, n) =
  Lib.with_exn HolKernel.list_mk_comb
    (Term.pure_inst [Type.alpha |-> Type.bool, Type.beta |-> dim_of w] fcp_index_tm,
     [w, n])
    (ERR "mk_index" "")
...
val s = syntax_fns "words" 0
   ...
   (fn (tm, ty) => Term.pure_inst [Type.alpha |-> ty] tm)
...
val s = syntax_fns "words" 1 HolKernel.dest_monop
   (fn (tm, w) => Term.mk_comb (Term.pure_inst [Type.alpha |-> dim_of w] tm, w))
...
val s = syntax_fns "words" 1
   ...
   (fn (tm, (x, w)) =>
       Term.mk_comb (Term.pure_inst [Type.alpha |-> dim_of w] tm,
...
val s = syntax_fns "words" 1
   ...
   (fn (tm, (a, b, c)) =>
       Term.mk_comb (Term.pure_inst [Type.alpha |-> dim_of a] tm,
...
val s = syntax_fns "words" 1
   ...
   (fn (tm, (w, ty)) =>
       Term.mk_comb
         (Term.pure_inst [Type.alpha |-> dim_of w, Type.beta |-> ty] tm, w))
...
val s = syntax_fns "words" 1
   ...
   (fn (tm, (l, ty)) =>
      ...
      in
         Term.mk_comb (Term.pure_inst [Type.alpha |-> d, Type.beta |-> ty] tm, l)
      end)
...
val s = syntax_fns "words" 2 HolKernel.dest_binop
   (fn (tm, (x, w2)) =>
       Term.list_mk_comb (Term.pure_inst [Type.alpha |-> dim_of w2] tm, [x, w2]))
...
val s = syntax_fns "words" 2 HolKernel.dest_binop
   (fn (tm, (w1, w2)) =>
       Term.list_mk_comb
         (Term.pure_inst [Type.alpha |-> dim_of w1, Type.beta |-> dim_of w2] tm,
          [w1, w2]))
...
val s = syntax_fns "words" 2
   ...
   (fn (tm, (n, l, ty)) =>
       Term.list_mk_comb (Term.pure_inst [Type.alpha |-> ty] tm, [n, l]))
...
val s = syntax_fns "words" 2 HolKernel.dest_binop
   (fn (tm, (w1, w2)) =>
      ...
      in
         Term.list_mk_comb
            (Term.pure_inst [Type.alpha |-> d1, Type.beta  |-> d2,
...
val s = syntax_fns "words" 2 HolKernel.dest_binop
   (fn (tm, (n, w)) =>
      ...
      in
         Term.list_mk_comb
            (Term.pure_inst [Type.alpha |-> d1, Type.beta |-> d2] tm, [n, w])
      end)
...
val s = syntax_fns "words" 3 HolKernel.dest_triop
   (fn (tm, (n1, n2, w)) =>
       Term.list_mk_comb (Term.pure_inst [Type.alpha |-> dim_of w] tm, [n1, n2, w]))
...
val s = syntax_fns "words" 3
   ...
   (fn (tm, (n1, n2, n3, ty)) =>
       Term.list_mk_comb (Term.pure_inst [Type.alpha |-> ty] tm, [n1, n2, n3]))
...
val s = syntax_fns "words" 3
   ...
   (fn (tm, (n1, n2, w, ty)) =>
       Term.list_mk_comb
          (Term.pure_inst [Type.alpha |-> dim_of w, Type.beta |-> ty] tm,
           [n1, n2, w]))
...
val s = syntax_fns "words" 4 dest_quadop
   (fn (tm, (n1, n2, w1, w2)) =>
       Term.list_mk_comb
          (Term.pure_inst [Type.alpha |-> dim_of w2, Type.beta |-> dim_of w1] tm,
           [n1, n2, w1, w2]))
