...
val fdiv = new_definition (
  ...
  => some_nan(X)
    | is_infinity(X) a \/ is_zero(X) b => sign(a) = sign(b) => plus_infinity(X) |  minus_infinity(X)
      | is_infinity(X) b => sign(a) = sign(b) => plus_zero(X) 
        | minus_zero(X)
          | zerosign(X) ( sign(a) = sign(b) => 0 | 1) (round(X) m (valof(X) a / valof(X) b)))`--);
...
val fcompare = new_definition (
  ...
  ( is_nan(X) a \/ is_nan(X) b => Un
    | is_infinity(X) a /\ (sign(a) = 1) => is_infinity(X) b /\ (sign(b) = 1) => Eq | Lt
      | is_infinity(X) a /\ (sign(a) = 0) => is_infinity(X) b /\ (sign(b) = 0) => Eq | Gt
