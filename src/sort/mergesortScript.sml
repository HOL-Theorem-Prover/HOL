...
`!R n l.
 ...
 srw_tac[][] >>
 ONCE_REWRITE_TAC [mergesortN_def] >>
 srw_tac[][SORTED_EQ, SORTED_DEF, sort2_sorted, sort3_sorted]
 >- (Cases_on `l` >>
     srw_tac[][])
 >- (Cases_on `l` >>
     srw_tac[][] >>
...
`!P R l1 l2.
 ...
 srw_tac[][merge_def, FILTER_APPEND] >>
 REV_FULL_SIMP_TAC (srw_ss()) [SORTED_EQ, FILTER_APPEND]
 >- metis_tac []
 >- metis_tac []
 >- (`FILTER P l1 = []`
           by (srw_tac[][FILTER_EQ_NIL] >>
               CCONTR_TAC >>
               fs [EXISTS_MEM] >>
...
Proof
 ...
 srw_tac[][] >>
 ONCE_REWRITE_TAC [mergesortN_def] >>
 srw_tac[][sort2_stable, sort3_stable] >>
 TRY (srw_tac[][stable_def] >> NO_TAC)
 >- (Cases_on `l` >>
     srw_tac[][stable_def])
 >- (Cases_on `l` >>
     srw_tac[][] >>
 ...
 >- (`len1 <= n`
             by (UNABBREV_ALL_TAC >>
                 fs [DIV2_def, DIV_LESS_EQ]) >>
     metis_tac [stable_cong, merge_stable, take_drop_partition, stable_trans,
                mergesortN_sorted])
QED
...
Theorem sort3_tail_correct:
  !neg R x y z.
    sort3_tail neg R x y z = if neg then REVERSE (sort3 R x y z)
                             else sort3 R x y z
...
Proof
 ...
 srw_tac[][sort2_tail_correct, sort3_tail_correct] >>
 fs [] >>
 every_case_tac >>
 fs [] >>
 UNABBREV_ALL_TAC >>
 srw_tac[][merge_tail_correct1] >>
