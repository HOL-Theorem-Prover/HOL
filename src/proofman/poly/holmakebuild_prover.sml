...
(* Access Globals refs at runtime (not captured at load time)
   because the refs are set by hol.ML after modules are loaded. *)
            ...
            | run_steps ((_, ftype, ftext) :: rest) =
                let val ok = (tactic_timeout timeout step_frag (ftype, ftext);
                             ...
                             handle TacticTimeout t =>
                               raise ProofFailed (NONE, mk_HOL_ERR "holmakebuildProver"
                ...
                in if ok then run_steps rest
                   else raise ProofFailed (NONE, mk_HOL_ERR "holmakebuildProver"
                     "fragment_stepped_prover"
        ...
        in
          ...
          handle e as HOL_ERR _ => raise ProofFailed (NONE, e)
               | TacticTimeout t => raise ProofFailed (NONE,
                   mk_HOL_ERR "holmakebuildProver" "fragment_stepped_prover"
                     ("tactic timeout (" ^
...
in
  ...
  | TacticTimeout t =>
    let val e = mk_HOL_ERR "holmakebuildProver" "fragment_stepped_prover"
                  ("tactic timeout (" ^
                   Real.fmt (StringCvt.FIX (SOME 1)) t ^ "s)")
    in
      drop_all_safe ();
...
in
  ...
  handle e as HOL_ERR _ => raise ProofFailed (NONE, e)
       | TacticTimeout t => raise ProofFailed (NONE,
           mk_HOL_ERR "holmakebuildProver" "goalfrag_prover"
...
(* basic_prover with checkpointing: replaces holmakebuild.basic_prover
   when --dumpheap is active without -g. *)
...
(* Register advanced provers when -g or --dumpheap is active.
   On mosml these Globals refs are always false, so basic_prover stays. *)
val () = if !g_flag andalso !dumpheap then
           Tactical.set_prover fragment_stepped_prover
         else if !g_flag then
           Tactical.set_prover goalfrag_prover
         else if !dumpheap then
           Tactical.set_prover basic_prover
         else
           ()  (* keep holmakebuild's basic_prover with CHEAT support *)
