...
datatype elem_internal
    ...
    | iEQDATATYPE of tyvar list * ParseDatatype.AST list
    | iABSDATATYPE of tyvar list * ParseDatatype.AST list
...
(* handle _ =>
   ...
   handle _ =>
     ...
     in dTyApp(pretype_of opr, pretype_of arg)
     end
   handle _ =>
 ...
 let open Portable ParseDatatype
     ...
     fun pp_tyvars [] = ()
       ...
       | pp_tyvars vlist =
          (begin_block CONSISTENT 0;
           add_string"(";
           pr_list pp_tyvar (fn () => add_string",") (fn ()=>()) vlist;
           add_string")";
           end_block())
         ...
         fun pp_tydec (tyvars,s) =
           (begin_block CONSISTENT 0;
             ...
             if List.length tyvars = 1
              then (pp_tyvar (hd tyvars); add_string(" "^s))
              else (add_string "(";
                    pr_list pp_tyvar (fn () => add_string",")
                                       (fn () => ()) tyvars;
                    add_string(") "^s));
            end_block())
     in begin_block CONSISTENT 0;
