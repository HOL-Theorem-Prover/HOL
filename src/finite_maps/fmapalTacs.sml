...
fun incr_merge_CONV key_conv = let
  fun im_c t = (
    (REWR_CONV im_one THENC RAND_CONV (
        RATOR_CONV (RAND_CONV (merge_CONV key_conv)) THENC im_c)) ORELSEC
    REWR_CONV im_zero ORELSEC REWR_CONV im_lengthen) t
  in im_c end
...
fun incr_build_CONV key_conv = let
  fun ib_c t = (
    (REWR_CONV ib_rec THENC RAND_CONV ib_c THENC
      incr_merge_CONV key_conv) ORELSEC
    REWR_CONV ib_base) t
  in ib_c end
...
fun merge_out_CONV key_conv = let
  fun mo_c t = (
    (REWR_CONV mo_some THENC
...
fun FAPPLY_CONV keyconv = let
  fun apf_c t = (
    (REWR_CONV FAPPLY_node THENC
  ...
  fun apf_i t = (
    (REWR_CONV FAPPLY_fmap_CONS THENC
      RATOR_CONV (RATOR_CONV (RAND_CONV keyconv)) THENC
      COND_CONV THENC
      TRY_CONV apf_i) ORELSEC
    REWR_CONV FAPPLY_fmap_NIL) t
...
fun ORWL_TO_FMAPAL orwl = let
  val (eqn, orwl) = CONJ_PAIR (CONV_RULE (REWR_CONV ORWL) orwl)
...
fun bt_to_orl_lb_ub_CONV keyconv = let
  fun orluc t = (
    ...
    (REWR_CONV lb_ub_acf_node THENC
      ...
      ((RATOR_CONV (RATOR_CONV (RAND_CONV (LAND_CONV keyconv))) THENC
        COND_EQ_LESS_CONV THENC
        ((RAND_CONV (RAND_CONV orluc) THENC orluc) ORELSEC orluc)) ORELSEC
       orluc))) t
  in orluc end
...
fun bt_to_orl_ub_CONV keyconv = let
  fun oruc t = (
    REWR_CONV ub_acf_nt ORELSEC
    (REWR_CONV ub_acf_node THENC
      RATOR_CONV (RATOR_CONV (RAND_CONV (LAND_CONV keyconv))) THENC
      COND_EQ_LESS_CONV THENC
      ((RAND_CONV (RAND_CONV (bt_to_orl_lb_ub_CONV keyconv)) THENC
        oruc) ORELSEC
       oruc))) t
  in oruc end
...
fun bt_to_orl_lb_CONV keyconv = let
  fun orlc t = (
    REWR_CONV lb_acf_nt ORELSEC
    (REWR_CONV lb_acf_node THENC
      RATOR_CONV (RATOR_CONV (RAND_CONV (LAND_CONV keyconv))) THENC
      COND_EQ_LESS_CONV THENC
      ((RAND_CONV (RAND_CONV orlc) THENC
        bt_to_orl_lb_ub_CONV keyconv) ORELSEC
       orlc))) t
  in orlc end
...
fun ORL_bt_lb_ub_CONV keyconv = let
  fun olu t = (
...
fun ORL_bt_lb_CONV keyconv = let
  fun ol t = (
    (REWR_CONV ORL_l_node THENC
      LAND_CONV (ORL_bt_lb_ub_CONV keyconv) THENC
      RAND_CONV ol THENC AND_CONV) ORELSEC
    REWR_CONV ORL_l_nt) t
  in ol end
...
fun ORL_bt_ub_CONV keyconv = let
  fun ou t = (
    (REWR_CONV ORL_u_node THENC LAND_CONV ou THENC
      RAND_CONV (ORL_bt_lb_ub_CONV keyconv) THENC
      AND_CONV) ORELSEC
    REWR_CONV ORL_u_nt) t
  in ou end
...
fun ORL_bt_CONV keyconv =
  (REWR_CONV ORL_node THENC
    LAND_CONV (ORL_bt_ub_CONV keyconv) THENC
    RAND_CONV (ORL_bt_lb_CONV keyconv) THENC
    AND_CONV) ORELSEC
  REWR_CONV ORL_nt
...
fun bt_to_orl_CONV keyconv = let
  fun oc t = (
...
fun FUNION_CONV keyconv ust = let
  val (s, t) =
    dest_binop ``$FUNION:('a|->'b) -> ('a|->'b) -> ('a|->'b)``
      (ERR "FUNION_CONV" "Not a FUNION term") ust
  in
    ORWL_TO_FMAPAL $ ORWL_FUNION keyconv
      (FMAPAL_TO_ORWL keyconv s) (FMAPAL_TO_ORWL keyconv t)
  end
...
fun inter_merge_CONV keyconv = let
  fun interm_c t = (
    (REWR_CONV imsome_some THENC
      RATOR_CONV (RATOR_CONV (RATOR_CONV (RAND_CONV keyconv))) THENC
      cpn_REWR_CONV THENC
      (RAND_CONV interm_c ORELSEC interm_c)) ORELSEC
    REWR_CONV imnone_some ORELSEC
    REWR_CONV imsome_none ORELSEC
    REWR_CONV imnone_none) t
  in interm_c end
...
fun diff_merge_CONV keyconv = let
  fun diffm_c t = (
    (REWR_CONV dmsome_some THENC
      RATOR_CONV (RATOR_CONV (RATOR_CONV (RAND_CONV keyconv))) THENC
      cpn_REWR_CONV THENC (RAND_CONV diffm_c ORELSEC diffm_c)) ORELSEC
    REWR_CONV dmnone_some ORELSEC
    REWR_CONV dmsome_none ORELSEC
    REWR_CONV dmnone_none) t
  in diffm_c end
...
fun DRESTRICT_CONV keyconv mst = let
  val (s, t) =
    dest_binop ``DRESTRICT:('a|->'b) -> 'a set -> ('a|->'b)``
      (ERR "DRESTRICT_CONV" "Not a DRESTRICT term") mst
  in
    if is_comb t andalso is_const (rator t) andalso
      fst (dest_const (rator t)) = "COMPL"
    then
      if is_comb (rand t) andalso is_const (rator (rand t)) andalso
        fst (dest_const (rator (rand t))) = "COMPL"
      then
        (RAND_CONV (REWR_CONV pred_setTheory.COMPL_COMPL) THENC
          DRESTRICT_CONV keyconv) mst
      else
        ORWL_TO_FMAPAL $ ORWL_DRESTRICT_COMPL keyconv
          (FMAPAL_TO_ORWL keyconv s) (ENUMERAL_TO_OWL keyconv (rand t))
    else
      ORWL_TO_FMAPAL $ ORWL_DRESTRICT keyconv
...
fun MAP_ELEM_CONV elemconv = let
  fun mec t =
    if is_const t then REFL t
    else (LAND_CONV elemconv THENC RAND_CONV mec) t
  in mec end
...
val T_OR = fetch "fmapal" "T_OR"
val F_OR = fetch "fmapal" "F_OR"

fun IN_FDOM_CONV keyconv = let
  fun ifc t = (
  ...
  fun ifl t = (
    (REWR_CONV infdomcons THENC RATOR_CONV (RAND_CONV keyconv) THENC
      ((REWR_CONV F_OR THENC ifl) ORELSEC REWR_CONV T_OR)) ORELSEC
    REWR_CONV notinfdomnil) t
...
fun list_rplacv_CONV eqconv = let
  fun lrcn t = (
...
val NOT_CONS_NIL_EQN = fetch "fmapal" "NOT_CONS_NIL_EQN"
fun fmap_FUPDATE_CONV eqconv t = let
  val (fm, pair) = dest_binop
    ``FUPDATE:('a |-> 'b) -> 'a # 'b -> ('a |-> 'b)``
  ...
  val th' = CONV_RULE
    (RAND_CONV (list_rplacv_CONV eqconv) THENC
...
fun bt_rplacv_CONV keyconv = let
  fun brcn t = (
...
val NOT_node_nt_EQN = fetch "fmapal" "NOT_node_nt_EQN"
fun FMAPAL_FUPDATE_CONV keyconv t = let
  val (fm, pair) = dest_binop
    ``FUPDATE:('a |-> 'b) -> 'a # 'b -> ('a |-> 'b)``
    (ERR "FUPDATE_CONV" "Not a FUPDATE term") t
  val (x,y) = dest_binop ``$, : 'a -> 'b -> 'a # 'b``
  ...
  val th' =
    CONV_RULE (RAND_CONV (bt_rplacv_CONV keyconv) THENC
      REWR_CONV LET_THM THENC BETA_CONV) th
  val th'' =
    CONV_RULE
      (RATOR_CONV (RATOR_CONV (RAND_CONV
...
fun FMAPAL_EXPR_TO_ORWL keyconv t = let
  val c = rator (rator t)
  in
    if is_const c then let
      val d = fst (dest_const c)
      in
        if d = "FMAPAL" then FMAPAL_TO_ORWL keyconv t
        else if d = "FUNION" then
          ORWL_FUNION keyconv
...
fun fmap_TO_ORWL keyconv cmp fmapl = let
  val th = ISPECL [cmp, rand fmapl] fmap_ORWL_thm
