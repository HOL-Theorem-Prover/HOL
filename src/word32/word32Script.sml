...
val RIGHT_REWRITE_RULE = GEN_REWRITE_RULE (RAND_CONV o DEPTH_CONV) empty_rewrites;
...
val WL_SUB_ONE   = SIMP_CONV arith_ss [WL_def] ``WL - 1``;

val WL_SUB_HB    = SIMP_CONV bool_ss [SUC_SUB,WL_def] ``WL - HB``;

val WL_SUB_SUC_X = SIMP_CONV arith_ss [WL_def] ``WL - SUC x``;
    ...
    THENL [
      ...
      A_RW_TAC [BITWISE_def,SET_def,REWRITE_RULE [SLICE_THM] BITS_SUC2]
        ...
        THENL [
           ...
           REWRITE_TAC [REWRITE_CONV [ADD_SUB,TIMES2] ``2 * a - a``,SUB_PLUS,EXP]
        ]
    ]
);
...
val BITS00 = GEN_ALL (REWRITE_CONV [SUC_SUB,DIV1,EXP,EXP_1,SYM TWO,BITS_THM] ``BITS 0 0 n``);
...
val ROR_LEM = GEN_ALL (RIGHT_REWRITE_RULE [GSYM RORw_def]
                 (SIMP_CONV bool_ss [RORw_def,FUNPOW_THM2] ``RORw (SUC x) n``));
...
val ASR_LEM = GEN_ALL (RIGHT_REWRITE_RULE [GSYM ASRw_def]
                 (REWRITE_CONV [ASRw_def,FUNPOW_THM2] ``ASRw (SUC x) n``));
