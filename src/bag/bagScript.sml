...
(`(!x. BAG_VAL EMPTY_BAG x = 0) /\
  (!x b e. BAG_VAL (BAG_INSERT e b) x = if (e=x) then 1 + BAG_VAL b x else BAG_VAL b x)`,
 CONJ_TAC THENL
 [RW_TAC hol_ss [EMPTY_BAG,BAG_VAL_DEF],
  RW_TAC hol_ss [BAG_VAL_DEF] THEN METIS_TAC [BAG_VAL_DEF,BAG_INSERT]]);
...
(`BAG_INN e n b = BAG_VAL b e >= n`,
 RW_TAC hol_ss [BAG_VAL_DEF, BAG_INN]);
...
("SUB_BAG_EQNS",
 ...
 RW_TAC hol_ss [SUB_BAG_EMPTY,SUB_BAG, BAG_INSERT, BAG_INN,
  ...
  [FULL_SIMP_TAC bool_ss [ARITH `a+1 >= n = a >=n \/ (n=a+1)`]
...
(*  :: MLSIG "val BAG_OF_SET   : ''a set -> ''a bag" *)
    ...
    @
    [MLSIG "val fromList : 'a list -> 'a bag",
     MLSIG "val toList   : 'a bag -> 'a list",
     MLSTRUCT "fun fromList alist = listML.FOLDL (fn s => fn a => BAG_INSERT(a,s)) EMPTY_BAG alist",
     MLSTRUCT "fun toList EMPTY_BAG = []\n\
              \    | toList (BAG_INSERT(a,s)) = a::toList s"]))
end;
