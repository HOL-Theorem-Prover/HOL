...
in
  fun initialize_lits th =
    if eq (concl th) F then ([], th) else (strip_disj (concl th), INIT th);
end;
...
fun hol_term_to_fol (parm : parameters) (tmV, tyV) =
  let
    ...
    and tm2fol tm =
      if op_mem eq tm tmV then mlibTerm.Var (fst (dest_var tm))
        ...
        let
          ...
          val () = assert (not (op_mem eq f tmV)) (ERR "hol_term_to_fol" "ho term")
        in
