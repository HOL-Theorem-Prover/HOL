...
(* When true, trace_theory recodes the PolyML heap dump from 64-bit to 32-bit
   before gzipping. Produces a backward-incompatible format marked by a
   leading magic byte 0xFF 0x00 0x00 0x00. *)
...
fun export_proof {file, tag} th =
  if !TraceMode.mode = TraceMode.NoTrace then () else let
    val prf = Thm.proof th
    val () = export (file, prf)
    in Thm.set_exported th (file, tag) end
