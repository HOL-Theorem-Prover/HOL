...
val _ = temp_TeX_notation {hol = "=~",            TeX = ("\\ensuremath{\\approx}", 1)};
val _ = temp_TeX_notation {hol = UTF8.chr 0x2248, TeX = ("\\ensuremath{\\approx}", 1)};
...
val _ = temp_TeX_notation {hol = "<</=",          TeX = ("\\ensuremath{\\prec}", 1)};
val _ = temp_TeX_notation {hol = UTF8.chr 0x227A, TeX = ("\\ensuremath{\\prec}", 1)};

Overload cardlt[local] = ``\s1 s2. ~(cardleq s2 s1)``(* cardlt *)
...
val _ = temp_set_fixity "<=_c" (Infix(NONASSOC, 450)); (* for cardleq *)
Overload "<=_c"[local] = ``cardleq``
Overload "<<="[local] = ``$<=_c``(* defined in pred_setTheory *)

val _ = temp_set_fixity "<_c" (Infix(NONASSOC, 450));  (* for cardlt *)
Overload "<_c"[local] = ``cardlt``
Overload "<</="[local] = ``$<_c``

val _ = temp_set_fixity ">=_c" (Infix(NONASSOC, 450)); (* for cardgeq *)
val _ = Unicode.temp_unicode_version {u = UTF8.chr 0x227D, tmnm = ">=_c"};
val _ = temp_TeX_notation {hol = ">=_c",          TeX = ("\\ensuremath{\\succcurlyeq}", 1)};
val _ = temp_TeX_notation {hol = UTF8.chr 0x227D, TeX = ("\\ensuremath{\\succcurlyeq}", 1)};

val _ = temp_set_fixity ">_c" (Infix(NONASSOC, 450));  (* for cardgt *)
val _ = Unicode.temp_unicode_version {u = UTF8.chr 0x227B, tmnm = ">_c"};
val _ = temp_TeX_notation {hol = ">_c",           TeX = ("\\ensuremath{\\succ}", 1)};
val _ = temp_TeX_notation {hol = UTF8.chr 0x227B, TeX = ("\\ensuremath{\\succ}", 1)};

val _ = temp_set_fixity "=_c" (Infix(NONASSOC, 450));  (* for cardeq *)
Overload "=_c"[local] = ``cardeq``
Overload "=~"[local] = ``$=_c``
