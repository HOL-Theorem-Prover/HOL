...
fun EQT_INTRO th = EQ_MP (SPEC (concl th) (GSYM iff_T)) th;
...
val th29 = store_thm
  ("th29", el 29 goals |> concl, MATCH_ACCEPT_TAC (GSYM cons_neq_nil));

val some_neq_none = hd(amatch``_ <> Data_Option_none``);

(* |- !x. Data_Option_none <> Data_Option_some x
val th29 = store_thm("th29", el 29 goals |> concl,
    MATCH_ACCEPT_TAC (GSYM some_neq_none));
 *)
...
val th79 = store_thm
  ...
  \\ qx_genl_tac[`a`,`b`]
  \\ CONV_TAC(DEPTH_CONV BETA_CONV)
  \\ match_mp_tac (PURE_REWRITE_RULE[and_imp_intro]th21)
  \\ conj_tac \\ strip_tac
  >- (
...
(* |- !f g M N. M = N /\ (!x. x = N ==> f x = g x) ==> LET f M = LET g N
   NOTE: this theorem is now the new "th6"
