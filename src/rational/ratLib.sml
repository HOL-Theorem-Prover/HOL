...
fun RAT_SAVE_TAC t1 (asm_list, goal) =
    let val subst_thm = SPEC t1 RAT_SAVE
    in
      (ASSUME_TAC subst_thm THEN LEFT_EXISTS_TAC THEN LEFT_EXISTS_TAC THEN
       FILTER_ASM_REWRITE_TAC (fn t => not (tmem t asm_list)) [] THEN
       POP_NO_TAC 0)
      (asm_list, goal)
    end
...
let
  ...
  fun insert_into_factor_list (term1:term) (i1:int,i2:int) (h::t) =
      if( fst h ~~ term1 ) then (fst h, (fst (snd h)+i1, snd (snd h)+i2) )::t
      else h::(insert_into_factor_list term1 (i1,i2) t)
  ...
  val desired_product =
      list_rat_mul
        (map list_rat_mul (reorder_product (count_factors [] factors)))
in
  (* this is basically proven by the specialised versions of RAT_MUL_LINV *)
  ((fn tx => EQT_ELIM (RAT_MULAC_CONV “^tx = ^desired_product”)) THENC
   (REWRITE_CONV (RAT_MUL_LID::RAT_MUL_RID::
                  (map (fn fx=> UNDISCH (SPEC fx RAT_MUL_LINV))
                       (map dest_rat_minv minv_factors))))) t1
end;
