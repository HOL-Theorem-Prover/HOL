...
datatype trace_step = datatype trace_step_dtype.trace_step

(* Proof trace hook: called after each inference rule.
   Set by TraceRecord module when tracing is enabled. Default: disabled. *)
val trace_hook : ((thm, term, hol_type) trace_step -> unit) option ref =
  ref NONE
...
in
fun disk_thm name ((d,ocl), termlist) = let
  val c = hd termlist
  val asl = tl termlist
...
fun disk_thm_dep ((d,ocl), termlist, src_thy, src_trace_id) = let
  val c = hd termlist
  val asl = tl termlist
  val result = mk_disk_thm (Tag.read_disk_tag (d,ocl),list_hyp asl,c)
in
...
fun save_dep thy (th as (THM(t,h,c,sid))) =
  let
    val depid_num = !thm_order
    val did = (thy, depid_num)
...
in
  fun compute (init as {cval_terms, cval_type, num_type, char_eqns}) =
    let
      val tag0 = merge_tags empty_tag (List.map (tag o snd) char_eqns)
      val char_eqns_raw = char_eqns
      ...
      val _ = tr (TR_COMPUTE_INIT (cval_terms, cval_type, num_type,
                                    char_eqns_raw))
    in
      fn code_eqs =>
        let
        ...
        in
          fn tm =>
            ...
            in
              tr (TR_COMPUTE (result, code_thms, tm));
              result
            end
        end
    end;
