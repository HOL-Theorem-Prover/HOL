...
struct
  open Word8Vector
    ...
    and transform ({A,B,C,D},i,buf) = let
      ...
      fun x (n) = Word32.fromLargeWord (Pack32Little.subVec (buf,n + off))
      val (a,b,c,d) = (A,B,C,D)
  ...
  fun b64i n = String.sub (b64, Word8.toInt n);
  datatype bstate = B0 | B2 of Word8.word | B4 of Word8.word;
  fun toBase64String v =
    let
      fun bytes2b64 (b,(B0,acc)) =
        let
          val bm = Word8.>> (b,0w2)
          val bl = Word8.<< (Word8.andb (b,0wx03), 0w4)
        in
          (B2 bl, bm :: acc)
        end
        | bytes2b64 (b, (B2 x, acc)) =
        let
          val bm = Word8.orb (x, Word8.>> (b,0w4))
          val bl = Word8.<< (Word8.andb (b,0wx0f), 0w2)
        in
          (B4 bl, bm :: acc)
        end
        | bytes2b64 (b, (B4 x, acc)) =
        let
          val bm = Word8.orb (x, Word8.>> (b,0w6))
          val bl = Word8.andb (b,0wx3f)
        in
          (B0, bl :: bm :: acc)
        end
      fun final (B0, acc) = acc
        | final (B2 x, acc) = x :: acc
        | final (B4 x, acc) = x :: acc
      val chrs = final (Word8Vector.foldl bytes2b64 (B0,[]) v)
    in
      String.implode (foldl (fn (h,t) => b64i h :: t) [] chrs)
    end;
