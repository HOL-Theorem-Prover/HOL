...
local
  ...
  fun q s = "\""^munge s^"\""
in
(* Generate unified batch script supporting --bare and --noquote flags.
...
fun emit_hol_script target {boss_qend, bare_qend} =
 ...
 in
    output "@echo off\n";
    output "rem The HOL script\n";
    output "rem (automatically generated by HOL configuration)\n\n";
    output "setlocal enabledelayedexpansion\n";
    output "set BARE=0\n";
    output "set NOQUOTE=0\n";
    output "set ARGS=\n";
    output "for %%A in (%*) do (\n";
    output "  if \"%%A\"==\"--bare\" (\n";
    output "    set BARE=1\n";
    output "  ) else if \"%%A\"==\"--noquote\" (\n";
    output "    set NOQUOTE=1\n";
    output "  ) else (\n";
    output "    set ARGS=!ARGS! %%A\n";
    output "  )\n";
    output ")\n\n";
    output "rem Select end script based on --bare\n";
    output "if %BARE%==1 (\n";
    output ("  set QEND=" ^ q bare_qend ^ "\n");
    output ") else (\n";
    output ("  set QEND=" ^ q boss_qend ^ "\n");
    output ")\n\n";
    output "rem Run with or without unquote filter\n";
    output "if %NOQUOTE%==1 (\n";
    output (String.concat["  call ", mosml, " -P full -I ", sigobj, " ",
                          std_prelude, " %ARGS% %QEND%\n"]);
    output ") else (\n";
    output (String.concat["  call ", qfilter, " | ", mosml, " -P full -I ",
