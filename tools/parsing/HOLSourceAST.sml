...
and dec =
  DecSemi of int (** ; *)
| DecVal of {val_: int, tyvars: ident seq, elems: valbind separated}
  (** val tyvarseq [rec] pat = exp [and [rec] pat = exp ...] *)
| DecFun of {fun_: int, tyvars: ident seq, fvalbind: fvalarm list separated}
...
| DecSignature of {signature_: int, elems: sigbind separated}
...
fun mkList (p, ls) =
  List {left = p, elems = {args = ls, seps = [], stop = p}, right = NONE, stop = p}
fun mkTuple (p, ls) =
  Tuple {left = p, elems = {args = ls, seps = [], stop = p}, right = NONE, stop = p}

fun mkLabEq (p, lab, exp) = LabEq {lab = (p, lab), eq = p, exp = exp}
fun mkRecord (p, ls) =
  Record {left = p, elems = {args = ls, seps = [], stop = p}, right = NONE, stop = p}
